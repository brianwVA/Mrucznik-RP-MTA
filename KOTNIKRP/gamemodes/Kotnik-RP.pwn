...
#define FIXES_TogglePlayerControllable 0
...
#define AC_MAX_MSGS_REC_DIFF 900
...
#include <fadescreen>
#include <timestamp>
#include <systempozarow>   //System Pożarów v0.1 by PECET
#include <true_random>
...
//#define RESOURCES_LINK "M-RPdownloads/"
#if !defined gpci
native gpci (playerid, serial [], len);
#endif

new MySQL:Database = MYSQL_INVALID_HANDLE; //db
...
//do implementacji w główny kod (możliwie w moduły)
...
{
	print("Loading M-RP 2.9 gamemode in progress...");
    ...
    SetWeatherEx(3);
	AllowInteriorWeapons(1); //broń w intkach
	ShowPlayerMarkers(0); //wyłączenie markerów graczy
	DisableInteriorEnterExits(); //wyłączenie wejść do intków z GTA
	EnableStuntBonusForAll(0); //brak hajsu za stunty
	ManualVehicleEngineAndLights(); //brak automatycznego włączania silnika i świateł
	ShowNameTags(1); //Pokazywanie nicków graczy
	SetNameTagDrawDistance(50.0); //Wyświetlanie nicków od 45 metrów
	//UsePlayerPedAnims(); // Animacja CJ 
		// - off (broń trzymana w obu rękach jest trzymana jedną, skiny chodzą swoim chodem)
		// - on  (broń trzymana jest normalnie, wszystkie skiny chodzą jak CJ)
	...
	//-------<[ streamer ]>-------
    // SA-MP/MTA bridge exposes the classic per-player object pool (999 IDs).
    // Asking the legacy Streamer for 2000 objects makes it overrun that pool
    // during login and can stall the MTA client until AC #95 HB-timeout.
    Streamer_SetVisibleItems(STREAMER_TYPE_OBJECT, 999);
    Streamer_SetTickRate(50);
	//Streamer_AmxUnloadDestroyItems(false);
	...
	TextDrawColour(Textdraw[5], 16711935);
	...
	TextDrawColour(Textdraw[6], 16711935);
	...
	TextDrawColour(Textdraw[7], 16711935);
	...
	TextDrawColour(SpeedometerTD[12], 0);
	...
	TextDrawColour(SpeedometerTD[0], 0);
	...
	TextDrawColour(SpeedometerTD[1], 0);
	...
	TextDrawColour(SpeedometerTD[2], 0);
	...
	TextDrawColour(SpeedometerTD[3], -253);
	...
	TextDrawColour(SpeedometerTD[4], -253);
	...
	TextDrawColour(SpeedometerTD[5], -141);
	...
	TextDrawColour(SpeedometerTD[6], -141);
	...
	TextDrawColour(SpeedometerTD[7], -1768515841);
	...
	TextDrawColour(SpeedometerTD[8], -141);
	...
	TextDrawColour(SpeedometerTD[9], -253);
	...
	TextDrawColour(SpeedometerTD[10], -253);
	...
	TextDrawColour(SpeedometerTD[11], -141);
	...
	print("Ladowanie magazynow...");
	Warehouse_Load();

	print("Ladowanie laboratoriow...");
	DrugLab_Load();

	print("Ladowanie skrzynek...");
	Crates_Load();

	print("Ladowanie skrytek...");
	Storage_Load();

	mysql_query(Database, "UPDATE `mru_konta` SET `online` = 0"); //reset uzytkownikow online
	mysql_query(Database, "UPDATE `mru_konta` SET `connected` = 0"); //reset uzytkownikow online dzialajace
	...
	for(new playerid = 0; playerid < MAX_PLAYERS; playerid++){
		...
		TextDrawColour(TextDrawInfo[playerid],-1);
		...
		TextDrawColour(TextDrawEXP[playerid],-1);
		TextDrawShowForPlayer(playerid, TextDrawEXP[playerid]);

		HUDTXDINFO[playerid] = TextDrawCreate(600.000000, 2.000000, "~y~[ID: 1]~w~ Norbert Gierczak.");
        ...
        TextDrawColour(HUDTXDINFO[playerid], -1);
	...
	TimeUpdater();
	//timery
	SetTimer("AktywujPozar", 10800000, true);//System Pożarów v0.1
	SetTimer("MainTimer", 1000, true);
	SetTimer("InitVehicleParams", 2000, false);
	#if !defined MRP_MTA_RUNTIME
		SetTimer("CheckChangeWeapon", 450, true);
	#endif
    //SetTimer("RPGTimer", 100, true);
	//Ustalanie wartości wind
	levelLock[FRAC_SN][5]=1;//Zamknięte
    ...
    DB_FreeResultSet(DB_ExecuteQuery(db_handle, "CREATE TABLE IF NOT EXISTS mru_legal (pID integer,weapon1 integer not null,weapon2 integer not null,weapon3 integer not null,weapon4 integer not null,weapon5 integer not null,weapon6 integer not null,weapon7 integer not null,weapon8 integer not null,weapon9 integer not null,weapon10 integer not null,weapon11 integer not null,weapon12 integer not null,weapon13 integer not null,unique (pID));"));
    DB_FreeResultSet(DB_ExecuteQuery(db_handle, "CREATE TABLE IF NOT EXISTS mru_opisy(uid INTEGER PRIMARY KEY AUTOINCREMENT, text VARCHAR, owner INT, last_used INT);"));
    DB_FreeResultSet(DB_ExecuteQuery(db_handle, "CREATE TABLE IF NOT EXISTS mru_kevlar(pID integer, offsetX FLOAT, offsetY FLOAT, offsetZ FLOAT, rotX FLOAT, rotY FLOAT, rotZ FLOAT, scaleX FLOAT, scaleY FLOAT, scaleZ FLOAT);"));
	...
	#if !defined MRP_MTA_RUNTIME
		// weapon-config jest zależny od pakietów SA-MP; MTA obsługuje obrażenia i synchronizację natywnie.
	...
	Log(serverLog, WARNING,
		...
		Streamer_CountItems(STREAMER_TYPE_ACTOR));
	printf("[M-RP port] Streamer items: objects=%d pickups=%d mapicons=%d labels=%d areas=%d actors=%d",
		Streamer_CountItems(STREAMER_TYPE_OBJECT),
		Streamer_CountItems(STREAMER_TYPE_PICKUP),
		Streamer_CountItems(STREAMER_TYPE_MAP_ICON),
		Streamer_CountItems(STREAMER_TYPE_3D_TEXT_LABEL),
	...
	Log(serverLog, WARNING, "Serwer został pomyślnie uruchomiony.");
...
#if defined MRP_MTA_RUNTIME
forward MRP_GetStreamerItemCount(type);
...
public OnGameModeExit()
{
	...
	Log(serverLog, WARNING, "Serwer został wyłączony.");
...
public OnPlayerWeaponShot(playerid, WEAPON:weaponid, BULLET_HIT_TYPE:hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	switch (hittype)
	{
		...
		case BULLET_HIT_TYPE_PLAYER:
		{
			...
			/*
			if(PlayerAC[playerid][PlayerAimed] == 0)
			{
				new bool:validweapon = true;
				if (weaponid == WEAPON_COLT45 || weaponid == WEAPON_SAWEDOFF || weaponid == WEAPON_UZI || weaponid == WEAPON_MP5 || weaponid == WEAPON_TEC9 || weaponid == WEAPON_AK47 || weaponid == WEAPON_M4 || weaponid == WEAPON_MOLTOV || weaponid == WEAPON_FLAMETHROWER)
				{
					validweapon = false;
				}
				if(validweapon == true)
				{
    		        PlayerAC[playerid][acWarn_Triggerbot]++;
    		        new string[144];
    		        format(string, sizeof(string), "Gracz %s [%d] moze miec triggerbota!! (%s ilosc detekcji)", GetNick(playerid), playerid, PlayerAC[playerid][acWarn_Triggerbot]);
...
public UpdatePlayer3DName(playerid)
{
	...
	format(string, sizeof(string), "%s ~y~(%d)~n~~w~%s", GetNickEx(playerid), playerid, (strlen(duty) ? (duty) : ("")));
    PlayerTextDrawSetString(playerid, TDEditor_TD[playerid][20], string);
	//PlayerTextDrawSetPreviewModel(playerid, TDEditor_TD[playerid][10], PlayerInfo[playerid][pSkin]);
	PlayerTextDrawShow(playerid, TDEditor_TD[playerid][20]);
	//PlayerTextDrawShow(playerid, TDEditor_TD[playerid][10]);
    return 1;
}
    ...
    if(GetPVarInt(playerid, "patrol-map") == 1 && GetPVarInt(playerid, "patrolmap") == 1)
    {
        ...
        else
        {
            ...
            if((clickedid == PatrolAlfa[0] || clickedid == PatrolAlfa[1]) && Patrolujacych[0] < 4)
            {
                ...
                SendClientMessage(playerid, COLOR_LIGHTBLUE, "Strefa patrolu to ALFA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                if(GetPVarInt(playerid, "patrol-duo") == 1) SendClientMessage(PatrolInfo[pat][patroluje][1], COLOR_LIGHTBLUE, "Strefa patrolu to ALFA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                Patrolujacych[0]++;
                GetPlayerName(playerid, fnick[0], MAX_PLAYER_NAME);
                GetPlayerName(PatrolInfo[pat][patroluje][1], fnick[1], MAX_PLAYER_NAME);
                format(str, 128, "{FFFFFF}»»{6A5ACD} CENTRALA: {FFFFFF}%s{6A5ACD} 10-30 na strefę Alfa, skład %s %s", PatrolInfo[pat][patname], fnick[0], fnick[1]);
            }
            else if((clickedid == PatrolBeta[0] || clickedid == PatrolBeta[1]) && Patrolujacych[1] < 4)
            {
                PatrolInfo[pat][patstrefa] = 2;
                SendClientMessage(playerid, COLOR_GREEN, "Strefa patrolu to BETA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                if(GetPVarInt(playerid, "patrol-duo") == 1) SendClientMessage(PatrolInfo[pat][patroluje][1], COLOR_GREEN, "Strefa patrolu to BETA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                Patrolujacych[1]++;
                GetPlayerName(playerid, fnick[0], MAX_PLAYER_NAME);
                GetPlayerName(PatrolInfo[pat][patroluje][1], fnick[1], MAX_PLAYER_NAME);
                format(str, 128, "{FFFFFF}»»{6A5ACD} CENTRALA: {FFFFFF}%s{6A5ACD} 10-30 na strefę Beta, skład %s %s", PatrolInfo[pat][patname], fnick[0], fnick[1]);
            }
            else if((clickedid == PatrolGamma[0] || clickedid == PatrolGamma[1]) && Patrolujacych[2] < 6)
            {
                PatrolInfo[pat][patstrefa] = 3;
                SendClientMessage(playerid, COLOR_RED, "Strefa patrolu to GAMMA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                if(GetPVarInt(playerid, "patrol-duo") == 1) SendClientMessage(PatrolInfo[pat][patroluje][1], COLOR_RED, "Strefa patrolu to GAMMA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                Patrolujacych[2]++;
                GetPlayerName(playerid, fnick[0], MAX_PLAYER_NAME);
                GetPlayerName(PatrolInfo[pat][patroluje][1], fnick[1], MAX_PLAYER_NAME);
                format(str, 128, "{FFFFFF}»»{6A5ACD} CENTRALA: {FFFFFF}%s{6A5ACD} 10-30 na strefę Gamma, skład %s %s", PatrolInfo[pat][patname], fnick[0], fnick[1]);
            }
            else if((clickedid == PatrolDelta[0] || clickedid == PatrolDelta[1]) && Patrolujacych[3] < 6)
            {
                PatrolInfo[pat][patstrefa] = 4;
                SendClientMessage(playerid, COLOR_YELLOW, "Strefa patrolu to DELTA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                if(GetPVarInt(playerid, "patrol-duo") == 1) SendClientMessage(PatrolInfo[pat][patroluje][1], COLOR_YELLOW, "Strefa patrolu to DELTA.{FFFFFF} Udaj się w to miejsce i pamiętaj o kodach radiowych!");
                Patrolujacych[3]++;
                GetPlayerName(playerid, fnick[0], MAX_PLAYER_NAME);
                GetPlayerName(PatrolInfo[pat][patroluje][1], fnick[1], MAX_PLAYER_NAME);
                format(str, 128, "{FFFFFF}»»{6A5ACD} CENTRALA: {FFFFFF}%s:{6A5ACD} 10-30 na strefę Delta, skład %s %s", PatrolInfo[pat][patname], fnick[0], fnick[1]);
            }
            else SendClientMessage(playerid, COLOR_YELLOW, "Brak wolnego miejsca w tej strefie, spróbuj inną.");
    ...
    else if(GetPVarInt(playerid, "patrolmap") == 1)
    {
        ...
        for(new i=0;i<MAX_PATROLS;i++)
        {
            if(clickedid == PatrolMarker[i] && PatrolInfo[i][pataktywny] == 1)
            {
                ...
                {
                    case 1: stanp="Sytuacja pod kontrolą";
                    case 2: stanp="Potrzebne wsparcie";
                    case 3: stanp="Pościg za podejrzanym";
...
{
	Turf_OnPlayerEnterDynamicArea(playerid, areaid);
...
{
	ProcessMinigameClick(playerid, playertextid);
	PlayerClickPlayerTextDrawRuleta(playerid, playertextid);
	if(GetPVarInt(playerid, "gatechose_active") == 1)   //Barierki
    {
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	if(PlayerInfo[playerid][pJailed] == 2)
	{
		...
		SendClientMessage(playerid, COLOR_GRAD2, "Strażnik zauważył, że coś kombinujesz. Wracasz do celi.");
	...
	{
		Player_RemoveFromVeh(playerid);
		ShowPlayerInfoDialog(playerid, "M-RP", "{FF542E}Jesteś sparaliżowany!\n{FFFFFF}Nie możesz wsiąść do pojazdu.");
		TogglePlayerControllable(playerid, 0);
		ApplyAnimation(playerid, "CRACK","crckdeth2",4.1,0,1,1,1,1,1);
		return 1;
	}
	//Sila
	if(GetPVarInt(playerid, "RozpoczalBieg") == 1)//Zabezpieczenie, jeśli jest podczas biegu
	{
		sendTipMessage(playerid, "Nie możesz wejść do pojazdu podczas biegu!"); 
		GetPlayerPos(playerid, pX,pY,pZ);
		SetPlayerPos(playerid, pX,pY,pZ+2);
	}
	if(MaPaczke[playerid] == 1 || PodnoszeniePaczki[playerid] == 1)
	{
		sendTipMessage(playerid, "Nie możesz wejść do pojazdu w trakcie pracy jako magazynier"); 
		GetPlayerPos(playerid, pX,pY,pZ);
		SetPlayerPos(playerid, pX,pY,pZ+2);
	}
	if(GetPlayerPing(playerid) >= 800 && !ispassenger)//Zabezpieczenie, jeśli ma za duży ping
	{
		sendTipMessage(playerid, "Twój ping jest stanowczo za wysoki! Odczekaj chwilę, zanim wsiądziesz do pojazdu"); 
		GetPlayerPos(playerid, pX,pY,pZ);
		SetPlayerPos(playerid, pX,pY,pZ+2);
	}
	if(GetVehicleModel(vehicleid) == 570 && ispassenger)
	{
		...
		{
			Player_RemoveFromVeh(playerid);
			ShowPlayerInfoDialog(playerid, "M-RP", "{FF542E}Nie posiadasz biletu!\n{FFFFFF}Kup go na dworcu za pomocą /kupbiletpociąg(/kbpo)");
		}
	}
	...
	else if (GetVehicleModel(vehicleid) == 525) sendTipMessageEx(playerid, COLOR_BROWN, "Wsiadłeś do holownika, naciśnij CTRL alby podholować wóz.");
	...
	if(!IsABoat(vehicleid) && !IsABike(vehicleid) && !IsACarBezSzyb(vehicleid)){
		new anyWindowOpen = false;
		if(IsVehicleWindowOpened(vehicleid, FL_DOOR) || IsVehicleWindowOpened(vehicleid, FR_DOOR) || 
		   IsVehicleWindowOpened(vehicleid, BL_DOOR) || IsVehicleWindowOpened(vehicleid, BR_DOOR)) {
			anyWindowOpen = true;
		}
		
		if(!anyWindowOpen) {
			SendClientMessage(playerid, COLOR_BIALY, "Pamietaj: masz zamknieta szybe w pojezdzie. Mozesz ja podniesc komenda /vszyba.");
		}
	}
	return 1;
...
public OnPlayerConnect(playerid)
{
	...
	OnPlayerConnectRuleta(playerid);
	EnablePlayerCameraTarget(playerid, 1);
	new strGPCI[41];
	gpci(playerid, strGPCI, sizeof(strGPCI));
	Log(connectLog, WARNING, "Gracz %s[id: %d, ip: %s, gpci: %s] połączył się z serwerem", GetNickEx(playerid), playerid, GetIp(playerid), strGPCI);
	...
	{
		SendClientMessage(playerid, COLOR_NEWS, "SERWER: Twój nick jest niepoprawny! Nick musi posiadać formę: Imię_Nazwisko!");
		KickEx(playerid);
		return 1;
	}*/
	...
	{
		SendClientMessage(playerid, COLOR_NEWS, "SERWER: Twój nick zawiera wulgaryzmy/niedozwolone słowa - zmień go!"); 
		KickEx(playerid);
		return 1;
	}
	SetRPName(playerid);
    ...
    SetPlayerMapIcon(playerid, 3, 544.3761, -1276.2046, 17.2482, 55, 0); //Grotti (wypożyczalnia aut)
    SetPlayerMapIcon(playerid, 4, 501.9365, -1358.5668, 16.1252, 45, 0); //Prolaps (za San News)
    SetPlayerMapIcon(playerid, 5, 1498.39, -1581.62, 13.54, 56, 0); //Urząd pracy
    ...
    SetPlayerMapIcon(playerid, 14, 1038.1844, -1339.7595, 13.7266, 17, 0); //Pączkarnia Allen
    ...
    SetPlayerMapIcon(playerid, 21, 2255.2629, -1333.2920, 23.9816, 12, 0); //Kościół
    ...
    SetPlayerMapIcon(playerid, 35, 1310.2568, -1370.4567, 13.3031, 34, 0); //Urząd
    SetPlayerMapIcon(playerid, 36, 1481.2053, -1768.3350, 18.5228, 34, 0); //Sąd
    SetPlayerMapIcon(playerid, 37, 913.59, -1003.91, 37.99, 30, 0); //Komisariat Główny
    ...
    SetPlayerMapIcon(playerid, 40, 2227.1052, -1723.2871, 13.2840, 54, 0); //Siłownia
    ...
    SetPlayerMapIcon(playerid, 43, 2269.7112, -74.8501, 26.7724, 34, 0); //Urząd w PC
    ...
    SetPlayerMapIcon(playerid, 51, 1352.4242, -1758.4613, 13.5078, 36, 0); //24/7 obok Urzędu
    SetPlayerMapIcon(playerid, 52, 1109.1722, -1796.2472, 16.5938, 56, 0); //Złodziej Aut
    ...
    SetPlayerMapIcon(playerid, 58, 1787.4432, -1866.6737, 13.5711, 52, 0); //Bankomat obok Dworca Głównego
    SetPlayerMapIcon(playerid, 59, 1833.0537, -1842.6494, 13.5781, 36, 0); //24/7 na Idlewood
    SetPlayerMapIcon(playerid, 62, 382.8541, -2079.4890, 7.5630, 9, 0); //Miejsce do wędkowania
    ...
    SetPlayerMapIcon(playerid, 65, 2273.1931, -77.6219, 26.5704, 52, 0); //Bankomat obok Urzędu w PC
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    Poker_OnPlayerDisconnect(playerid, reason);
	OnPlayerDisconnectRuletka(playerid);
	...
	Log(connectLog, WARNING, "Gracz %s[id: %d] rozłączył się, powód: %d", GetNickEx(playerid), playerid, reason);
	...
	mysql_query_format("UPDATE `mru_konta` SET `connected`=0, `online` = 0 WHERE `UID` = %d", PlayerInfo[playerid][pUID]);
	//test
    ...
    if(Spectate[playerid] == INVALID_PLAYER_ID)
    {
		...
		if(GetPVarString(playerid, "maska_nick", nick, 24))
			format(reString, sizeof(reString), "SERWER: Gracz znajdujący się w pobliżu wyszedł z serwera (%s, powód: %s [%s]).", GetNick(playerid), DisconnectReason[reason], GetNickEx(playerid));
		else
    		format(reString, sizeof(reString), "SERWER: Gracz znajdujący się w pobliżu wyszedł z serwera (%s, powód: %s).", GetNick(playerid), DisconnectReason[reason]);
	...
	if(!IsPlayerNPC(playerid)){
		if(gPlayerLogged[playerid] != 0){
			new left[128];
			format(left, sizeof(left), "[Left] %s (%d) wyszedł z serwera (%s).", GetNickEx(playerid), GetPlayerIDFromName(GetNick(playerid)), DisconnectReason[reason]);
			OOCLeft(left);
		}
	}
	...
	{
		new string[128];
		format(string, sizeof(string), "[Marcepan Marks] Zabrałem graczowi %s broń [dał /q podczas AJ DM2]", GetNickEx(playerid));
    ...
    if(GetPVarInt(playerid, "budka-Mobile") != 999) {
        ...
        if(GetPVarInt(caller, "budka-Mobile") == playerid) {
            sendTipMessage(caller, "**biiip biiip** połączenie zostało przerwane ((Wyjście z gry))", COLOR_PAPAYAWHIP);
    ...
    {
		SendPlayerMessageToAll(COLOR_NEWS, "(( Wywiad zakończony - gracz wyszedł z gry ))");
	...
	if(Mobile[playerid] != INVALID_PLAYER_ID)
	{
		...
		{
			SendClientMessage(Mobile[playerid], COLOR_YELLOW, "Gracz, z którym prowadziłeś rozmowę telefoniczną, wyszedł z gry.");
		}
		StopACall(playerid);
	}
	if(firstDutyAdmin[playerid] == 1 && PlayerInfo[playerid][pAdmin] > 0
	|| firstDutyAdmin[playerid] == 1 && PlayerInfo[playerid][pNewAP] > 0
	|| firstDutyAdmin[playerid] == 1 && IsAScripter(playerid))//Jeżeli admin był na duty, wykonuje zapis w logi 
	{
		...
		//Log dla 0Verte [Nick][UID] [HH:mm] [Bany] [Warny] [AJ] [Kicki] [Inne] [Reporty+zapytania] [/w] [/w2] [powod zakończenia służby]
	...
	|| (IsAScripter(playerid) && iloscPozaDuty[playerid] >= 1))//Gdy nie był na admin duty, ale wykonywał akcje
	{
	...
	else if(Kajdanki_Uzyte[playerid] != 0) //gdy skuwający da /q
	{
    ...
    if(GetPVarInt(playerid, "kostka"))
    {
        ...
        SendClientMessage(id, COLOR_RED, "Współzawodnik opuścił serwer, otrzymujesz zwrot wpłaconej kwoty wraz z podatkiem.");
    ...
    if(PlayerTied[playerid] >= 1 || (PlayerCuffed[playerid] >= 1 && pobity[playerid] == 0 && PlayerCuffed[playerid] < 3) || Kajdanki_JestemSkuty[playerid] >= 1 || poscig[playerid] == 1)
	{
        ...
        //format(string, 130, "%s dostanie Marcepana za możliwe: %s (%s)", GetNickEx(playerid), powod, codal); marcepan
		format(string, 130, "%s - %s podczas akcji %s", GetNickEx(playerid), codal, powod);
        SendAdminMessage(COLOR_P@, string);
	}

	UnCuffedAction(playerid); //Usuwanie kajdanek przy rozłączeniu
    ...
    if(GetPVarInt(playerid, "patrol") != 0) {
        ...
        {
        	sendTipMessageEx(PatrolInfo[patrol][patroluje][0], COLOR_PAPAYAWHIP, "Partner opuścił patrol. 10-33!");
			RunCommand(PatrolInfo[patrol][patroluje][0], "/patrol", "stop");
		}
		if(PatrolInfo[patrol][patroluje][1] != INVALID_PLAYER_ID)
		{
        	sendTipMessageEx(PatrolInfo[patrol][patroluje][1], COLOR_PAPAYAWHIP, "Partner opuścił patrol. 10-33!");
    ...
    if(ScigaSie[playerid] != 666 && IloscCH[playerid] != 0)
	{
	    ...
	    format(string, sizeof(string), "Wyścig: {FFFFFF}%s wyszedł z gry", sendername);
    	foreach(new i : Player)
    	{
	...
	if(GotHit[playerid] > 0)
	{
	    if(GetChased[playerid] < 500)
	    {
	        ...
	        {
	        	SendClientMessage(GetChased[playerid], COLOR_YELLOW, "Twój cel opuścił serwer.");
...
{
	format(PlayerInfo[playerid][pDeathAnimLib], 32, "%s", "SWEET");
	format(PlayerInfo[playerid][pDeathAnimName], 32, "%s", "Sweet_injuredloop");
...
public OnPlayerDamageDone(playerid, Float:amount, issuerid, WEAPON:weapon, bodypart)
{
	if(!IsPlayerNPC(playerid) && !IsPlayerNPC(issuerid))
	{
		if(issuerid != INVALID_PLAYER_ID)
		{
			...
			if(amount > 0 && issuerid != playerid) {
				CreateNewCombatLog(playerid);
				CreateNewCombatLog(issuerid);
			}
		}
...
public OnPlayerDamage(&playerid, &Float:amount, &issuerid, &WEAPON:weapon, &bodypart)
{
	...
	if(issuerid != INVALID_PLAYER_ID) // PvP
    {
		...
		if(GetPlayerWeapon(issuerid) == 23 && GetDistanceBetweenPlayers(playerid,issuerid) < 30)
		{
			if(GroupPlayerDutyPerm(issuerid, PERM_POLICE)) {
				if(gettime() - GetPVarInt(issuerid, "lastDamage") < 60) return sendErrorMessage(issuerid, "Nie możesz tego użyć podczas walki!");
				if(gettime() - GetPVarInt(playerid, "lastDamage") < 60) return sendErrorMessage(issuerid, "Nie możesz tego użyć podczas walki!");
				SetTimerEx("DostalTazerem", 30000, false, "i", playerid);
				new string[128];
				GameTextForPlayer(playerid, "DOSTALES TAZEREM! ODCZEKAJ CHWILE!", 3000, 5);
				GameTextForPlayer(issuerid, "~g~TRAFILES W GRACZA!!", 3000, 5);
		...
		if(IsPlayerInAnyVehicle(playerid))
		{
			...
			if(CarData[VehicleUID[carID][vUID]][c_UpgradeWindows] == 1 && carHealth > 500.0 && !IsPlayerInDriveByMode(playerid))
			{
				return 0;
			}
		}
...
public OnPlayerDeath(playerid, killerid, WEAPON:reason)
{
	...
	if(!IsPlayerNPC(playerid))
	{
		...
		Log(damageLog, WARNING, "%s został zabity przez %s, powód: %d, strefa: %s", 
			GetPlayerLogName(playerid),
			IsPlayerConnected(killerid) ? GetPlayerLogName(killerid) : sprintf("%d", killerid),
			reason, Area_GetName(PlayerInfo[playerid][pCurrentArea], false)
		);
	}
	GetPlayerPos(playerid, PlayerInfo[playerid][pPos_x], PlayerInfo[playerid][pPos_y], PlayerInfo[playerid][pPos_z]);
	...
	if(IsPlayerConnected(playerid))
	{
		...
		if(GetPVarInt(playerid, "skip_bw") == 0)
		{
			if(PlayerInfo[playerid][pInjury] > 0) //TRYB BW (GDY ZGINIE JAK MA RANNEGO)
			{
				...
				if(TalkingLive[playerid] != INVALID_PLAYER_ID)
				{
					SendPlayerMessageToAll(COLOR_NEWS, "NEWS: Wywiad zakończony - nasz rozmówca przerwał wywiad.");
					new talker = TalkingLive[playerid];
					TalkingLive[playerid] = INVALID_PLAYER_ID;
					TalkingLive[talker] = INVALID_PLAYER_ID;
				}
				...
				if(Mobile[playerid] != INVALID_PLAYER_ID)
				{
					SendClientMessage(playerid, COLOR_YELLOW, "Jesteś ranny - połączenie zakończone.");
					if(Mobile[playerid] >= 0)
					{
						SendClientMessage(Mobile[playerid], COLOR_YELLOW, "Słychać nagły trzask i połączenie zostaje zakończone.");
					}
					StopACall(playerid);
				}
				...
				if(ScigaSie[playerid] != 666 && IloscCH[playerid] != 0)
				{
					format(string, sizeof(string), "Wyścig: {FFFFFF}%s zginął jak prawdziwy ścigant [*]", GetNickEx(playerid));
				...
				{
					lowcaz[playerid] = 501;
					SendClientMessage(playerid, COLOR_YELLOW, "Zlecenie zostało anulowane - nie możesz wziąć teraz zlecenia na tego samego gracza!");
				}
				if(GetPVarInt(playerid, "ZjadlDragi") == 1)
				{
					new FirstValue = GetPVarInt(playerid, "FirstValueStrong");
					SetPVarInt(playerid, "ZjadlDragi", 0);
					sendTipMessage(playerid, "Z powodu śmierci twój boost (dragów) został wyłączony, zażyj kolejną dawkę!"); 
					KillTimer(TimerEfektNarkotyku[playerid]);
					SetStrong(playerid, FirstValue);
				}
				...
				if(IsPlayerConnected(killerid))
				{
					...
					if(PlayerInfo[playerid][pHeadValue] > 0) //hitmani muszą dobić, żeby zaliczyło kontrakt
					{
						if(IsPlayerInGroup(killerid, 8) || PlayerInfo[killerid][pLider] == 8)
						{
							if(GoChase[killerid] == playerid)
							{
								//jeśli zabity miał kajdanki
								if(Kajdanki_JestemSkuty[playerid] != 0) // gdy skuty da /q
								{
									format(string, sizeof(string), "* Więzień %s został zastrzelony przez Hitmana (MK). Następnym razem zadbaj o bezpieczeństwo swojego wiꟸnia *", GetNick(playerid));
						...
						if(count == 1 || count == 11 || count == 22 || count == 33 || count == 44 || count == 55 || count == 2)
						{
							if(!(GroupPlayerDutyPerm(playerid, PERM_POLICE)))
							{
								...
								if(count == 1 || count == 11 || count == 22 || count == 33 || count == 44 || count == 55)
								{
									...
									format(string, sizeof(string), "* Jesteś w więzieniu na %d Sekund i straciłeś $%d gdyż uciekałeś lub strzelałeś do funkcjonariusza policji.", PlayerInfo[playerid][pJailTime], CenaZabicia);
									SendClientMessage(playerid, COLOR_LIGHTRED, string);
									SendClientMessage(playerid, COLOR_LIGHTBLUE, "Jeżeli nie chcesz aby taka sytuacja powtórzyła się w przyszłości, skorzystaj z usług prawnika który zbije twój WL.");
									WantLawyer[playerid] = 1;
								}
								else if(count == 2)
								{
									PlayerInfo[playerid][pJailed] = 2;
									format(string, sizeof(string), "* Jesteś w DeMorgan na %d Sekund i straciłeś $%d gdyż uciekałeś lub strzelałeś do funkcjonariusza policji", PlayerInfo[playerid][pJailTime], CenaZabicia);
									SendClientMessage(playerid, COLOR_LIGHTRED, string);
									SendClientMessage(playerid, COLOR_LIGHTBLUE, "Jeżeli nie chcesz aby taka sytuacja powtórzyła się w przyszłości, skorzystaj z usług prawnika który zbije twój WL.");
								}
								return 1; //zrespawnuj gracza w więzieniu
							}
						}
					}
			...
			else //TRYB RANNEGO
			{
				...
				{
					//kajdanki
					if(Kajdanki_Uzyte[playerid] != 0) //gdy skuwający dostanie rannego
					{
					...
					if(IsPlayerConnected(killerid))
					{
						...
						if(PlayerInfo[playerid][pHeadValue] > 0) //hitmani muszą dobić, żeby zaliczyło kontrakt
						{
							if(IsPlayerInGroup(killerid, 8) || PlayerInfo[killerid][pLider] == 8)
							{
								if(GoChase[killerid] == playerid)
								{
									format(string, sizeof(string), "* Dobij %s, żeby wypełnić kontrakt *", GetNick(playerid));
...
public OnPlayerSpawn(playerid)
{
	...
	// The MTA AMX bridge can deliver this callback before the streamer's first
	// periodic player update. Refresh it explicitly so dynamic entrances,
	// objects and labels become visible immediately after login and respawn.

	//turfy @_@
	for(new z = 0; z < MAX_GZONE; z++)
	{
		new zoneid = gZoneInfo[z][gZoneID];
		if(zoneid == -1) continue;
		new gangid = gZoneInfo[z][gZoneGangID];
		// set alpha (low byte) to 0x66 for RGBA format
		new color = ((0xFFFFFFFF & 0xFFFFFF00) | 0x66);
		if(gangid > 0) {
			new flashColor = ((GroupInfo[gangid][g_Color] & 0xFFFFFF00) | 0x66);
			color = flashColor;
		}
	...
	{
		sendErrorMessage(playerid, "Zespawnowałeś się, a nie jesteś zalogowany! Zostałeś wyrzucony z serwera.");
		KickEx(playerid);
		return 0;
	}
	else
    ...
    if(PlayerInfo[playerid][pPK] > 24) {
        format(string, sizeof(string), "* Przekroczyłeś limit 24 PK. Tracisz prawo jazdy na 1 DZIEÑ");
    ...
    if(GroupIsLeader(playerid, FRAC_SN))
    {
...
#if defined MRP_MTA_RUNTIME
	forward MRP_MTA_RestoreHUD(playerid);
	public MRP_MTA_RestoreHUD(playerid)
	{
...
SetPlayerSpawnPos(playerid)
{
	...
	else if(PlayerInfo[playerid][pJailed] == 1)
	{
		...
		SendClientMessage(playerid, COLOR_LIGHTRED, "Twój wyrok nie dobiegł końca, wracasz do więzienia.");
		TogglePlayerControllable(playerid, 0);
		Wchodzenie(playerid);
	}
	...
	{
		if(PlayerInfo[playerid][pInjury] > 0) ZdejmijBW(playerid, 3000);
		SendClientMessage(playerid, COLOR_LIGHTRED, "Twój wyrok nie dobiegł końca, wracasz do więzienia stanowego");
	...
	/*MARCEPAN IS OFF else if(PlayerInfo[playerid][pJailed] == 10)//Marcepan Admin Jail
	{
	    ...
	    new sendername[MAX_PLAYER_NAME + 1];
	    GetPlayerName(playerid, sendername, sizeof(sendername));
		format(string, sizeof(string), "* Zostałeś uwieziony w Admin Jailu przez Admina Marcepan_Marks. Powod: /q podczas akcji");
		SendClientMessage(playerid, COLOR_LIGHTRED, string);
		ResetPlayerWeapons(playerid);
		UsunBron(playerid);
		...
		PlayerInfo[playerid][pMuted] = 1;
		SetPlayerPos(playerid, 1481.1666259766,-1790.2204589844,156.7875213623);
		format(string, sizeof(string), "Zostałeś ukarany na 15 minut. Powod: /q podczas akcji");
		SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
		format(string, sizeof(string), "AdmCmd: %s zostal uwieziony w 'AJ' przez Admina Marcepan_Marks. Powod: /q podczas akcji + zabieram połowę kasy i broń", sendername);
		SendClientMessageToAll(COLOR_LIGHTRED, string);
		format(string, sizeof(string), "Dodatkowo zabrano z twojego portfela %d$ i wyzerowano twoje bronie oraz zabrano połowę matsów", kaseczka);
        SendClientMessage(playerid, COLOR_LIGHTRED, string);
        Log(punishmentLog, WARNING, "%s dał /q podczas akcji więc zabrano mu %d$, %d materiałów oraz broń.", GetPlayerLogName(playerid), kaseczka, TakeMats(playerid, CountMats(playerid)/2));
    ...
    else
    {
		...
		if(GetPVarInt(playerid, "spawn") == 2 || (GetCombatLogByPUID(PlayerInfo[playerid][pUID]) >= 0 && FirstSpawn[playerid] > 0 /* XDDDD */))
		{
			Wchodzenie(playerid);
			SetPlayerPos(playerid, PlayerInfo[playerid][pPos_x], PlayerInfo[playerid][pPos_y], PlayerInfo[playerid][pPos_z]);
			SetPlayerInterior(playerid, PlayerInfo[playerid][pIntSpawn]);
			SetPlayerVirtualWorld(playerid, PlayerInfo[playerid][pVW]);
			SetTimerEx("Storage_CheckPlayerOnSpawn", 1500, false, "i", playerid);
			if(GetPLocal(playerid) == PLOCAL_INNE_BANK || GetPLocal(playerid) == PLOCAL_FRAC_DMV)
	        {
				sendTipMessage(playerid, "W banku nie wolno mieć broni! Zostanie Ci ona przywrócona po śmierci.");
		...
		{
			#if !defined MRP_LEGACY_HOUSES
			if(PlayerInfo[playerid][pSpawnHouseID] > -1) //Spawn w domu, nowy system.
	  		{
				new houseid = PlayerInfo[playerid][pSpawnHouseID];
				if(House[houseid][h_Owner] != PlayerInfo[playerid][pUID]) {
					PlayerInfo[playerid][pSpawnHouseID] = -1;
					SendClientMessage(playerid, COLOR_YELLOW, "Juz nie jestes wlascicielem tego domu, spawnujesz sie jako cywil.");
		    ...
		    if(PlayerInfo[playerid][pSpawn] == 0 && PlayerInfo[playerid][pSpawnHouseID] == -1) //Normalny spawn
		    {
		        SetPlayerInteriorEx(playerid, 0);
		        PlayerInfo[playerid][pLocal] = 255;
				SetPlayerVirtualWorld(playerid, 0);
				if(PlayerInfo[playerid][pGrupa][0] > 0 || PlayerInfo[playerid][pGrupa][1] > 0 || PlayerInfo[playerid][pGrupa][2] > 0 || PlayerInfo[playerid][pGrupa][3] > 0 || PlayerInfo[playerid][pGrupa][4] > 0)
				{
					...
					if(group > 0)
					{
						...
						{
							SendClientMessage(playerid, COLOR_YELLOW, "Twoja grupa nie ma jeszcze spawnu - spawnujesz się jako cywil");
							SetPlayerPos(playerid, gRandomPlayerSpawns[0][0], gRandomPlayerSpawns[0][1], gRandomPlayerSpawns[0][2]);
							SetPlayerFacingAngle(playerid, gRandomPlayerSpawns[0][3]);
							SetPlayerVirtualWorld(playerid, 0);
							SetPlayerInterior(playerid, 0);
					...
					{
						SendClientMessage(playerid, COLOR_YELLOW, "Twój wybór spawnu grupy jest niepoprawny - spawnujesz się jako cywil");
						SetPlayerPos(playerid, gRandomPlayerSpawns[0][0], gRandomPlayerSpawns[0][1], gRandomPlayerSpawns[0][2]);
						SetPlayerFacingAngle(playerid, gRandomPlayerSpawns[0][3]);
						SetPlayerVirtualWorld(playerid, 0);
						SetPlayerInterior(playerid, 0);
				...
				else if(PlayerInfo[playerid][pJob] > 0 && PlayerInfo[playerid][pSpawnHouseID] == -1) //Spawn Prac
				{
		    ...
		    else if(PlayerInfo[playerid][pSpawn] == 1 && PlayerInfo[playerid][pSpawnHouseID] == -1) //Spawn przed domem / motelem
		    {
		        new i;
				...
				else
				{
					if(PlayerInfo[playerid][pMotRoom] != 0) 
					{
						...
						if(MotelRooms[i][mtrOwnerUID] == PlayerInfo[playerid][pUID]) // Sprawdza czy gracz jest jeszcze właścicielem
						{
						...
						} else PlayerInfo[playerid][pMotRoom] = 0; // Gracz nie jest już właścicielem, można ustawić na 0
					}
	  		...
	  		else if(PlayerInfo[playerid][pSpawn] == 2 && PlayerInfo[playerid][pSpawnHouseID] == -1) //Spawn w środku domu / motelem
	  		{
	  		    new i, h, m;
                if(PlayerInfo[playerid][pDom] != 0)
           	 		i = PlayerInfo[playerid][pDom];
	...
	if(GetPVarInt(playerid, "RozpoczalBieg") == 1)
	{
		if(GetPVarInt(playerid, "WybralBieg") == 1)
		{
			if(OszukujewBiegu[playerid] == 0)
			{
				...
				{
					CreateNewRunCheckPoint(playerid, 1709.3523,-1461.3938,13.5469, 3, "Zaliczyłeś pierwszy przystanek, kolejny jest już oznaczony!", 0, true);
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 1)
				{
					CreateNewRunCheckPoint(playerid, 1707.8762,-1584.3118,13.5453, 3, "Zaliczyłeś drugi przystanek, kolejny jest już oznaczony!", 0, true); 
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 2)
				{
					CreateNewRunCheckPoint(playerid, 1625.7415,-1608.9004,13.7188, 3, "Zaliczyłeś trzeci przystanek, kolejny jest już oznaczony!", 5,true); 
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 3)
				{
					CreateNewRunCheckPoint(playerid, 1538.9513,-1724.1267,13.5469, 3, "Zaliczyłeś czwarty przystanek, kolejny jest już oznaczony!", 0, true); 
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 4)
				{
					CreateNewRunCheckPoint(playerid, 1322.6306,-1724.9469,13.5469, 3, "Zaliczyłeś piąty przystanek, kolejny jest już oznaczony!", 0, true); 
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 5)
				{
					CreateNewRunCheckPoint(playerid, 1318.4052,-1841.7726,13.5469, 3, "Zaliczyłeś szósty przystanek, kolejny jest już oznaczony!", 0, true); 
				}
				if(GetPVarInt(playerid, "ZaliczylBaze") == 6)
				{
					CreateNewRunCheckPoint(playerid,  1382.2340,-1811.7761,13.5469, 3, "Zaliczyłeś siódmy (przedostatni) przystanek, kolejny jest już oznaczony!", 0, true); 
				}
		...
		else if(GetPVarInt(playerid, "WybralBieg") == 2)
		{
			if(GetPVarInt(playerid, "ZaliczylBaze") == 0)
			{
				CreateNewRunCheckPoint(playerid,  535.0668,-1364.9790,15.8432, 2, "Zaliczyłeś pierwszy checkpoint! Następny został już oznaczony", 0, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 1)
			{
				CreateNewRunCheckPoint(playerid,  339.2540,-1526.9476,33.3757, 2, "Zaliczyłeś drugi checkpoint! Następny został oznaczony", 0, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 2)
			{
				CreateNewRunCheckPoint(playerid,  317.4830,-1632.8326,33.3125, 2, "Zaliczyłeś trzeci checkpoint! Następny został oznaczony, a ty się nie poddajesz!", 5, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 3)
			{
				CreateNewRunCheckPoint(playerid,  364.1078,-1805.8809,7.8380, 2, "Zaliczyłeś czwarty checkpoint! Piąty jest już oznaczony!", 0, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 4)
			{
				CreateNewRunCheckPoint(playerid,  664.4612,-1859.3246,5.4609, 2, "Zaliczyłeś Piąty checkpoint! Zostały jeszcze dwa!", 5, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 5)
			{
			...
			if(GetPVarInt(playerid, "ZaliczylBaze") == 6)
			{
				CreateNewRunCheckPoint(playerid,  1000.8669,-1857.4419,12.8146, 2, "Zaliczyłeś siódmy checkpoint! Już widać ostatni", 5, true); 
			}
			if(GetPVarInt(playerid, "ZaliczylBaze") == 7)//Ostatni
			{
		...
		else//Jeśli wybrał dialog do pokazania punktu startowego trasy
		{
			DisablePlayerCheckpoint(playerid);
			sendTipMessageEx(playerid, COLOR_P@, "Aby rozpocząć bieg wpisz w tym miejscu [/biegnij]"); 
			SetPVarInt(playerid, "RozpoczalBieg", 0);
		}
	}
	...
	else if(CP[playerid]==1)
	{
	    ...
	    { 
			...
			if(IsPlayerInRangeOfPoint(playerid, 50, -1548.3618,123.6438,3.2966))
			{
				...
				{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 2, będziesz więcej zarabiać oraz szybciej ukraść nowe auto."); }
				else if(PlayerInfo[playerid][pJackSkill] == 100)
				{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 3, będziesz więcej zarabiać oraz szybciej ukraść nowe auto."); }
				else if(PlayerInfo[playerid][pJackSkill] == 200)
				{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 4, będziesz więcej zarabiać oraz szybciej ukraść nowe auto."); }
				else if(PlayerInfo[playerid][pJackSkill] == 400)
				{ SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 5, będziesz najwięcej zarabiać oraz najszybciej kraść auta."); }
				new level = PlayerInfo[playerid][pJackSkill];
				if(level >= 0 && level <= 50)
				{
					new money = PRICE_STEAL_CAR_1;
					new finalPrice = money;
					if(IsPlayerPremiumOld(playerid)) { finalPrice += floatround(finalPrice * 0.1); }
					new h, m, s;
					gettime(h, m, s);
					if(h >= 18) { finalPrice += floatround(finalPrice * 0.15); }
					format(string, sizeof(string), "Sprzedałeś pojazd za $%d, następny pojazd możesz ukraść za 15 minut.", finalPrice);
					SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
					DajKaseDone(playerid, finalPrice);//moneycheat
	    ...
	    if(FirstKartWinner == 999)
	    {
	        ...
	        foreach(new i : Player)
	        {
	            if(IsPlayerConnected(i))
	            {
		            ...
		            {
		                format(string, sizeof(string), "* %s ukończyłeś wyścig jako pierwszy !",name);
	    ...
	    else if(SecondKartWinner == 999)
	    {
	        ...
	        foreach(new i : Player)
	        {
	            if(IsPlayerConnected(i))
	            {
		            if(PlayerKarting[i] != 0 && PlayerInKart[i] != 0)
		            {
		                format(string, sizeof(string), "* %s ukończyłeś wyścig jako drugi !",name);
		                SendClientMessage(i, COLOR_WHITE, string);
		            }
				}
			}
	    ...
	    else if(ThirdKartWinner == 999)
	    {
	        ...
	        foreach(new i : Player)
	        {
	            if(IsPlayerConnected(i))
	            {
		            if(PlayerKarting[i] != 0 && PlayerInKart[i] != 0)
		            {
		                format(string, sizeof(string), "* %s ukończyłeś wyścig jako trzeci.",name);
		                SendClientMessage(i, COLOR_WHITE, string);
		                SendClientMessage(i, COLOR_WHITE, "** Koniec wyścigu **");
		                CP[i] = 0;
		                DisablePlayerCheckpoint(i);
		            }
				}
	...
	else if(zawodnik[playerid] == 1)
	{
		if(okregi[playerid] == 5)
		{
		    if(iloscwygranych == 0)
		    {
			    new sendername[MAX_PLAYER_NAME + 1];
			    GetPlayerName(playerid, sendername, sizeof(sendername));
				format(string, sizeof(string), "Wygrał %s - ukończył wyścig zajmując 1 miejsce !!!.", sendername);
			...
			else if(iloscwygranych == 1)
			{
			    new sendername[MAX_PLAYER_NAME + 1];
			    GetPlayerName(playerid, sendername, sizeof(sendername));
				format(string, sizeof(string), "%s ukończył wyścig zajmując 2 miejsce !!.", sendername);
				ProxDetectorW(500, -1106.9854, -966.4719, 129.1807, COLOR_WHITE, string);
				DisablePlayerCheckpoint(playerid);
		        DisablePlayerCheckpoint(playerid);
		        zawodnik[playerid] = 0;
			...
			else if(iloscwygranych == 2)
			{
			    new sendername[MAX_PLAYER_NAME + 1];
			    GetPlayerName(playerid, sendername, sizeof(sendername));
				format(string, sizeof(string), "%s ukończył wyścig zajmując 3 miejsce !.", sendername);
				ProxDetectorW(500, -1106.9854, -966.4719, 129.1807, COLOR_WHITE, string);
				DisablePlayerCheckpoint(playerid);
		        DisablePlayerCheckpoint(playerid);
		        zawodnik[playerid] = 0;
			...
			else
			{
			    ...
			    new sendername[MAX_PLAYER_NAME + 1];
			    GetPlayerName(playerid, sendername, sizeof(sendername));
				format(string, sizeof(string), "%s ukończył wyścig zajmując %d miejsce !.", sendername, iloscwygranych);
				ProxDetectorW(500, -1106.9854, -966.4719, 129.1807, COLOR_WHITE, string);
				DisablePlayerCheckpoint(playerid);
		        DisablePlayerCheckpoint(playerid);
		        zawodnik[playerid] = 0;
	...
	if(ScigaSie[playerid] != 666 && IloscCH[playerid] != 0 && Scigamy != 666)
	{
	    ...
	    else if(IloscCH[playerid] == Wyscig[Scigamy][wCheckpointy])
	    {
	        ...
	        if(Ukonczyl == 1)
	        {
        		...
        		SendClientMessage(playerid, COLOR_LIGHTGREEN, "| Ukończyłeś wyścig jako pierwszy! |");
		        ...
		        {
		        	format(string, sizeof(string), "Komunikat wyścigu: {FFFFFF}%s wygrał wyścig %s", GetNickEx(playerid), Wyscig[Scigamy][wNazwa]);
		        	foreach(new i : Player)
		        	{
	        ...
	        else
	        {
	            if(Ukonczyl >= IloscZawodnikow)
		        {
		            SendClientMessage(playerid, COLOR_LIGHTGREEN, "Ukończyłeś wyścig jako ostatni - cienias!");
					format(string, sizeof(string), "Komunikat wyścigu: {FFFFFF}%s ukończył wyścig jako ostatni !", GetNickEx(playerid));
					WyscigMessage(COLOR_YELLOW, string);
		            KoniecWyscigu(-2);
		        }
		        else
		        {
		            format(string, sizeof(string), "Ukończyłeś wyścig jako %d !", Ukonczyl);
					SendClientMessage(playerid, COLOR_LIGHTGREEN, string);
					format(string, sizeof(string), "Komunikat wyścigu: {FFFFFF}%s ukończył wyścig jako %d !", GetNickEx(playerid), Ukonczyl);
					WyscigMessage(COLOR_YELLOW, string);
	            	Ukonczyl++;
		        }
	        }
	...
	if(IsValidDynamicObject(objectid))
	{
        ...
        if(response < EDIT_RESPONSE_UPDATE && GetPVarInt(playerid, "Barier-id") != 0)
        {
            ...
            if(!IsPlayerInRangeOfPoint(playerid, 5.0, x, y, z))
            {
                ...
                SendClientMessage(playerid, -1, "Jesteś za daleko.");
		...
		else if(response == EDIT_RESPONSE_UPDATE && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
        	...
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Musisz być na swojej strefie!");
                SetDynamicObjectPos(objectid, X, Y, Z);
			}
		}
		else if( response == EDIT_RESPONSE_FINAL && GetPVarInt(playerid, "CreatingGraff") == 1)
		{
			...
			if( GetPVarInt(playerid, "zoneid") == -1 || (ZoneControl[GetPVarInt(playerid, "zoneid")] != frac && ZoneControl[GetPVarInt(playerid, "zoneid")]-100 != OnDuty[playerid]-1) )
        	{
				SendClientMessage(playerid, 0xFF0000FF, "Nie byłeś na swojej strefie!");
                graffiti_DeleteMySQL(f);
				graffiti_ZerujZmienne(playerid);
				graffiti_Zeruj(f);
				return 1;
			...
			new akcja[150];
			format(akcja,sizeof(akcja),"* %s wyciąga spray i tworzy nim napis.",GetNick(playerid));
            ProxDetector(40.0, playerid, akcja, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			format(akcja, sizeof(akcja), "%s [%d] stworzył nowe graffiti [ID: %d], lokalizacja: %s", GetNick(playerid), playerid, f, pZone);
			SendAdminMessage(COLOR_PANICRED, akcja);
			Log(serverLog, WARNING, "%s stworzył nowe graffiti %s, lokalizacja: %s", GetPlayerLogName(playerid), GetGraffitiLogText(f), pZone);
		}
...
public OnRconLoginAttempt(ip[], password[], success)
{
    ...
    else
    {
        if(player != -1)
        {
			...
			if(!dini_Exists(name) || gPlayerLogged[player] != 1)
			{
				new str[128];
				format(str, 128, "RCON: Użytkownik %s (%d) próbował się zalogować na rcona bez wymaganych uprawnień!", GetNickEx(player), player);
...
/*public OnRconCommand(cmd[])
{
    ...
    {
		Profiler_Start();
        SendClientMessageToAll(0xFFFFFFFF, "Profilowanie wystartowane.");
        return 1;
    }
	else if(strcmp(cmd, "profile stop", true) == 0)
    {
		Profiler_Stop();
        SendClientMessageToAll(0xFFFFFFFF, "Profilowanie zatrzymane.");
        return 1;
    }
	else if(strcmp(cmd, "profile dump", true) == 0)
    {
		Profiler_Dump();
        SendClientMessageToAll(0xFFFFFFFF, "Dump stworzony.");
        return 1;
    }
    print(cmd);
	return 1;
...
public OnPlayerPickUpDynamicPickup(playerid, pickupid)
{
    ...
    {
        SendClientMessage(playerid, COLOR_LIGHTBLUE, "|_______________Wybór skina - dostępne komendy_______________|");
        SendClientMessage(playerid, COLOR_WHITE, "{3CB371}/ubranie{FFFFFF}- zabija i przenosi do zwykłego menu wyboru skinów (wybierałka). Tylko dla cywili.");
    ...
    for (new i = 0; i < MAX_DROPPED_MONEY; i++)
	{
		...
		{
			return OnPickupMoney(playerid, pickupid);
		}
	}
...
public OnPlayerStateChange(playerid, PLAYER_STATE:newstate, PLAYER_STATE:oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)
    {
        	...
        	if(CarData[lcarid][c_OwnerType] == CAR_OWNER_SPECIAL)
        	{
 				if(CarData[lcarid][c_Owner] == RENT_CAR)
    			{
					if (CarData[lcarid][c_Rang]-1 != playerid)
					{
						...
						{
							format(stringDialog, sizeof(stringDialog), "Będąc nowym graczem, możesz wypożyczyć ten pojazd całkowicie za darmo!", costBike);	
						}
						else
						{
							format(stringDialog, sizeof(stringDialog), "Możesz wypożyczyć ten pojazd\nCena: %d$ za 15 minut", costBike);
						}
		    			TogglePlayerControllable(playerid, 0);
						HireCar[playerid] = vehicleid;
						ShowPlayerDialogEx(playerid, 7079, DIALOG_STYLE_MSGBOX, "Wypożyczalnia pojazdów", stringDialog, "Wynajmij", "Wyjdź¸");
					}
				}
			}
        ...
        if(!ToggleSpeedo[playerid])
        {
			...
			PlayerTextDrawColour(playerid, SpeedometerPDP[playerid][7], -1);
		...
		if(IsACopCar(GetPlayerVehicleID(playerid)) && IsAPolicja(playerid, 0)) sendTipMessageEx(playerid, COLOR_BLUE, "Połączyłeś się z komputerem policyjnym, wpisz /mdc aby zobaczyć kartotekę policyjną");
        if(newstate == PLAYER_STATE_DRIVER) TJD_CallEnterVeh(playerid, GetPlayerVehicleID(playerid));
    }
    else if(oldstate == PLAYER_STATE_DRIVER)
    {
		if(KradniecieWozu[playerid] == INVALID_VEHICLE_ID) //kradzież aut
		{
    	...
    	{
    	    new sendername[MAX_PLAYER_NAME + 1], komunikat[128];
	...
	if(newstate == PLAYER_STATE_PASSENGER) // TAXI & BUSSES
	{
	    ...
	    foreach(new i : Player)
	    {
			if(IsPlayerInVehicle(i, vehicleid) && GetPlayerState(i) == 2 && TransportDuty[i] > 0)
			{
				...
				{
					format(string, sizeof(string), "* Potrzebujesz $%d aby wejść.", TransportValue[i]);
					SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
				...
				else
				{
					if(TransportDuty[i] == 1)
					{
						...
						SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
						format(string, sizeof(string), "* Klient %s wszedł do Twojej taryfy.", name);
						SendClientMessage(i, COLOR_LIGHTBLUE, string);
                        if(PlayerInfo[playerid][pLevel] < 3)
                        {
                            ZabierzKaseDone(playerid, floatround(TransportValue[i]/4));//moneycheat
                            sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Jesteś nowym graczem, obowiązuje Cie rabat 75 procent na taksówkę.");
                        }
                        else
                        {
					...
					{
						format(string, sizeof(string), "* Zapłaciłeś $%d Za bilet.", TransportValue[i]);
						SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
						format(string, sizeof(string), "* Klient %s wszedł do autobusu i skasował bilet.", name);
						SendClientMessage(i, COLOR_LIGHTBLUE, string);
	...
	{// 38 / 49 / 56 = SS
        ...
        if(newcar <= CAR_End) //do kradziezy
        {
		    ...
		    {
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Możesz ukraść ten wóz, wpisz /kradnij aby spróbować to zrobić.");
				new engine, lights, alarm, doors, bonnet, boot, objective;
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	/*print("checking OMP!");
	if (!IsPlayerUsingOfficialClient(playerid) || (!IsUsingOmp(playerid) && !IsPlayerUsingOmp(playerid)))
	{
		if(!IsAProductionServer()) SendClientMessage(playerid, -1, "[DEBUG] Official: %d | Omp: %d | OmpStd: %d", IsPlayerUsingOfficialClient(playerid), IsUsingOmp(playerid), IsPlayerUsingOmp(playerid));
		SendClientMessage(playerid, COLOR_NEWS, "SERWER: Musisz korzystać z najnowszej wersji launchera Open.MP, aby grać na tym serwerze!"); 
		ShowPlayerDialogEx(playerid, 9999, DIALOG_STYLE_MSGBOX, "{FF0000}NIEPRAWIDŁOWA WERSJA OPEN.MP", "{FFFFFF}Musisz korzystać z najnowszej wersji launchera Open.MP, aby grać na tym serwerze. \n\nPobierz najnowszą wersję\n{a6a6a6}M-RP\n{a6a6a6}https://github.com/openmultiplayer/launcher/releases/", "OK", "");
		SetTimerEx("KickExTimer",2500,0,"d",playerid);
		return 0;
	}*/
...
PayDay()
{
	new string[128], account,playername2[MAX_PLAYER_NAME + 1],
        ebill;

	UpdateFuelPrice(random(5)+1);
	...
	foreach(new i : Player)
	{
		if(IsPlayerConnected(i))
		{
		    if(PlayerInfo[i][pLevel] > 0)
		    {
			    ...
			    if(MoneyMessage[i]==1)
				{
				    SendClientMessage(i, COLOR_LIGHTRED, "Nie spłaciłeś długu, wierzyciele nasłali na ciebie Policję !");
					PoziomPoszukiwania[i] += 1;
					SetPlayerCriminal(i,INVALID_PLAYER_ID, "Niespłacanie długu");
				}
				...
				if(PlayerInfo[i][pPayDay] >= 5)
				{
					...
					{
						if(PlayerInfo[i][pAccount] >= TaxValue)
						{
							Tax += TaxValue;//Should work for every player online
							SetPlayerBank(i, PlayerInfo[i][pAccount] - TaxValue);
						}
						else
						{
							PoziomPoszukiwania[i] += 1;
							SetPlayerCriminal(i,INVALID_PLAYER_ID, "Niespłacanie podatku");
						}
						PlayerInfo[i][pExp]++;
						PlayerPlayMusic(i);
						new wyplata_duty = 0;
						if(GroupPlayerDutyPerm(i, PERM_POLICE) //frakcje, które mają dostęp do wypłat
						...
						|| GroupPlayerDutyPerm(i, PERM_PAYDAY))	   //inne grupy
						{
							if(OnDuty[i] > 0)
							{
								if(PlayerInfo[i][pDutyTime] >= 1800 && GetPVarInt(i, "duty-act")+1800 < PlayerInfo[i][pDutyTime]) //30 minut
								{
									...
									if (GroupPlayerDutyPerm(i, PERM_RESTAURANT))
									{
										wyplata_duty = RandomEx(PRICE_PAYDAY_MIN_RESTAURANT, PRICE_PAYDAY_MAX_RESTAURANT);
									}
									if(GroupPlayerDutyPerm(i, PERM_MEDIC)) 
									{
										wyplata_duty = RandomEx(PRICE_PAYDAY_MIN_MEDIC, PRICE_PAYDAY_MAX_MEDIC);
									}
									if(GroupPlayerDutyPerm(i, PERM_POLICE)){
										wyplata_duty = RandomEx(PRICE_PAYDAY_MIN_COPS, PRICE_PAYDAY_MAX_COPS);
									}
									else{
									...
									Log(payLog, WARNING, "%s otrzymał %d$ za służbe %s", GetPlayerLogName(i), wyplata_duty, ginfo);
								}
								else
									SendClientMessage(i, COLOR_GRAD1, "  Nie przegrałeś wystarczająco minut na duty, żeby otrzymać wypłatę.");
							}
						}
						SendClientMessage(i, COLOR_WHITE, "|___ STAN KONTA ___|");
						format(string, sizeof(string), "  Wypłata za służbę: $%d   Podatek: -$%d", wyplata_duty, TaxValue);
						SendClientMessage(i, COLOR_GRAD1, string);
						if(PlayerInfo[i][pDom] != 0 || PlayerInfo[i][pPbiskey] != 255)
						{
							ebill = (PlayerInfo[i][pAccount]/10000)*(PlayerInfo[i][pLevel]);
							format(string, sizeof(string), "  Rachunek za prąd: -$%d", ebill);
							SendClientMessage(i, COLOR_GRAD1, string);
							
							//DajKase(i, checks);
							if(PlayerInfo[i][pAccount] > 0)
							{
								SetPlayerBank(i, PlayerInfo[i][pAccount] - ebill);
							}
							else
							{
								ebill = 0;
					...
					{
						SendClientMessage(i, COLOR_LIGHTRED, "* Byłeś AFK zbyt długo, aby dostać wypłatę.");
					}
					...
					if(((kaska[i] >= 10000000 || PlayerInfo[i][pAccount] >= 10000000) && PlayerInfo[i][pLevel] <= 2) && !DEVELOPMENT)
					{
						...
						Log(punishmentLog, WARNING, "%s dostał bana za 10MLN i 1 lvl (Portfel: %d$, Bank: %d$)", GetPlayerLogName(i), kaska[i], PlayerInfo[i][pAccount]);
						KickEx(i);
					}
					...
					SetWeatherEx(2+random(19));//Pogoda dla każdego
				...
				{
				    SendClientMessage(i, COLOR_LIGHTRED, "* Nie grasz wystarczająco długo, aby dostać wypłatę.");
				}
				SetPlayerWantedLevelEx(i, PoziomPoszukiwania[i]);
			}
			format(string, sizeof(string), "Obecna cena za litr paliwa na stacji: %d$", MULT_TANKOWANIE);
			SendClientMessage(i, COLOR_GRAD1, string);
		}
	}
	...
	if(10 <= shifthour <= 22)
	{
  	...
  	SendClientMessageToAll(COLOR_YELLOW, "Odliczanie do respawnu rozpoczęte");
	...
	if(shifthour == 4)
	{
	    ...
	    SendClientMessageToAll(COLOR_YELLOW, "UWAGA!! ZARAZ NASTĄPI RESTART SERWERA!!!!");
	    SendClientMessageToAll(COLOR_YELLOW, "UWAGA!! ZARAZ NASTĄPI RESTART SERWERA!!!!");
	    SendClientMessageToAll(COLOR_YELLOW, "UWAGA!! ZARAZ NASTĄPI RESTART SERWERA!!!!");
	    SendClientMessageToAll(COLOR_YELLOW, "UWAGA!! ZARAZ NASTĄPI RESTART SERWERA!!!!");
	    SendClientMessageToAll(COLOR_YELLOW, "UWAGA!! ZARAZ NASTĄPI RESTART SERWERA!!!!");
...
{	
	...
	{
	    new camMode, Keys[ 3 ];
		camMode = GetPlayerCameraMode(playerid);
		GetPlayerKeys(playerid, Keys[ 0 ], Keys[ 1 ], Keys[ 2 ]);
	    if((camMode == 53 || camMode == 7) || Keys[ 0 ] == KEY_FIRE)
	    {
	        PlayerAC[playerid][PlayerAimed] = 1;
	    }
	}
	*/
    ...
    systempozarow_OnPlayerUpdate(playerid);//System Pożarów v0.1
        ...
        if(vid != LastVehicleID[playerid])
        {
            if(GetTickDiff(GetTickCount(), VehicleIDChangeTime[playerid]) < 2000)
            {
                ...
                if(VehicleIDChanges[playerid] > MAX_VEHICLE_ID_CHANGES)
                {
                    ...
                    {
                        format(acstr, 128, "%s może lagować autami i dostał kicka Użyj /gotoczit aby to sprawdzić!", GetNickEx(playerid));
    ...
    if(noclipdata[playerid][cameramode] == CAMERA_MODE_FLY)
	{
		...
		if(noclipdata[playerid][udold] != ud || noclipdata[playerid][lrold] != lr)
		{
			...
			{   // All keys have been released, stop the object the camera is attached to and reset the acceleration multiplier
				if(GetPVarInt(playerid, "flymode-using-streamer") == 1) StopDynamicObject(noclipdata[playerid][flyobject]);
				else StopPlayerObject(playerid, noclipdata[playerid][flyobject]);
...
public OnPassswordVerify(playerid,bool:success){
    new nick[MAX_PLAYER_NAME + 1], string[512];
	GetPlayerName(playerid, nick, sizeof(nick));
	//success = true; // DO USUNIĘCIA [DEBUG]
 	if(success){
		//poprawne hasło

		//----------------------------
		//ładowanie konta i zmiennych:
		//----------------------------
		if( !MruMySQL_LoadAccount(playerid) )
		{
			SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {FF0000}Krytyczny błąd konta. Zgłoś zaistniałą sytuację na forum.");
			Log(serverLog, ERROR, "Krytyczny błąd konta %s (pusty rekord?)", nick);
			KickEx(playerid);
			return 1;
		}
		...
		//złe hasło
		SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {33CCFF}Złe hasło.");
		...
		ShowPlayerDialogEx(playerid, 230, DIALOG_STYLE_PASSWORD, "Logowanie", string, "Zaloguj", "Wyjdź");
		gPlayerLogTries[playerid] += 1;
		if(gPlayerLogTries[playerid] == 3)
		{
			SendClientMessage(playerid, COLOR_WHITE, "[SERVER] {33CCFF}Złe hasło. Zostajesz zkickowany.");
			ShowPlayerDialogEx(playerid, 239, DIALOG_STYLE_MSGBOX, "Kick", "{FF0000}Dostałeś kicka za wpisanie złego hasła 3 razy pod rząd!", "Wyjdź", "");
			KickEx(playerid);
		}
		return 1;
 	}
...
OnPlayerLogin(playerid, password[])
{
	new string[WHIRLPOOL_LEN + 1], accountPass[200], salt[200];
    
	MruMySQL_ReturnPassword(GetNick(playerid), accountPass, salt);
	if(strlen(accountPass) == (WHIRLPOOL_LEN - 1))
	{
		// Format starszych kont Mrucznika i kont tworzonych bez forum.
		WP_Hash(string, sizeof(string), sprintf("%s%s%s", ServerSecret, password, salt));
	}
	else
	{
		// Format haseł MyBB używany przez konta powiązane z forum.
...
public OnPlayerKeyStateChange(playerid,newkeys,oldkeys)
{
	...
	if(newkeys==KEY_YES) //Klawisz Y
	{
		if(IsPlayerInRangeOfPoint(playerid,3,1479.4830,-1672.9886,14.0469)) //Kod od mikolaja
		{
			...
			{
				sendTipMessage(playerid, "Odebrałeś już prezent!");
				return 0;
			}
			else
			{
				...
				if(losowyprezent == 1)
				{
					sendTipMessage(playerid, "Dostałeś prezent od mikołaja, 2500$ wleciało do twojej kieszeni!");
					DajKaseDone(playerid, 2500);
					PlayerInfo[playerid][pMikolaj] = 1;
					printf("[PREZENT MIKOLAJ] Gracz: %s[%d] odebral 2500$", name, playerid);
				}
				if(losowyprezent == 2)
				{
					DajKP(playerid, IsPlayerPremiumOld(playerid) ? (PremiumInfo[playerid][pExpires] + 172800) : (gettime() + 172800), true);
					sendTipMessage(playerid, "Dostałeś prezent od mikołaja, premium na okres 2 dni jest twoje!");
					PlayerInfo[playerid][pMikolaj] = 1;
					printf("[PREZENT MIKOLAJ] Gracz: %s[%d] odebral premium na 2 dni", name, playerid);
				}
				if(losowyprezent == 3)
				{
					DajMC(playerid, 100);
					sendTipMessage(playerid, "Dostałeś prezent od mikołaja, 100 mrucznik coinsów jest w twoim portfelu!");
					PlayerInfo[playerid][pMikolaj] = 1;
					printf("[PREZENT MIKOLAJ] Gracz: %s[%d] odebral 100 mrucznik coinsow", name, playerid);
				}
			}
		}
	}
		    ...
		    if(!IsPlayerInAnyVehicle(playerid))
		    {
		        if(GetPlayerVirtualWorld(playerid) == 21)
		        {
		            if(newkeys & KEY_SPRINT)
		            {
						...
						{
			                new ImieGracza[MAX_PLAYER_NAME + 1],string[64];
	...
	if ((newkeys==KEY_ACTION)&&(IsPlayerInAnyVehicle(playerid))&&(GetPlayerState(playerid)==PLAYER_STATE_DRIVER))
	{
		...
		{
			SendClientMessage(playerid,0x00FFFFFF, "Próbujesz podczepić pojazd");
			new Float:pX, Float:pY, Float:pZ;
			GetPlayerPos(playerid,pX,pY,pZ);
			...
			if (!found)
			{
				SendClientMessage(playerid,COLOR_BROWN, "Nie ma w pobliżu żadnych samochodów.");
			}
            return 0;
		}
	}
	...
	if((newkeys & KEY_SPRINT) && newkeys & KEY_WALK)
	{
		...
		#if !defined MRP_LEGACY_HOUSES
		else if(House_Enter(playerid) || House_Exit(playerid))
		{
			
		}
		#endif
		else if(Motel_Wejdz(playerid) || Motel_Wyjdz(playerid))
		{
			...
			if(IsAtPlaceGetHP(playerid))
			{
				if(GetPlayerHealth(playerid, health) <= PlayerInfo[playerid][pHealth])
				{
					if(TimerJedzenie[playerid] == 0)
					{
						...
						sendTipMessageEx(playerid, COLOR_RED, "Kupiłeś jedzenie"); 
					...
					{
						sendTipMessage(playerid, "Odczekaj chwilę!"); 
					}
				}
				else
				{
					sendErrorMessage(playerid, "Nie możesz wziąć więcej HP!");
					sendTipMessage(playerid, "Aby móc brać więcej HP ulepsz HP w /ulepszenia!"); 
					return 1;
				}
			}
		}
			    ...
			    for(new v; v < MAX_VEHICLES; v++)
			    {
					...
					if(IsAInteriorVehicle(playerid))
					{
		          		...
		          		if(IsVehicleStreamedIn(v, playerid) && IsPlayerInRangeOfPoint(playerid, 10.0, vehx, vehy, vehz))
		          		{
							...
							{
							    SendClientMessage(playerid, COLOR_GREY, "Interior jest zamknięty!");
							    return 0;
							}
		          		}
					}
		...
		if(PRESSED(KEY_FIRE))
		{
			if(GetPlayerWeapon(playerid) == 46)
			{
				...
				if(vehicleid != -1)
				{
					...
					{
						SetPlayerArmedWeapon(playerid, 0); //chowanie spadochronu
        				return ShowPlayerInfoDialog(playerid, "M-RP", "Schowaj spadochron zanim w coś uderzysz."); 
					}
				}
			}
		}

		if(GetPlayerWeapon(playerid) == 34 || GetPlayerWeapon(playerid) == 43)  //usuwanie obiektu maski podczas celowania snajperką/aparatem i przywracanie
		{
			new nick[32];
			if(GetPVarString(playerid, "maska_nick", nick, 24))
			{
...
public OnVehicleDeath(vehicleid, killerid)
{
	if(GetVehicleModel(vehicleid) == 577)
	{
        foreach(new i : Player)
		{
			...
			{
				SendClientMessage(i, COLOR_GREY, " Samolot rozbił się!");
				SetPlayerHealth(i, 0);
		...
		{
            new str[64];
            format(str, 64, "Szok! Samolot KT rozbił się i zginęło %d osób!", osoby);
...
public OnPlayerText(playerid, text[])
{
	...
	if(text[0] == '@')
	{
		...
		if(groupcheck >= 1 && groupcheck <= allowedSlots)
		{
			strdel(text, 0, 3);
		...
		if(strlen(text) > 31)
		{
			sendTipMessage(playerid, "Nieprawidłowa długość znaków animacji"); 
			return 0;
		}
	...
	{
		sendTipMessageEx(playerid, TEAM_CYAN_COLOR, "Nie możesz mówić gdyż jesteś uciszony");
		return 0;
	}
	    ...
	    if (strcmp("tak", text, true) == 0)
		{
		    if(GotProposedBy[playerid] < 999)
		    {
			    if(IsPlayerConnected(GotProposedBy[playerid]))
				{
				    ...
				    format(string, sizeof(string), "Ksiądz: %s czy chcesz wziąć %s na swoją żone? (wpisz 'tak', cokolwiek innego anuluje ślub).", giveplayer,sendername);
			...
			else if(ProposedTo[playerid] < 999)
			{
			    if(IsPlayerConnected(ProposedTo[playerid]))
				{
					...
					{
						format(string, sizeof(string), "Ksiądz: %s i %s zostaliście mężem i żoną, możecie się pocałować.", sendername, giveplayer);
						SendClientMessage(playerid, COLOR_WHITE, string);
				   		format(string, sizeof(string), "Ksiądz: %s i %s zostaliście mężem i żoną, możecie się pocałować.", giveplayer, sendername);
						SendClientMessage(ProposedTo[playerid], COLOR_WHITE, string);
						format(string, sizeof(string), "Kościół: Mamy nową pare, %s & %s zostali zaręczeni.", sendername, giveplayer);
						OOCNews(COLOR_WHITE, string);
					}
					else if(PlayerInfo[playerid][pSex] == 1 && PlayerInfo[ProposedTo[playerid]][pSex] == 1)
					{
					    format(string, sizeof(string), "Ksiądz: %s i %s Zostaliście mężem i mężem, możecie się pocałować.", sendername, giveplayer);
						SendClientMessage(playerid, COLOR_WHITE, string);
				   		format(string, sizeof(string), "Ksiądz: %s i %s Zostaliście mężem i mężem, możecie się pocałować.", giveplayer, sendername);
						SendClientMessage(ProposedTo[playerid], COLOR_WHITE, string);
						format(string, sizeof(string), "Kościół: Mamy nową gejowską pare, %s & %s zostali zaręczeni.", sendername, giveplayer);
						OOCNews(COLOR_WHITE, string);
					}
					else if(PlayerInfo[playerid][pSex] == 2 && PlayerInfo[ProposedTo[playerid]][pSex] == 2)
					{
					    format(string, sizeof(string), "Ksiądz: %s i %s Zostaliście żoną i żoną, możecie się pocałować.", sendername, giveplayer);
						SendClientMessage(playerid, COLOR_WHITE, string);
				   		format(string, sizeof(string), "Ksiądz: %s i %s Zostaliście żoną i żoną, możecie się pocałować.", giveplayer, sendername);
						SendClientMessage(ProposedTo[playerid], COLOR_WHITE, string);
						format(string, sizeof(string), "Kościół: Mamy nową lesbijską pare, %s & %s zostali zaręczeni.", sendername, giveplayer);
						OOCNews(COLOR_WHITE, string);
					}
		...
		else
		{
		    if(GotProposedBy[playerid] < 999)
		    {
				if(IsPlayerConnected(GotProposedBy[playerid]))
				{
					GetPlayerName(playerid, sendername, sizeof(sendername));
					GetPlayerName(GotProposedBy[playerid], giveplayer, sizeof(giveplayer));
					format(string, sizeof(string), "* Nie chcesz poślubić %s, nie powiedziałeś 'tak'.",giveplayer);
				    SendClientMessage(playerid, COLOR_YELLOW, string);
				    format(string, sizeof(string), "* %s nie chce cię poślubić gdyż nie powiedział 'tak'.",sendername);
	...
	if(ConnectedToPC[playerid] == 255)
	{
	    ...
	    if ((strcmp("Contracts", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("Contracts")) || (strcmp("Kontrakty", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("Kontrakty")))
		{
		    ...
		    {
		        SendClientMessage(playerid, COLOR_GREY, "Tylko Hitmani z 4 rangą mogą sprawdzać listę kontraktów !");
		        return 0;
		    }
		...
		else if ((strcmp("News", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("News")))
		{
				...
				if(!strlen(x_nr)) {
					...
					SendClientMessage(playerid, COLOR_WHITE, "UżYJ: News [numer] aby skasowac 'News Delate [numer]' lub 'News delete all'");
				...
				{
				    if(GroupPlayerDutyRank(playerid) < 3) { SendClientMessage(playerid, COLOR_GREY, "Musisz miec 3 rangę aby pisać newsy Hitman Agency !"); return 0; }
				    if(News[hTaken1] == 0)
				    {
				        GetPlayerName(playerid, sendername, sizeof(sendername));
						...
						News[hTaken1] = 1;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Umieściłeś news na kanale Hitman Agency.");
						return 0;
				    }
				    else
				    {
				...
				{
				    if(GroupPlayerDutyRank(playerid) < 3) { SendClientMessage(playerid, COLOR_GREY, "Musisz mieć 3 rangę aby pisać newsy na kanale Hitman Agency !"); return 0; }
				    if(News[hTaken2] == 0)
				    {
				        GetPlayerName(playerid, sendername, sizeof(sendername));
				    ...
				    {
				        SendClientMessage(playerid, COLOR_GREY, "Ten numer jest aktualnie w użyciu !");
				        return 0;
				    }
				}
				...
				else if(strcmp(x_nr,"delete",true) == 0)
				{
				    if(GroupPlayerDutyRank(playerid) < 4)
				    {
				        SendClientMessage(playerid, COLOR_GREY, "Musisz mieć 4 rangę aby usuwać newsy z kanału Hitman Agency !");
				        return 0;
				    }
				    new string1[MAX_PLAYER_NAME + 1];
				    new x_tel[128];
					...
					if(!strlen(x_tel)) {
					    SendClientMessage(playerid, COLOR_WHITE, "UżYJ: News delete [numer] lub News delete all.");
					    return 0;
					}
                    if(strcmp(x_tel,"1",true) == 0)
                    {
						...
						News[hTaken1] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś newsa numer (1) Z kanału Hitman Agency.");
						return 0;
                    }
                    else if(strcmp(x_tel,"2",true) == 0)
                    {
						...
						News[hTaken2] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś newsa numer (2) Z kanału Hitman Agency.");
						return 0;
                    }
                    else if(strcmp(x_tel,"3",true) == 0)
                    {
						...
						News[hTaken3] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś newsa numer (3) Z kanału Hitman Agency.");
						return 0;
                    }
                    else if(strcmp(x_tel,"4",true) == 0)
                    {
						...
						News[hTaken4] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś newsa numer (4) Z kanału Hitman Agency.");
						return 0;
                    }
                    else if(strcmp(x_tel,"5",true) == 0)
                    {
						...
						News[hTaken5] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś newsa numer (5) Z kanału Hitman Agency.");
						return 0;
                    }
                    else if(strcmp(x_tel,"all",true) == 0)
                    {
						...
						format(string1, sizeof(string1), "Nikt");	strmid(News[hContact5], string1, 0, strlen(string1));
						News[hTaken5] = 0;
						SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Skasowałeś wszystkie newsy z kanału Hitman Agency.");
						return 0;
                    }
                    else
                    {
		...
		else if ((strcmp("Givehit", tmp, true, strlen(tmp)) == 0) && (strlen(tmp) == strlen("Givehit")))
		{
		    if(GroupPlayerDutyRank(playerid) < 4)
		    {
		        SendClientMessage(playerid, COLOR_GREY, "Musisz mieć 4 rangę aby dawać kontrakty Hitmanom !");
		        return 0;
		    }
		    if(hitfound == 0)
		    {
		        SendClientMessage(playerid, COLOR_GREY, "Nie sprawdziłeś jeszcze kontraktów, zrób to w laptopie (wpisz 'kontrakty') !");
		        return 0;
		    }
		    tmp = strtok(text, idx);
		    if(!strlen(tmp))
			{
				SendClientMessage(playerid, COLOR_GRAD1, "UżYJ: Givehit [playerid]");
				return 0;
			}
			...
			if(IsPlayerConnected(giveplayerid))
			{
			    if(giveplayerid != INVALID_PLAYER_ID)
			    {
				    ...
				    {
				        SendClientMessage(playerid, COLOR_GREY, "Ten Hitman wykonuje już jakieś zlecenie !");
						return 0;
				    }
				    if(IsPlayerConnected(hitmanid))
				    {
		    			...
		    			format(string, sizeof(string), "* Hitman %s, dał zlecenie %s na zabicie: %s(ID:%d), nagroda: $%d.", sendername, giver, giveplayer, hitmanid, PlayerInfo[hitmanid][pHeadValue]);
						...
						ConnectedToPC[playerid] = 0; //rozłączanie z laptopem po akcji
				        return 0;
				    }
				    else
				    {
				        SendClientMessage(playerid, COLOR_GREY, "Osoby, na którą jest zlecenie, nie ma na serwerze. Spróbuj póź¸niej !");
				        return 0;
				    }
				}
				return 0;
	...
	if(CallLawyer[playerid] == 111)
	{
		...
		{
		    GetPlayerName(playerid, sendername, sizeof(sendername));
		    format(string, sizeof(string), "** %s jest w więzieniu i potrzebuje prawnika, jedź¸ na komisariat.", sendername);
	    	SendJobMessage(2, TEAM_AZTECAS_COLOR, string);
	    	SendJobMessage(2, TEAM_AZTECAS_COLOR, "* Kiedy będziesz juz na komisariacie, spytaj się policjanta o /akceptuj prawnik.");
	    	SendClientMessage(playerid, COLOR_LIGHTRED, "Jeśli policjant się zgodzi, prawnik będzie mógł uwolnić cię za opłatą.");
		...
		{
		    SendClientMessage(playerid, COLOR_LIGHTRED, "Nie ma żadnych prawników na serwerze, czas odsiadki rozpoczęty.");
		    WantLawyer[playerid] = 0;
			CallLawyer[playerid] = 0;
		    return 0;
		}
	...
	if(TalkingLive[playerid] != INVALID_PLAYER_ID)
	{
		new SanNews_nick[MAX_PLAYER_NAME + 1];
		GetPlayerName(playerid, SanNews_nick, sizeof(SanNews_nick));
		if(IsPlayerInGroup(playerid, 9) || GroupIsLeader(playerid, 9))
		{//todo
			if(strlen(text) < 78)
			{
		...
		else
		{
			if(strlen(text) < 78)
			{
				...
				format(string, sizeof(string), "%s mówi: %s", SanNews_nick, text);
				format(wywiad_string, sizeof(wywiad_string), "Gość wywiadu %s: %s", SanNews_nick, text);
				ProxDetector(10.0, playerid, string, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
				SetPlayerChatBubble(playerid,text,COLOR_FADE1,10.0,8000);
				OOCNews(COLOR_LIGHTGREEN, wywiad_string);
				SendDiscordMessage(DISCORD_SAN_NEWS, wywiad_string);
	...
	if(Mobile[playerid] != INVALID_PLAYER_ID && Callin[playerid] != CALL_NONE)
	{
		...
		{
			new org = (Mobile[playerid] - EMERGENCY_NUMBERS) * -1; //wzór na wyłuskanie organizacji z numeru
			if(Mobile[playerid] == POLICE_NUMBER || Mobile[playerid] == SHERIFF_NUMBER)
			{
				if(strlen(text) > 82)
				{
					SendClientMessage(playerid, COLOR_ALLDEPT, "Centrala: Niestety, nie rozumiem. Proszę powtórzyć ((max 75 znaków))");
					return 0;
				}
				new id, message[128];
			...
			new turner[MAX_PLAYER_NAME + 1], pZone[MAX_ZONE_NAME];
			new wanted[144];
			GetPlayerName(playerid, turner, sizeof(turner));
			GetPlayer2DZone(playerid, pZone, sizeof(pZone));
			SendClientMessage(playerid, TEAM_CYAN_COLOR, "Centrala: Zgłosimy to wszystkim jednostkom w danym obszarze.");
			SendClientMessage(playerid, TEAM_CYAN_COLOR, "Dziękujemy za zgłoszenie");
			format(wanted, sizeof(wanted), "Centrala: Otrzymano zgłoszenie: %s", text);
			GroupSendMessage(org, COLOR_ALLDEPT, wanted, true);
			format(wanted, sizeof(wanted), "Centrala: Nadawca: %s, lokalizacja zgłoszenia: %s", turner, pZone);
			GroupSendMessage(org, COLOR_ALLDEPT, wanted, true);
			if(org == 4 && (PlayerInfo[playerid][pBW] > 0 || PlayerInfo[playerid][pInjury] > 0)) PlayerRequestMedic[playerid] = 1;

			SendClientMessage(playerid, COLOR_GRAD2, "Rozmowa zakończona...");
			StopACall(playerid);
		}
		else
		{
			...
			{
				sendErrorMessage(playerid, "Gracz jeszcze nie odebrał telefonu!");
			}
			...
			{
				SendClientMessage(playerid, COLOR_YELLOW, "Nikt się nie odzywa.");
			}
		}
		return 0;
	}
...
public OnVehicleRespray(playerid, vehicleid, color1, color2)
{
    ...
    {
        ChangeVehicleColours(vehicleid, CarData[VehicleUID[vehicleid][vUID]][c_Color][0], CarData[VehicleUID[vehicleid][vUID]][c_Color][1]);
        return 0;
    }
