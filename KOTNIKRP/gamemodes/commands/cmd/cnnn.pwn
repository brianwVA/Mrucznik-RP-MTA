...
YCMD:cnnn(playerid, params[], help)
{
	if (PlayerInfo[playerid][pAdmin] >= 5000)
	{
		...
		{
			sendTipMessage(playerid, "Użyj /cnnn <typ> [cnnn formattextu ~n~=Nowalinia ~r~=Czerwony ~g~=Zielony ~b~=Niebieski ~w~=Biały ~y~=Żółty]");
			return 1;
		}

		if(txtid == 2)
		{
			sendTipMessageEx(playerid, COLOR_GRAD2, "2 jest niedostępna");
			return 1;
		}
		new string[128];
