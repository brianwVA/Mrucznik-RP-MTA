...
YCMD:pl(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /pl(pokazlicencje) [playerid/CzęśćNicku]");
			return 1;
		}
		...
		if(IsPlayerConnected(giveplayerid))
		{
			if(giveplayerid != INVALID_PLAYER_ID)
			{
				...
				{
				    if(giveplayerid == playerid) { sendTipMessageEx(playerid, COLOR_GREY, "Nie możesz pokazać licencji samemu sobie, użyj /licencje!"); return 1; }
					...
					SendClientMessage(giveplayerid, COLOR_GREY, string);
					format(string, sizeof(string), "** Licencja pływanie: %s.", text2);
					SendClientMessage(giveplayerid, COLOR_GREY, string);
					format(string, sizeof(string), "** Karta wędkarska: %s.", text3);
					SendClientMessage(giveplayerid, COLOR_GREY, string);
					format(string, sizeof(string), "** Licencja broń: %s.", text5);
					SendClientMessage(giveplayerid, COLOR_GREY, string);
					format(string, sizeof(string), "* %s pokazał tobie licencje.", sendername);
					SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "* Pokazałeś licencje %s.", giveplayer);
