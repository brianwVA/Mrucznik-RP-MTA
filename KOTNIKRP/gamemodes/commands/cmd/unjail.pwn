...
YCMD:unjail(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /unaj [ID gracza]");
			return 1;
		}
		    ...
		    if(IsPlayerConnected(playa))
		    {
		        if(playa != INVALID_PLAYER_ID)
		        {
		            if(PlayerInfo[playa][pJailed] == 3)
		            {
						...
						format(string, sizeof(string), "* Uwolniłeś %s.", giveplayer);
						_MruAdmin(playerid, string);
						format(string, sizeof(string), "* Zostałeś uwolniony przez %s.", GetNickEx(playerid));
						_MruAdmin(playa, string);
						format(string, sizeof(string), "ADMCMD: Administrator %s uwolnił %s z AJ", GetNickEx(playerid), giveplayer);
						SendMessageToAdmin(string, COLOR_RED);
                        Log(punishmentLog, WARNING, "Admin %s uwolnił %s z AJ (typ: %d, czas: %ds)", 
						...
						{
							format(string, sizeof(string), "Info: %s nie zostanie zabrana broń bo dostał UnJail", GetNick(playa));
