...
YCMD:wplac(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if(gPlayerLogged[playerid] == 1)
        {
			...
			{
				sendTipMessage(playerid, "Użyj /wplac [kwota]");
				format(string, sizeof(string), "Masz teraz $%d na swoim koncie.", PlayerInfo[playerid][pAccount]);
				sendTipMessage(playerid, string);
                if(GraczBankomat(playerid)) {
                    sendTipMessage(playerid, "UWAGA! Przy wpłatach przez bankomat prowizja to 6 procent!", COLOR_LIGHTBLUE);
                }
				return 1;
			}
			...
			{
				sendTipMessage(playerid, "Maksymalnie w banku możesz trzymać 100 milionów!"); 
				return 1;
			}
			...
			SetPlayerBank(playerid, PlayerInfo[playerid][pAccount] + (cashdeposit - bankomat_fee));

			Log(payLog, WARNING, "%s wpłacił na swoje konto %d$. Koszt wpłaty %d, nowy stan %d$.", GetPlayerLogName(playerid), cashdeposit-bankomat_fee, bankomat_fee, PlayerInfo[playerid][pAccount]);

            if(GraczBankomat(playerid)) {
                format(string, sizeof(string), "Wpłaciłeś $%d na swoje konto (prowizja %d), obecny stan to: $%d ", cashdeposit-bankomat_fee, bankomat_fee, PlayerInfo[playerid][pAccount]);
