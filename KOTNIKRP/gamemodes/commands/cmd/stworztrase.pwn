		...
		if(IsANoA(playerid) || IsAKt(playerid) || HasPerm(playerid, PERM_RACING))
		{
			if(GroupPlayerDutyRank(playerid) >= 5)
			{
				...
				if(ow == -1)
				{
					...
					ShowPlayerDialogEx(playerid, 1420, DIALOG_STYLE_LIST, "Kreator wyścigów: Wybierz slot", trasy, "Wybierz", "Anuluj");
				}
				else
				{
					format(string, sizeof(string), "Gracz [%d] %s jest w trakcie tworzenia wyscigu. Poczekaj aż skończy", ow, GetNick(ow));
					sendTipMessage(playerid, string);
				}
			}
			else
			{
				sendErrorMessage(playerid, "Potrzebujesz 5 rangi by tworzyć wyścigi!");
			}
		}
		else
		{
