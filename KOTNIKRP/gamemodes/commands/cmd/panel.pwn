...
YCMD:panel(playerid, params[], help)
{
    ...
    if(isnull(params))
    {
        ...
        format(str, sizeof(str), "{FFFFFF}» PANEL KAR (%s{FFFFFF})\n{FFFFFF}» Sprawdź konto gracza", (kary & 0b10) ? ("{00FF00}Uprawnienia") : ("{FF0000}Brak upr."));
        ShowPlayerDialogEx(playerid, D_PANEL_ADMINA, DIALOG_STYLE_LIST, "K-RP » Panel administracyjny", str, "Wybierz", "Wyjdź");
    }
    else
    {
        ...
        if(strcmp(sub, "unban", true) == 0)
        {
            ...
            {
                sendErrorMessage(playerid, "Uprawnienia: Nie posiadasz wystarczających uprawnień.");
                return 1;
            }
            if(strlen(var) < 1 || strlen(var) > MAX_PLAYER_NAME)
            {
                sendErrorMessage(playerid, "Niepoprawna długosc!");
                return 1;
            }
            ...
            Log(punishmentLog, WARNING, "Admin %s odbanował %s", GetPlayerLogName(playerid), var);
            return 1;
        }
        ...
        else if(strcmp(sub, "unwarn", true) == 0)
        {
			...
			{
                SendClientMessage(playerid, COLOR_RED, "Gracz jest online, używam komendy /unwarn");
				
				new txt[64];
				format(txt, sizeof(txt), "%d", id);
            ...
            if(warny > 0)
            {
                ShowPlayerWarns(playerid, MruMySQL_GetUIDFromName(var));
				/*if(AddPunishment(-1, var, playerid, gettime(), PENALTY_UNWARN, 0, powod, 0) == 1) {
					//MruMySQL_Unwarn(var);
					format(str, sizeof(str), "AdmCmd: Konto gracza %s zostało unwarnowane przez %s, powod: %s", var, GetNickEx(playerid), powod);
					ABroadCast(COLOR_YELLOW,str,1);
					Log(punishmentLog, WARNING, "Admin %s unwarnował %s, powod: %s", GetPlayerLogName(playerid), var, powod);
				}*/
            }
