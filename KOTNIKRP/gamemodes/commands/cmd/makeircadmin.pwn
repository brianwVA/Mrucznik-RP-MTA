...
YCMD:makeircadmin(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /makeircadmin [playerid/CzęśćNicku] [NrKanału]");
			return 1;
		}

		if(channel > 10 || channel < 0) { sendTipMessage(playerid, "Numer nie może wynosić 0, albo więcej niż 10!"); return 1; }
		if(IsAScripter(playerid) || Uprawnienia(playerid, ACCESS_MAKELEADER))
		{
		    if(IsPlayerConnected(giveplayerid))
		    {
		        if(giveplayerid != INVALID_PLAYER_ID)
		        {
					...
					format(string, sizeof(string), "* Dałeś %s admina kanału IRC nr %d.", giveplayer, channel);
					SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "* %s Mianował cię administratorem kanału IRC nr %d.", GetNickEx(playerid), channel);
