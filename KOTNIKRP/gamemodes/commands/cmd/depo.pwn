...
YCMD:depo(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if(isnull(params))
        {
            sendTipMessage(playerid, "Użyj /depo [tekst]");
            return 1;
        }
        if(gMuteDepo[playerid] == 1) return sendTipMessage(playerid, "Nie możesz pisać na depo, zablokowałeś ten czat (Użyj /togdepo)");
