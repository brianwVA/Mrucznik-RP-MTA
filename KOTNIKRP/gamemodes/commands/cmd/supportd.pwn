YCMD:supportusun(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /supportusun [SupID] [powód]");
			return 1;
		}
        ...
        format(string, sizeof(string), "Usunąłeś zgłoszenie o ID: %d od %s, powód: %s", supportid, suppList[supportid][suppOwner], powod);
		sendTipMessageEx(playerid, COLOR_LIGHTBLUE, string);
        format(string, sizeof(string), "Twoje zgłoszenie zostało usunięte przez: %s, powód: %s", GetNick(playerid), powod);
