...
{
    if(GroupIsVLeader(playerid, FRAC_NG))
    {
        new lKey;
        sscanf(params, "D(0)", lKey);
        if(lKey != 0)
        {
            if(!(0 < lKey < 6666)) return sendTipMessage(playerid, "Użyj /kod [kod(od 0 do 6666)]");
            STANOWE_GATE_KEY = lKey;
            MruMySQL_KodStanowca(lKey);
            SendClientMessage(playerid, COLOR_GRAD2, "Zmieniono");
        }
    }
    if(VAR_NGKeypad) return sendErrorMessage(playerid, "Ktoś już wpisuje kod");
