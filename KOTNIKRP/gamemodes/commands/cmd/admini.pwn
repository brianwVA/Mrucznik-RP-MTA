...
YCMD:admini(playerid, params[], help)
{
	if(IsPlayerConnected(playerid))
	{
		...
		SendClientMessage(playerid, -1, "Lista administratorów na służbie:"); 
		foreach(new i : Player)
		{
			if(HiddenAdmin[i]) continue; 
			if(GetPlayerAdminDutyStatus(i) == 1)
			{
				...
				{
					format(string, sizeof(string), "{FFFFFF}Pół-Admin: {00C0FF}%s {FFFFFF}({00C0FF}%s{FFFFFF}) {FFFFFF}[ID: %d] [P@LVL: %d]", GetNickEx(i), PlayerInfo[i][pNickOOC], i, PlayerInfo[i][pNewAP]); 
				}
				else if(PlayerInfo[i][pZG] > 0)
				{
					switch (PlayerInfo[i][pZG]) {
						...
						case 8:  format(string, sizeof(string), "{FFFFFF}Świetny ZG: {7AA1C9}%s {FFFFFF}({7AA1C9}%s{FFFFFF}) [ID: %d]",		GetNickEx(i), PlayerInfo[i][pNickOOC], i);
						...
						case 10: format(string, sizeof(string), "{FFFFFF}Ćwierć admin: {7AA1C9}%s {FFFFFF}({7AA1C9}%s{FFFFFF}) [ID: %d]",	GetNickEx(i), PlayerInfo[i][pNickOOC], i);
					}
				}
				sendTipMessage(playerid, string); 
				activeAdmins = true;
		...
		if(!activeAdmins) 
		{
			...
			foreach(new i : Player)
			{
				...
				{
					format(string, sizeof(string), "{888888}Pół-Admin: {00C0FF}%s {888888}({00C0FF}%s{888888}) {888888}[ID: %d] [P@LVL: %d]", GetNickEx(i), PlayerInfo[i][pNickOOC], i, PlayerInfo[i][pNewAP]); 
					sendTipMessage(playerid, string); 
				}
				...
				else if(PlayerInfo[i][pZG] > 0)
				{
					switch (PlayerInfo[i][pZG]) {
						...
						case 8:  format(string, sizeof(string), "{888888}Świetny ZG: {7AA1C9}%s {888888}({7AA1C9}%s{888888}) [ID: %d]",		GetNickEx(i), PlayerInfo[i][pNickOOC], i);
						...
						case 10: format(string, sizeof(string), "{888888}Ćwierć admin: {7AA1C9}%s {888888}({7AA1C9}%s{888888}) [ID: %d]",	GetNickEx(i), PlayerInfo[i][pNickOOC], i);
					}
					sendTipMessage(playerid, string); 
				}
			}
