			...
			if(GroupIsVLeader(playerid, 0)) //TU WPISAC SLOT GRUPY JEZELI TAKA ZOSTANIE STWORZONA
			{
				if(IsPlayerInRangeOfPoint(playerid, 3.0, 1203.39294, -1772.58777, 13.71199))
				{
					...
					if(poolStatus == 0)
					{
						...
						ShowPlayerDialogEx(playerid, 1093, DIALOG_STYLE_TABLIST, "Laptop Lidera", string, "Wybierz", "Odrzuć");
					}
					else
					{
				...
				{
					sendTipMessage(playerid, "Nie jesteś przy laptopie lidera"); 
					return 1;
				}
			}
			else
			{
				sendErrorMessage(playerid, "Nie jesteś liderem basenu tsunami!"); 
				return 1;
			}
		}
		else
		{
			sendErrorMessage(playerid, "Nie jesteś pracownikiem basenu!"); 
			return 1;
		}
