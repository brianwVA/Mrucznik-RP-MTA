...
YCMD:bp(playerid, params[], help)//blokada pisania
{
	...
	{
		sendTipMessage(playerid, "Użyj /bp [ID gracza] [czas (w godzinach)] [nazwa chatu]");
		return 1;
	}
		...
		if(IsPlayerConnected(giveplayerid))
		{
			if(giveplayerid != INVALID_PLAYER_ID)
			{
				if(czas <= 8 && czas >= 0)
				{
					...
					{
						format(string, sizeof(string), "AdmCmd: %s dostał Blokadę Pisania od %s na %d godzin. Powód: %s", GetNick(giveplayerid), GetNickEx(playerid), czas, text);
