   				...
   				if(playa != INVALID_PLAYER_ID)
			    {
				    if(GetDistanceBetweenPlayers(playerid,playa) < 10)
					{
					    if(kaska[playerid] > 10)
						{
		        			...
		        			format(string, sizeof(string), "* Oczyściłeś kartoteki gracza %s (-10$)",giveplayer);
						    SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
					        format(string, sizeof(string), "* Prawnik %s oczyścił kartoteki Policji na twój temat",sendername);
					        SendClientMessage(playa, COLOR_LIGHTBLUE, string);
					        format(string, sizeof(string),"* %s wykonuje parę telefonów i oczyszcza kartoteki %s.", sendername, giveplayer);
						...
						{
						    sendErrorMessage(playerid, "Nie masz wystarczającej ilości pieniędzy ($10).");
						}
                    }
					else
					{
