...
YCMD:wywalzsad(playerid,params[], help) //GSA
{
    if(GroupPlayerDutyPerm(playerid, PERM_BOR) || (IsPlayerInGroup(playerid, FAMILY_SAD)))
	{
	   ...
	   {
		    new odbiorca;
		    if(sscanf(params,"k<fix>",odbiorca)) return sendTipMessage(playerid,"Użyj /wywalzsad [ID-Gracza]");
            new str[128];
            format(str, 128, "Zostales wyrzucony z sądu przez %s", GetNick(playerid));
            SetPlayerVirtualWorld(odbiorca, 0);
			SendClientMessage(odbiorca,0xFFFFFFFF,str);
            format(str, 128, "* %s wyrzuca z sądu %s", GetNick(playerid), GetNick(odbiorca));
            ProxDetector(30.0, playerid, str, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            SetPlayerPos(odbiorca,1309.9658, -1367.2878, 13.7324);
        }
        else
        {
        	sendTipMessageEx(playerid,0xFFFFFFFF,"Nie jestes w sądzie!");
        }
    }
	return 1;
}
