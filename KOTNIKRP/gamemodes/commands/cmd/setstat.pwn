...
YCMD:setstat(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /setstat [playerid/CzęśćNicku] [statcode] [amount]");
			...
			SendClientMessage(playerid, COLOR_GRAD2, "|12 Łowca Nagród |13 Prawnik |14 Mechanik |15 Reporter |16 ZlodziejAut |17 Diler Zioła");
				...
				if (PlayerInfo[playerid][pAdmin] >= 5000)
				{
					switch (stat)
					{
						...
						case 2:
						{
							...
							format(string, sizeof(string), "Życie na spawnie gracza wynosi teraz %d ", amount);
						}
						case 3:
						{
						...
						case 6:
						{
							...
							Item_Add("Telefon (KP)", ITEM_OWNER_TYPE_PLAYER, PlayerInfo[playerid][pUID], ITEM_TYPE_PHONE, amount, 0, true, playerid);
						...
						case 10:
						{
							format(string, sizeof(string), "Ta funkcja została wyłączona", amount);
						}
						case 11:
						{
							return sendErrorMessage(playerid, "Komenda zablokowana.");
						...
						case 12:
						{
							...
							format(string, sizeof(string), "Skill Łowcy Nagród gracza zmieniony na %d", amount);
						}
						case 13:
						{
						...
						case 16:
						{
							...
							format(string, sizeof(string), "Skill złodzieja samochodów gracza zmieniony na %d", amount);
						}
						case 17:
						{
							PlayerInfo[giveplayerid][pDrugsSkill] = amount;
							format(string, sizeof(string), "Skill Dilera Zioła gracza zmieniony na %d", amount);
						}
						case 18:
						{
						...
						default:
						{
							format(string, sizeof(string), "Zły kod statystyk", amount);
						}

					}
					SendClientMessage(playerid, COLOR_GRAD1, string);
					Log(adminLog, WARNING, "Admin %s zmienił %s statystykę: %s", GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), string);
				}
				else
				{
