...
YCMD:lot(playerid, params[], help)
{
    ...
    {
		sendTipMessageEx(playerid, COLOR_WHITE, "Użyj /lot [nazwa]");
	...
	if(strcmp(data,"bilet",true) == 0)
	{
        if(PlayerToPoint(3.0,playerid,1593.3000488281,-2285.6000976563,-75)||PlayerToPoint(3.0,playerid,1593.3000488281,-2280,-75)||PlayerToPoint(3.0,playerid,1674.8, 1442.3, 14.8)||PlayerToPoint(1.5,playerid,-1391.9000244141,-299.79998779297,-74.699996948242))
	    {
    	    if(kosztbiletu>0)
    	    {
    	        ...
    	        {
    	            sendTipMessageEx(playerid, COLOR_GREY, " Kupiłeś bilet !");
    	        ...
    	        {
    	            format(string, sizeof(string), " Nie stać cię na bilet ($%d) !", kosztbiletu);
	    ...
	    else return sendTipMessageEx(playerid, COLOR_GREY, " Nie jesteś przy kasie lotniska Los Santos !");
	}
    if(CheckPlayerPerm(playerid, PERM_TAXI))
    {
       			...
       			if(IsPlayerInRangeOfPoint(playerid, 10,1941.0000,-2294.5068,13.4720) || IsPlayerInRangeOfPoint(playerid, 10, -1344.1194,-229.2785,14.0624) || IsPlayerInRangeOfPoint(playerid, 10, 1565.5919,1448.4813,10.7384))
    	        {
    	    	    ...
    	    	    foreach(new i : Player)
	    	        {
	    	            if(PlayerInfo[i][pWsamolocieLS]==1)
							...
							sendTipMessageEx(i, COLOR_GREY, " Nie możesz teraz opuścić samolotu.");
    				...
    				format(string, sizeof(string), "LSAD: W samolocie znajduje się %d osób", osoby);
    				SendClientMessage(playerid, COLOR_YELLOW, string);
    			}
    			else
    			{
                    sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś na lotnisku !");
    			}
    		}
    		else
    		{
    			sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś w samolocie !");
    		}
    	}
    	    ...
    	    if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 577)
    	    {
    	    	if(IsPlayerInRangeOfPoint(playerid, 10,1941.0000,-2294.5068,13.4720) || IsPlayerInRangeOfPoint(playerid, 10, -1344.1194,-229.2785,14.0624) || IsPlayerInRangeOfPoint(playerid, 10, 1565.5919,1448.4813,10.7384))
    	        {
    	    	    ...
    	    	    foreach(new i : Player)
	    	        {
	    	            if(PlayerInfo[i][pWsamolocieLS]==1)
   		 				{
							format(string, sizeof(string), "LSAD: Pilot %s zakończył lot!", sendername);
							SendClientMessage(i, COLOR_YELLOW, string);
							sendTipMessageEx(i, COLOR_GREY, " Możesz teraz opuścić samolotu.");
							PlayerInfo[i][pWlociej]=0;
						}
			        }
    				SendClientMessage(playerid, COLOR_YELLOW, "LSAD: Zakańczasz lot");
    			}
    			else
    			{
    			     sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś na lotnisku !");
    	...
    	else if(strcmp(data,"koszt",true) == 0)
    	{
    	    ...
    	    {
                new koszt;
        		if(sscanf(sub, "d", koszt)) return sendTipMessage(playerid, "Użyj /lot koszt [ilosc]");

    		    if(koszt < PRICE_LOT_MIN || koszt > PRICE_LOT_MAX) { sendTipMessageEx(playerid, COLOR_GREY, "Cena biletu nie może być mniejsza od "#PRICE_LOT_MIN"$ i nie większa od "#PRICE_LOT_MAX"$"); return 1; }
                kosztbiletu = koszt;
                if(kosztbiletu>0)
    		    {
        			GetPlayerName(playerid, sendername, sizeof(sendername));
    			...
    			format(string, sizeof(string), "AirLine: Pilot %s ustalił nową cenę biletu ($%d)", sendername, kosztbiletu);
    	...
    	else if(strcmp(data,"skoki",true) == 0)
    	{
    	    if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 577)
    	    {
        		foreach(new i : Player)
    	        {
    	            if(PlayerInfo[i][pWsamolocieLS]==1)
     				{
    					SendClientMessage(i, COLOR_YELLOW, "LSAD: Pilot otworzył klapę skokową! Szykuj się do skoków!");
    					PlayerInfo[i][pMozeskakacAT]=1;
    				}
    	        }
    			SendClientMessage(playerid, COLOR_YELLOW, "LSAD: Otwierasz klapę skokową");
    		}
    		else
    		{
    			sendTipMessageEx(playerid, COLOR_GREY, " Nie jesteś w samolocie !");
    	...
    	else if(strcmp(data,"k",true) == 0 || strcmp(data,"komunikat",true) == 0)
    	{
    	    if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 577)
    	    {
    	    	GetPlayerName(playerid, sendername, sizeof(sendername));
                new result[128];
                if(sscanf(sub, "s[128]", result)) return sendTipMessageEx(playerid, COLOR_GRAD2, "Użyj /lot (/k)omunikat [tekst]");
    			foreach(new i : Player)
    	        {
    	            if(PlayerInfo[i][pWsamolocieLS]==1)
     				{
    ...
    else sendErrorMessage(playerid, "Nie jesteś z Korporacji Transportowej !");
	return 1;
}
