...
YCMD:ulepsz(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			format(string, sizeof(string), "UŻYJ: /ulepsz [nazwaulepszenia] (Posiadasz %d Punktów Ulepszenia)",PlayerInfo[playerid][gPupgrade]);
		...
		if (PlayerInfo[playerid][gPupgrade] > 1)
		{
			if (PlayerInfo[playerid][pLevel] >= 1 && (strcmp(gunid, "hp", true, strlen(gunid)) == 0))
			{
				if (PlayerInfo[playerid][pSHealth] < 50)
				{
					...
					format(string, 128, "Nowe Ulepszenie: Teraz będziesz się spawnował z %.2f Życiem (+5)",PlayerInfo[playerid][pSHealth]+50);
				}
				else
				{
					SendClientMessage(playerid, COLOR_GRAD6, "Masz maksymalną ilość HP na spawnie");
					return 1;
				}
			}
			...
			else if (PlayerInfo[playerid][pLevel] >= 3 && (strcmp(gunid, "matsiarz", true, strlen(gunid)) == 0))
			{
				...
				{
					SendClientMessage(playerid, COLOR_GRAD6, "Masz maksymalny poziom Skąpca !");
					return 1;
				}
			}
