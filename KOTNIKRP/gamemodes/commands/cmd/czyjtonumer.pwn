...
YCMD:czyjtonumer(playerid, params[], help)
{
	if (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid))
	{
		...
		{
			sendTipMessage(playerid, "Użyj /czyjtonumer [numer]");
			sendTipMessage(playerid, "FUNKCJA: Pokazuje do kogo należy numer telefonu.");
			return 1;
		}
		
		if(number <= 0)
		{
			sendErrorMessage(playerid, "Numer telefonu musi być większy niż 0!");
			return 1;
		}
