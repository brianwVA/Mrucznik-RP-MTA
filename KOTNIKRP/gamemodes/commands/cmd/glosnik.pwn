...
YCMD:glosnik(playerid, params[], help)
{
    ...
    {
        if(SanDuty[playerid] == 0 && GroupPlayerDutyPerm(playerid, PERM_NEWS)) return sendErrorMessage(playerid, "Musisz być na służbie San News");
		if(GroupPlayerDutyPerm(playerid, PERM_NEWS))
		{
			if(SANradio == 0)
			{
				...
				if(inpos)
				{
					ShowPlayerDialogEx(playerid, 765, DIALOG_STYLE_LIST, "Wybierz muzykę", "Mrucznik Radio 1\nMrucznik Radio 2\nDisco polo\nDance100\nPrzeboje\nHip hop\nParty\nWłasna", "Wybierz", "Anuluj");
				}
			}
			else
			{
				...
				sendTipMessageEx(playerid, COLOR_GREY, "Głośnik wyłączony!");
		...
		else if(CheckPlayerPerm(playerid, PERM_CLUB))
		{
			if(KLUBOWEradio == 0)
			{
				...
				if(inpos)
				{
					ShowPlayerDialogEx(playerid, 768, DIALOG_STYLE_LIST, "Wybierz muzykę", "Mrucznik Radio 1\nMrucznik Radio 2\nDisco polo\nDance100\nPrzeboje\nHip hop\nParty\nWłasna", "Wybierz", "Anuluj");
				}
			}
			else
			{
	...
	{
	    sendErrorMessage(playerid, "Nie jesteś z SAN News/Ibizy!");
	    return 1;
	}
    return 1;
}
