...
YCMD:pban(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if (PlayerInfo[playerid][pAdmin] >= 1 || IsAScripter(playerid))
		{
			...
			{
                sendTipMessage(playerid, "Użyj /pban [NICK GRACZA OFFLINE] [powod]");
                return 1;
            }
            new giveplayerid;
			...
			{
			    sendErrorMessage(playerid, "Nie możesz zbanować tego gracza (jest online (na serwerze))");
				return 1;
			}
			...
			if(AddPunishment(-1, nick, playerid, gettime(), PENALTY_BAN, 0, result, 0) == 1) {
				...
				{
					format(string, sizeof(string), "Admin %s zbanował %s (Offline). Powód: %s", GetNickEx(playerid), nick, result);
