        ...
        if(IsAtBar(playerid))
        {
            if(PlayerDrunk[playerid] < 20)
            {
				...
				if( sscanf(params, "s[16]", x_nr))
				{
					...
					SendClientMessage(playerid, COLOR_WHITE, "UŻYJ: /pij [nazwa]");
			  		SendClientMessage(playerid, COLOR_GREY, "Dostępne nazwy: Piwo ($6), Wodka ($10), Whiskey ($10), Woda ($1), Sprunk($3)");
			  		SendClientMessage(playerid, COLOR_GREY, "Dostępne nazwy: Soczek ($3), Wino ($10), Denaturat ($2), Jabol ($2), twojastara($30)");
			    ...
			    if(strcmp(x_nr,"piwo",true) == 0)
				{
					...
					{
					    SendClientMessage(playerid, COLOR_WHITE, "Barman: Musisz być pełnoletni aby pić alkohole.");
					    return 1;
					}
				...
				{
				    SetPlayerSpecialAction(playerid, SPECIAL_ACTION_DRINK_SPRUNK);
				    Item_Add("Sok pomarańczowy", ITEM_OWNER_TYPE_PLAYER, PlayerInfo[playerid][pUID], ITEM_TYPE_SPRUNK, 0, 0, true, playerid);
				    ZabierzKaseDone(playerid, 3);
				    if(health < 100)
					{
				...
				else if(strcmp(x_nr,"denaturat",true) == 0)
				{
    				...
    				SendClientMessage(playerid, COLOR_WHITE, "Barman: Mam nadzieje że nie będziesz tego pił, gdyż denaturat do tego nie służy.");
				...
				else if(strcmp(x_nr,"twojastara",true) == 0)
				{
				    if(PlayerInfo[playerid][pAge] < 18)
					{
					    SendClientMessage(playerid, COLOR_WHITE, "Barman: Musisz być pełnoletni aby pić swoją starą.");
					    return 1;
					}
					SetPlayerSpecialAction(playerid, SPECIAL_ACTION_DRINK_SPRUNK);
            ...
            {
                sendTipMessageEx(playerid, COLOR_GREY, "Nie możesz już więcej wypić, jesteś pijany !");
                return 1;
            }
        }
        else
        {
            sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś w barze !");
            return 1;
        }
    }
    return 1;
