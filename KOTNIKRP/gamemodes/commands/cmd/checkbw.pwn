...
YCMD:checkbw(playerid, params[], help)
{
    if(PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid))
	{
		...
		if(sscanf(params, "k<fix>", id)) return sendTipMessage(playerid, "Użyj /checkbw [ID]");
		...
		else
		{
			...
			format(string, sizeof(string), "Graczowi %s zostało do końca BW: %d sekund %s", GetNick(id), typeint, type); 
