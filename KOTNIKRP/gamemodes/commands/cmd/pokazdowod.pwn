...
YCMD:pokazdowod(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /dowod [id gracza]");
			return 1;
		}
		if (PlayerInfo[playerid][pDowod] == 0)
		{
			sendErrorMessage(playerid, "Nie posiadasz dowodu, wyrób go w urzędzie !");
			return 1;
		}
		...
		if(IsPlayerConnected(giveplayerid))
		{
			if(giveplayerid != INVALID_PLAYER_ID)
			{
			    if (ProxDetectorS(8.0, playerid, giveplayerid) && Spectate[giveplayerid] == INVALID_PLAYER_ID)
				{
                    ...
                    // 
                    if(PlayerInfo[playerid][pJob] == 1) { jtext = "Łowca Nagród"; }
                    ...
                    //else if(PlayerInfo[playerid][pJob] == 4) { jtext = "Diler Zioła"; }
                    //else if(PlayerInfo[playerid][pJob] == 5) { jtext = "Złodziej Aut"; }
			       ...
			       //
					...
					SendClientMessage(giveplayerid, COLOR_WHITE, string);
					format(string, sizeof(string), "- Płeć: %s", atext);
					SendClientMessage(giveplayerid, COLOR_WHITE, string);
					...
					//
					format(string, sizeof(string), "* %s pokazał Ci swój dowód osobisty.", sendername);
					SendClientMessage(giveplayerid, COLOR_PURPLE, string);
					format(string, sizeof(string), "* Pokazałeś dowód graczowi %s.", giveplayer);
