...
YCMD:zaparkuj(playerid, params[], help)
{
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		if(!IsCarOwner(playerid, lVeh)) return sendErrorMessage(playerid, "Ten pojazd nie należy do Ciebie!");
		if(IsAPlane(lVeh))
		{
			...
			if((IsPlayerInRangeOfPoint(playerid, Dom[PlayerInfo[playerid][pDom]][hLadowisko], Dom[PlayerInfo[playerid][pDom]][hWej_X], Dom[PlayerInfo[playerid][pDom]][hWej_Y],  Dom[PlayerInfo[playerid][pDom]][hWej_Z]) && PlayerInfo[playerid][pDom] != 0) || strcmp(pZone, "Las Venturas Airport", true) == 0 || strcmp(pZone, "Lotnisko", true) == 0 || strcmp(pZone, "Easter Bay Airport", true) == 0 || strcmp(pZone, "Verdant Meadows", true) == 0)
			{
				...
				format(string, sizeof(string), "Twój %s został zaparkowany w tym miejscu!", VehicleNames[GetVehicleModel(lVeh)-400]);
			...
			else
			{
		    	...
		    	{
					sendTipMessage(playerid, "Samoloty i helikoptery możesz parkować tylko na lotnisku (Aby móc parkować je koło domu, kup lądowisko w domu)");
				}
				else
				{
				    format(string, sizeof(string), "Z obecnym levelem lądowiska możesz zaparkować swój pojazd latający tylko %d metrów od domu", Dom[PlayerInfo[playerid][pDom]][hLadowisko]);
	...
	{
	    sendErrorMessage(playerid, "Nie jesteś w wozie.");
	}
	return 1;
}
