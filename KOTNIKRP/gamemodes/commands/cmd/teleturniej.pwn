...
{
    if(!(IsPlayerInGroup(playerid, 9) || PlayerInfo[playerid][pLider] == 9)) return 1;
	...
	if(IsPlayerInRangeOfPoint(playerid,2,675.6801, -1339.5690, 30.3864))
	{
		...
		for(new i=0; i< MAX_PLAYERS; i++)
		{
			...
			{
				format(string, sizeof(string), "* %s zadając pytanie rozpoczyna teleturniej",ImieGracza);
    			SendClientMessage(i,COLOR_PURPLE,string);
				new Imie1[MAX_PLAYER_NAME + 1];
				new Imie2[MAX_PLAYER_NAME + 1];
				new Imie3[MAX_PLAYER_NAME + 1];
