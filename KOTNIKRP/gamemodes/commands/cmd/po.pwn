...
YCMD:po(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if( (IsABOR(playerid) || IsAPolicja(playerid)) && OnDuty[playerid] > 0)
        {
    		...
    		{
    			sendTipMessage(playerid, "Użyj /(su)spect [playerid/CzęśćNicku] [popelnione przestepstwo]");
    			return 1;
    		}
    		if(IsPlayerConnected(giveplayerid))
    		{
    			if(giveplayerid != INVALID_PLAYER_ID)
    			{
    				...
    				{
    					sendTipMessage(playerid, "   Nie możesz dawać Wanted Level policjantom na służbie!");
    				}
    			}
    		}
    		else
        ...
        } else sendErrorMessage(playerid, " Nie jesteś funkcjonariuszem PD/FBI/NG/BOR na służbie!");
	}
	return 1;
}
