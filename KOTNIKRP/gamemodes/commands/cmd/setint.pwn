...
YCMD:setint(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /setint [nick/id] [interiorid]");
			return 1;
		}
		...
		if (PlayerInfo[playerid][pAdmin] >= 1 || IsAScripter(playerid) || PlayerInfo[playerid][pNewAP] >= 1)
		{
			...
			format(string, sizeof(string), "Ustawiłeś %s interior nr %d.", GetNick(gracz), intid);
			SendClientMessage(playerid, COLOR_GRAD1, string);
			format(string, sizeof(string), "Admin %s ustawił ci interior nr %d.", GetNickEx(playerid), intid);
