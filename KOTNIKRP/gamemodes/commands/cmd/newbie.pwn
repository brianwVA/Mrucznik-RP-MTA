...
YCMD:newbie(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        ...
        {
            sendTipMessageEx(playerid, COLOR_GRAD2, "Chat newbie jest wyłączony!");
			return 1;
        }
        if(PlayerInfo[playerid][pMuted] == 1)
		{
			sendTipMessageEx(playerid, TEAM_CYAN_COLOR, "Nie możesz mówić gdyż zostałeś uciszony !");
			return 1;
		}
		if(isnull(params))
		{
			sendTipMessage(playerid, "Użyj (/n)ewbie [tekst]");
			return 1;
		}
		...
		if (strfind(params , "ip:" , true)>=0 ||strfind(params , "-roleplay" , true)>=0 || strfind(params , "Nicolson" , true)>=0 || strfind(params , "roleplay." , true)>=0  || strfind(params , " ip" , true)>=0 || strfind(params , "794" , true)>=0 || strfind(params , "772" , true)>=0 || strfind(params , "797" , true)>=0 || strfind(params , "795" , true)>=0 || strfind(params , "775" , true)>=0 || strfind(params , "91." , true)>=0 || strfind(params , "91," , true)>=0)
		{
			...
			Log(warningLog, WARNING, "%s reklamuje na chacie newbie: %s", GetPlayerLogName(playerid), params);
		}
		else if (strfind(params , "kurwa" , true)>=0 ||strfind(params , "huj" , true)>=0 || strfind(params , "pizda" , true)>=0 || strfind(params , "cipa" , true)>=0  || strfind(params , "n00b" , true)>=0 || strfind(params , "noob" , true)>=0 || strfind(params , "pierdole" , true)>=0 || strfind(params , "debil" , true)>=0 || strfind(params , "fiut" , true)>=0 || strfind(params , "kutas" , true)>=0 || strfind(params , "jeb" , true)>=0 || strfind(params , "ssij" , true)>=0)
		{
			...
			Log(warningLog, WARNING, "%s przeklina na chacie newbie: %s", GetPlayerLogName(playerid), params);
		}
		...
		else
		{
			...
			{
			    sendTipMessageEx(playerid, COLOR_GRAD3, "Odczekaj 30 sekund zanim napiszesz nową wiadomość");
			    return 1;
			}
		    ...
		    {
				format(nobchat, sizeof(nobchat), "(({AA3333} Pół Administrator [%d] %s: {8D8DFF}%s ))", PlayerInfo[playerid][pAdmin], GetNickEx(playerid), params);
			}
			...
			{
			    format(nobchat, sizeof(nobchat), "(({7B68EE} Początkujący [%d] %s: {8D8DFF}%s ))", playerid, GetNickEx(playerid), params);
			}
			else if(PlayerInfo[playerid][pLevel] == 5)
			{
			    format(nobchat, sizeof(nobchat), "(({7B68EE} Przeciętniak [%d] %s: {8D8DFF}%s ))", playerid, GetNickEx(playerid), params);
			}
			...
			{
			    format(nobchat, sizeof(nobchat), "(({7B68EE} Doświadczony [%d] %s: {8D8DFF}%s ))", playerid, GetNickEx(playerid), params);
			}
			...
			{
			    format(nobchat, sizeof(nobchat), "(({7B68EE} Wszechwiedzący [%d] %s: {8D8DFF}%s ))", playerid, GetNickEx(playerid), params);
			}
			OOCNewbie(nobchat);
			...
			Log(chatLog, WARNING, "%s newbie: %s", GetPlayerLogName(playerid), params);
		}
	}
	return 1;
}
