...
YCMD:pblok(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		if (PlayerInfo[playerid][pAdmin] >= 1)
		{
			...
			{
				sendTipMessage(playerid, "Użyj /pblock [NICK GRACZA OFFLINE] [powod]");
				return 1;
			}
			...
			if(AddPunishment(-1, nick, playerid, gettime(), PENALTY_BLOCK, 0, result, 0) == 1) {
				...
				{
					format(string, sizeof(string), "Admin %s zablokował (offline) konto gracza %s. Powód: %s", GetNickEx(playerid), nick, result);
