...
YCMD:respawnplayer(playerid, params[], help)
{
    ...
    new v;
    if(sscanf(params, "k<fix>", v)) return sendTipMessage(playerid, "Użyj /respawnplayer [ID/Nick]");
    ...
    //wiadomosci
    new str[128];
    format(str, 128, "$System$ » Zostałeś zrespawnowany przez admina %s", GetNickEx(playerid));
    SendClientMessage(v, COLOR_LIGHTGREEN, str);
    format(str, 128, "$System$ » Zrespawnowałeś gracza o nicku %s", GetNick(v));
