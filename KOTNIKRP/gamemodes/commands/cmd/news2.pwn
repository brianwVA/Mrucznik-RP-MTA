...
YCMD:news2(playerid, params[], help)
{
	...
	if(IsPlayerConnected(playerid))
	{
		if(GroupPlayerDutyPerm(playerid, PERM_NEWS) && GroupPlayerDutyRank(playerid) >= 5)
		{
			...
			{
				sendTipMessage(playerid, "Użyj /news2 [text]"); 
				return 1;
			}
			format(string, sizeof(string), "Ustawiłeś nowy komunikat na blok informacyjny"); 
			sendTipMessageEx(playerid, COLOR_P@, string); 
			format(string, sizeof(string), "~y~NR %s:~w~%s", GetNick(playerid), Odpolszcz(text)); 
			SendNews_2(string); 
		}
		else{
			sendErrorMessage(playerid, "Ta komenda jest dostępna od 5 stopnia w San News!"); 
			return 1;
		}
	}
	return 1;
