...
YCMD:spec(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if(IsAScripter(playerid) || PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || PlayerInfo[playerid][pZG] >= 3 || IsAGameMaster(playerid))
        {
            ...
            new pid;
			if(sscanf(params, "k<fix>", pid)) return sendTipMessage(playerid, "Użyj /spec [Nick lub ID gracza] - Aby wyłączyć - Klawisz skoku");
            else if(pid == INVALID_PLAYER_ID) return sendErrorMessage(playerid, "Cel nie jest osiagalny.");
            else if(pid == playerid) return sendErrorMessage(playerid, "Nie możesz podglądać siebie samego.");
			else if(PlayerInfo[pid][pAdmin] >= 1 && PlayerInfo[playerid][pAdmin] < 1000) return sendErrorMessage(playerid, "Nie możesz specować tego admina!"); 
			...
			Log(adminLog, WARNING, "Admin %s użył /spec na graczu %s", GetPlayerLogName(playerid), GetPlayerLogName(pid));
			...
			if(playerTargetSpec[playerid] != INVALID_SPECTATE_ID)
			{
				...
				format(string, sizeof(string), "[CMD_USE_WARNING] Admin %s [%d] podgląda  %s [%d]", 
				GetNickEx(playerid), 
				playerid,
				GetNick(pid),
				pid); 
