...
YCMD:namierz(playerid, params[], help)
{
	if(IsPlayerConnected(playerid))
	{
		if(PlayerInfo[playerid][pJob] == 1 || IsPlayerInGroup(playerid, FRAC_FBI) || IsAHA(playerid) || (IsAPolicja(playerid) && GetPlayerSkin(playerid) == 285))
		{
			...
			{
				sendTipMessage(playerid, "Szukałeś już numeru telefonu, spróbuj ponownie za 2 minuty!");
				return 1;
			}
            new givenumber;
			...
			{
				sendTipMessage(playerid, "Użyj /namierz [numer telefonu]");
				return 1;
			}
            ...
            {
                foreach(new x : PlayerItems[i]) 
				{
					new p = PlayerItem[i][x][player_item_id];
					if(p == -1) continue;
            ...
            if(giveplayerid == INVALID_PLAYER_ID) return sendTipMessage(playerid, "Nie udało się namierzyć numeru...");
			if(giveplayerid == playerid)
			{
				sendErrorMessage(playerid, "Nie możesz szukać samego siebie!"); return 1;
			}
			if(Spectate[giveplayerid] != INVALID_PLAYER_ID)
			{
				sendErrorMessage(playerid, "Nie możesz namierzyć tego gracza.");
				return 1;
			}
            ...
            format(msg, sizeof(msg), "Numer telefonu %d znajduje się w okolicy %s. Strefa zaznaczona na mapie kolorem różowym.", givenumber, pZone);
			...
			//SendClientMessage(playerid, COLOR_GREY, "Skill (Łowca Nagród/Detektyw) +1");
            sendNotification(playerid, "PRACA", "~b~~h~Skill (Detektyw) +1", 5000);
			if(PlayerInfo[playerid][pDetSkill] == 50)
			{ 
                //SendClientMessage(playerid, COLOR_YELLOW, "* Twój skill Łowcy Nagród jest teraz na poziomie 2, możesz teraz szukać dłużej."); 
			...
			//else
			//{
			//	sendErrorMessage(playerid, "Złe ID/Nick !");
			//}
		}
		else
		{
			sendErrorMessage(playerid, "Nie jesteś agentem FBI lub operatorem SWAT!");
		}
	}
	return 1;
}
