...
YCMD:setveh(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /zmienauto [playerid/CzęśćNicku] [Nr. z listy (nie UID!)] [UID pojazdu]");
			return 1;
		}
		        ...
		        if(para1 != INVALID_PLAYER_ID)
		        {
		            if(nrcar >= 1 && nrcar <= MAX_CAR_SLOT)
					{
                        ...
                        if(level != 0)
                        {
                            ...
                            if(lID == -1)
                            {
                                sendTipMessage(playerid, "Pojazd nie był wczytany do systemu, inicjalizacja ...", COLOR_GRAD2);
                        ...
                        {
                            format(string, 256, "Gracz posiadał auto (UID: %d) na slocie %d. Pojazd zostaje bez własciciela.", CarData[PlayerInfo[para1][pCars][nrcar-1]][c_UID], nrcar);
                            SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
                            format(string, 256, "Admin %s zmienił Ci auto (UID: %d) na slocie %d. Pojazd zostaje bez własciciela.", GetNickEx(playerid), CarData[PlayerInfo[para1][pCars][nrcar-1]][c_UID], nrcar);
                        ...
                        if(PlayerInfo[para1][pCars][nrcar-1] != 0)
                        {
                            Log(serverLog, WARNING, "Gracz %s posiadał auto %s na slocie %d. Pojazd zostaje bez własciciela.", 
                        ...
                        if(level != 0)
                        {
    						...
    						SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
    						Log(adminLog, WARNING, "Admin %s dał %s auto %s na slot %d", 
                        ...
                        {
                            format(string, sizeof(string), " Usunięto pojazd ze slotu %d graczowi %s.", nrcar, GetNick(para1));
    						SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
    						Log(adminLog, WARNING, "Admin %s usunął %s auto %s ze slotu %d", 
                                GetPlayerLogName(playerid),
                                GetPlayerLogName(para1),
                                GetCarDataLogName(lID),
                                nrcar);
