...
YCMD:setdrunk(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
    {
		...
		if (PlayerInfo[playerid][pAdmin] >= 35 || IsAScripter(playerid) )
		{
			...
			{
				sendTipMessage(playerid, "Użyj /setdrunk [ID] [poziom (2000 wyłącza | max 50000)]");
				return 1;
			}
			...
			if(IsPlayerConnected(giveplayerid))
            {
				...
				else if(level > 50000) return sendTipMessage(playerid, "Użyj /setdrunk [ID] [poziom (2000 wyłącza | max 50000)]");

				SetPlayerDrunkLevel(giveplayerid, level);
				format(string, sizeof(string), "AdmCmd: %s [%d] ustawił %s [%d] efekt pijactwa na [%d]", 
					GetNickEx(playerid), 
					playerid,
