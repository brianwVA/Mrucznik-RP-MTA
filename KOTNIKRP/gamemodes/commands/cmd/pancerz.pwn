	...
	if(PlayerInfo[playerid][pDomWKJ] == PlayerInfo[playerid][pDom] || PlayerInfo[playerid][pDomWKJ] == PlayerInfo[playerid][pWynajem] && Dom[PlayerInfo[playerid][pDom]][hUL_D] != 0)
	{
		...
		if(IsPlayerInRangeOfPoint(playerid, 50.0, Dom[dom][hInt_X], Dom[dom][hInt_Y], Dom[dom][hInt_Z]))
		{
			if(Dom[dom][hKami] == 1)
			{
                if(pancerz >= 50.0)
				{
					sendTipMessageEx(playerid, COLOR_GRAD2, "Posiadasz już pancerz.");
					return 1;
				}
				SetPlayerArmour(playerid, 50);
				format(string, sizeof(string), "%s wyciąga kamizelkę kuloodporną i zakłada ją na siebie.", GetNick(playerid));
				ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
				//format(string, sizeof(string), "---[%s użył kamizelki domowej]---", GetNick(playerid));
				//SendAdminMessage(COLOR_GREEN, string);	
				return 1;
            }
            else
            {
                sendTipMessage(playerid, "Musisz kupić pancerz jako dodatek w domu!");
            }
        }
        else
        {
            sendTipMessage(playerid, "Musisz być w domu!");
        }
    }
    return 1;
}          