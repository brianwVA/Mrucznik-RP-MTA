...
{
    //SendClientMessage(playerid, COLOR_RED, "Komenda wyłączona na czas naprawy. Przepraszamy za utrudnienia.");
    /*
    if(PlayerInfo[playerid][pConnectTime] < 4) return sendErrorMessage(playerid, "Opis dostępny od 4 godzin online!");
    new var[8], id=-1;
    ...
    if(strlen(var) == 4 && (strcmp(var, "usuń", true) == 0 || strcmp(var, "usun", true) == 0))
    {
        if(id != -1 && PlayerInfo[playerid][pAdmin] >= 1)
        {
            ...
            {
                new str[64];
                format(str, 64, "(OPIS) - Administrator %s usunął Twój opis.", GetNick(playerid, true));
                SendClientMessage(id, COLOR_PURPLE, str);
                format(str, 64, "(OPIS) - Usunąłeś opis graczowi %s.", GetNick(id, true));
    ...
    if(PlayerInfo[playerid][pBP] == 0)
    {
        ...
        format(lStr, 256, "%s\n» Ustaw opis\n» Zmień opis\n» {FF0000}Usuń", lStr);
        ShowPlayerDialogEx(playerid, D_OPIS, DIALOG_STYLE_LIST, "Opis postaci", lStr, "Wybierz", "Wyjdź");
    }
    else
    {
