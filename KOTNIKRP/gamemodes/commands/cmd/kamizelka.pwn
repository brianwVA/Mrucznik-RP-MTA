    ...
    if(IsPlayerConnected(playerid))
    {
        ...
        if(IsAPolicja(playerid) || IsPlayerInGroup(playerid, 7) || PlayerInfo[playerid][pLider] == 7 || IsPlayerInGroup(playerid, 6))
        {
			...
			|| IsPlayerInRangeOfPoint(playerid, 5.0, 609.0364,-555.1090,19.4573))
            {
                ...
                {
                    if(kaska[playerid] < PRICE_KAMIZELKA) return sendErrorMessage(playerid, "Nie stać cię na kamizelke");
                    ZabierzKaseDone(playerid, PRICE_KAMIZELKA);
                    sendTipMessageEx(playerid, COLOR_P@, "Zapłaciłeś $"#PRICE_KAMIZELKA" za kamizelkę - wpisz /dopasuj aby dopasować."); 
                    format(string, sizeof(string), "* %s wyciąga z szafki i ubiera kamizelkę.", sendername);
