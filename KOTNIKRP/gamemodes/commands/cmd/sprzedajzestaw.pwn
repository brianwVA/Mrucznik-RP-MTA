...
YCMD:sprzedajzestaw(playerid, params[], help)
{
	if(PlayerInfo[playerid][pJob] == JOB_MECHANIC)
	{
		...
		new id;
		if(sscanf(params, "k<fix>", id)) return sendTipMessage(playerid, "Użyj /sprzedajzestaw [id]");
		if(IsPlayerConnected(id))
		{
			if(id != INVALID_PLAYER_ID)
			{
				if(id == playerid)  return sendTipMessage(playerid, "Nie możesz sprzedać sobie zestawu.");
				...
				if(PlayerInfo[id][pFixKit] >= 3) return sendTipMessageEx(playerid, 0xB52E2BFF, "Ten gracz posiada maksymalną ilość zestawów");
    ...
    {
        sendErrorMessage(playerid, "Komenda dostępna tylko dla mechanika.");
    }
	return 1;
}