YCMD:supporty(playerid, params[], help)
{
    ...
    {
        new tekst[3000], string[128], skrocony[13];
        if(PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pZG] >= 3 || IsAScripter(playerid) || PlayerInfo[playerid][pNewAP] >= 1){
            ...
            strcat(tekst, "{d9d9d9}SuppID\t{d9d9d9}Zgłaszający\t{d9d9d9}Powód" );
            for (new i = 1; i < MAX_SUPPORTS; i++)
            {
                // Pomijamy zgłoszenia puste
            ...
            ShowPlayerDialogEx(playerid, DIALOG_SUPPORT_LISTA, DIALOG_STYLE_TABLIST_HEADERS, "{d9d9d9}Zgłoszenia - SUPPORTS", tekst, "Wybierz", "Wyjdź" );
		}
		else
		{
