...
YCMD:patrol(playerid, params[], help)
{
    ...
    if(sscanf(params, "s[32]S()[32]", var, sign)) return sendTipMessageEx(playerid, COLOR_GRAD2, "Dostępne parametry: start, stop, mapa, akceptuj, lista, vsign");
    if(strcmp(var, "start", true) == 0)
    {
        ...
        if(!IsPlayerInAnyVehicle(playerid)) return sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, " Musisz być w wozie.");

        new pat = GetPatrolID();
        if(pat == -1) return sendTipMessageEx(playerid, -1, "Brak wolnych slotów na patrol.");
        SetPVarInt(playerid, "patrol-id", pat);

        ShowPlayerDialogEx(playerid, DIALOG_PATROL_NAME, DIALOG_STYLE_INPUT, "Konfiguracja patrolu » Nazwa", "Wprowadź nazwę patrolu (kryptonim)", "Dalej", "Anuluj");
    ...
    else if(strcmp(var, "stop", true) == 0)
    {
        ...
        if(PatrolInfo[pat][patroluje][1] == playerid)
        {
            ...
            sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Opuściłes patrol.");
            sendTipMessageEx(id, COLOR_PAPAYAWHIP, "Wspólnik opuscił patrol.");
        }
        else if(PatrolInfo[pat][patroluje][0] != playerid) return sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Nie jestes założycielem tego patrolu.");
        else
        {
            ...
            if(!GetPVarInt(playerid, "patrol-Wyszedl")) {
                sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Zakończono patrol.");
            } else {
                sendTipMessageEx(id, COLOR_PAPAYAWHIP, "Wspólnik opuścił patrol. 10-33!");
            }
        }
    }
    else if(strcmp(var, "vsign", true) == 0)
    {
        if(strlen(sign) <= 0) return sendTipMessage(playerid, "Użyj: /patrol vsign [kryptonim / usun]");

        if(IsPlayerInAnyVehicle(playerid))
        {
            new carid = GetPlayerVehicleID(playerid);
            if(!IsACopCar(carid))
                return sendErrorMessage(playerid, "Musisz być w pojeździe policyjnym.");
            ...
            vSigny[carid] = 1;
            //
        } else return sendTipMessage(playerid, "Musisz być w pojeździe.");
    }
    else if(strcmp(var, "akceptuj", true) == 0)
    {
        if(GetPVarInt(playerid, "patrol-dec") == 0) return sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Nikt nie oferował Ci patrolu.");
        if(GetPVarInt(playerid, "patrol") == 1) return sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Aktualnie jestes na patrolu.");
        new id = GetPVarInt(playerid, "patrol-parent");
        ...
        if(PatrolInfo[pat][pattime] != GetPVarInt(id, "patrol-time")) return sendTipMessageEx(playerid, COLOR_PAPAYAWHIP, "Oferta patrolu wygasła.");
        new str[128], nick[MAX_PLAYER_NAME+1];
        GetPlayerName(playerid, nick, MAX_PLAYER_NAME);
        format(str, 128, "PATROL »» %s zaakceptował Twoją propozycję.", nick);
