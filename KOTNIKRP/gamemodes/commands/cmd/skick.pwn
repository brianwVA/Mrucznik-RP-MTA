...
YCMD:skick(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /skick [playerid/CzęśćNicku]");
			return 1;
		}
		...
		if (PlayerInfo[playerid][pAdmin] >= 5000 || PlayerInfo[playerid][pNewAP] == 4 || PlayerInfo[playerid][pAdmin] == 7)
		{
			...
			if(IsPlayerConnected(giveplayerid))
			{
			    if(giveplayerid != INVALID_PLAYER_ID)
			    {
		            ...
		            {
		                sendTipMessageEx(playerid, COLOR_WHITE, "Nie mozesz zkickować Admina !");
		                return 1;
		            }
					Log(punishmentLog, WARNING, "Admin %s ukarał %s karą cichego kicka", 
