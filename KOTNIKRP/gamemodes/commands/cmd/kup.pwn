...
{
	ShowPlayerDialogEx(playerid, D_SHOP_CATEGORY, DIALOG_STYLE_LIST, "Sklep 24/7", "Produkty spożywcze\nSprzęt\nInne", "Dalej", "Zamknij");	
}
	...
	if(IsPlayerConnected(playerid))
	{
		...
		else if(PlayerToPoint(5.0, playerid, 809.4175,-1420.2585,-22.6193))
		{
				if(kasjerkaWolna == 666)
				{
					...
					format(string, sizeof(string), "Bilet zwykły\t %d$\nBilet VIP\t %d$", cenaNorm, cenaVIP);
					ShowPlayerDialogEx(playerid, 6997,DIALOG_STYLE_LIST,  "Vinyl-Club", string, "Akceptuj", "Odrzuć"); 
				}
				else
				{
					format(string, sizeof(string), "Aktualnie kasjerka obsługuje %s - odczekaj chwile!", GetNick(kasjerkaWolna));
		...
		|| IsPlayerInRangeOfPoint(playerid, 2.0, 833.8511,-1391.2689,-17.6433))
		{
			if(PlayerInfo[playerid][pAge] >= 18)
			{
				...
				ShowPlayerDialogEx(playerid, 6996, DIALOG_STYLE_TABLIST_HEADERS, "Vinyl-Club", stringBig, "Pij", "Odrzuć", true);
			}
			else
			{
				SendClientMessage(playerid, -1, "Barman_Jarosław mówi: Nieletnim nie sprzedajemy alkoholu!");
				return 1;
			}
		}
		else 
		{
			sendErrorMessage(playerid, "Nie jesteś w miejscu, w którym można użyć tej komendy.");
			return 1;
		}
	}
	return 1;
