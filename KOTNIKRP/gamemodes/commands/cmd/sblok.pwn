...
YCMD:sblok(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        ...
        {
            sendTipMessage(playerid, "Użyj /sblock [id/nick] [powod]");
            return 1;
        }
		...
		if (PlayerInfo[playerid][pAdmin] >= 5000 || PlayerInfo[playerid][pNewAP] == 4)
		{
        	...
        	if(IsPlayerConnected(giveplayerid))
        	{
            	if(giveplayerid != INVALID_PLAYER_ID)
            	{
	                ...
	                Log(punishmentLog, WARNING, "Admin %s ukarał %s karą cichego blocka, powód: %s", 
						GetPlayerLogName(playerid),
						GetPlayerLogName(giveplayerid),
						result);
		            PlayerInfo[playerid][pBlock] = 1;
					SendClientMessage(giveplayerid, COLOR_NEWS, "Twoje konto zostało zablokowane za zły nick. Jeśli block jest niesłuszny wejdź na M-RP i napisz prośbę o UN BLOCK");
					KickEx(giveplayerid);
