...
{
    if(GetPVarInt(playerid, "CommandSpam") > gettime()) return sendErrorMessage(playerid, "Odczekaj 15 sekund przed wysłaniem kolejnej wiadomości.");
    if(isnull(params))
        return sendTipMessage(playerid, "Użyj: /ado [tekst]");
    ...
    if(door == 0 && business == INVALID_BIZ_ID)
        return sendTipMessage(playerid, "Nie znajdujesz się przy drzwiach!");
    if(strlen(params) < 78) 
