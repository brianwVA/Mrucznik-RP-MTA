		...
		if(IsPlayerInAnyVehicle(playerid))
		{
			...
			if(IsAInteriorVehicle(model))
			{
				if(Car_GetOwnerType(vehicleid) == CAR_OWNER_GROUP) //grupy
				{
					...
					{
						return sendTipMessageEx(playerid, COLOR_LIGHTGREEN, "Ten pojazd nie należy do Twojej grupy!");
					}
				}
				...
				{
					return sendTipMessageEx(playerid, COLOR_LIGHTGREEN, "Ten pojazd nie należy do Ciebie!");
				}
				
				if(VehicleUID[vehicleid][vIntLock] == 0)
				{
				    VehicleUID[vehicleid][vIntLock] = 1;
					sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Otworzyłeś interior");
				}
				else
				{
				    VehicleUID[vehicleid][vIntLock] = 0;
					sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Zamknąłeś interior");
				}
		    }
		    else
		    {
	   	...
	   	{
	   	    sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś w wozie");
	   	}
	}
	return 1;
}
