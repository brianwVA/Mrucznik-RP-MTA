...
YCMD:windalock(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj: /windalock [nr poziomu]");
			return 1;
		}
        ...
        || IsPlayerInRangeOfPoint(playerid,3,290.7577,-1604.3273,134.6113)/*Biura SAN NEWS*/)
		{
			...
			{
				sendErrorMessage(playerid, "Nie masz uprawnień - nie jesteś z San News"); 
				return 1;
			}
			if(level > 7 || level < 0)
			{
			...
			{
				sendErrorMessage(playerid, "Nie masz wystarczających uprawnień aby otwierać to piętro!"); 
				return 1;
			}
		...
		)
		{
			...
			{
				sendErrorMessage(playerid, "Nie masz uprawnień do otwierania/zamykania windy!"); 
				return 1;
			}
			if(level > 11 || level < 0)
			{
        ...
        IsPlayerInRangeOfPoint(playerid,2.0, 1161.8228, -1337.0521, 31.6112)) && (IsPlayerInGroup(playerid, 4) || PlayerInfo[playerid][pLider] == 4))
        {
		    ...
		    {
		    	LSMCWindap0 = 1;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Zamknąłeś poziom nr [0]!");
		    ...
		    {
		    	LSMCWindap0 = 0;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Otworzyłeś poziom nr [0]!");
				GetPlayerName(playerid, nick, sizeof(nick));
		    ...
		    {
		    	LSMCWindap2 = 1;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Zamknąłeś poziom nr [2]!");
				GetPlayerName(playerid, nick, sizeof(nick));
		    ...
		    {
		    	LSMCWindap2 = 0;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Otworzyłeś poziom nr [2]!");
				GetPlayerName(playerid, nick, sizeof(nick));
		    ...
		    {
		    	LSMCWindap8 = 1;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Zamknąłeś poziom nr [8]!");
				GetPlayerName(playerid, nick, sizeof(nick));
		    ...
		    {
		    	LSMCWindap8 = 0;
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Otworzyłeś poziom nr [8]!");
				GetPlayerName(playerid, nick, sizeof(nick));
		    ...
		    {
				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Błąd! Możesz zarządzać tylko poziomami [0], [2], [8]!");
			}
		}
		else
		{
			sendErrorMessage(playerid, "Nie znajdujesz się obok windy, którą możesz zarządzać!");
		}
	}
	return 1;
}
