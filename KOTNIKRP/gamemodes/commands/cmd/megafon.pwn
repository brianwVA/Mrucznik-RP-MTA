...
YCMD:megafon(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		if(isnull(params))
		{
			sendTipMessage(playerid, "Użyj (/m)egafon [tekst]");
			return 1;
		}
		...
		{
			format(string, sizeof(string), "[Prowadzący %s: %s]", sendername, params);
			ProxDetector(50.0, playerid, string,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			return 1;
		}
		//=========================[DLA SĄDU]======================================
		if(GetPlayerVirtualWorld(playerid)== 15 || GetPlayerVirtualWorld(playerid)== 16 || GetPlayerVirtualWorld(playerid)== 17)
		{
			...
			|| IsPlayerInRangeOfPoint(playerid,5,1313.1859,-1295.6798,79.7320))//Oskarżony
			{
				format(string, sizeof(string), "[Oskarżony %s: %s]", sendername, params);
			...
			|| IsPlayerInRangeOfPoint(playerid,5,1306.6122,-1295.5017,79.7320))//Oskarżyciel
			{
				format(string, sizeof(string), "[Oskarżyciel %s: %s]", sendername, params);
				ProxDetector(100.0, playerid, string,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
				return 1;
			}
			...
			{
				format(string, sizeof(string), "[Świadek %s: %s]", sendername, params);
				ProxDetector(100.0, playerid, string,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
				return 1;
			}
		}
        ...
        else if((GroupPlayerDutyPerm(playerid, PERM_GOV)) && GroupPlayerDutyRank(playerid) >= 5)
        {
            ...
            {
                format(string, sizeof(string), "[Wykładowca %s: %s]", sendername, params);
                ProxDetector(60.0, playerid, string,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
            }
            else
            {
        ...
        else if(IsPlayerInGroup(playerid, FAMILY_SAD) && GroupRank(playerid, FAMILY_SAD) > 2)
        {
            ...
            {
                format(string, sizeof(string), "[Sędzia %s: %s]", sendername, params);
			    ProxDetector(100.0, playerid, string,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
            }
            else
            {
                return sendTipMessage(playerid, "Jako sędzia możesz używać mikrofonu tylko na sali rozpraw!");
            }
        }
		//grupy - flagi
