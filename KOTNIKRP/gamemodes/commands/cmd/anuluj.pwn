...
YCMD:anuluj(playerid, params[], help)
{
	...
	if( sscanf(params, "s[16]", x_job))
	{
		...
		SendClientMessage(playerid, COLOR_WHITE, "UŻYJ: /anuluj [nazwa]");
		SendClientMessage(playerid, COLOR_GREY, "Dostępne nazwy: Sex, Dragi, Naprawa, Prawnik, Ochrona, Wywiad, Tankowanie, Auto, Boks, Kontrakt");
		SendClientMessage(playerid, COLOR_GREY, "Dostępne nazwy: Taxi, Bus, Medyk, Mechanik, Gazeta, Mandat, Swiadek, Slub, Rozwod, taxicall");
		SendClientMessage(playerid, COLOR_GREY, "Dostępne nazwy: Tuning, Tatuaz");
	...
	else if(strcmp(x_job,"mandat",true) == 0) { SendClientMessage(TicketOffer[playerid], COLOR_PANICRED, "Podejrzany nie zgodził się zapłacić mandatu, jedź z nim na komisariat" ); TicketOffer[playerid] = 999; TicketMoney[playerid] = 0; }
	else if(strcmp(x_job,"medyk",true) == 0) { if(IsPlayerConnected(MedicCall)) { if(MedicCall == playerid) { MedicCall = 999; } else { SendClientMessage(playerid, COLOR_GREY, "   Nie zamawiałeś medyka !"); return 1; } } }
	else if(strcmp(x_job,"mechanik",true) == 0) { if(IsPlayerConnected(MechanicCall)) { if(MechanicCall == playerid) { MechanicCall = 999; } else { SendClientMessage(playerid, COLOR_GREY, "   Nie zamawiałeś  mechanika !"); return 1; } } }
	else if(strcmp(x_job,"tatuaz",true) == 0) { 
		new offer = GetPVarInt(playerid, "Tattoo-Offer");
	...
	{
		if(IsPlayerInGroup(playerid, 10) || PlayerInfo[playerid][pLider]==10)
 		{
   			if(TaxiCall < 999)
      		{
        		if(TransportDuty[playerid] == 1 && TaxiCallTime[playerid] > 0)
   			...
   			{
	           sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś z Korporacji !");
   			}
   		}
   	}
		...
		if(IsPlayerInGroup(playerid, 8) || PlayerInfo[playerid][pLider]==8)
 		{
      		...
      		{
				GroupSendMessage(8, COLOR_YELLOW, sprintf("Kontrakt na %s, nagroda za wykonanie $%d.", GetNickEx(GoChase[playerid]), PlayerInfo[GoChase[playerid]][pHeadValue]));
				GotHit[playerid] = 0;
   			...
   			{
	           sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś z HA !");
   			}
   		}
   	}
	...
	{ return 1; }
	new string[32];
	format(string, sizeof(string), "* Anulowałeś: %s.", x_job);
