				...
				if(IsAInteriorVehicle(model))
				{
	          		...
	          		if(IsPlayerInRangeOfPoint(playerid, 15.0, vehx, vehy, vehz))
	          		{
                        if(PlayerInfo[playerid][pJailed] != 0)
                        {
                            ...
                            {
								Log(punishmentLog, WARNING, "%s dostał bana od antycheata, powód: Ucieczka przed AJ");
                                Ban(playerid);
                            }
                            return 1;
                        }
	          		    if(VehicleUID[v][vIntLock] == 1 || GetVehicleModel(v) == 570)
	          		    {
							...
							{
								sendErrorMessage(playerid, "Nie posiadasz biletu! Kup go na dworcu za pomocą /kupbiletpociąg(/kbpo)");
								return 1;
							}
							else
							{
						...
						{
						    sendTipMessage(playerid, "Interior jest zamknięty! Otwórz go za pomocą /lockint");
						    return 1;
						}
	          		}
				}
		    }
		    sendTipMessage(playerid, "Jesteś zbyt daleko od pojazdu do którego można wejść w ten sposób");
		    return 1;
	   	}
	}
	return 1;
