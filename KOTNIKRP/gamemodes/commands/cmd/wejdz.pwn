...
//======================================================================
        if(GetPVarInt(playerid, "AC-izolacja") != 0) return sendTipMessageEx(playerid, COLOR_PANICRED, "Jesteś odizolowany, nie możesz używać tej komendy.");
        ...
        // Boczne wejscie od góry do intka banku
        else if(IsPlayerInRangeOfPoint(playerid, 5.0, 1487, -1004, 46))
        {
            SetPlayerVirtualWorld(playerid, 2);
            SetPlayerInterior(playerid, 0);
            SetPlayerPos(playerid, 1464.07, -1010.61, 74.21);
            Wchodzenie(playerid);
            PlayerInfo[playerid][pLocal] = PLOCAL_INNE_BANK;
        }
//======================================================================
//==================[Windy pod /wejdz]==================================
//======================================================================
        ...
        {
            ShowPlayerDialogEx(playerid, 121, DIALOG_STYLE_LIST, "Wybierz pomieszczenie", "Salka Konferencyjna\nBiura\nPiwnice", "Wybierz", "Wyjdź");
        }
        else if(IsPlayerInRangeOfPoint(playerid,3,1450.6615,-1819.2279,77.9613) 
			||  (IsPlayerInRangeOfPoint(playerid,5,1285.5134, -1329.1538, 13.5492) && IsAUrzednik(playerid, 0))
            ||  IsPlayerInRangeOfPoint(playerid,3,1481.5200,-1821.0967,58.1563))
        {
            ShowPlayerDialogEx(playerid,122,DIALOG_STYLE_LIST,"Winda: Wybierz Piętro","[Poziom 0] Zaplecze\n[Poziom 9] Główna sala urzędu\n[Poziom 10] Kancelaria burmistrza","Wybierz","Wyjdź");
        }
		...
		{
            ShowPlayerDialogEx(playerid, D_WINDA_LSFD, DIALOG_STYLE_LIST, "Winda", "Garaż\nPierwsze piętro\nDrugie piętro\nDach", "Wybierz", "Wyjdź");
		}
        else if((IsPlayerInRangeOfPoint(playerid,4,1568.7660,-1691.4886,5.8906) && IsAPolicja(playerid, 0) && GetPlayerVirtualWorld(playerid) == 2)//parking Dolny LSPD 1568.7660,-1691.4886,5.8906
            ...
            || (IsPlayerInRangeOfPoint(playerid,4,1564.9027,-1665.8291,28.4815) && GetPlayerVirtualWorld(playerid) == 28)//Sala przesłuchań
            || (IsPlayerInRangeOfPoint(playerid,4,1543.3915,-1643.2813,28.4881) && IsAPolicja(playerid, 0) && GetPlayerVirtualWorld(playerid) == 29)//więzienie
            || (IsPlayerInRangeOfPoint(playerid,4,1564.92, -1665.82, 28.39) && IsAPolicja(playerid, 0) && GetPlayerVirtualWorld(playerid) == 0)//dach lspd
            || (IsPlayerInRangeOfPoint(playerid,4,1571.29,-1635.58, 13.56) && IsAPolicja(playerid, 0) && GetPlayerVirtualWorld(playerid) == 0))//gorny LSPD
        {
            ShowPlayerDialogEx(playerid,WINDA_LSPD,DIALOG_STYLE_LIST,"Winda","[Poziom -2]Więzienie\n[Poziom -1]Parking Dolny\n[Poziom 0] Parking Górny\n[Poziom 1]Recepcja\n[Poziom 2]Biuro komendanta\n[Poziom 3]Biura\n[Poziom 4]Konferencyjne\n[Poziom 5]Sale przesłuchań\n[Poziom 6]Dach","Jedz","");
        }
            ...
            || PlayerToPoint(2.0, playerid,1581.6999511719, 1444.5, 14.10000038147))//LS
        {
            ...
            if(IsPlayerInRangeOfPoint(playerid, 25, PosX,PosY,PosZ))
            {
                if(planeisstopped == 1 || IsPlayerInFraction(playerid, FRAC_KT, 5000))
                {
                    ...
                    {
                        GameTextForPlayer(playerid, "Witamy w samolocie! Znajdź swoje miejsce", 4000, 3);
                ...
                {
                    sendTipMessageEx(playerid, COLOR_GREY, "Drzwi samolotu są zamknięte !");
                }
            }
            else
            {
                 sendTipMessageEx(playerid, COLOR_GREY, "Nie ma w pobliżu samolotu !");
            }
        }
...
//======================================================================
//=================[Nieznane - nieokreślone - nieprzeniesione]==========
//======================================================================
       ...
       //wejscie do vinyl
        ...
        {
            if(vinylStatus == 0 && !CheckPlayerPerm(playerid, PERM_CLUB))
            {
                sendTipMessage(playerid, "Vinyl jest aktualnie zamknięty! Skontaktuj się z właścicielem klubu."); 
                return 1;
            }
            if(GetPVarInt(playerid, "Vinyl-bilet") < 1 && !CheckPlayerPerm(playerid, PERM_CLUB))
            {
        ...
        else if(PlayerToPoint(1.5, playerid, 395.9647, -1811.1703, 7.8789) && GetPlayerVirtualWorld(playerid) == 21)
        {
            ...
            {
                if(!CheckPlayerPerm(playerid, PERM_CLUB))
                {
            ...
            if  (!IsAPolicja(playerid, 0))
			{
                ...
                if(wyrzuc)
                {
                    ...
                    SendClientMessage(playerid, COLOR_WHITE, "Nie chcesz kłopotów, więc oddajesz swój arsenał ochroniarzowi Ibizy.");
        ...
        else if (PlayerToPoint(2.0, playerid,-2170.0593,641.1239,1052.3817)) //Zakład bukmaherski WEJSCIE NA GORE
        {
        ...
        {
            if(ApprovedLawyer[playerid] == 1 || IsPlayerInGroup(playerid, FRAC_FBI))
                SetPlayerPos(playerid,-1674.4122,917.7183,-52.4141); //nowe komi drzwi w celach
            else
                SendClientMessage(playerid, COLOR_GRAD1, "Musisz mieć pozwolenie prawnicze aby przejść");
        ...
        else if (IsPlayerInRangeOfPoint(playerid, 5.0, 276.32934570313,122.20029449463,1004.1166992188)) // Pokój przesłuchań wejs
        {
            if(IsAPolicja(playerid, 0))
            {
                SetPlayerPos(playerid, 2371.5246582031, 586.56274414063, 4893.2431640625); // Pokój przesłuchań sro
                TogglePlayerControllable(playerid, 0);
                Wchodzenie(playerid);
                SetPlayerInterior(playerid, 10);
                return 1;
            ...
            {
                SendClientMessage(playerid, COLOR_GREY, "Tylko dla policjantów i więźniów w kajdankach");
                return 1;
            }
        }
		//AirTower LS
...
//======================================================================
//=================[BASEN TSUNAMI WEJŚCIA]==============================
//======================================================================
        else if (IsPlayerInRangeOfPoint(playerid, 5.0,  1205.45874, -1765.66833, 13.54815)) // basen wejście
        {
			if(poolStatus == 1)
			{
				if(Kredyty[playerid] >= 200)
				{
					SetPlayerPos(playerid, 1213.3364,-1807.0619,70.0682); // basen środek
					GameTextForPlayer(playerid, "~w~Szatnie~n~ ~r~by Simeone & Sergio", 5000, 1);
					sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Aby się przebrać podejdź do szafek i wpisz /przebierz");
					...
					new string[128];
					format(string, sizeof(string), "Pozostało Ci %d kredytów", Kredyty[playerid]);
				...
				{
					sendTipMessageEx(playerid, COLOR_GREY, "Nie masz wystarczającej ilości kredytów.");
					return 1;
				}
			}
			else
			{
				SendClientMessage(playerid, -1, "Pani Janina mówi: Przykro mi! Szef jeszcze nie otworzył, przyjdź później.");
				return 1;
			}
        }
        else if (IsPlayerInRangeOfPoint(playerid, 5.0, 1213.28540, -1790.80505, 70.25735)) // sauny wejście
        {
            if(Kredyty[playerid] >= 75)
            {
                SetPlayerPos(playerid, 1274.3020,-1756.0800,48.5812); // sauny środek
...
//======================================================================
//=================[IBIZA WEJSĆIA I SKRYPT==============================
//======================================================================
        //IBIZA
        else if(IsPlayerInRangeOfPoint(playerid, 2.0, 397.6700,-1811.8844,14.7359) && GetPlayerVirtualWorld(playerid) == 0)
        {
            if(!IbizaDach) return sendTipMessageEx(playerid, 0xB52E2BFF, "Wyjście na dach jest zamknięte");
            Wchodzenie(playerid);
        ...
        //wejście do bileterki
        else if(IsPlayerInRangeOfPoint(playerid, 12.0, 1740.8485,-2471.0625,14.2266) && CheckPlayerPerm(playerid, PERM_CLUB) && GetPlayerVirtualWorld(playerid) == 1) //RANGA
        {
        ...
        {
            if(IbizaZamek) return sendTipMessageEx(playerid, 0xB52E2BFF, "Klub jest w tej chwili zamknięty");
            if(!GetPVarInt(playerid, "IbizaBilet") && !CheckPlayerPerm(playerid, PERM_CLUB)) return sendTipMessageEx(playerid, 0xB52E2BFF, "Nie posiadasz biletu, kup go w kasie"); //RANGA
            ...
            SendClientMessage(playerid, 0x88aa62FF, "((UWAGA! W naszym klubie często można się spotkać z wiadomościami typu: Audio stream.. aby wyłączyć ten komunikat wpisz /audiomsg))");
        ...
        // BAR - wejscie
        else if(IsPlayerInRangeOfPoint(playerid, 2.0, 1898.4817,-2494.8752,13.6266) && GetPlayerVirtualWorld(playerid) == 1 && CheckPlayerPerm(playerid, PERM_CLUB)) //RANGA
        {
        ...
        {
            if(!IbizaBiuro) return sendTipMessageEx(playerid, 0xB52E2BFF, "Biuro jest w tej chwili zamknięte");
        ...
        {
            if(IbizaZamek) return sendTipMessageEx(playerid, 0xB52E2BFF, "Klub jest w tej chwili zamknięty");
            if(!CheckPlayerPerm(playerid, PERM_CLUB)) return sendTipMessageEx(playerid, 0xB52E2BFF, "Nie możesz tędy przejść, nie jesteś z Ibiza Club!"); //RANGA
            TogglePlayerControllable(playerid, false);
...
//======================================================================
            ...
            Wejdz(playerid, 1302.8000488281,-988.20001220703,38.299999237061, 1301.8000488281,-1083.9000244141,5972.2001953125, 5.0);//główne wejście
			...
			//BIZNESY
            if(SprawdzBiznesy(playerid)) return 1;

            //DOMY
            #if !defined MRP_LEGACY_HOUSES
            if(House_Enter(playerid)) return 1;
            #endif
        }
    }
    return 1;
