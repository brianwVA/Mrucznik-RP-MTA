...
YCMD:pogodaall(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
    {
        if(PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || PlayerInfo[playerid][pZG] >= 7 || IsAScripter(playerid))
		{
			...
			{
			    sendTipMessage(playerid, "Użyj: /pogodaall [pogodaid]");
			    return 1;
			}
            ...
            new string[128];
            format(string, 128, "CMD_Info: /pogodaall użyte przez %s [%d]", GetNickEx(playerid), playerid);
            SendCommandLogMessage(string);
			Log(adminLog, WARNING, "Admin %s użył /pogodaall", GetPlayerLogName(playerid));
