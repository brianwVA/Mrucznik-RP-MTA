...
YCMD:boombox(playerid, params[], help)
{
    ...
    if(PlayerInfo[playerid][pAdmin] > 0 || PlayerInfo[playerid][pNewAP] > 0)
    {
        if(strcmp(sub, "szukaj", true) == 0)
        {
            ...
            if(!isnull(var))
            owner = strval(var);
            SendClientMessage(playerid, COLOR_GREEN, "Boomboxy w pobliżu:");
        ...
        if(strcmp(sub, "usun", true) == 0)
        {
            ...
            if(sscanf(var, "d", bbxid))
                return sendTipMessage(playerid, "Użyj: /boombox usun [id (możesz sprawdzić poprzez /boombox szukaj)]");
    ...
    if(id != -1)
    {
        if(isnull(sub))
        {
            ...
            {
                if(!IsPlayerInRangeOfPoint(playerid, 1.4, BoomBoxData[id][BBD_x], BoomBoxData[id][BBD_y], BoomBoxData[id][BBD_z])) return sendTipMessageEx(playerid, COLOR_GRAD2, "Musisz być obok boomboxa.");
        ...
        else
        {
            ...
            if(strcmp(sub, "znajdz", true) == 0 || strcmp(sub, "znajdź", true) == 0)
            {
                SetPlayerCheckpoint(playerid, BoomBoxData[id][BBD_x], BoomBoxData[id][BBD_y], BoomBoxData[id][BBD_z], 2.0);
                SendClientMessage(playerid, -1, "Jeżeli nie możesz znaleźć boombox'a poproś administratora o usunięcie go.");
                return 1;
            }
            ...
            if(strcmp(sub, "off", true) == 0)
            {
                ...
                BBD_Turn(id);
                format(var, sizeof(var), "** %s wyłącza boomboxa.", GetNick(playerid));
            ...
            else if(strcmp(sub, "on", true) == 0)
            {
                ...
                BBD_Turn(id);
                format(var, sizeof(var), "** %s włącza boomboxa.", GetNick(playerid));
	            ProxDetector(15.0, playerid, var, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            }
            else if(strcmp(sub, "url", true) == 0)
            {
                ...
                BBD_Turn(id);
                format(var, sizeof(var), "** %s zmienia nutę w boomboxie", GetNick(playerid));
	            ProxDetector(15.0, playerid, var, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            }
        }
    }
    else
    {
        ...
        id = BBD_GetID();
        if(id == -1) return sendTipMessageEx(playerid, COLOR_GRAD2, "Osiągnięto limit boomboxów (15).");
