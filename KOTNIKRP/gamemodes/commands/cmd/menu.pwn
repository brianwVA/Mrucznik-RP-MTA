...
YCMD:menu(playerid, params[], help)
{
    ...
    if(!OnDuty[playerid]) return sendTipMessage(playerid, "Nie jesteś na służbie!");

    new string[428], giveplayerid;
    if(sscanf(params, "r", giveplayerid))
        return sendTipMessage(playerid, "Użyj: /menu [ID/nick gracza]");
    ...
    if(strlen(string) < 30)
        return sendErrorMessage(playerid, "Twoja grupa nie posiada żadnych produktów!");
