...
YCMD:givegun(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /givegun [ID gracza] [Broń] [Amunicja]");
		...
		if(gun < 1||gun > 46)
		{ sendTipMessageEx(playerid, COLOR_GRAD1, "Błędne ID broni!"); return 1; }

		if(ammo <1||ammo > 99999)
		{ sendTipMessageEx(playerid, COLOR_GRAD1, "Amunicja może być od 1 do 99999 !"); return 1; }
		if(PlayerInfo[playerid][pAdmin] >= 5000  || IsAScripter(playerid))
		{
		    if(IsPlayerConnected(playa))
		    {
		        if(playa != INVALID_PLAYER_ID)
		        {
					...
					{
					    SendClientMessage(playerid, COLOR_GREY, "Broń zablokowana lub nie istnieje !");
					    return 1;
					}
					...
					Log(adminLog, WARNING, "Admin %s dał graczowi %s broń %s", GetPlayerLogName(playerid), GetPlayerLogName(playa), GetWeaponLogName(gun, ammo));
				}
			}
		}
		else
