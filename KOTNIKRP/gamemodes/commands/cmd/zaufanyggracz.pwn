...
YCMD:zaufanygracz(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		if(isnull(params))
		{
			sendTipMessage(playerid, "Użyj /zg [admin chat]");
			return 1;
		}
		switch (PlayerInfo[playerid][pZG]) {
			...
			case 8: format(string, sizeof(string), "*%d Świetny ZG %s: %s",PlayerInfo[playerid][pZG], sendername, params);
			case 9: format(string, sizeof(string), "*%d Znakomity ZG %s: %s",PlayerInfo[playerid][pZG], sendername, params);
			case 10: format(string, sizeof(string), "*%d Ćwierć admin %s: %s",PlayerInfo[playerid][pZG], sendername, params);
		}

		if(PlayerInfo[playerid][pAdmin] > 0) format(string, sizeof(string), "*%d Admin %s: %s",PlayerInfo[playerid][pAdmin], sendername, params);
		else if(PlayerInfo[playerid][pNewAP] > 0) format(string, sizeof(string), "*%d Pół-Admin %s: %s",PlayerInfo[playerid][pNewAP], sendername, params);
		
		SendZGMessage(0xFFF08080, string);
