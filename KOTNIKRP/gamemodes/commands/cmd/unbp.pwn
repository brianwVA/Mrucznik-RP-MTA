...
YCMD:unbp(playerid, params[], help)
{
	if(PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid))
	{
		...
		{
			sendTipMessage(playerid, "Użyj /unbp [ID GRACZA]"); 
			return 1;
		}
		...
		format(string, sizeof(string), "Administrator %s zdjął Ci blokadę pisania.", GetNickEx(playerid));
		sendTipMessageEx(giveplayerid, COLOR_P@, string);
		format(string, sizeof(string), "Administrator %s zdjął blokadę dla %s.", GetNickEx(playerid), GetNick(giveplayerid));
