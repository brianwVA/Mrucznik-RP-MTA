...
{
	new string[128];
	new giveplayer01[MAX_PLAYER_NAME + 1];
	new giveplayer02[MAX_PLAYER_NAME + 1];
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /tp [playerid/CzęśćNicku] [playerid/CzęśćNicku]");
			return 1;
		}
		...
		if (IsPlayerConnected(plo) && IsPlayerConnected(plo1))
		{
		    if(plo != INVALID_PLAYER_ID && plo1 != INVALID_PLAYER_ID)
		    {
				...
				if (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || PlayerInfo[playerid][pZG]==10 || Uprawnienia(playerid, ACCESS_PANEL) || IsAScripter(playerid) || IsAGameMaster(playerid))
				{
					...
					{
						sendTipMessageEx(playerid, COLOR_GRAD1, "Na serwerze są admini, nie możesz użyć tej komendy");
						return 1;
					}
					...
					format(string, sizeof(string), "Zostałeś teleportowany do %s (ID: %d) przez Admina %s.", giveplayer02, plo, GetNickEx(playerid));
