...
YCMD:wyjdz(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
	{
		...
		else if(IsPlayerInRangeOfPoint(playerid, 2, 304.5840,-142.0804,1004.0625) && GetPlayerInterior(playerid) == 7) //strzelnica wyjscie
		{
			SetPlayerPos(playerid, 305.0363,-128.1189,999.6016);
		}
		else if(IsPlayerInRangeOfPoint(playerid, 2, 1994.08, -2127.61, 30.56) && GetPlayerVirtualWorld(playerid) == 25 && PlayerInfo[playerid][pJailed] == 0) // Wyjście z celi
		{
			SetPlayerPos(playerid, 1983.33, -2122.39, 30.58);
		...
		else if(IsPlayerInRangeOfPoint(playerid, 4, 1213.3364,-1807.0619,70.0682)) //basen środek 
		{
		...
		else if((IsPlayerInRangeOfPoint(playerid,4,1543.3915,-1643.2813,28.4881) || IsPlayerInRangeOfPoint(playerid,4,1555.1351,-1643.0166,28.4881)) && GetPlayerVirtualWorld(playerid) == 29 && !IsAPolicja(playerid) && PlayerInfo[playerid][pJailed] == 0) //miejsce do /wyjdz dla osób które nie są więźniami
        {
            Wchodzenie(playerid);
            SetPlayerVirtualWorld(playerid, 25);
		...
		{
			ShowPlayerDialogEx(playerid, 696, DIALOG_STYLE_LIST, "Winda:", "[Poziom -1] Parking wewnętrzny\n[Poziom 0] Parking zewnętrzny\n[Poziom 1] Centrala GSA\n[Poziom 2] Sale Treningowe\n[Poziom 3] Dach", "Wybierz", "Anuluj");
		}
		//winda FBI
        ...
        else if (PlayerToPoint(2.0, playerid,211.7162,1811.4824,21.8594) && (PlayerInfo[playerid][pJailed] == 0)) //Wejście do więzienia stanowego
        {
            ...
            if(IsPlayerInGroup(playerid, 2) || PlayerInfo[playerid][pLider] == 2)
            {
            ...
            {
                SetPlayerPos(playerid,-1606.0139,818.9503,-29.4141); //Wejście do więzienia stanowego
		...
		else if (IsPlayerInRangeOfPoint(playerid, 5.0, 1193.2720,-1774.8802,13.7282)) // basen tylko dla personelu
		{
			...
			{
			    sendTipMessageEx(playerid, COLOR_GREY, "Nie jesteś upoważniony do korzystania z wejścia dla personelu");
			}
		}
		...
		//Tory szkoleniowe
		else if (IsPlayerInRangeOfPoint(playerid, 5.0, -653.34765625,-5448.5634765625,13.368634223938) || // wybieralka fbi Strzelnica FBI środek
				IsPlayerInRangeOfPoint(playerid, 5.0, 1703.9327392578,141.29598999023,30.903503417969) || // wybieralka fbi Strzelnica terenowa
				IsPlayerInRangeOfPoint(playerid, 5.0, 1581.8731689453,5490.7412109375,329.73870849609) || // wybieralka fbi spadochrin wejście środek
				IsPlayerInRangeOfPoint(playerid, 5.0, 2236.0786132813,-6891.217773475,21.423152923584) || // wybieralka fbi Porwanie w domu środek
				IsPlayerInRangeOfPoint(playerid, 5.0, -433.55599975586,-3812.807373046,176.7035064697) || // wybieralka fbi Terroryści na statku
				IsPlayerInRangeOfPoint(playerid, 5.0, 1630.513671875,5232.2626953125,2.6707870960236)) // wybieralka fbi spadochrin wejście środek
		{
    	...
    	//wyjście bileterki
    	else if(IsPlayerInRangeOfPoint(playerid, 2.0, 397.6174,-1806.2030,7.8381) && CheckPlayerPerm(playerid, PERM_CLUB) && GetPlayerVirtualWorld(playerid) == 0) //RANGA
    	{
    	...
    	// BAR - wyjscie
    	else if(IsPlayerInRangeOfPoint(playerid, 2.0, 1900.5961,-2494.9373,13.6266) && GetPlayerVirtualWorld(playerid) == 1 && IsPlayerInGroup(playerid, 18)) //RANGA
    	{
    	...
    	//wyjście boczne
    	else if(IsPlayerInRangeOfPoint(playerid, 2.0, 1917.3785,-2465.4368,15.4266) && GetPlayerVirtualWorld(playerid) == 1 && CheckPlayerPerm(playerid, PERM_CLUB)) //RANGA
    	{
        ...
        else if (IsPlayerInRangeOfPoint(playerid, 3.0, 1763.4243, -1128.0543, 224.1499))//Wyjście
		{
		...
		{
			Wyjdz(playerid, 738.5865, -1427.7610, 23.5927, 738.8545, -1428.7880, 13.5927, 5.0, "~w~Zapraszamy ponownie!"); //wyjście jetty przy ulicy
			Wyjdz(playerid, 734.0619, -1451.6720, 22.5920, 725.6371, -1440.3321, 13.5360, 5.0, "~w~Zapraszamy ponownie!"); //wyjście jetty pod schodami	
		    ...
		    Wyjdz(playerid, 1302.8000488281,-988.20001220703,38.299999237061, 1301.8000488281,-1083.9000244141,5972.2001953125, 5.0);//główne wejście
			...
			if(Motel_Room_CheckWyjdz(playerid)) return 1;

			if(SprawdzBiznesy(playerid)) return 1;
			#if !defined MRP_LEGACY_HOUSES
			if(House_Exit(playerid)) return 1;
			#endif
		}
