...
YCMD:departament(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        if(GroupPlayerDutyPerm(playerid, PERM_DEPO))
        {
            ...
            if(isnull(params))
            {
                SendClientMessage(playerid, COLOR_GRAD2, "UŻYJ: (/d)epartament [tekst]");
                return 1;
            }
			...
			{
				sendErrorMessage(playerid, "Dobry admin nie powinien robić OOC w IC! Pisz poprzez /depo [treść]");
				return 1;
			}
        ...
        {
            SendClientMessage(playerid, COLOR_GRAD2, "Nie jesteś na służbie !");
            return 1;
        }
    }
    return 1;
