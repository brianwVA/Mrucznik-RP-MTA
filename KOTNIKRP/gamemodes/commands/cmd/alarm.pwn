    ...
    if(IsPlayerInGroup(playerid, FRAC_BOR) && GroupRank(playerid, FRAC_BOR) > 2 || IsPlayerInGroup(playerid, 22) && GroupRank(playerid, 22) > 2)
	{
        ...
        || IsPlayerInRangeOfPoint(playerid,5,1315.6658,-1318.4017,74.5859))
    	{
            if(!GATE_SAD_ALARM)
            {
                ...
                SendRadioMessage(FRAC_BOR, 0xFFFF0000, "[Słuchawka bluetooth] Uwaga! K3 w 0-6! W sądzie włączył się alarm! Sprawdź to!");
				new akcja[150];
				format(akcja,sizeof(akcja),"* %s wciska czerwony guzik i włącza alarm.",GetNick(playerid));
                ProxDetector(30.0, playerid, akcja, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
                format(akcja, 150, "> %s uruchomił alarm w sądzie!", GetNick(playerid));
            ...
            {
                SendRadioMessage(FRAC_BOR, 0xFFFF0000, "[Słuchawka bluetooth] Alarm w sądzie odwołany.");
				new akcja[150];
				format(akcja,sizeof(akcja),"* %s wciska czerwony guzik i wyłącza alarm.",GetNick(playerid));
                ProxDetector(30.0, playerid, akcja, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			...
			if(IsAtTheDMVWindows(playerid))
			{
				if(GetPlayerVirtualWorld(playerid) == 50)
				{
					if(DMV_ALARM == 0)
					{
						...
						format(string, sizeof(string), "[Słuchawka bluetooth] Uwaga! %s aktywował alarm w Urzędzie - rzucaj wszystko i ruszaj tam!", GetNick(playerid));
						SendRadioMessage(FRAC_BOR, 0xFFFF0000, string);
						format(string, sizeof(string), "[RADIO] Uwaga! Kryj się! %s aktywował alarm w Urzedzie.", GetNick(playerid));
						SendRadioMessage(FRAC_GOV, 0xFFFF0000, string);
						
						//AKCJA 
						format(string,sizeof(string),"* %s wciska czerwony guzik pod biurkiem i włącza alarm.",GetNick(playerid));
						...
						foreach(new i : Player)
						{
							if(GetPlayerVirtualWorld(playerid) == 50)
							{
								sendTipMessage(i, "[GŁOŚNIKI URZĘDU MIASTA] ALARM W URZEDZIE MIASTA! UDAJ SIĘ DO BEZPIECZNEJ STREFY");
							}
					...
					{
						//POWIADOMIENIE DLA BOR I DMV
						format(string, sizeof(string), "[Słuchawka bluetooth] %s wyłączył alarm w Urzędzie Miasta!", GetNick(playerid));
						SendRadioMessage(FRAC_BOR, 0xFFFF0000, string);
						format(string, sizeof(string), "[RADIO] %s wyłączył alarm w Urzędzie Miasta!", GetNick(playerid));
						SendRadioMessage(FRAC_GOV, 0xFFFF0000, string);
						
						//AKCJA 
						format(string,sizeof(string),"* %s wciska czerwony guzik pod biurkiem i wyłącza alarm.",GetNick(playerid));
						ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
						//MOVE OBJECTS:
