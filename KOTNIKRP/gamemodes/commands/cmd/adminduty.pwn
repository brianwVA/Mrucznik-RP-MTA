...
YCMD:adminduty(playerid, params[], help)
{
	...
	{	
		if(GetPlayerAdminDutyStatus(playerid) == 0)
		{
			...
			{
				sendErrorMessage(playerid, "Jesteś na służbie IC, graj IC!");  
				return 1;
			}
		...
		else if(GetPlayerAdminDutyStatus(playerid) == 1)//Wykonuje zejście z duty
		{	
	...
	{
		sendErrorMessage(playerid, "Nie jesteś administratorem, więc nie możesz tego użyć!"); 
