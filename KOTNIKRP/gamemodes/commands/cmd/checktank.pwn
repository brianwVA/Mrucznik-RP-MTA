...
YCMD:checktank(playerid, params[], help)
{
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /checktank [id gracza/część nicku]");
			return 1;
		}
        ...
        if (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid))
        {
            ...
            if(!carid) return sendErrorMessage(playerid, sprintf("%s [%d] musi znajdować się w pojeździe!", GetNick(giveid), giveid));
