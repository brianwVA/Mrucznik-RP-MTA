    ...
    if(IsPlayerConnected(playerid)){
        ...
        if(gPlayerLogged[playerid] == 1){
            if(sscanf(params, "is[128]", giveplayerid, text)){
                sendTipMessage(playerid, "Użyj /report [playerid] [tekst]"); 
                return 1;
            }
            if(IsPlayerConnected(giveplayerid)){
                if(giveplayerid == playerid){
                        sendTipMessage(playerid, "Nie możesz zreportować sam siebie. Użyj /support w innym wypadku niż zgłoszenie gracza!"); 
                        return 1;
                }
                if(GetPlayerAdminDutyStatus(giveplayerid) == 1){
                    sendTipMessage(playerid, "Zgłoszenia dotyczące członków administracji składamy na forum!");
                    return 1;
                }
                if(IsPlayerNPC(giveplayerid)){
                    sendTipMessage(playerid, "Błędne ID gracza!");
                    return 1;
                }
                if(GetPVarInt(playerid, "wyreportowany") > 1){
                    sendTipMessageFormat(playerid, "Odczekaj %d sekund przed następnym wysłaniem zgłoszenia!", GetPVarInt(playerid, "wyreportowany"));
                    return 1;
                }
                if (strlen(text) <= 0){
                ...
                if( reportid == -1 ) return sendTipMessage( playerid, "Osiągnięto maksymalną liczbę zgłoszeń!" );
                ...
                SendClientMessage(playerid, 0x008000AA, "Twój report został wysłany do administracji, oczekuj na reakcję zanim napiszesz kolejny!");
