...
YCMD:kick(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		{
			sendTipMessage(playerid, "Użyj /kick [playerid/CzęśćNicku] [reason]");
			return 1;
		}
		if(giveplayerid == 65535)
		{
			...
			{
				sendTipMessageEx(playerid, COLOR_GRAD2, "Ten gracz ma zbugowane ID. Wpisz jego ID zamiast nicku aby go zkickować.");
				return 1;
			}
		}
		if(!IsPlayerConnected(giveplayerid))
		{
			sendErrorMessage(playerid, "Błędne ID gracza!"); 
			return 1;
		}
		...
		if (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || PlayerInfo[playerid][pZG] >= 1 || IsAScripter(playerid))
		{
			...
			if(IsPlayerConnected(giveplayerid) || true)//bug z id
			{
			    if(giveplayerid != INVALID_PLAYER_ID)
			    {
					...
					{
						sendTipMessage(playerid, "Nie możesz dawać /kick administratorowi!");
						return 1;
					}
                    if(PlayerInfo[giveplayerid][pAdmin] >= 1) return sendTipMessageEx(playerid, COLOR_WHITE, "Nie mozesz zkickować Admina !");
  					if(PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pZG] >= 2 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid))
  					{
						...
						if(AddPunishment(giveplayerid, GetNick(giveplayerid), playerid, gettime(), PENALTY_KICK, 0, result, 0) == 1) {
							...
							{
								format(string, sizeof(string), "Admin %s zkickował %s. Powód: %s", GetNickEx(playerid), GetNick(giveplayerid), (result));
								SendPunishMessage(string, giveplayerid); 
							}
						} else {
							sendErrorMessage(playerid, "Wystąpił błąd!"); 
						}
						return 1;
					}
				}
