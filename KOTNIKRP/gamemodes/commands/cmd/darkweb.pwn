...
YCMD:darkweb(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
		...
		else if(PlayerInfo[playerid][pConnectTime] == 0 && PlayerInfo[playerid][pLevel] == 1) return sendErrorMessage(playerid, "Aby pisać na darkwebie musisz przegrać 1h na serwerze!");
		else if(GetPlayerAdminDutyStatus(playerid) == 1) return sendErrorMessage(playerid, "Nie możesz pisać na darkwebie podczas służby administratora!");
        ...
        else if(isnull(params)) return SendClientMessage(playerid, COLOR_GRAD2, "UŻYJ: /darkweb [tekst]");
		...
		else
		{
			...
			format(string, sizeof(string), "Darkweb: %s, Kontakt: %d", params, GetPhoneNumber(playerid));
			format(admstring, sizeof(admstring), "Darkweb %s, Kontakt: %d [%s]", params, GetPhoneNumber(playerid), GetNick(playerid));
			foreach(new i : Player)
			{
				if(IsPlayerConnected(i))
				{
					if(!gNews[i] && PlayerPersonalization[i][PERS_AD] == 0)
					{
						...
						{
							SendClientMessage(i, COLOR_GREY, admstring);
						}
						else
						{
							if(IsAPolicja(i)) continue;
							SendClientMessage(i, COLOR_GREY, string);
						}
					}
				}
			}
			Log(chatLog, WARNING, "%s Darkweb: %s", GetPlayerLogName(playerid), params);
