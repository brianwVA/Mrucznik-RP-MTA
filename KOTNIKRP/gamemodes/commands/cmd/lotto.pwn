...
YCMD:lotto(playerid, params[], help)
{
    ...
    if(IsPlayerConnected(playerid))
    {
        ...
        {
            sendTipMessageEx(playerid, COLOR_GREY, "Posiadasz już los loteryjny !");
            return 1;
        }
        if(kaska[playerid] < PRICE_LOTTO)
        {
            sendTipMessageEx(playerid, COLOR_GREY, "Potrzebujesz "#PRICE_LOTTO"$ aby kupić los lotto !");
            return 1;
        }
        new lottonr;
		if( sscanf(params, "d", lottonr))
		{
			sendTipMessage(playerid,"Użyj /lotto [number]");
			return 1;
		}

		if(lottonr < 1 || lottonr > 80) { sendTipMessageEx(playerid, COLOR_GREY, "Numer lotto od 1 do 80 !"); return 1; }
		format(string, sizeof(string), "* Kupiłeś los lotto z numerem: %d.", lottonr);
