...
{
    if(!GroupPlayerDutyPerm(playerid, PERM_FIREDEPT)) return sendTipMessageEx(playerid, COLOR_GRAD2, "Nie jesteś na służbie grupy z takimi uprawnieniami.");
    if(JobDuty[playerid] == 0 && OnDuty[playerid] == 0) return sendTipMessageEx(playerid, COLOR_GRAD2, "Nie jesteś na służbie.");
    new Float:lPos[3];
    GetPlayerPos(playerid, lPos[0], lPos[1], lPos[2]);
    new lID = Oil_GetIDAtPosition(lPos[0], lPos[1], lPos[2], 2.5);
    if(lID == -1) return sendTipMessageEx(playerid, COLOR_GRAD2, "Brak plam w pobliżu.");
