...
YCMD:zd(playerid, params[], help)
{
		...
		if(IsPlayerConnected(playerid))
		{
			if(PlayerInfo[playerid][pJob] == 10)
			{
				if(IS_KomunikacjaMiejsca(Veh))
				{
					...
					{
						sendErrorMessage(playerid, "Drzwi autobusu są już zamknięte !");
					}
				}
				else
				{
					sendErrorMessage(playerid, "Nie jesteś w autobusie Korporacji !");
				}
			}
			else
			{
	...
	{
	sendErrorMessage(playerid, "Nie jesteś z Korporacji Transportowej !");
	}
	return 1;
}
