...
saveLegale(playerid) {
	...
	DB_FreeResultSet(DB_ExecuteQuery(db_handle, lStr));
}
...
loadKamiPos(playerid)
{
	...
	db_result = DB_ExecuteQuery(db_handle, lStr);

	if(DB_GetRowCount(db_result)) {
		SetPVarFloat(playerid, "k_offsetX", DB_GetFieldFloatByName(db_result, "offsetX"));
		SetPVarFloat(playerid, "k_offsetY", DB_GetFieldFloatByName(db_result, "offsetY"));
		SetPVarFloat(playerid, "k_offsetZ", DB_GetFieldFloatByName(db_result, "offsetZ"));
		SetPVarFloat(playerid, "k_rotX", DB_GetFieldFloatByName(db_result, "rotX"));
		SetPVarFloat(playerid, "k_rotY", DB_GetFieldFloatByName(db_result, "rotY"));
		SetPVarFloat(playerid, "k_rotZ", DB_GetFieldFloatByName(db_result, "rotZ"));
		SetPVarFloat(playerid, "k_scaleX", DB_GetFieldFloatByName(db_result, "scaleX"));
		SetPVarFloat(playerid, "k_scaleY", DB_GetFieldFloatByName(db_result, "scaleY"));
		SetPVarFloat(playerid, "k_scaleZ", DB_GetFieldFloatByName(db_result, "scaleZ"));
...
noAccessMessage(id) {
    return SendClientMessage(id,COLOR_FADE2,"»» Nie posiadasz dostępu do tej komendy");
}
...
ToggleInwigilacja(playerid, adminid)
{
	if(PlayerInfo[playerid][pPodPW] == 0)
	{
		...
		SendCommandLogMessage(sprintf("Admin %s [%d] włączył inwigilacje dla %s [%d]", GetNickEx(adminid), adminid, GetNick(playerid), playerid));
		Log(adminLog, WARNING, "Admin %s włączył inwigilacje /w dla gracza %s", GetPlayerLogName(adminid), GetPlayerLogName(playerid));
	}
	else if(PlayerInfo[playerid][pPodPW] == 1)
	{
		PlayerInfo[playerid][pPodPW] = 0;
		SendCommandLogMessage(sprintf("Admin %s [%d] wyłączył inwigilacje dla %s [%d]", GetNickEx(adminid), adminid, GetNick(playerid), playerid));
		Log(adminLog, WARNING, "Admin %s wyłączył inwigilacje /w dla gracza %s", GetPlayerLogName(adminid), GetPlayerLogName(playerid));
	}
	return 1;
}
...
Odpolszcz(text[])
{
	...
	while ((i_pos = strfind(string, "Ą", false, i_pos)) != -1) string[i_pos] = 'A';
	while ((i_pos = strfind(string, "ą", false, i_pos)) != -1) string[i_pos] = 'a';
	while ((i_pos = strfind(string, "Ć", false, i_pos)) != -1) string[i_pos] = 'C';
	while ((i_pos = strfind(string, "ć", false, i_pos)) != -1) string[i_pos] = 'c';
	while ((i_pos = strfind(string, "Ę", false, i_pos)) != -1) string[i_pos] = 'E';
	while ((i_pos = strfind(string, "ę", false, i_pos)) != -1) string[i_pos] = 'e';
	while ((i_pos = strfind(string, "Ł", false, i_pos)) != -1) string[i_pos] = 'L';
	while ((i_pos = strfind(string, "ł", false, i_pos)) != -1) string[i_pos] = 'l';
	while ((i_pos = strfind(string, "Ń", false, i_pos)) != -1) string[i_pos] = 'N';
	while ((i_pos = strfind(string, "ń", false, i_pos)) != -1) string[i_pos] = 'n';
	while ((i_pos = strfind(string, "Ś", false, i_pos)) != -1) string[i_pos] = 'S';
	while ((i_pos = strfind(string, "ś", false, i_pos)) != -1) string[i_pos] = 's';
	while ((i_pos = strfind(string, "Ó", false, i_pos)) != -1) string[i_pos] = 'O';
	while ((i_pos = strfind(string, "ó", false, i_pos)) != -1) string[i_pos] = 'o';
	while ((i_pos = strfind(string, "Ż", false, i_pos)) != -1) string[i_pos] = 'Z';
	while ((i_pos = strfind(string, "ż", false, i_pos)) != -1) string[i_pos] = 'z';
	while ((i_pos = strfind(string, "Ź", false, i_pos)) != -1) string[i_pos] = 'Z';
	while ((i_pos = strfind(string, "ź", false, i_pos)) != -1) string[i_pos] = 'z';
	return string;
}

FormatAdvertString(text[])
{
	new string[256], i_pos;
	format(string, sizeof(string), text);
	while ((i_pos = strfind(string, "[", false, i_pos)) != -1) string[i_pos] = '(';
	while ((i_pos = strfind(string, "]", false, i_pos)) != -1) string[i_pos] = ')';
	while ((i_pos = strfind(string, "{", false, i_pos)) != -1) string[i_pos] = '(';
	while ((i_pos = strfind(string, "}", false, i_pos)) != -1) string[i_pos] = ')';
	while ((i_pos = strfind(string, "|", false, i_pos)) != -1) string[i_pos] = '-';
	...
	while ((i_pos = strfind(string, "*", false, i_pos)) != -1) string[i_pos] = ' ';
	while ((i_pos = strfind(string, "~", false, i_pos)) != -1) string[i_pos] = ' ';
	format(string, sizeof(string), "%s",  Odpolszcz(string));
	return string;
}
...
Taxi_FareEnd(playerid)
{
	...
	format(string, sizeof(string), "Zakończyłeś służbę, zarobiłeś $%d dla Korporacji Transportowej", TransportMoney[playerid]);
	...
	if(!IsPlayerInGroup(playerid, FRAC_KT))
	{
		...
		Log(payLog, WARNING, "%s zarobił $%d na służbie taksówkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
	}
    else
    {
        Sejf_Add(FRAC_KT, floatround(TransportMoney[playerid]/2));
		DajKaseDone(playerid, floatround(TransportMoney[playerid]/2));
		Log(payLog, WARNING, "%s zarobil $%d dla KT na służbie taksówkarza.", GetPlayerLogName(playerid), TransportMoney[playerid]);
    }
    ...
    if(IsPlayerConnected(taxidriver))
	{
	    ...
	    if(PlayerInfo[playerid][pLevel] < 3)
	    {
			if(kaska[playerid] < floatround(TransportCost[playerid]*0.25))
			{
				...
				SetPlayerCriminal(playerid,INVALID_PLAYER_ID, "Kradzież taksówkarza(brak pieniędzy na spłate)");
				format(string, sizeof(string), "Klient nie posiadał pełnej kwoty.");
...
GetPlayerFrac(playerid) // Duplikat funkcji żeby mi się nie pierdoliło z legacy funkcjami
{
...
CarOpis_Usun(playerid, vehicleid, message=false)
{
    ...
    if(CarOpis[vehicleid] != Text3D:INVALID_3DTEXT_ID)
    {
        ...
        if(message)
        {
            SendClientMessage(playerid, COLOR_YELLOW, "Opis: Usunięto.");
        }
        return 1;
    }
    return 0;
...
public Dopalaj(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		{
			SendClientMessage(playerid, TEAM_CYAN_COLOR, "Dopalacz deaktywowany - zbyt mała prędkość");
			return 1;
		}
	}
	else
	{
		SendClientMessage(playerid, TEAM_CYAN_COLOR, "Dopalacz deaktywowany - wysiadłeś z wozu");
		return 1;
	}
	return 1;
}
	...
	foreach(new i : Player)
	{
	    if(IsPlayerConnected(i))
	    {
	        if(PlayerPaintballing[i] != 0)
	        {
	            ...
	            SendClientMessage(i, COLOR_YELLOW, "Mecz Pintballa rozpoczęty, skończy się za 4 minuty.");
...
public PreparePaintball()
{
    foreach(new i : Player)
	{
		if(PlayerPaintballing[i] != 0)
		{
			SendClientMessage(i, COLOR_YELLOW, "Mecz Pintballa rozpocznie się za 20 sekund, proszę czekać.");
		}
	}
...
public ZestawNaprawczy_CountDown(playerid, vehicleid)
{
	...
	{
		SendClientMessage(playerid, COLOR_PANICRED, "* [ZESTAW NAPRAWCZY] Anulowano naprawę pojazdu.");
	...
	if (ZestawNaprawczy_Timer[playerid] > 0)
	{
		...
		{
			SendClientMessage(playerid, COLOR_PANICRED, "* [ZESTAW NAPRAWCZY] Anulowano naprawę pojazdu. Nie byłeś w dostatecznie blisko pojazdu.");
			ZestawNaprawczy_Timer[playerid] = 30;
			ZestawNaprawczy_Warning[playerid] = 0;
			DeletePVar(playerid, "Use_ZestawNaprawczy");
		}
	...
	else
	{
        ...
        SetVehicleHealth(vehicleid, CarData[VehicleUID[vehicleid][vUID]][c_MaxHP] );
		CarData[VehicleUID[vehicleid][vUID]][c_HP] = CarData[VehicleUID[vehicleid][vUID]][c_MaxHP] ;
		DeletePVar(playerid, "Use_ZestawNaprawczy");
	}
	return 1;
}
...
public CountDownVehsRespawn()
{
	...
	else
	{
		...
		SendClientMessageToAll(COLOR_PANICRED, "|___Nastąpił respawn nieużywanych pojazdów___|");
		Count = 20;
...
public Wchodzenie(playerid) //Zmiana na inteligentny system odmrażania
{
    if(GetPVarInt(playerid, "enter-check") == 0)
    {
		g_ProtectionEnabled[playerid] = true;
		SetTimerEx("DisableProtection", 3000, false, "i", playerid); // 10 sekund (10000 milisekund)
    ...
    else
    {
        ...
        else
        {
            ...
            SetPVarInt(playerid, "enter-check", 0);
            new str[128];
            SendClientMessage(playerid, -1, "(OBJ LOADER Inteligencja) - Błąd z wczytaniem obiektów, nie można dodać nowych - zgłos to z debugiem:");
...
public TiNzPJwGUI(playerid)
{
	...
	format(string, sizeof(string), "* Słychać dźwięk końca czasu na rozwiąznie testu ((%s))", playername);
	ProxDetector(40.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	ShowPlayerDialogEx(playerid, 13, DIALOG_STYLE_MSGBOX, "Czas minął!", "Przykro nam!\nUpłyną czas na udzielenie poprawnej odpowiedzi.\nOblewasz egzamin i możesz go zdać ponownie za 2h.", "Wyjdz", "");
	return 1;
}
...
public naczasbicie(playerid, playerid_atak){
	...
	SendClientMessage(playerid_atak, COLOR_NEWS, "Wygrałeś bitwę ponieważ broniący za długo wpisywał znaki!");
...
poscig[playerid] = 0;
SendClientMessage(playerid,COLOR_PANICRED,"|_________________Tryb Pościgu ZAKOŃCZONY_________________|");
SendClientMessage(playerid,COLOR_WHITE,"Czas na złapanie cię dobiegł końca.");
SendClientMessage(playerid,COLOR_WHITE,"Jeżeli nie przeprowadzasz już żadnej akcji RP, możesz bezpiecznie wyjść z gry");
...
{
	new engine, lights, alarm, doors, bonnet, boot, objective, Float:health, sendername[MAX_PLAYER_NAME + 1], string[256];
	...
	if(Gas[carid] > 3)
	{
		...
		if(rand <= health)
		{
			format(string, sizeof(string), "* silnik odpalił (( %s ))", sendername);
		...
		{
		    format(string, sizeof(string), "* silnik nie odpalił (( %s ))", sendername);
			ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
		}
	}
	else
	{
	 	format(string, sizeof(string), "* na desce rozdzielczej pojawia się informacja o braku paliwa (( %s ))", sendername);
		ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	}
	return 1;
}
        ...
        switch(KradziezEtap[i])
        {
            ...
            case 2:
            {
                if(PlayerInfo[i][pJackSkill] < 50) {
                    ...
                    KradziezTick[i] = gettime()+2;
                }
                format(komunikat, sizeof(komunikat), "* %s odkręca osłonkę i wyjmuje kolorowe kabelki.", GetNick(i));
                ProxDetector(20.0, i, komunikat, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
                KradziezEtap[i] = 3;
            }
            case 3:
            {
                ...
                new mnoznik = 25 + (skillz * 13);
				if(kradnij <= mnoznik)
				{
					format(komunikat, sizeof(komunikat), "* %s łączy odpowiednie kabelki i wyłączył alarm.", GetNick(i));
					ProxDetector(20.0, i, komunikat, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
					TogglePlayerControllable(i, 1);
					new carid = GetPlayerVehicleID(i);
					if(Gas[carid] <= 10) Gas[carid] += 15;
					NieSpamujKradnij[i] = 0;
					SendClientMessage(i, COLOR_GRAD2, "Skill złodzieja aut +1");
					PlayerInfo[i][pJackSkill] ++;
					KradniecieWozu[i] = GetPlayerVehicleID(i);
					Iter_Add(StolenVehicles[i], carid);
					if(PlayerInfo[i][pJackSkill] == 50)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 2, masz teraz większe szanse że uda ci się ukraść wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 100)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 3, masz teraz większe szanse że uda ci się ukraść wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 200)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 4, masz teraz większe szanse że uda ci się ukraść wóz."); }
					else if(PlayerInfo[i][pJackSkill] == 400)
					{ SendClientMessage(i, COLOR_YELLOW, "* Twój skill złodzieja samochodów wynosi teraz 5, masz teraz większe szanse że uda ci się ukraść wóz."); }
				}
				else
				{
					new vehi = GetPlayerVehicleID(i);
					new engine, lights, alarm, doors, bonnet, boot, objective;
					format(komunikat, sizeof(komunikat), "* %s źle łączy kabelki po czym włącza się alarm.", GetNick(i));
					ProxDetector(20.0, i, komunikat, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
					...
					SendClientMessage(i, COLOR_PANICRED, "Zaraz tu będą gliny! Uciekaj!");
					TogglePlayerControllable(i, 1);
					...
					format(komunikat, sizeof(komunikat), "HQ: Zgłoszono próbę kradzieży %s na %s.", VehicleNames[GetVehicleModel(vehi)-400], pZone);
...
KoniecWyscigu(playerid)
{
	new string[256];
	if(playerid == -1)
		format(string, sizeof(string), "Komunikat wyścigowy: {FFFFFF}Wyscig %s zakończony przez system.", Wyscig[Scigamy][wNazwa]);
	else if(playerid == -2)
		format(string, sizeof(string), "Komunikat wyścigowy: {FFFFFF}Wyscig %s zakończony - wszyscy dojechali do mety!", Wyscig[Scigamy][wNazwa]);
	else
		format(string, sizeof(string), "Komunikat wyścigowy: {FFFFFF}Wyscig %s zakończony przez %s.", Wyscig[Scigamy][wNazwa], GetNickEx(playerid));
...
PrawoJazdyRandomGUITest(playerid, questions_available[], questions_count)
{
    ...
    new questions_content[][] =
    {
        ...
        "Ile wynosi dopuszczalna prędkość\nprzy holowaniu w terenie zabudowanym?\n\nPodaj w cyfrach",
        "Którą stroną jezdni należy jechać?\n\nPodaj jeden wyraz",
        "Co należy rozstawić po wypadku?\n\nPodaj jeden wyraz",
        "Dozwolona prędkość na autostradzie?\n\nPodaj w cyfrach",
        "Dozwolona prędkość na mieście?\n\nPodaj w cyfrach",
        "Jaka jest maksymalna, dozwolona\nprędkość na drodze szybkiego ruchu?\n\nPodaj w cyfrach",
        "Co trzeba wyciągnąć po wypadku\nby ostrzec o nim?\n\nPodaj jeden wyraz"
    };
		...
		if(IsPlayerInRangeOfPoint(i, 500, -1106.9854, -966.4719, 129.1807))
		{
            ...
            foreach(new di : Player)
			{
			    if(zawodnik[di] == 1)
			    {
			        if(okregi[di] >= 1)
			        {
			            new iplayer[MAX_PLAYER_NAME + 1];
			            new string[256];
				        GetPlayerName(di, iplayer, sizeof(iplayer));
				        format(string, sizeof(string), "%s - %d okrążeń", iplayer, okregi[di]);
...
GetPos2DZone(zone[], len, Float:x, Float:y)
{
 	for(new i = 0; i != sizeof(gSAZones); i++ )
 	{
		if(x >= gSAZones[i][SAZONE_AREA][0] && x <= gSAZones[i][SAZONE_AREA][3] && y >= gSAZones[i][SAZONE_AREA][1] && y <= gSAZones[i][SAZONE_AREA][4])
		{
		    return format(zone, len, "%s", gSAZones[i][SAZONE_NAME]);
		}
	}
	return 0;
}
...
GetPlayer2DZone(playerid, zone[], len)
{
	...
	{
		return format(zone, len, "Nieznane ((AJ))");
	}
	new Float:x, Float:y, Float:z;
	GetPlayerPos(playerid, x, y, z);
 	for(new i = 0; i != sizeof(gSAZones); i++ )
...
stock UndressPlayer(playerid, bool:dressup, colorID=0)
{
	...
	if(dressup)
	{
        ...
        isNaked[playerid] = 0;
        format(string, sizeof(string), "%s ubiera się.", GetNick(playerid));
	...
	else
	{
        ...
        format(string, sizeof(string), "%s rozbiera się.", GetNick(playerid));
        ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
        sendTipMessage(playerid, "Aby ubrać się spowrotem, ponownie użyj komendy /rozbierz.");
        isNaked[playerid] = 1;
	}
	return 1;
}
...
Kostka_Wygrana(playerid, loser, kasa, bool:quit=false)
{
    ...
    Log(payLog, WARNING, "%s wygrał rzuty kostką z %s na kwotę %d$ %s", GetPlayerLogName(playerid), GetPlayerLogName(loser), kasa, quit ? "(quit)" : "");
    Sejf_Add(11, podatek);
...
PlayerText:CreatePlayerBackgroundTextDraw(playerid, Float:Xpos, Float:Ypos, Float:Width, Float:Height)
{
    ...
    PlayerTextDrawColour(playerid, txtBackground,-1);
...
HandlePlayerItemSelection(playerid, selecteditem)
{
    ...
    if(id == -1) return SendClientMessage(playerid, -1, "Osiągnięto limit barierek.");
...
SearchingHit(playerid)
{
			        ...
			        foreach(new k : Player)
					{
						if(IsPlayerConnected(k))
						{
				        	if(IsPlayerInGroup(k, 8) || PlayerInfo[k][pLider] == 8)
				        	{
				                ...
				                SendClientMessage(k, COLOR_DBLUE, "*** Wiadomość: Nowe zlecenia są już dostępne. ***");
								...
								SendClientMessage(k, COLOR_YELLOW, "Wpisz Givehit [hitmanid], aby podpisać kontrakt z hitmanem.");
	...
	if(searchhit == 0)
	{
	    SendClientMessage(playerid, COLOR_GREY, "   Nie ma dostępnych zleceń !");
	}
	return 0;
}

public PaintballEnded()//nowe domy biznes ważne
{
	new string[256];
	    ...
	    if(IsPlayerConnected(i))
	    {
	        if(PlayerPaintballing[i] != 0)
	        {
	            if(IsPlayerConnected(PaintballWinner))
	            {
	                ...
	                format(string,sizeof(string), "** %s wygrał mecz Pintballa z %d trafieniami **",name,PaintballWinnerKills);
	                SendClientMessage(i, COLOR_WHITE, string);
	            }
	            ResetPlayerWeapons(i);
...
StartZuzling()
{
    foreach(new i : Player)
	{
	    if(IsPlayerConnected(i))
	    {
	        if(zawodnik[i] == 1)
	        {
	            SendClientMessage(i, COLOR_GREEN, "Zielone światło, go go go !");
	            PlayerPlaySound(i, 1057, 0.0, 0.0, 0.0);
...
UsunBron(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		PlayerInfo[playerid][pGun0] = 0;//Pięść lub kastet
		PlayerInfo[playerid][pGun1] = 0;//Broń biała
		PlayerInfo[playerid][pGun2] = 0;//Pistolety
		PlayerInfo[playerid][pGun3] = 0;//Shotguny
		PlayerInfo[playerid][pGun4] = 0;//Broń maszynowa
		PlayerInfo[playerid][pGun5] = 0;//Karabiny
		PlayerInfo[playerid][pGun6] = 0;//Snajperki
		PlayerInfo[playerid][pGun7] = 0;//Ciężka broń
		PlayerInfo[playerid][pGun8] = 0;//Broń wybuchowa
		PlayerInfo[playerid][pGun9] = 0;//Sprej/Gasnica/Aparat
		PlayerInfo[playerid][pGun10] = 0;//Nietypowa broń biała
...
DajBronieFrakcyjne(playerid, grupa)
{
	...
	else if(!strcmp(GroupInfo[grupa][g_ShortName], "FBol", true)) //fbi
	{
	    ...
	    {
	        //layerInfo[playerid][pGun4] = 29; PlayerInfo[playerid][pAmmo4] = 1030;
	        //layerWeapons[playerid][weaponLegal5] = 1;
	    }
...
PrzywrocBron(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 2) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 3) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 4) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 5) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 6) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 7) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 8) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 11) == -1)
		...
		{
			if(Weapons_HasActiveWeaponOnSlot(playerid, 12) == -1)
		...
		foreach(new i : PlayerItems[playerid])
		{
			...
			if(Item[itemid][i_ItemType] != ITEM_TYPE_WEAPON || Item[itemid][i_Used] != 1 || Item[itemid][i_Value1] <= 0 || Item[itemid][i_Value2] <= 0) continue;
...
WyjmijBron(playerid)
{
    ...
    ShowPlayerDialogEx(playerid, 8241, DIALOG_STYLE_LIST, "Wybierz broń którą chcesz wyjąć", bron, "Wybierz", "Cofnij");
}
...
SchowajBron(playerid)
{
	...
	new bron28itrzyipol[2000];
 	...
 	ShowPlayerDialogEx(playerid, 8242, DIALOG_STYLE_LIST, "Wybierz broń którą chcesz schować", bron28itrzyipol, "Wybierz", "Cofnij");
}
...
ListaBroni(playerid)
{
    ...
    a9 = Dom[dom][hS_A9];
    format(bron, sizeof(bron), "%s\n%s\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\tAmunicja:%d\n%s\n%s",b0,b1,b2,a2,b3,a3,b4,a4,b5,a5,b6,a6,b7,a7,b8,a8,b9,a9,b10,b11);
    ShowPlayerDialogEx(playerid, 8243, DIALOG_STYLE_LIST, "Lista broni", bron, "Usuń", "Cofnij");
}
...
PrzyczepKogut(playerid, veh)
{
    ...
    {
        SendClientMessage(playerid, COLOR_WHITE, "Na tym wozie nie można przyczepić koguta");
    }
...
IsAtBar(playerid)
{
    if(IsPlayerConnected(playerid))
	{
		...
		{//Basen "tsunami i Włoska restauracja
		    return 1;
 		}
...
IsABoat(carid)
{
	if(GetVehicleModel(carid) == 472 || GetVehicleModel(carid) == 473 || GetVehicleModel(carid) == 493 || GetVehicleModel(carid) == 595 || GetVehicleModel(carid) == 484 || GetVehicleModel(carid) == 430 || GetVehicleModel(carid) == 453 || GetVehicleModel(carid) == 452 || GetVehicleModel(carid) == 446 || GetVehicleModel(carid) == 454)//łodzie
	{
		return 1;
	}
	return 0;
...
JoinChannel(playerid, number, line[])
{
    if(IsPlayerConnected(playerid))
	{
	    ...
	    {
	        SendClientMessage(playerid, COLOR_GREY, "   Złe hasło !");
	    }
	}
	return 1;
}
...
JoinChannelNr(playerid, number)
{
	if(IsPlayerConnected(playerid))
	{
	    ...
	    {
			format(string, sizeof(string), "* %s opuścił ten kanał.", sendername);
		...
		{
		    format(string, sizeof(string), "* Dołączyłeś do kanału numer %d jako Administartor.", channel);
			SendClientMessage(playerid, COLOR_YELLOW, string);
		}
		else
		{
		    format(string, sizeof(string), "* Dołączyłeś do kanału numer %d, Administrator kanału: %s.", channel, IRCInfo[number][iAdmin]);
			SendClientMessage(playerid, COLOR_YELLOW, string);
		}
		format(string, sizeof(string), "MOTD: %s.", IRCInfo[number][iMOTD]);
		SendClientMessage(playerid, COLOR_YELLOW, string);
		format(string, sizeof(string), "* %s dołączył do tego kanału.", sendername);
	...
	foreach(new i : Player)
	{
		if(IsPlayerConnected(i))
		{
			if(PlayerInfo[i][pLottoNr] > 0)
			{
				...
				{
					format(string, sizeof(string), "Niestety nie udało Ci się wygrać loterii z numerem %d.", PlayerInfo[i][pLottoNr]);
					SendClientMessage(i, COLOR_WHITE, string);
	...
	foreach(new i2 : Player)
	{
		if(status == 1)
		{
			if(winners == 0)
			{
				...
				format(string, sizeof(string), "Nikt nie wygrał. Nagroda została podwyższona do %d$", Jackpot);
		...
		if(status == 2)
		{
			if(winners == 0)
			{
				...
				Jackpot = rand;
				format(string, sizeof(string), "Lotto rozpoczyna kolejną loterię, kasa wyjścia to %d$", Jackpot);
				OOCOff(COLOR_WHITE, string);
				status = 1;
				break;
			}
			if(PlayerInfo[i2][pLottoNr] > 0 && PlayerInfo[i2][pLottoNr] == number)
			{
				...
				format(string, sizeof(string), "%s wygrał w totolotku %d$", GetNick(i2), kasaWin);
				OOCOff(COLOR_WHITE, string);
...
SetPlayerCriminal(playerid,declare,reason[], bool:sendmessage=true)
{
	if(IsPlayerConnected(playerid))
	{
		...
		new turned[MAX_PLAYER_NAME + 1];
		new turner[MAX_PLAYER_NAME + 1];
		new turnmes[128];
		new wantedmes[128];
		...
		if (declare == INVALID_PLAYER_ID)
		{
			...
			else if(PoziomPoszukiwania[playerid] >= 5)
			{
		...
		format(turnmes, sizeof(turnmes), "Popełniłeś przestępstwo ( %s ). Zgłosił: %s.",reason,turner);
				...
				foreach(new i : Player)
				{
					if(IsPlayerConnected(i))
					{
					    if(IsAPolicja(i) && (OnDutyCD[i] || OnDuty[i]))
					    {
					        if(gCrime[i] == 0)
					        {
								format(cbjstore, sizeof(turnmes), "HQ: Przestępstwo: %s | Nadawca: %s",reason, turner);
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		new atext[20];
		if(PlayerInfo[targetid][pSex] == 1) { atext = "Mężczyzna"; }
		...
		new f1text[40], f2text[40], f3text[40], f4text[40], f5text[40];
		new r1text[40], r2text[40], r3text[40], r4text[40], r5text[40];
		...
		if(PlayerInfo[targetid][pGrupa][3] != 0)
		{
			new group = PlayerInfo[targetid][pGrupa][3];
			if(strlen(GroupInfo[PlayerInfo[targetid][pGrupa][3]][g_Name]) <= 12 && strlen(GroupInfo[PlayerInfo[targetid][pGrupa][3]][g_ShortName]) > 1)
				format(f4text, sizeof(f4text), "%s", GroupInfo[PlayerInfo[targetid][pGrupa][3]][g_Name]);
			else
				format(f4text, sizeof(f4text), "%s", GroupInfo[PlayerInfo[targetid][pGrupa][3]][g_ShortName]);
			format(r4text, sizeof(r4text), "%s", GroupRanks[group][PlayerInfo[targetid][pGrupaRank][3]]);
		}
		else {f4text = "Brak"; r4text = "Brak";}
		if(PlayerInfo[targetid][pGrupa][4] != 0)
		{
			new group = PlayerInfo[targetid][pGrupa][4];
			if(strlen(GroupInfo[PlayerInfo[targetid][pGrupa][4]][g_Name]) <= 12 && strlen(GroupInfo[PlayerInfo[targetid][pGrupa][4]][g_ShortName]) > 1)
				format(f5text, sizeof(f5text), "%s", GroupInfo[PlayerInfo[targetid][pGrupa][4]][g_Name]);
			else
				format(f5text, sizeof(f5text), "%s", GroupInfo[PlayerInfo[targetid][pGrupa][4]][g_ShortName]);
			format(r5text, sizeof(r5text), "%s", GroupRanks[group][PlayerInfo[targetid][pGrupaRank][4]]);
		}
		else {f5text = "Brak"; r5text = "Brak";}
		...
		format(coordsstring, sizeof(coordsstring), "Level:[%d] Płeć:[%s] Wiek:[%d] Pochodzenie:[%s] Zdrowie:[%.1f] Kasa:[$%d] Bank:[$%d] Telefon:[%d]", level,atext,age,otext,shealth+50, cash, account, pnumber);
		SendClientMessage(playerid, COLOR_GRAD1,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Konto Premium:[%s] ślub z:[%s] On-Line:[%d] LottoNr:[%d] Praca:[%s] Punkty karne:[%d]", drank,PlayerInfo[targetid][pMarriedTo],ptime,lotto,jtext, PlayerInfo[targetid][pPK]);
		SendClientMessage(playerid, COLOR_GRAD2,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Ryb Złowionych:[%d] Największa Ryba:[%d] Przestępstwa:[%d] Czas Aresztu:[%d] Smierci będąc Poszukiwanym:[%d]", fishes,bigfish,crimes,arrests,warrests );
		SendClientMessage(playerid, COLOR_GRAD3,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Zabić:[%d] śmierci:[%d] Bonus Levelowy:[$%d] Respekt:[%d/%d] WL:[%d] Skin ID: [%d] MrucznikCoins:[%d]",kills,deaths,costlevel,exp,expamount,wanted, skin, PremiumInfo[targetid][pMC]);
		SendClientMessage(playerid, COLOR_GRAD4,coordsstring);
		format(coordsstring, sizeof(coordsstring), "Drugs:[%d] Mats:[%d] Warny:[%d] Dostępnych zmian nicków:[%d] Siła:[%d] Głód:[%.1f%%]",drugs,mats,PlayerInfo[targetid][pWarns],znick, PlayerInfo[targetid][pStrong], PlayerInfo[targetid][pHunger]);
		SendClientMessage(playerid, COLOR_GRAD5,coordsstring);
		format(coordsstring, sizeof(coordsstring), "JobSkin:[%d] Apteczki:[%d] Zestawy:[%d] Pokój:[%d] Dom:[%d] Klucz Wozu:[%d]", PlayerInfo[targetid][pJobSkin], PlayerInfo[targetid][pHealthPacks],PlayerInfo[targetid][pFixKit], MotelRooms[PlayerInfo[playerid][pMotRoom]][mtrRoomNum], housekey,PlayerInfo[targetid][pKluczeAuta]);
		SendClientMessage(playerid, COLOR_GRAD5, coordsstring);
		format(coordsstring, sizeof(coordsstring), "Grupa 1:[%s] Grupa 2:[%s] Grupa 3:[%s] Grupa 4:[%s] Grupa 5:[%s]", f1text, f2text, f3text, f4text, f5text);
		SendClientMessage(playerid, COLOR_GRAD6, coordsstring);
		format(coordsstring, sizeof(coordsstring), "Ranga 1:[%s] Ranga 2:[%s] Ranga 3:[%s] Ranga 4:[%s] Ranga 5:[%s]", r1text, r2text, r3text, r4text, r5text);
		SendClientMessage(playerid, COLOR_GRAD6, coordsstring);
		SendClientMessage(playerid, COLOR_GREEN,"_______________________________________");
	}
}
...
PokazStatyDialogBronie(playerid)
{
	...
	for(new i = 0; i < 6; i++)
	{
		...
		format(line, sizeof(line), "{d9d9d9}%s: \t- poziom %d [%d]\n", weapon, skill, PlayerInfo[playerid][pWeaponSkill][i]);
		strcat(string, line);
	}
	return ShowPlayerDialogEx(playerid, DIALOG_STATS_UMIEJETNOSCI, DIALOG_STYLE_TABLIST, "Umiejętności postaci", string, "Zamknij", "");
}
...
PokazStatyDialog(playerid)
{
	...
	//
	new atext[20];
	if(PlayerInfo[playerid][pSex] == 1) { atext = "Mężczyzna"; }
	...
	if (cash > 0) { format(string, sizeof(string), "%s\nGotówka:\t\t{9ACD32}%d$", string, cash); }
	else if (cash == 0) { format(string, sizeof(string), "%s\nGotówka:\t\t{fffafa}%d$", string, cash); }
	else if (cash < 0) { format(string, sizeof(string), "%s\nGotówka:\t\t{ff0000}%d$", string, cash); }
	...
	new Float:glod = PlayerInfo[playerid][pHunger];
	new Float:pragnienie = PlayerInfo[playerid][pThirst];
	new Float:glod_procent = 100.0 - glod;
	new Float:pragnienie_procent = 100.0 - pragnienie;
	
	if(HUDShown[playerid] == 0){
		if(glod_procent >= 75.0)
			format(string, sizeof(string), "%s\nGłód:\t\t\t{228B22}%.1f%%", string, glod_procent);
		else if(glod_procent >= 50.0 && glod_procent < 75.0)
			format(string, sizeof(string), "%s\nGłód:\t\t\t{DAA520}%.1f%%", string, glod_procent);
		else if(glod_procent >= 25.0 && glod_procent < 50.0)
			format(string, sizeof(string), "%s\nGłód:\t\t\t{FF8C00}%.1f%%", string, glod_procent);
		else
			format(string, sizeof(string), "%s\nGłód:\t\t\t{B22222}%.1f%%", string, glod_procent);
		
		if(pragnienie_procent >= 75.0)
			format(string, sizeof(string), "%s\nPragnienie:\t\t{228B22}%.1f%%", string, pragnienie_procent);
		else if(pragnienie_procent >= 50.0 && pragnienie_procent < 75.0)
			format(string, sizeof(string), "%s\nPragnienie:\t\t{DAA520}%.1f%%", string, pragnienie_procent);
		else if(pragnienie_procent >= 25.0 && pragnienie_procent < 50.0)
			format(string, sizeof(string), "%s\nPragnienie:\t\t{FF8C00}%.1f%%", string, pragnienie_procent);
		else
			format(string, sizeof(string), "%s\nPragnienie:\t\t{B22222}%.1f%%", string, pragnienie_procent);
	}
	...
	format(string, sizeof(string), "%s\n{d9d9d9}---", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyświetl posiadane dokumenty", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyświetl posiadane tatuaże", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyświetl statystyki postaci", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyświetl umiejętności postaci", string);
	format(string, sizeof(string), "%s\n\t{d9d9d9}Wyświetl umiejętności strzelania", string);
...
public MRP_ShopPurchaseCar(playerid, model, cena)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
        SendClientMessage(playerid, COLOR_GRAD2, "Wyjdź z pojazdu.");
        return 0;
    }
    new komunikat[256];
    ...
    //Info
    format(komunikat, sizeof(komunikat), "Kupiłeś unikatowy %s za %d MC. Komendy auta znajdziesz w /auto. Gratulujemy zakupu!",VehicleNames[model-400], cena);

    _MruAdmin(playerid, komunikat);

	Log(premiumLog, WARNING, "%s kupił unikatowy pojazd %s za %dMC", GetPlayerLogName(playerid), GetVehicleLogName(CarData[carid][c_ID]), cena);
    //TODO
    if(carid >= MAX_CARS)
    {
        sendErrorMessage(playerid, "Nie można stworzyć pojazdu! Możliwe przepełnienie, auto zostało kupione lecz nie możesz go użyć.");
        return 1;
    }
...
{
    if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_GRAD2, "Wyjdź z pojazdu.");
    new komunikat[256];
	if(kaska[playerid] >= cena)
	{
	    if(model >= 400 && model <= 611)
	    {
            ...
            if(carid == -1) return SendClientMessage(playerid, COLOR_PANICRED, "Nie można stworzyć pojazdu! Rekord nie został dodany do bazy.");
            //Assign
            Car_MakePlayerOwner(playerid, carid);

            //Info
            format(komunikat, sizeof(komunikat), "Kupiłeś %s za %d$. Komendy auta znajdziesz w /auto. Gratulujemy zakupu!",VehicleNames[model-400], cena);
			...
			Log(payLog, WARNING, "%s kupił pojazd %s za %d$", GetPlayerLogName(playerid), GetVehicleLogName(CarData[carid][c_ID]), cena);
            //TODO
            if(carid >= MAX_CARS) return SendClientMessage(playerid, COLOR_PANICRED, "Nie można stworzyć pojazdu! Możliwe przepełnienie, auto zostało kupione lecz nie możesz go użyć.");

			PutPlayerInVehicleEx(playerid, CarData[carid][c_ID], 0);
		}
		else
		{
		    SendClientMessage(playerid, COLOR_WHITE, "BŁĄD - zły model! Zgłoś okoliczności na forum.");
		}
	}
	else
 	{
 	    format(komunikat, sizeof(komunikat), "Nie stać Cię na zakup tego pojazdu. Kosztuje on %d$ a Ty posiadasz tylko %d$.",cena, kaska[playerid]);
...
#include "mrp_legacy_houses_io.pwn"
#else
ZapiszDom(id)
{
	return 0;
...
StworzDom(playerid, interior, oplata)
{
	...
	{
	    SendClientMessage(playerid, COLOR_RED, "Ten interior został usunięty");
	    return 1;
	} // loled
	if(interior == 33)
	{
	...
	{
	    SendClientMessageToAll(COLOR_RED, "BŁĄD PRZY TWORZENIU DOMU");
	    return 1;
	}
	else
	{
		...
		new string2[256];
		format(string2, sizeof(string2), "Dom %d || NrDom %d || Parcela %s || Cene ogólna %d$.", dld, Dom[dld][hDomNr], pZone, floatround(cenadomu, floatround_ceil));
...
Dom_ChangeInt(playerid, dld, interior)
{
	...
	Log(adminLog, WARNING, "Admin %s zmienił interior domu %d z %d na %d", GetPlayerLogName(playerid), dld, oldInt, interior);
	//
	ZapiszDom(dld);
	return 1;
}
...
Dom_ChangeOwner(playerid, dom, forid)
{
	...
	Log(adminLog, WARNING, "Admin %s zmienił właściciela domu %d z %s na %s", GetPlayerLogName(playerid), dom, Dom[dom][hWlasciciel], GetPlayerLogName(forid));
...
L_StworzDom(playerid, kategoria, oplata)
{
	...
	if(dini_Exists(string))
	{
	    SendClientMessageToAll(COLOR_RED, "BŁAD PRZY TWORZENIU DOMU (DOM INSTNIEJE)");
	    return 1;
	}
	else
	{
		...
		{
		    SendClientMessageToAll(COLOR_RED, "BŁAD PRZY TWORZENIU DOMU (ZŁA KATEGORIA)");
	    	return 1;
		}
		Dom[dld][hVW] = dld;
		Dom[dld][hPW] = 0;//pokoi wynajmowanych
...
KupowanieDomu(playerid, dom, platnosc, intType = 0)
{
	...
	if(!dini_Exists(string))
	{
        format(string, 256, "BŁAD PRZY KUPOWANIU DOMU - Dom %d gracz %d", dom, playerid);
	...
	else
	{
		...
		{
			SendClientMessage(playerid, COLOR_RED, "Dom już jest kupiony!");
			return 1;
		}
		new str2[256];
	    ...
	    if(platnosc == 1)
	    {
	        if(kaska[playerid] >= cenadomu && cenadomu != 0 && cenadomu > 0 && kaska[playerid] != 0 && kaska[playerid] > 0)
	        {
				...
				format(str2, sizeof(str2), "Gratulacje! Kupiłeś dom za %d$. Masz teraz %d$.", cenadomu, kaska[playerid]);
				SendClientMessage(playerid, COLOR_NEWS, str2);
			}
			else
			{
			    SendClientMessage(playerid, COLOR_NEWS, "Nie stać cię!!!!");
			    return 1;
			}
	    }
	    if(platnosc == 2)
		{
	        ...
	        {
				SetPlayerBank(playerid, PlayerInfo[playerid][pAccount]-cenadomu);
				format(str2, sizeof(str2), "Gratulacje! Kupiłeś dom za %d$. Zostało one odliczone od twojego konta bankowego które wynosi teraz %d", cenadomu, PlayerInfo[playerid][pAccount]);
				SendClientMessage(playerid, COLOR_PANICRED, str2);
			}
			else
			{
			    SendClientMessage(playerid, COLOR_PANICRED, "Nie stać cię!!!!");
			    return 1;
			}
		}
		printf(str2);
	    ...
	    SendClientMessage(playerid, COLOR_NEWS, "Aby zobaczyć komendy domu wpisz /dompomoc");
		if(intType == 1)
		{
			SendClientMessage(playerid, COLOR_NEWS, "INFO: Wybrałeś własny interior. Możesz go teraz zbudować lub wynająć kogoś do pomocy.");
			SendClientMessage(playerid, COLOR_NEWS, "INFO: Pod komendą /dom możesz ustawić pozycję wejścia do domu po zbudowaniu interioru.");
		}
		Log(payLog, WARNING, "%s kupił dom %s za %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), cenadomu);
		FirstHouse(playerid);
...
ZlomowanieDomu(playerid, dom)
{
	...
	if(dini_Exists(plik))
	{
		...
		if(playerid != 9999)
		{
		    new GeT4[MAX_PLAYER_NAME + 1];
		    GetPlayerName(playerid, GeT4, sizeof(GeT4));
			PlayerInfo[playerid][pDom] = 0;
			DajKaseDone(playerid, (Dom[dom][hCena]/2));
			new GeT2[512];
			format(GeT2, sizeof(GeT2), "Sprzedałeś swój dom za %d$. Osoby wynajmujące zostały wyeksmitowane. Przedmioty w sejfie oraz dodatki do domu przepadły.", (Dom[dom][hCena]/2));
			SendClientMessage(playerid, COLOR_NEWS, GeT2);
			Log(payLog, WARNING, "%s zezłomował dom %s i dostał %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), (Dom[dom][hCena]/2));
			format(GeT, sizeof(GeT), "Brak");
		...
		else
		{
			...
			{
				Log(serverLog, WARNING, "Dom %s został zezłomowany z powodu nieaktywności właściciela %s.", GetHouseLogName(dom), Dom[dom][hWlasciciel]);
	...
	{
	    SendClientMessage(playerid, COLOR_NEWS, "BŁAD - Twój dom nie istnieje, zgłoś to na forum !!");
	}
    return 1;
}
...
NaprawSpojnoscWlascicielaDomu(playerid)
{
	...
	if(domcheck == 1)
	{
		if(Dom[dom][hKupiony] == 0)
		{
			Dom[dom][hKupiony] = 1;
			ZapiszDom(dom);
			SendClientMessage(playerid, COLOR_PANICRED, "Wykryto bug z niekupionym domem, został on automatycznie naprawiony. Jeżeli komunikat będzie się powtarzał lub wystąpią inne bugi, zgłoś to koniecznie na forum!");
			Log(serverLog, WARNING, "%s posiadał bug z niekupionym domem %s. Naprawiono.", GetPlayerLogName(playerid), GetHouseLogName(dom));
		}
	}
	else if(domcheck == 0)
	{
		...
		{
			DajKaseDone(playerid, (Dom[dom][hCena]/2));
			SendClientMessage(playerid, COLOR_PANICRED, "Twój dom został zabrany z powodu nieaktywności, otrzymujesz połowę wartosci domu!");
			Log(payLog, WARNING, "%s stracił dom %s z powodu nieaktywności i otrzymał %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
		else
		{
			DajKaseDone(playerid, (Dom[dom][hCena]/2));
			SendClientMessage(playerid, COLOR_PANICRED, "Wykro bug z dwoma właścicielami! Jesteś drugim właścicielem, więc tracisz dom, otrzymujesz połowę wartosci domu.");
			Log(payLog, WARNING, "%s stracił dom %s z powodu nieaktywności (2 właścicieli) i otrzymał %d$", GetPlayerLogName(playerid), GetHouseLogName(dom), Dom[dom][hCena]/2);
			PlayerInfo[playerid][pDom] = 0;
		}
	}
	else
	{
		SendClientMessage(playerid, COLOR_PANICRED, "Twój dom nie istnieje! Prawdopodobnie został usunięty lub coś poszło nie tak.");
...
KupowanieDodatkow(playerid, dom)
{
	...
	if(Dom[dom][hGaraz] == 0)
	{
		format(Garaz, sizeof(Garaz), "Garaż\t\t\tLevel: %d\tCena: %d$\n", 1, 1000000);
	}
	else
	{
	...
	{
	    format(Ladowisko, sizeof(Ladowisko), "Lądowisko\t\tLevel: %d\tCena: %d$\n", 1, 10000000);
	}
	else
	{
    ...
    format(RTV, sizeof(RTV), "Sprzęt RTV\t\tLevel: %d\tCena: %d$\n", Dom[dom][hRTV]+1, 0*(Dom[dom][hRTV]+1));
    ...
    ShowPlayerDialogEx(playerid, 826, DIALOG_STYLE_LIST, "Kupowanie dodatków", strALL, "Wybierz", "Wyjdź");
    return 1;
}
...
Do_WnetrzaWozu(playerid, vehicleid, model)
{
	...
	else if(model == 570)//Wagony KT
	{
			...
			sendTipMessage(playerid, ">>Pomyślnie wszedłeś do pociągu! Bilet został zużyty!");
...
LadujInteriory()
{
    ...
    IntInfo[47][Int_X] = 1284.1958;//WOŚP
    ...
    IntInfo[48][Int_X] = 2149.0562;//Julia 1 - ładny
    ...
    IntInfo[49][Int_X] = 2150.1968;//Julia 2 - ładny mały
    ...
    IntInfo[50][Int_X] = 2126.7422;//Julia 3 - dwupiętrowy
    ...
    IntInfo[51][Int_X] = 2131.7869;//Julia 4 - ładny
    ...
    IntInfo[53][Int_X] = 2091.5286;//Julia 6 - ładny
...
stock ShowPlayerInfoDialog(playerid, caption[], info[], bool:dialogTimer=false)
{
	...
	{
		sendErrorMessage(playerid, "Odczekaj 15 sekund przed wywołaniem kolejnego dialogu");
	}
	return 1;
}
...
FunkcjaK(string[])
{
	...
	if(strval(string) != 0 && strlen(string) > 1)
	{
		...
		if(ilosc_k != 0)
		{
			...
			x=floatstr(string);
			return floatround((ilosc_k==1) ? (x*1000) : ((ilosc_k==2) ? (x*1000000) : (x*1000000000)), floatround_tozero); //zbugowane potęgi: x*(1000^ilosc_k)
		}
	}
...
public OPCLogin(playerid, status)
{
    new nick[MAX_PLAYER_NAME + 1];
	...
	if(status == 0)
	{
		...
		#if defined MRP_MTA_RUNTIME
			// Kotnik otwiera dialog dopiero po kliknieciu tekstury ekranu
			// logowania (mdl-2074). Ta tekstura nie istnieje w natywnym MTA,
			// wiec gracz pozostawal na pustej kamerze bez mozliwosci wejscia.
			// W MTA przechodzimy bezposrednio do standardowego dialogu hasla.
			SetTimerEx("OPCLogin", 250, false, "ii", playerid, 1);
		#else
		HUD_ShowLoginScreen(playerid);
		if(IsValidTimer(LoginScreenTimer[playerid])) KillTimer(LoginScreenTimer[playerid]);
		LoginScreenTimer[playerid] = SetTimerEx("FixSelectTextDraw", 1000, true, "d", playerid);
		#endif
	...
	else if(status == 1)
	{
		...
		if(result == -1 || result == 1) //logowanie
		{
			//Logowanie
			new string[512];
			SendClientMessage(playerid, COLOR_YELLOW, "Wszedłeś właśnie na serwer o wielkich możliwościach! Witaj na M-RP.");
			format(string, sizeof(string), "{adc7e7}Witaj na M-RP, serwerze o wielkich możliwościach\nZaloguj się lub dołącz już dzisiaj do nas i baw się razem z nami w świecie GTA.\n\nKonto o takim nicku jest już stworzone.\nJest Twoja? Wpisz hasło i zaloguj się.\nJeżeli nie wyjdź i wejdź na nowym nicku!");
			ShowPlayerDialogEx(playerid, D_LOGIN, DIALOG_STYLE_PASSWORD, "Logowanie", string, "Zaloguj", "Wyjdź");
			gPlayerAccount[playerid] = 1; //logowanie
			PlayerInfo[playerid][pUID] = MruMySQL_GetUIDFromName(nick);
			if(PlayerInfo[playerid][pUID] < 1)
			{
				sendErrorMessage(playerid, "Błędnie przypisany UID konta.");
				KickEx(playerid);
				return 0;
			}
		}
		...
		{
			new string[256];
			SendClientMessage(playerid, COLOR_YELLOW, "Zalogowałeś się właśnie na serwer o wielkich możliwościach! Zaloguj się.");
			format(string, sizeof(string), "Nick podobny do %s jest zarejestrowany.\n{FF0000}Podobny nick jest już w bazie, sprawdź wielkosć znaków.", nick);
		...
		{
			#if defined MRP_MTA_RUNTIME
				// Oryginalny Kotnik zakładał rejestrację przez nieistniejące już
				// forum i natychmiast kickował każdy nieznany nick. W MTA musi być
				// dostępny zwykły dialog tworzenia konta, inaczej nowy gracz widzi
				// tylko "NETWORK TROUBLE" chwilę po wejściu na serwer.
				SendClientMessage(playerid, COLOR_YELLOW, "Nie znaleziono konta o tym nicku. Utwórz nowe konto M-RP.");
				ShowPlayerDialogEx(playerid, D_REGISTER, DIALOG_STYLE_PASSWORD,
					"Rejestracja konta",
					"Wpisz hasło, którego chcesz używać do logowania na tym koncie.\nHasło musi mieć od 1 do 64 znaków.",
					"Rejestruj", "Wyjdź");
			...
			{
				SendClientMessage(playerid, COLOR_RED, "Rejestracja została wyłączona na czas ataków, przepraszamy za uniedogodnienia.");
				KickEx(playerid);
			}
			#endif
		}
...
LoadServerInfo()
{
    ServerInfo="\0";
	new Cache:result;
    result = mysql_query(Database, "SELECT `info` FROM `mru_serverinfo` WHERE `aktywne`=1 LIMIT 1");
    if(cache_num_rows())
    {
        cache_get_value_index(0, 0, ServerInfo);
        new lPos=0;
...
LoadConfig()
{
    new data[256], Cache:result;
    result = mysql_query(Database, "SELECT * FROM `mru_config`");
    if(cache_num_rows())
    {
		new i = 0;
        cache_get_value_index(i, 0, RadioSANUno );
		cache_get_value_index(i, 1, RadioSANDos );
		cache_get_value_index_int(i, 2, ZONE_DISABLED );
		cache_get_value_index_int(i, 3, ZONE_DEF_TIME );
		cache_get_value_index(i, 4, data );
		cache_get_value_index_int(i, 5, STANOWE_GATE_KEY );
		cache_get_value_index_int(i, 6, TJD_Materials);
	...
	result = mysql_query(Database, "SELECT `changelog` FROM `mru_config`");
	if(cache_num_rows())
	{
		cache_get_value_index(0, 0, Changelog);
    ...
    print("Wczytano podstawową konfigurację");
}
...
IsNickCorrect(nick[])
{
	...
	return 0;*/
	return IsRPName(nick, true, true);
}

CheckAlfaNumeric(password[])
{
    new charsets[46] = "0123456789aąbcćdeęfghijklłmnńoóprsśtuvwyzżźxq";
...
GetVehicleMaxPassengersOld(iModel) //RydeR
{
    if(400 <= iModel <= 611)
    {
        static
...
Zone_StartAttack(zoneid, attacker, defender)
{
    ...
    if(attacker > 100) //gangi
    {
        if(defender > 100) //gang-gang
        {
            foreach(new i : Player)
            {
				new allowedSlots = IsPlayerPremiumOld(i) ? MAX_PLAYER_GROUPS_PREMIUM : MAX_PLAYER_GROUPS;
				new frac;
				for(new x = 0; x < allowedSlots; x++)
				{
    ...
    printf("[GangZone] Atak na strefę %d przez %d. Atakuje %d osób broni %d osób.", zoneid, attacker, ZoneAttackData[zoneid][0], ZoneAttackData[zoneid][1]);
}
...
Zone_GangUpdate(bool:cash=false)
{
    ...
    MySQL_TQueryInline(Database, using inline OnZoneLoaded, "SELECT * FROM `mru_strefylimit`");
    if(cash)
    {
...
{
    new Cache:result;
    result = mysql_query(Database, "SELECT * FROM `mru_strefy`");
    new id, kontrol, expire, time=gettime();
    for(new i = 0; i < cache_num_rows(); i++)
    {
		cache_get_value_index_int(0, 0, id);
		cache_get_value_index_int(0, 1, kontrol);
		cache_get_value_index_int(0, 2, expire);
...
{
	if(IsABike(veh)) return sendErrorMessage(playerid, "Nie możesz użyć /lock na motocyklu");
    new engine, lights, alarm, doors, bonnet, boot, objective;
...
MoveCamera(playerid)
{
	...
	if(GetPVarInt(playerid, "flymode-using-streamer") == 1) MoveDynamicObject(noclipdata[playerid][flyobject], X, Y, Z, speed);
	else MovePlayerObject(playerid, noclipdata[playerid][flyobject], X, Y, Z, speed);
...
CancelFlyMode(playerid)
{
	...
	if(GetPVarInt(playerid, "flymode-using-streamer") == 1) DestroyDynamicObject(noclipdata[playerid][flyobject]);
	else DestroyPlayerObject(playerid, noclipdata[playerid][flyobject]);
	noclipdata[playerid][cameramode] = CAMERA_MODE_NONE;
	DeletePVar(playerid, "flymode-using-streamer");
...
FlyMode(playerid, typ)
{
    ...
    if(noclipdata[playerid][flyobject] == INVALID_OBJECT_ID) 
	{
		noclipdata[playerid][flyobject] = CreateDynamicObject(19300, X, Y, Z, 0, 0, 0, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid), playerid);
		SetPVarInt(playerid, "flymode-using-streamer", 1);
		SendClientMessage(playerid, -1, " Błąd limitu obiektów - using CreateDynamicObject");
	}
	else
		DeletePVar(playerid, "flymode-using-streamer");
	...
	if(GetPVarInt(playerid, "flymode-using-streamer") == 1) AttachCameraToDynamicObject(playerid, noclipdata[playerid][flyobject]);
	else AttachCameraToPlayerObject(playerid, noclipdata[playerid][flyobject]);
...
Patrol_Init()
{
    ...
    TextDrawColour(PatrolGammaSq, -1);
    ...
    TextDrawColour(PatrolDeltaSq, -1);
    ...
    for(new i=0;i<MAX_PATROLS;i++)
    {
        ...
        TextDrawColour(PatrolMarker[i], -1);
...
{
	new string[144], sendername[MAX_PLAYER_NAME + 1];
	GetPlayerName(playerid, sendername, sizeof(sendername));
	...
	if(red)
	{
		format(string, sizeof(string), "=: %s %s GPS %s :=", sendername, (PDGPS == playerid) ? ("deaktywował") : ("aktywował"), (PDGPS == playerid) ? ("- odwołuję RED") : ("potrzebne wsparcie! - CODE RED"));
		PDGPS = (PDGPS == playerid) ? (-1) : (playerid);
	}
	else
	{
		format(string, sizeof(string), "=: %s %s GPS %s :=", sendername, (PDGPS == playerid) ? ("deaktywował") : ("aktywował"), (PDGPS == playerid) ? ("") : ("potrzebne wsparcie!"));
		PDGPS = (PDGPS == playerid) ? (-1) : (playerid);
	}
...
Patrol_CreateMarker(patrolid, Float:x, Float:y, type)
{
    ...
    TextDrawColour(PatrolMarker[patrolid], -1);
...
Oil_OnPlayerPress(playerid, keys)
{
    ...
    {
        //Co się dzieje gdy wcisnięto niepoprawny przycisk
        new lStr[16];
...
Oil_Destroy(lID)
{
    ...
    foreach(new i : Player)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[ERS] Usunąłeś plamę oleju! Otrzymujesz "#PRICE_PLAMA"$! [ERS]");
        DajKaseDone(i, PRICE_PLAMA);
        GroupSendMessage(4, COLOR_GREEN, "[ERS] Strażak usunął plamę oleju! Na konto frakcji wpływa "#PRICE_PLAMA"$! [ERS]");
		SendMessageToAdmin(sprintf("PLAMY: Gracz %s [%d] usunął plamę oleju i otrzymał za to "#PRICE_PLAMA"$!", GetNickEx(i), i), COLOR_LIGHTRED);
		Log(payLog, WARNING, "%s usunął plamę oleju i otrzymał za to "#PRICE_PLAMA"$", GetPlayerLogName(i));
...
ChangePlayerName(playerid, name[])
{
    if(strlen(name) >= 21)
	{
		SendClientMessage(playerid, COLOR_RED, "Nowy nick może mieć maksymalnie 20 znaków!");
		return 0;
	}

    if(toupper(name[0]) != name[0])
    {
        SendClientMessage(playerid, COLOR_RED, "Litery początkowe muszą zaczynać się od dużej.");
        return 0;
    }
    if(name[strlen(name)-1] == '_')
    {
        SendClientMessage(playerid, COLOR_RED, "Nie moze byc podłogi na końcu.");
        return 0;
    }
    for(new i=0;i<strlen(name);i++)
    {
        if(name[i] == ' ')
        {
            SendClientMessage(playerid, COLOR_RED, "Nick zawiera spację, zmień to.");
		    return 0;
        }
    }
    new stabcount=0;
    ...
    {
        SendClientMessage(playerid, COLOR_RED, "Nick nie zawiera podkresleń, lub ma ich ponad 2.");
		return 0;
    }
	...
	{
		SendClientMessage(playerid, COLOR_YELLOW, "Ten Nick jest już zajęty przez innego gracza.");
		return 0;
	}
...
TJD_Exit()
{
    ...
    if(MYSQL_SAVING) mysql_query(Database, lStr);
}
...
TJD_Load()
{
    ...
    new lStr[64];
    format(lStr, 64, "Liczba materiałów\n{99311E}%d\n{646464}(/zlecenie)", TJD_Materials);
...
TJD_JobEnd(playerid, bool:quiter=false)
{
    ...
    new money = ile*96;
    format(str, 64, "Praca zakończona. Przewiozłes %d paczek.", ile);
    SendClientMessage(playerid, -1, str);
    if(quiter) money/=2, ile/=2;
    format(str, 64, "Zarabiasz $%d oraz otrzymujesz %d umiejętnosci.", money, floatround(ile/5, floatround_floor));
        ...
        if(idx != 0)
        {
            ...
            else
            {
				if(IsPlayerInRangeOfPoint(playerid, 20, TransportJobData[idx2][eTJDEndX], TransportJobData[idx2][eTJDEndY], TransportJobData[idx2][eTJDEndZ]))
				{
					...
					{
						new string[128];
						format(string, sizeof(string), "AdmCmd: %s zostal zkickowany przez Admina: Marcepan_Marks, powód: teleport (kurier)", GetNick(playerid));
                        SendPunishMessage(string, playerid);
						Log(punishmentLog, WARNING, "%s dostał kicka od antycheata, powód: teleport (kurier)", GetPlayerLogName(playerid));
			        	KickEx(playerid);
					}
					idx = (idx-1)/2;
...
public TJD_LoadTime(playerid, count, maxcount)
{
    ...
    if(count == maxcount)
    {
        ...
        SendClientMessage(playerid, 0x00FF00FF, "Towar załadowany, udaj się na miejsce wypakunku.");
        TogglePlayerControllable(playerid, 1);
    }
    else
    {
...
public TJD_UnloadTime(playerid, count, maxcount)
{
    if(count == maxcount)
    {
        ...
        format(str, 64, "Towar wyładowany, zarabiasz %d$", ile);
        SendClientMessage(playerid, 0x00FF00FF, str);

        DajKaseDone(playerid, ile);
		Sejf_Add(FRAC_KT, floatround(ile/2));
		Log(payLog, WARNING, "%s dostarczył towar i dostał $%d (do sejfu KT trafiło $%d)", GetPlayerLogName(playerid), ile, ile/2);
        ...
        if(ile != TransportJobData[idx][eTJDMoney]) SendClientMessage(playerid, 0x00FF00FF, "Pamiętaj, że im szybciej jedziesz, tym więcej gubisz towaru!");
        TogglePlayerControllable(playerid, 1);
    }
    else
    {
...
TJD_CreateTXD(playerid)
{
    ...
    PlayerTextDrawColour(playerid, TXD_TJDCounter[playerid], -1);
        ...
        if(CarData[caruid][c_Rang] != 0)
        {
            if(IsPlayerConnected(playerid))
            {
                if(GetPlayerVehicleID(playerid) == veh)
                {
                    ...
                    RemovePlayerFromVehicleEx(playerid);
                    SendClientMessage(playerid, COLOR_YELLOW, "Czas wypożyczenia minął!");
                }
...
ZaladujSamochody()
{
    ...
    CAR_End = id;
    printf("Wczytano %d aut do kradzieży", CAR_End);
	return 1;
}
...
Support_ShowTickets(playerid)
{
    ...
    if(strlen(str) > 10)
    {
        ShowPlayerDialogEx(playerid, D_SUPPORT_LIST, DIALOG_STYLE_LIST, "Zgłoszenia", str, "Wybierz", "Wyjdź");
...
BuyDrinkOnClub(playerid, const dName[], dCost, dLVL, dAction)
{
	...
	if(GetPVarInt(playerid, "jestPrzyBarzeVIP") == 1)
	{
		...
		{
			format(string, sizeof(string), "%s kupił w barze %s - zaczyna pić", GetNick(playerid), dName);
			ProxDetector(10.0, playerid, string, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
		...
		{
			sendErrorMessage(playerid, "Nie masz takiej ilości gotówki!");
			return 1;
		}
	}
...
ShowPersonalization(playerid, value)
{
	...
	if(value == 2)
	{
		...
		{
			strdel(persona_A, 0, 64);
			strins(persona_A, "Ogłoszenia [AD]\t{FF6A6A}OFF\n", 0);
		}
		else if(PlayerPersonalization[playerid][PERS_AD] == 0)
		{
			strdel(persona_A, 0, 64);
			strins(persona_A, "Ogłoszenia [AD]\t{80FF00}ON\n", 0);
		}
		...
		if(PlayerPersonalization[playerid][PERS_OG_TYPE] == 0)
			strins(persona_F, "Ogłoszenia (/og)\t{80FF00}CHAT\n", 0);
		else if(PlayerPersonalization[playerid][PERS_OG_TYPE] == 1)
			strins(persona_F, "Ogłoszenia (/og)\t{80FF00}TEXTDRAW\n", 0);
		else
			strins(persona_F, "Ogłoszenia (/og)\t{80FF00}OBA\n", 0);
			
		//Komunikaty aresztowania
		new persona_G[64];
		if(PlayerPersonalization[playerid][PERS_ARREST_MSG] == 0)
			strins(persona_G, "Komunikaty /aresztuj\t{80FF00}ON\n", 0);
		else
			strins(persona_G, "Komunikaty /aresztuj\t{FF6A6A}OFF\n", 0);
			
		//Komunikaty news
		new persona_H[64];
		if(PlayerPersonalization[playerid][PERS_NEWS_MSG] == 0)
			strins(persona_H, "Komunikaty /news\t{80FF00}ON\n", 0);
		else
			strins(persona_H, "Komunikaty /news\t{FF6A6A}OFF\n", 0);
			
		format(string, sizeof(string), "%s%s%s%s%s%s%s%s", persona_A, persona_B, persona_C, persona_D, persona_E, persona_F, persona_G, persona_H);
		ShowPlayerDialogEx(playerid, D_PERS_CHAT, DIALOG_STYLE_TABLIST, "M-RP", string, "Akceptuj", "Wyjdz");
	}
	if(value == 3)
	{
	...
	if(value == 4)
	{
		...
		{
			strdel(persona_B, 0, 64);
			strins(persona_B, "Wyświetlanie nicków\t{80FF00}ON\n", 0);
		}
		else if(PlayerPersonalization[playerid][PERS_NICKNAMES] == 1)
		{
			strdel(persona_B, 0, 64);
			strins(persona_B, "Wyświetlanie nicków\t{FF6A6A}OFF\n",0);
		}
...
stock SetPlayerTW(playerid, valueTime, time, weather)
{
	...
	sendTipMessage(playerid, "Trwa inicjowanie pogody i czasu - chwilę to potrwa!");
	return 1;
}

forward OnPlayerTakeDamageWeaponHack(playerid, WEAPON:weaponid, fakekillid);
public OnPlayerTakeDamageWeaponHack(playerid, WEAPON:weaponid, fakekillid)
{
	...
	{
		new string[128];
		format(string, sizeof string, "ACv2 [#2002]: %s może mieć weapon hack. | Jeśli fakekill, to: %s .", GetNickEx(playerid), GetNickEx(fakekillid));
		SendCommandLogMessage(string);
...
SetWeaponValue(playerid, slot, weaponid, ammo, legal)
{
	switch(slot)
	{
...
{
	SendClientMessage(forplayerid, COLOR_WHITE, sprintf("--- Ostatnie wiadomości gracza %s: ---", GetNick(playerid)));
...
ShowPlayerDamaged(playerid, forplayerid)
{
	...
	new atakujacy[MAX_PLAYER_NAME + 1];
	if(index != 0)
	{
		for(new i = index-1; i>=0; i--)
		{
			...
			format(string, sizeof(string), "%d:%d:%d | %s zadał mu %0.1fHP z %s[%d]", godzina, minuta, sekunda, atakujacy, hp, weapon_decoded, Obrazenia[playerid][i][WEAPONID]);
...
ShowPlayerDamage(playerid, forplayerid)
{
	...
	new weapon_decoded[24];
	new godzina,minuta,sekunda,Float:hp;
	new broniacy[MAX_PLAYER_NAME + 1];
	if(index != 0)
	{
		for(new i = index-1; i>=0; i--)
		{
			...
			format(string, sizeof(string), "%d:%d:%d | zadał %0.1fHP graczowi %s z %s[%d]", godzina, minuta, sekunda, hp, broniacy, weapon_decoded, Obrazenia[playerid][i][WEAPONID]);
			SendClientMessage(forplayerid, COLOR_LIGHTGREEN, string);
		}
	}
	for(new i= 19; i >= index; i--)
...
{
	return (strfind(result, "ucieczka", true) != -1 || strfind(result, "poscig", true) != -1 || strfind(result, "pościg", true) != -1 || strfind(result, "ucieka", true) != -1);
}
...
PursuitMode(playerid, giveplayerid)
{
	if(ProxDetectorS(80.0, playerid, giveplayerid))
	{
		...
		{
			SendClientMessage(playerid,COLOR_LIGHTBLUE,"Rozpocząłeś pościg! Trwa on 10 minut.");
			SendClientMessage(giveplayerid,COLOR_PANICRED,"|_________________Tryb Pościgu_________________|");
			SendClientMessage(giveplayerid,COLOR_WHITE,"Służby porządkowe ruszyły za tobą w pościg! W takim wypadku najlepiej się poddać!");
			SendClientMessage(giveplayerid,COLOR_WHITE,"W trybie pościgu nie mozesz wyjść z gry, zginąć oraz być AFK.");
			SendClientMessage(giveplayerid,COLOR_WHITE,"Złamanie tych zasad skutkuje karą nadawaną automatycznie.");
	...
	{
		sendErrorMessage(playerid, "Gracz jest za daleko by nadać mu tryb pościgu.");
	}
}
...
public DeathAdminWarning(playerid, killerid, reason)
{
    ...
    if(killerid == INVALID_PLAYER_ID)
    {
        ...
        {
            format(string, sizeof(string), "{FF66CC}DeathWarning: %s [%d] umarł (%s)", 
                GetNick(playerid), 
                playerid, 
                (reason <= 46) ? GunNames[reason] : DeathNames[reason-46]
            );
    ...
    {
        Log(warningLog, WARNING, "%s %s %s z broni o id %d będąc w aucie (możliwe DB/CK2).", 
            GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid), reason);
        
        SendClientMessage(killerid, COLOR_YELLOW, "DriveBy jest zakazane, robiąc DriveBy możesz zostać ukarany przez admina!");
        
        if(PlayerInfo[killerid][pLevel] <= 1)
        {
            format(string, sizeof(string), "AdmWarning: %s[%d] %s %s[%d] z DB, dostał kicka !", 
                killername, killerid, bwreason, GetNick(playerid), playerid);
            SendMessageToAdmin(string, COLOR_YELLOW);
            Log(punishmentLog, WARNING, "Gracz %s dostał kicka od systemu za Drive-By", GetPlayerLogName(killerid));
            SendClientMessage(killerid, COLOR_PANICRED, "Dostałeś kicka za Drive-By do ludzi.");
            KickEx(killerid);
            SetPVarInt(playerid, "skip_bw", 1);
            return 1;
        }
        
        format(string, sizeof(string), "AdmWarning: %s[%d] %s %s[%d] będą w aucie (możliwe DB/CK2) [GunID %d]!", 
            killername, killerid, bwreason, GetNick(playerid), playerid, reason);
        SendMessageToAdmin(string, COLOR_YELLOW);
        return 1;
    }
    ...
    switch(reason)
    {
        case 38:
        {
            new vehicleModel = GetVehicleModel(GetPlayerVehicleID(killerid));
            if(vehicleModel != 425)
            {
                format(string, sizeof(string), "AdmWarning: %s[%d] %s %s[%d] z miniguna, podejrzane !", 
                    killername, killerid, bwreason, GetNick(playerid), playerid);
                SendMessageToAdmin(string, COLOR_YELLOW);
                Log(warningLog, WARNING, "%s zabił gracza %s używając miniguna", 
                    GetPlayerLogName(killerid), GetPlayerLogName(playerid));
            }
            else
            {
                format(string, sizeof(string), "{FF66CC}DeathWarning: {FFFFFF}%s[%d] %s %s[%d] z Huntera",  
                    killername, killerid, bwreason, GetNick(playerid), playerid);
            }
            SendMessageToAdminEx(string, COLOR_P@, 2);
        }
        case 41:
        {
            format(string, sizeof(string), "AdmWarning: %s[%d] %s %s[%d] ze spreya!", 
                killername, killerid, bwreason, GetNick(playerid), playerid);
            SendMessageToAdmin(string, COLOR_YELLOW);
            Log(warningLog, WARNING, "%s %s gracza %s używając spray'a", 
                GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid));
            SendMessageToAdminEx(string, COLOR_P@, 2);
        }
        default:
        {
            if(reason > 0 && reason <= 54)
            {
                format(string, sizeof(string), "{FF66CC}DeathWarning: {FFFFFF}%s[%d] %s %s[%d] z %s STREFA: %s", 
                    killername, killerid, bwreason, GetNick(playerid), playerid, 
                    (reason <= 46) ? GunNames[reason] : DeathNames[reason-46], Area_GetName(PlayerInfo[playerid][pCurrentArea], false));
                SendMessageToAdminEx(string, COLOR_P@, 2);
            }
            
            if(!GetPVarInt(playerid, "skip_bw") && PlayerInfo[playerid][pInjury] > 0)
            {
                if(lowcaz[killerid] == playerid && lowcap[playerid] != killerid && poddaje[playerid] != 1)
                {
                    format(string, sizeof(string), "AdmWarning: łowca Nagród %s[%d] %s %s[%d] bez oferty /poddajsie !", 
                        killername, killerid, bwreason, GetNick(playerid), playerid);
                    SendMessageToAdmin(string, COLOR_YELLOW);
                    Log(warningLog, WARNING, "łowca nagród %s %s gracza %s bez oferty /poddajsie", 
                        GetPlayerLogName(killerid), bwreason, GetPlayerLogName(playerid));
                }
            }
        }
...
public CuffedAction(playerid, cuffedid)
{
	...
	CreateNewCombatLog(cuffedid);
	return 1;
}

public UnCuffedAction(playerid)
{
	new cuffed_by = Kajdanki_PDkuje[playerid];
	Kajdanki_JestemSkuty[cuffed_by] = 0;
	Kajdanki_PDkuje[cuffed_by] = 0;
	Kajdanki_Uzyte[cuffed_by] = 0;
	Kajdanki_SkutyGracz[cuffed_by] = 0;
...
PrintDutyTime(playerid, reset = 1)
{
	...
	godziny = floatround( sekundy / 3600 );
	minuty = floatround( sekundy / 60 % 60 );
	format(string, sizeof(string), "[DUTY] Spędziłeś na służbie %d minut %d godzin.", minuty, godziny);
	SendClientMessage(playerid, COLOR_GREEN, string);
	if(reset)
	{
		sendTipMessage(playerid, "Twój czas na służbie zostanie wyzerowany za 15 minut.");
		SetPVarInt(playerid, "reset-dutytime", gettime()+900);
		if(OnDuty[playerid] > 0)
		{
			new orgid = GetPlayerGroupUID(playerid, OnDuty[playerid]-1);
			Log(serverLog, WARNING, "%s spędził na służbie %d minut %d godzin [%s]", GetPlayerLogName(playerid), minuty, godziny, GetGroupLogName(orgid));
		}
	}
	return 1;
}
...
ShowNewRadio(playerid)
{
	...
	ShowPlayerDialogEx(playerid, D_NEWRADIO, DIALOG_STYLE_LIST, "Radio", string, "Wybierz", "Wyjdź");
}
...
ShowDeskaRozdzielcza(playerid)
{
	...
	new recznystatus[16];
	GetVehicleParamsEx(veh,engine,lights,alarm,doors,bonnet,boot,objective);
	if(lights == 1) format(taknieL, sizeof(taknieL), "Wyłącz");
	else format(taknieL, sizeof(taknieL), "Włącz");
	...
	if(IsVehicleWindowOpened(veh, FL_DOOR)) format(szybastatus, sizeof(szybastatus), "Zamknij");
	else format(szybastatus, sizeof(szybastatus), "Otwórz");
	if(handbrake[veh]) format(recznystatus, sizeof(recznystatus), "Zwolnij");
	else format(recznystatus, sizeof(recznystatus), "Zaciągnij");
	format(komunikat, sizeof(komunikat), "Światła (%s)\nMaska (%s)\nBagażnik (%s)\nSzyba (%s)\nRęczny (%s)", taknieL, taknieBON, taknieBOT, szybastatus, recznystatus); // max. 79char
	if(veh > CAR_End)
	{
		if(CarData[VehicleUID[veh][vUID]][c_Neon] != 0)
		{
			new taknieNeo[16];
			if(VehicleUID[veh][vNeon]) format(taknieNeo, sizeof(taknieNeo), "Wyłącz");
			else format(taknieNeo, sizeof(taknieNeo), "Włącz");
...
SetTrunkState(playerid, vehicleid, op, action = 1)
{
	...
	if(op && action) RunCommand(playerid, "ja", "otwiera bagażnik");
	else if(action)	 RunCommand(playerid, "ja", "zamyka bagażnik");
	return 1;
}
...
UpdateRybyTop()
{
	...
	inline OnFishLoaded()
	{
		for(new i = 0; i < cache_num_rows(); i++)
		{
			...
			cache_get_value_index_int(i, 1, biggest);
			printf("Nick: %s", nick);
	...
	MySQL_TQueryInline(Database, using inline OnFishLoaded, "SELECT `Nick`, `BiggestFish` FROM `mru_konta` ORDER BY `BiggestFish` DESC LIMIT 10");
	format(RybyTopString, sizeof(RybyTopString), "{279127}Największe ryby:\n");
}

stock LoadCustomModels()
{
	new objects, skins;
	inline OnCustoModelsLoad()
	{
		for(new index; index != cache_num_rows(); index++)
		{
			
			new db_type, db_vw, db_baseid, db_newid, db_dff[64], db_txt[64], db_disabled;
			cache_get_value_int(index, "type", db_type);
			cache_get_value_int(index, "vw", db_vw);
			cache_get_value_int(index, "baseid", db_baseid);
			cache_get_value_int(index, "newid", db_newid);
			cache_get_value(index, "dff", db_dff, 64);
			cache_get_value(index, "txt", db_txt, 64);
			cache_get_value_int(index, "disabled", db_disabled);
			if(IsValidCustomModel(db_newid)) continue;
			if(db_disabled) continue;
			if(db_type == 0)
			{
				AddSimpleModel(db_vw, db_baseid, db_newid, db_dff, db_txt);
				objects++;
			}
			else
			{
				AddCharModel(db_baseid, db_newid, db_dff, db_txt);
				skins++;
			}
		}
		printf("[MODELS] Zaladowano %d modeli (%d obiektow i %d skinow)", objects+skins, objects, skins);
	}
	MySQL_TQueryInline(Database, using inline OnCustoModelsLoad, "SELECT * FROM `models`");
}
...
new allowed_chars[] = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890 ,.[]|ąĄłŁęʿŻćĆóӜŚńŃ()";
stock CheckBlockedChars(const tekst[]) {
	for (new i = 0; tekst[i] != '\0'; i++) {
		new found = 0;
		for (new j = 0; allowed_chars[j] != '\0'; j++) {
			if (tekst[i] == allowed_chars[j]) {
				found = 1;
				break;
			}
		}
		if (!found) return i; // return position of first not allowed character
	}
...
FormatNumber(number) {
   ...
   format(string, 15, "%d", number);

   if (strlen(string) < sizeof string) {
      if (number >= 1000 && number < 10000) strins(string, ".", 1, sizeof string);

      else if (number >= 10000 && number < 100000) strins(string, ".", 2, sizeof string);

      else if (number >= 100000 && number < 1000000) strins(string, ".", 3, sizeof string);

      else if (number >= 1000000 && number < 10000000) strins(string, ".", 1, sizeof string), strins(string, ".", 5, sizeof string);

      else if (number >= 10000000 && number < 100000000) strins(string, ".", 2, sizeof string), strins(string, ".", 6, sizeof string);

      else if (number >= 100000000 && number < 1000000000) strins(string, ".", 3, sizeof string), strins(string, ".", 7, sizeof string);

      else if (number >= 1000000000 && number < 10000000000) {
        strins(string, ".", 1, sizeof string),
        strins(string, ".", 5, sizeof string),
        strins(string, ".", 9, sizeof string);
      }

      else format(string, 10, "%d", number);
   }

   return string;
}

stock UpdateFuelPrice(newPrice)
{
    MULT_TANKOWANIE = newPrice;
    return 1;
}

SetPlayerBank(playerid, amount)
{
	Log(moneyLog, WARNING, sprintf("%s zmienił stan konta bankowego z %d na %d", GetPlayerLogName(playerid), PlayerInfo[playerid][pAccount], amount));
	new oldmoney = PlayerInfo[playerid][pAccount];
...
// -55 = koła w prawo
// -54 = koła w lewo
// -1 = brak sterowania A i D
// 0 - normalne
SetPlayerDrunkHandling(playerid, value){
       new BitStream:bs = BS_New();
       BS_WriteValue(bs,PR_UINT32, value);
       PR_SendRPC(bs, playerid, 150, PR_LOW_PRIORITY,  PR_RELIABLE_ORDERED);
       BS_Delete(bs);
}

stock AddZeroToTime(time)
{
	new ftime[5];
	if(time < 10)
		format(ftime, sizeof(ftime), "0%d", time);
	else
		format(ftime, sizeof(ftime), "%d", time);
	return ftime;
}
//EOF
