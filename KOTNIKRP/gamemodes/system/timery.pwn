...
public SprzedajMatsTimer(playerid,giveplayerid)
{
	if(GetPVarInt(giveplayerid, "OKupMats") == 1)
	{
    	...
    	sendErrorMessage(giveplayerid, "Sprzedaż mats została anulowana!");
	}
	if(GetPVarInt(playerid, "OSprzedajMats") == 1)
	{
		SetPVarInt(playerid, "OSprzedajMats", 0);
		sendErrorMessage(playerid, "Sprzedaż mats została anulowana!");
	}
	return 1;
}
...
public Naprawa(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		format(string, sizeof(string), "* Naprawiłeś pojazd %s, otrzymujesz $%d.",giveplayer,RepairPrice[playerid]);
		SendClientMessage(RepairOffer[playerid], COLOR_LIGHTBLUE, string);
		format(string, sizeof(string),"* Mechanik %s naprawia pojazd %s i chowa narzędzia do skrzynki.",giveplayer,VehicleNames[GetVehicleModel(RepairCar[playerid])-400]);
	...
	{
		SendClientMessage(playerid, -1, "Naprawa przerwana - wyszedłeś z pojazdu");
	}
    return 1;
}
...
public SetTimeAndWeather(playerid)
{
	...
	sendTipMessage(playerid, "Pomyślnie ustalono pogodę i czas dla VW"); 
...
public glosuj_admin_ankieta()
{
	...
	SendAdminMessage(COLOR_RED, "=====[WYNIKI GŁOSOWANIA]======"); 
	format(string, sizeof(string), "Użytkownicy, którzy głosowali na TAK: %d", glosowanie_admina_tak);
	SendAdminMessage(COLOR_P@, string);
	format(string, sizeof(string), "Użytkownicy, którzy głosowali na NIE: %d", glosowanie_admina_nie);
	SendAdminMessage(COLOR_P@, string);
...
public KomunikatCzas(playerid)
{
	...
	if(komunikatMinuty[playerid] == 60)
	{
		...
		sendTipMessage(playerid, "Zakończono odliczanie - Możesz ponownie wysłać komunikat"); 
...
public KomunikatCzasZerowanie(playerid)
{
	...
	if(komunikatMinutyZerowanie[playerid] == 5)
	{
		new string[128];
		format(string, sizeof(string), "null"); 
		sendTipMessage(playerid, "Zakończono odliczanie - Możesz ponowie wysłać komunikat"); 
		SetPVarString(playerid, "trescOgloszenia", string);
...
forward BusinessTypeKomunikatCzas(typeid);
public BusinessTypeKomunikatCzas(typeid)
{
	if(typeid < 0 || typeid >= 64) return 1;
	if(BusinessTypeCooldown[typeid] > 0)
	{
		BusinessTypeCooldown[typeid]--;
		if(BusinessTypeCooldown[typeid] <= 0)
		{
			BusinessTypeCooldown[typeid] = 0;
			KillTimer(BusinessTypeKomunikatTime[typeid]);
			BusinessTypeKomunikatTime[typeid] = 0;
		}
	}
	return 1;
}
...
public AktywujPozar()
{
	...
	if(losowy == 1)
	{
    	...
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: DOM JEDNORODZINNY - IDLEWOOD", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH KUCHENKI GAZOWEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
   		GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: DOM JEDNORODZINNY - IDLEWOOD",1);
	...
	else if(losowy == 2)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: SKLEP 24/7 W OKOLICACH DMV", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: AWARIA INSTALACJI ELEKTRYCZNEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK SKLEPU", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: SKLEP 24/7 W OKOLICACH DMV",1);
	...
	else if(losowy == 3)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: PIZZERIA IDLEWOOD", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: AWARIA KUCHENKI GAZOWEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK RESTAURACJI", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: PIZZERIA IDLEWOOD",1);
	...
	else if(losowy == 4)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: SKLEP BINCO - GANTON", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: SKLEP BINCO - GANTON",1);
	...
	else if(losowy == 5)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: STACJA PALIW - TEMPLE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH DYSTRYBUTORA", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK STACJI", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: STACJA PALIW - TEMPLE",1);
	...
	else if(losowy == 6)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: KONTENERY - LOTNISKO LOS SANTOS", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: OCHRONA LOTNISKA", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: KONTENERY - LOTNISKO LOS SANTOS",1);
	...
	else if(losowy == 7)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: PĄCZKARNIA NA MARKET", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH INSTALACJI ELEKTRYCZNEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: PĄCZKARNIA NA MARKET",1);
	...
	else if(losowy == 8)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: BAR W DILLIMORE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: BAR W DILLIMORE",1);
	...
	else if(losowy == 9)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: URZĄD MIASTA PALOMINO CREEK", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH INSTALACJI ELEKTRYCZNEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK URZĘDU MIASTA", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: URZĄD MIASTA PALOMINO CREEK",1);
	...
	else if(losowy == 10)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: SKLEP 24/7 - IDLEWOOD", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: SKLEP 24/7 - IDLEWOOD",1);
	...
	else if(losowy == 11)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: MOTEL JEFFERSON", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: MOTEL JEFFERSON",1);
	...
	else if(losowy == 12)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: SALON AUT", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: SALON AUT",1);
	...
	else if(losowy == 13)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: GUNSHOP OBOK BAZY LSFD", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: PODPALENIE", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRACOWNIK", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: GUNSHOP OBOK BAZY LSFD",1);
	...
	else if(losowy == 14)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: BUDYNEK OBOK BIUROWCA FBI", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH INSTALACJI ELEKTRYCZNEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: BUDYNEK OBOK BIUROWCA FBI",1);
	...
	else if(losowy == 15)
	{
		GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "           UWAGA: WYBUCHŁ POŻAR!", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "MIEJSCE POŻARU: BUDYNEK OBOK VINYL CLUB", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "PRZYCZYNA POŻARU: WYBUCH INSTALACJI ELEKTRYCZNEJ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "ZGŁOSIŁ: PRZECHODZIEŃ", true);
    	GroupSendMessage(FRAC_ERS, 0xAA3333AA, "         !!!!UDAJ SIĘ NA MIEJSCE!!!!", true);
    	GroupSendMessage(FRAC_ERS, 0xFFFFFFAA, fraction_name, true);
    	ABroadCast(COLOR_YELLOW,"[SYSTEM POŻARÓW] Aktywowano pożar: BUDYNEK OBOK VINYL CLUB",1);
	...
	if(IsPlayerPaused(playerid) || IdleCount[playerid] >= MAX_IDLE_COUNT)
	{
		...
		if(afktime > 600 && (PlayerInfo[playerid][pAdmin] >= 1 || PlayerInfo[playerid][pNewAP] >= 1 || IsAScripter(playerid)))
		{
			if(GetPlayerAdminDutyStatus(playerid) == 1)
			{
				...
				{
					SendClientMessage(playerid, 0xAA3333AA, "Nie wolno afczyć podczas @Duty! Otrzymujesz Kicka za AFK (15min)");
...
public MainTimer()
{
    ...
    PatrolGPS();
	for(new i = 0, j = MAX_PLAYERS - 1; i <= j; i++)
	{
    	Ograniczenia(i);
	}
    //ADD TIME
	...
	foreach(new i : Player)
	{
		...
		if(GetPlayerPing(i) >= 2000 && PlayerInfo[i][pAdmin] == 0)
		{
			if(gPlayerLogged[i] == 1)
			{
				...
				{
					SendClientMessage(i, COLOR_PANICRED, "Dostałeś kicka, POWÓD: Ping większy niż 2 000");
					KickEx(i);
				}
			}
		}
			...
			if(playerState >= 1 && playerState <= 6)
			{
				...
				if(ac_val)
				{
					...
					else
					{
						...
						{
							format(string, sizeof(string), "Dostałeś kicka od systemu, powód: Spawn Broni [%d]", ac_val);
                    ...
                    switch(weaponID)
                    {
                        ...
                        case 4://nóż
    					{
    						if(PlayerInfo[i][pGun1] != 4 && !IsPlayerInGroup(i, 8) && PlayerInfo[i][pLider] != 8)
    						{
                                ...
                                {
        							SendClientMessage(i, COLOR_PANICRED, "Dostałeś kicka od systemu, powód: Spawn Broni");
        							KickEx(i);
                                }
    						}
    					}
			...
			if(PlayerPos[i][0] == PlayerPos[i][3] && PlayerPos[i][1] == PlayerPos[i][4])
			{
				...
				if(IdleCount[i] == MAX_IDLE_COUNT)
				{
					if(afk_timer[i] == -1)
					{
						...
						SendClientMessage(i, 0xAA3333AA, sprintf("Zostałeś oznaczony jako AFK, ponieważ od %d minut się nie poruszyłeś.", MAX_IDLE_COUNT));
					}
				}
				//KickEx(i);
			}
...
public RPGTimer()
{
	foreach(new i : Player)
	{
        ...
        {
			MruDialog(i, "ACv2: Kod #2005", "Zostałeś wyrzucony za weapon hack RPG.");
			format(string, sizeof string, "ACv2 [#2005]: %s został wyrzucony za weapon hack RPG.", GetNick(i));
			...
			if(floatround(VS,floatround_round) >= 350)
			{
				if(PlayerSlapperWarning[i] >= 5)
				{
					if(AddPunishment(i, GetNick(i), -1, gettime(), PENALTY_BAN, 0, "Anti-Slapper", 0) == 1) {
						...
						Log(punishmentLog, WARNING, "%s został zbanowany za podejrzenie slappera (%d speeda on-foot, %d ostrzeżeń)", GetPlayerLogName(i), floatround(VS,floatround_round), PlayerSlapperWarning[i]);
						KickEx(i);
					}
				}
				else
...
public JednaSekundaTimer()
{
    ...
    new plname[MAX_PLAYER_NAME + 1], Float:angle,Lost = 0, trigger = 0,winner[MAX_PLAYER_NAME + 1], loser[MAX_PLAYER_NAME + 1],titel[MAX_PLAYER_NAME + 1];
    ...
    foreach(new i : Player)
	{
		...
		if(RingTone[i] > 0 && Mobile[i] >= 0)
		{
			...
			else
			{
				...
				if(RingTone[i]%12 == 0 || RingTone[i] == 1)
				{
					...
					if(slotKontaktu >= 0)
					{
						format(string, sizeof(string), "Twój telefon dzwoni, (aby odebrać wpisz: /od) dzwoniący: %s (%d)", Kontakty[i][slotKontaktu][eNazwa], GetPhoneNumber(caller));
					}
					else
					{
						format(string, sizeof(string), "Twój telefon dzwoni, (aby odebrać wpisz: /od) dzwoniący: %d", GetPhoneNumber(caller));
					}
					SendClientMessage(i, COLOR_YELLOW, string);
					format(string, sizeof(string), "* Telefon %s zaczyna dzwonić.", GetNick(i));
		...
		if(PlayerInfo[i][pJailed] > 0)
		{
			...
			if(PlayerInfo[i][pJailed] == 3)
			{
				...
				{
					sendErrorMessage(i, "Nie znajdujesz się w AJ! Pozycja przywrócona do poprawnej!");
					SetPlayerSpawn(i); 
				}
			}
			if(PlayerInfo[i][pJailTime] <= 0 && WantLawyer[i] == 0)
			{
				...
				else if(PlayerInfo[i][pJailed] == 3)
				{
					...
					{
						format(string, sizeof(string), "[Marcepan Marks] Zabrałem graczowi %s broń [Odsiedział karę za DM2]", GetNick(i));
						SendAdminMessage(COLOR_PANICRED, string);
						format(string, sizeof(string), "%s zabrałem twoją broń. Z pozdrowieniami - Marcepan Marks", GetNick(i));
				...
				PlayerInfo[i][pJailed] = 0;
				SendClientMessage(i, COLOR_GRAD1,"   Wolność! Odsiedziałeś karę, mamy nadzieje że to cię czegoś nauczyło.");
		...
		if(State == PLAYER_STATE_DRIVER)
		{
			if(IsPlayerInRangeOfPoint(i, 7.0, 2064.0703,-1831.3167,13.3853) || IsPlayerInRangeOfPoint(i, 5.0, 1351.0012,-1818.5981,13.3031) || IsPlayerInRangeOfPoint(i, 7.0, 1024.8514,-1022.2302,31.9395) || IsPlayerInRangeOfPoint(i, 7.0, 486.9398,-1742.4130,10.9594) || IsPlayerInRangeOfPoint(i, 7.0, -1904.2325,285.3743,40.8843)  || IsPlayerInRangeOfPoint(i, 7.0, 720.0876,-458.3574,16.3359) || IsPlayerInRangeOfPoint(i, 7.0, -2425.9668,1023.2122,50.1248) || IsPlayerInRangeOfPoint(i, 7.0, 1972.6704,2163.9829,10.7942) || IsPlayerInRangeOfPoint(i, 7.0, -100.3769,1115.7079,19.4688) || IsPlayerInRangeOfPoint(i, 7.0, -1420.5669,2584.1997,55.5703))
			{
				if(naprawiony[i] == 0)
				{
					...
					if(health <= CarData[VehicleUID[vehicleid][vUID]][c_MaxHP] - 1)
					{
						if(IsACopCar(GetPlayerVehicleID(i)) && OnDuty[i] > 0)
						{
							sendTipMessage(i, "Zapłaciłeś $0 za wizytę w warsztacie pojazdem służbowym");
						}
						else
						{
							sendTipMessageFormat(i, "Zapłaciłeś $%d za wizytę w warsztacie", PRICE_PAYNSPRAY);
		...
		if(GetPlayerWeapon(i) != 0 && PlayerInfo[i][pWeaponBlock] > gettime())
		{
			...
			SetPlayerArmedWeapon(i, 0);
			MruDialog(i, "Blokada broni", sprintf("Posiadasz aktywną blokadę broni, nie możesz korzystać z niej przez %d minut.", floatround(sec / 60 % 60)));
		}

		// serce zapisu broni
		foreach(new xx : PlayerItems[i])
		{
			new itemid = PlayerItem[i][xx][player_item_id];
			if(itemid != -1)
			{
				if(Item[itemid][i_ItemType] == ITEM_TYPE_WEAPON && Item[itemid][i_Used] && Item[itemid][i_Value1] > 0)
				{
					new weapondata[2];
					GetPlayerWeaponData(i, GetWeaponSlot(Item[itemid][i_Value1]), weapondata[0], weapondata[1]);
					if(weapondata[0] == Item[itemid][i_Value1] && Item[itemid][i_Value2] > weapondata[1])
					{
						Item[itemid][i_Value2] = weapondata[1];
					}
				}
			}
		}
		if(State >= 1 && State <= 6)
		{
			...
			if(MaZapisanaBron(i))//if(PlayerInfo[i][pGun2] >= 2) - dziwne, że wcześniej nikt tego błędu nie zauważył.
			{
                weaponID = GetPlayerWeapon(i);
		...
		if(MedicTime[i] > 0)
		{
			...
			{
				format(string, sizeof(string), "DOKTOR: %d zostałeś uleczony.", i);
		...
		if(WantLawyer[i] >= 1)
		{
			...
			if(WantLawyer[i] >= 20)
			{
				SendClientMessage(i, COLOR_LIGHTRED, "Nie możesz już wezwać prawnika, Czas odsiadki rozpoczęty.");
				WantLawyer[i] = 0;
				CallLawyer[i] = 0;
			}
		}
		if(TutTime[i] >= 1 && !IsPlayerNPC(i))
		{
			...
			if( x > 0.1 || y > 0.1)
			{
				SendClientMessage(i, COLOR_LIGHTBLUE, "AC: Kick za ucieczkę z samouczka!");
				KickEx(i);
			}
			TutTime[i] += 1;
			if(TutTime[i] == 3)
			{
				...
				SendClientMessage(i, COLOR_PURPLE, "|____ Tutorial: Wstęp ____|");
				SendClientMessage(i, COLOR_WHITE, "Witaj na serwerze M-RP! Zostałeś tutaj odesłany w celu przedstawienia informacji na temat gry!");
				SendClientMessage(i, COLOR_WHITE, "Znajdujesz się na serwerze typu Role Play, inaczej odzwierciedlanie realnego życia w grze.");
				SendClientMessage(i, COLOR_WHITE, "Zapoznałeś się ze znaczeniem słowa Role Play musisz poznać zasady panujące na naszym serwerze.");
				SendClientMessage(i, COLOR_WHITE, "W tym celu przejdziesz drobny samouczek tekstowy, który w pełni przygotuje Cię do rozgrywki!"); 
			}
			...
			{
				SendClientMessage(i, COLOR_WHITE, "Na tym serwerze obowiązuje absolutny zakaz jakiegokolwiek DeathMatch`u(DM).");
				SendClientMessage(i, COLOR_WHITE, "Jeśli zdecydujesz się kogoś zabić, musisz mieć naprawdę ważny do tego powód.");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze nie chcemy trzymać osób, które bezmyślnie zabijają, jest to nie wporządku.");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze obowiązkowo trzeba mieć nick typu Imie_Nazwisko (np. Jan_Kowalski), jeśli masz inny poproś admina o zmianę go.");
			}
			...
			{
				SendClientMessage(i, COLOR_WHITE, "Jeżeli widzisz, że ktoś łamie regulamin, powiadom o tym dostępnych administratorów przez komendę /report.");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze pod żadnym pozorem nie wolno używać Bugów, należy natychmiast zgłaszać je do administracji. ");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze zakazuje się używania jakichkolwiek wspomagaczy(cheatów), które ułatwiają tobie rozgrywke.");
				SendClientMessage(i, COLOR_WHITE, "Osoba korzystająca z Cheatów i Bugów może zostać ostrzeżona, bądź zbanowana.");
			}
			...
			{
				SendClientMessage(i, COLOR_WHITE, "Aby zacząć rozgrywke na serwerze, musisz poznać zasadę OOC");
				SendClientMessage(i, COLOR_WHITE, "Wszystko co tutaj znajdziesz jest bardzo ważne, więc czytaj uważnie i to zapamiętaj.");
				SendClientMessage(i, COLOR_WHITE, "OOC to wszystko co dotyczy realnego świata, zakazuje się przenoszenia tych informacji do gry, podczas odgrywania postaci.");
				SendClientMessage(i, COLOR_WHITE, "Na serwerze Chat OOC znajduje się pod komendą /b.");
				SendClientMessage(i, COLOR_WHITE, "Jeżeli czegoś nie rozumiesz bądź nie wiesz odnośnie rozgrywki, możesz zapytać o to innych graczy na chacie OOC.");
				SendClientMessage(i, COLOR_WHITE, "Poprawnie napisany przykładowy tekst na chacie OOC wygląda tak: /b Możesz mi pomóc z rozgrywką? Nie rozumiem jak to zrobić.");
			}
			...
			{
				SendClientMessage(i, COLOR_WHITE, "Aby zacząć rozgrywke na serwerze, musisz wiedzieć co to jest IC, oraz poprawnie to interpretować.");
				SendClientMessage(i, COLOR_WHITE, "IC to przeciwieństwo OOC, inaczej rzeczy związane z uniwersum gry.");
				SendClientMessage(i, COLOR_WHITE, "Na tym chacie wczuwamy się w naszą odgrywaną postać. - Piszemy jedynie rzeczy z nią związane."); 
				SendClientMessage(i, COLOR_WHITE, "Na serwerze, na chacie IC możesz rozmawiać w różne sposoby: /l (Chat Lokalny), /s (Szept) /k (Krzyk).");
				SendClientMessage(i, COLOR_WHITE, "Poprawnie napisany przykładowy tekst na chacie IC wygląda tak: /l Witam serdecznie, jestem zainteresowany pana ofertą pracy.");
			}
			...
			else if(TutTime[i] == 98)
			{
				...
				SendClientMessage(i, COLOR_WHITE, "Przykładowo widzisz nick z nad głowy gracza OOC, nastepnie wykorzystujesz to do gry IC i odnosisz się do niego po imieniu.");
				SendClientMessage(i, COLOR_WHITE, "Inna sytuacja, ktoś mówi na chacie OOC, że jest liderem LSPD, po czym pytasz go IC o pracę w LSPD, jest to MG inaczej Meta Gaming.");
				SendClientMessage(i, COLOR_WHITE, "PG to inaczej przymuszanie do odegrania czynności bez możliwości reakcji");
				SendClientMessage(i, COLOR_WHITE, "Podchodzisz do gracza, następnie odgrywasz /me zabija Johna, to jest PG inaczej Power Gaming.");
			}
			else if(TutTime[i] == 112)
			{
				...
				SendClientMessage(i, COLOR_PURPLE, "|____ Tutorial: Zakończenie ____|");
			}
			else if(TutTime[i] == 114)
			{
				SendClientMessage(i, COLOR_WHITE, "Wszystkich powyższych wymienionych zasad powinieneś się trzymać.");
				SendClientMessage(i, COLOR_WHITE, "Zapewne masz jeszcze sporo pytań dotyczących gry. Spokojnie, znajdziesz na nie odpowiedź!");
				SendClientMessage(i, COLOR_WHITE, "Możesz śmiało pytać administratora (/admins), poprzez zapytania (/zapytaj), bądź też poprzez chat dla nowych graczy /newbie. To już koniec tekstowego samouczka. ");
				SendClientMessage(i, COLOR_WHITE, "Zasad, poradników i pomocy jest znacznie więcej na naszym forum! Odwiedź je: M-RP");
			}
			...
			else if(TutTime[i] == 125)
			{
				...
				SendClientMessage(i, COLOR_NEWS, "Witamy Cię serdecznie, pierwszy raz na serwerze.");
				SetPlayerSkinEx(i, PlayerInfo[i][pSkin]);
				SetPlayerSpawn(i);
				SetPlayerSpawnPos(i);
				SetPlayerInterior(i, 0);
				SetPlayerVirtualWorld(i, 0);
				format(string, sizeof(string), "Czy chcesz skorzystać z pomocy administracji w celu lepszego zapoznania się z serwerem?");
				ShowPlayerDialogEx(i, NOWY_GRACZ, DIALOG_STYLE_MSGBOX, "Witamy na M-RP", string, "Tak", "Nie");
			}
		}
				...
				if(trigger)
				{
					if(Lost == 1)
					{
						if(IsPlayerConnected(Boxer1) && IsPlayerConnected(Boxer2))
						{
								...
								if(IsPlayerConnected(TBoxer))
								{
									if(TBoxer != Boxer2)
									{
										if(PlayerInfo[Boxer2][pJob] == 12)
										{
											...
											SaveBoxer();
											format(string, sizeof(string), "Boks News: %s wygrał walkę z Mistrzem %s i teraz to ON jest Mistrzem Boksu.",  titel, loser);
											OOCOff(COLOR_WHITE,string);
										}
										else
										{
											SendClientMessage(Boxer2, COLOR_LIGHTBLUE, "* zostałbyś mistrzem bokserskim gdybyś miał pracę boxera !");
										}
									}
									else
									{
										GetPlayerName(TBoxer, titel, sizeof(titel));
										format(string, sizeof(string), "Boks News: Mistrz boksu %s wygrał walkę z %s.",  titel, loser);
										OOCOff(COLOR_WHITE,string);
										Titel[TitelWins] = PlayerInfo[TBoxer][pWins];
							...
							format(string, sizeof(string), "* Przegrałeś walkę z %s.", winner);
							SendClientMessage(Boxer1, COLOR_LIGHTBLUE, string);
							GameTextForPlayer(Boxer1, "~r~you lost", 3500, 1);
							format(string, sizeof(string), "* Wygrałeś walkę z %s.", loser);
							SendClientMessage(Boxer2, COLOR_LIGHTBLUE, string);
							GameTextForPlayer(Boxer2, "~r~Wygrales", 3500, 1);
							if(GetPlayerHealth(Boxer1, health) < 20)
							{
								SendClientMessage(Boxer1, COLOR_LIGHTBLUE, "* Czujesz się wyczerpany, idź coś zjeść.");
								SetPlayerHealth(Boxer1, 30.0);
							}
							else
							{
								SendClientMessage(Boxer1, COLOR_LIGHTBLUE, "* Czujesz się wspaniale, pomimo odbytego pojedynku.");
								SetPlayerHealth(Boxer1, 50.0);
							}
							if(GetPlayerHealth(Boxer2, health) < 10)
							{
								SendClientMessage(Boxer2, COLOR_LIGHTBLUE, "* Czujesz się wykończony, idź coś zjeść.");
								SetPlayerHealth(Boxer2, 30.0);
							}
							else
							{
								SendClientMessage(Boxer2, COLOR_LIGHTBLUE, "* Czujesz się wspaniale, pomimo odbytego pojedynku.");
					...
					else if(Lost == 2)
					{
						if(IsPlayerConnected(Boxer1) && IsPlayerConnected(Boxer2))
						{
							...
							if(TBoxer < 255)
							{
								if(IsPlayerConnected(TBoxer))
								{
									if(TBoxer != Boxer1)
									{
										...
										{
											SendClientMessage(Boxer1, COLOR_LIGHTBLUE, "* zostałbyś mistrzem bokserskim gdybyś miał pracę boxera !");
										}
									}
									else
									{
							...
							{
								SendClientMessage(Boxer2, COLOR_LIGHTBLUE, "* Czujesz się wyczerpany, idź coś zjeść.");
								SetPlayerHealth(Boxer2, 30.0);
							}
							else
							{
		...
		if(StartingPaintballRound == 1 && AnnouncedPaintballRound == 0)
		{
			...
			{
				SendClientMessage(i, COLOR_YELLOW, "Mecz Pintball`u rozpocznie się za 15 sekund (Aby dołączyło więcej uczestników).");
			}
		}
		if(StartingKartRound == 1 && AnnouncedKartRound == 0)
		{
			...
			{
				SendClientMessage(i, COLOR_YELLOW, "Wyścig gokartów rozpocznie się za 15 sekund (Aby dołączyło więcej uczestników).");
			}
		}
		...
		if(RestaurantCallTime[i] > 0)
		{
			if(RestaurantCallTime[i] == 60) { RestaurantCallTime[i] = 0; DisablePlayerCheckpoint(i); PlayerPlaySound(i, 1056, 0.0, 0.0, 0.0); GameTextForPlayer(i, "~r~Czerwony Marker ulegl destrukcji", 2500, 1); }
			else
			{
				format(string, sizeof(string), "%d", 60 - RestaurantCallTime[i]);
				GameTextForPlayer(i, string, 1500, 6);
				RestaurantCallTime[i] += 1;
			}
		}
		if(Robbed[i] == 1)
		{
		...
		if(PlayerCuffed[i] == 2 || PlayerCuffed[i] == 3 || PlayerTied[i] == 1)
		{
			if(PlayerCuffedTime[i] <= 0)
			{
				...
				format(string, sizeof(string), "* %s po wielu próbach poluzował sznur i jest wolny.", winner);
				ProxDetector(30.0, i, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	...
	foreach(new i : Player)
   	{
		...
		if(Refueling[i] == 1)
		{
			...
			if(kaska[i] >= FillUp+4)
			{
				...
				if(IsACopCar(GetPlayerVehicleID(i)) && OnDuty[i] > 0)
				{
					format(string,sizeof(string),"Pojazd służbowy zatankowany za: $0.");
			...
			{
				format(string,sizeof(string),"Nie posiadasz dość pieniędzy ($%d) aby zatankować ten pojazd.",FillUp);
...
public PlayersCheckerMinute()
{
	foreach(new j : Player)
	{
		...
		{
			CheckPlayerInAction(j);
			if(PlayerInfo[j][pFishes] >= 5) 
			{ 
				if(FishCount[j] >= 19) //10 minut
				{
			...
			if(GetPlayerSkin(j) == 0 && GetPlayerAdminDutyStatus(j) == 0 && GetPVarInt(j, "JestPodczasWjezdzania") == 0 && GetPVarInt(j, "IsAGetInTheCar") == 0)
			{
				...
				else 
				{
					...
					sendTipMessage(j, "Posiadałeś skin CJ-a ID [0] - przywróciliśmy Ci domyślny skin. Uważasz, że to błąd? Zgłoś utratę w dziale błędów.");
					sendTipMessage(j, "[.] opisz dokładnie co się stało, np. dostałeś unfrakcję lub jesteś w rodzinie, frakcji. Pomocna będzie każda informacja.");
				}
			}
		}
	}
...
public CarCheck()
{
	...
	foreach(new j : Player)
	{
        if(kaska[j] < 0)
		{
			if(MoneyMessage[j]==0)
			{
				format(string, sizeof(string), "Masz długi, musisz zarobić do następnej wypłaty $%d inaczej naślemy na ciebie policje.",PlayerInfo[j][pCash]);
        ...
        if(PlayerInfo[j][pCarLic] > 1000)
        {
            if(lTime > PlayerInfo[j][pCarLic])
            {
                ...
                SendClientMessage(j, COLOR_LIGHTRED, "Blokada licencji na prowadzenie pojazdu minęła!");
            }
        }
	}	
    //Kolczatki 23.08
...
public CheckTankMode(playerid, giveid, carid, Float:vhealth)
{
    ...
    format(string, sizeof(string), "Admin %s [%d] sprawdził %s [%d]. Wynik: %s", GetNickEx(playerid), playerid, GetNick(giveid), giveid, (result ? "{fad052}prawdopodobny tankmode (zalecany /spec)" : "{fa5252}negatywny (brak tankmode)"));
    SendCommandLogMessage(string);
