...
// 	CreateDynamic3DTextLabel("Wiedzą już o nas!\nBez obaw, nie poddamy się tak łatwo\nPoszukaj naszych dilerów materiałami na terenie fabryki", 0xAA3333AA, 2139.1128,-2288.9380,20.6646, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
...
// 	FabrykaMats_ActorLabel = CreateDynamic3DTextLabel("Handlarz materiałami\nNaciśnij 'Y' aby pogadać", COLOR_GRAD1, ActorX, ActorY, labelZ, 7.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0);
...
public OnPlayerEditAttachedObject(playerid, response, index, modelid, boneid, Float:fOffsetX, Float:fOffsetY, Float:fOffsetZ, Float:fRotX, Float:fRotY, Float:fRotZ, Float:fScaleX, Float:fScaleY, Float:fScaleZ)
{
	...
	if(response)
	{
		...
		//chwilowe rozwiązanie (póki nie poszerzę tablicy) - tak aby gracz nie musiał edytować za każdym razem 
...
Player_CanUseCar(playerid, vehicleid)
{
	...
	if (IsAnAmbulance(vehicleid))
	{
		...
		{
			sendTipMessageEx(playerid, COLOR_GRAD1, "Nie jesteś medykiem!");
			return 0;
		}
	}
	...
	if(VehicleUID[vehicleid][vUID] != 0)
    {
	    ...
	    if(CarData[lcarid][c_OwnerType] == CAR_OWNER_FRACTION)// wszystkie auta grup
	    {
            ...
            {
                sendTipMessageEx(playerid,COLOR_GREY, "Nie jesteś uprawniony do kierownia tym pojazdem.");
                return 0;
            }
            if(CarData[lcarid][c_Owner] == 11)
            {
	        	...
	        	{
                	format(string, sizeof(string), "Aby kierować tym pojazdem potrzebujesz %d rangi!", CarData[lcarid][c_Rang]);
        ...
        else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_SPECIAL) //specjalne
        {
            if(CarData[lcarid][c_Owner] == CAR_ZUZEL)
            {
    		    ...
    		    {
    				sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Nie jesteś zawodnikiem żużlowym, zgłoś się do administracji jeśli chcesz nim zostać.");
    				return 0;
    			}
            }
        }
        else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_JOB) //reszta do pracy
        {
            if(CarData[lcarid][c_Owner] == PlayerInfo[playerid][pJob])
            {
                ...
                if(wywal)
                {
                    ...
                    sendTipMessageEx(playerid,COLOR_GREY,sprintf("Aby prowadzić ten pojazd potrzebujesz %d skilla w zawodzie %s.", CarData[lcarid][c_Rang], JobNames[CarData[lcarid][c_Owner]]));
					sendTipMessageEx(playerid,COLOR_GREY,sprintf("Twoje punkty skilla w zawodzie %s wynoszą: %d pkt", JobNames[CarData[lcarid][c_Owner]], skill));
                    return 0;
                }
				if(GetVehicleModel(vehicleid) == 578 && PlayerInfo[playerid][pLevel] == 1)
				{
					format(string, sizeof(string), "Musisz mieć 2 level aby prowadzić tym pojazdem.");
                    sendTipMessageEx(playerid,COLOR_GREY,string);
					return 0;
				}
            }
            else
            {
                ...
                format(string, sizeof(string), "Aby prowadzić ten pojazd musisz być w zawodzie %s.", JobNames[CarData[lcarid][c_Owner]]);
                sendTipMessageEx(playerid,COLOR_GREY,string);
				return 0;
            }
            if(CarData[lcarid][c_Owner] == JOB_BUSDRIVER) sendTipMessageEx(playerid, COLOR_YELLOW, "SERVER: Wpisz /trasa aby rozpocząć pracę");
        }
            ...
            if(IsCarOwner(playerid, vehicleid, true))
	        {
                if(CarData[lcarid][c_Keys] != 0 && CarData[lcarid][c_Owner] != PlayerInfo[playerid][pUID])
                {
                    if(CarData[lcarid][c_Keys] != PlayerInfo[playerid][pUID])
    	       	    {
                        sendTipMessageEx(playerid, COLOR_NEWS, "Kluczyki od tego pojazdu zostały zabrane przez właściciela.");
	       	...
	       	{
	       	    sendTipMessageEx(playerid, COLOR_NEWS, "Nie masz kluczy do tego pojazdu, a zabezpieczenia są zbyt dobre abyś mógł go ukraść.");
	       		return 0;
	       	}
        }
    }
	if(IsACopCar(vehicleid))
	{
	    if(IsAPolicja(playerid))
	    {
	        if(OnDuty[playerid] == 0)
	        {
	            ...
	            {
	            	sendTipMessageEx(playerid, COLOR_GREY, "Musisz być na służbie aby jeździć autem policyjnym !");
                    return 0;
	            }
	        }
	    }
	...
	if(IsABoat(vehicleid))
	{
		...
		{
		    sendTipMessageEx(playerid, COLOR_GREY, "Nie wiesz w jaki sposób pływać łodzią, więc decydujesz się ją opuścić !");
		    return 0;
		}
	}
	else if(IsAPlane(vehicleid))
	{
		...
		{
			sendTipMessageEx(playerid, COLOR_GREY, "Nie wiesz w jaki sposób latać samolotem, więc decydujesz się go opuścić !");
			return 0;
		}
	}
