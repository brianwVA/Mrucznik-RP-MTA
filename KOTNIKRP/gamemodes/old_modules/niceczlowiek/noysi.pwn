...
/*opis_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid==4192)
	{
		...
		if( dg_value == DG_DESC_DELETE )
		{
			...
			sendTipMessage(playerid, "Usunąłeś swój opis");
		}
		else if( dg_value == DG_DESC_ADD)
		{
			ShowPlayerDialogEx(playerid, 4193, DIALOG_STYLE_INPUT, "Ustaw opis postaci", "Wpisz nowy opis postaci\t", "Ok", "Wyjdź");
		}
...
LoadPrzewinienia()
{
    ...
    while(fread(file, line))
    {
        ...
        switch(typKary) {
            ...
            case 2: typKaryStr = ""#KARA_SREDNIA"Kara średnia";
            case 3: typKaryStr = ""#KARA_CIEZKA"Kara ciężka";
            case 4: typKaryStr = ""#KARA_BARDZOCIEZKA"Kara bardzo ciężka";
...
ShowPodrecznik(playerid, page=HELP_MAIN)
{
	...
	if( page == HELP_MAIN )
	{
		format(mainstring, sizeof(mainstring), ""#KARA_TEKST"Zasady panujące na serwerze");
	    ...
	    format(mainstring, sizeof(mainstring), "%s\n"#KARA_STRZALKA"    »» "#KARA_TEKST2"Jak uzyskać dodatkową pomoc", mainstring);
	    format(mainstring, sizeof(mainstring), "%s\n"#KARA_TEKST"Inne", mainstring);
	    format(mainstring, sizeof(mainstring), "%s\n"#KARA_STRZALKA"    »» "#KARA_TEKST2"Komendy dostępne na serwerze", mainstring);

	    ShowPlayerDialogEx(playerid, 1590, DIALOG_STYLE_LIST, "Podręcznik", mainstring, "Dalej", "Wyjdź");
	}	
	if( page == HELP_TYPY_KAR )
	{
		new typy_kar[500];
        format(typy_kar, sizeof(typy_kar), "Stopień\t\t\tKara");
        format(typy_kar, sizeof(typy_kar), "%s\n"#KARA_NIEZNACZNA"Kara Nieznaczna (I stopień)\t\t\t"#KARA_TEKST2"Do 5 minut AJ", typy_kar);
        format(typy_kar, sizeof(typy_kar), "%s\n"#KARA_LEKKA"Kara Lekka (II stopień)\t\t\t"#KARA_TEKST2"Do 10 minut AJ", typy_kar);
        format(typy_kar, sizeof(typy_kar), "%s\n"#KARA_SREDNIA"Kara Średnia (III stopień)\t\t\t"#KARA_TEKST2"Do 20 minut AJ", typy_kar);
        format(typy_kar, sizeof(typy_kar), "%s\n"#KARA_CIEZKA"Kara Ciężka (IV stopień)\t\t\t"#KARA_TEKST2"Do 30 minut AJ, Warn", typy_kar);
        format(typy_kar, sizeof(typy_kar), "%s\n"#KARA_BARDZOCIEZKA"Kara Bardzo Ciężka (V stopień)\t\t\t"#KARA_TEKST2"Warn, Ban", typy_kar);
        ...
        ShowPlayerDialogEx(playerid, 1591, DIALOG_STYLE_TABLIST_HEADERS, "Typy Kar", typy_kar, "Ok", "Wróć");
	}

	if( page == HELP_PRZEWINIENIA ) 
	{
		ShowPlayerDialogEx(playerid, 1592, DIALOG_STYLE_TABLIST_HEADERS, "Przewinienia", przewinienia, "Ok", "Wróć");
	}

	if( page == HELP_ZASADYKAR )
	{
		new zasady[2000];
		format(zasady, sizeof(zasady), "1. Admin ma prawo nadać karę tylko i wyłącznie za czynności przewidziane w LKiZ oraz o wysokości przewidzianej w LKiZ. \n\tZa jedną czynność niezgodną z LKiZ gracz może otrzymać tylko 1 karę.");
        format(zasady, sizeof(zasady), "%s\n \n2. Jeżeli czynność jest zabroniona w LKiZ, ale wykonywanie tej czynności przez gracza nikomu nie przeszkadza oraz jest \n\takceptowana przez wszystkich obecnych graczy, admin może się powstrzymać od ukarania (tylko gdy jest pewien, że akcja nie przeszkadza żadnemu graczowi który ją widzi/słyszy).", zasady);
        format(zasady, sizeof(zasady), "%s\n \n3. Staramy się nadawać karę adekwatnie do sytuacji oraz postawy gracza", zasady);
        format(zasady, sizeof(zasady), "%s\n \n4. Obowiązuje zakaz łączenia wartości kar. Jeżeli gracz popełnił 2 czyny do 15minut AJ oraz 1 do 30minut AJ to dajemy mu karę maksymalnie 30 minut AJ \n\ti resztę jego przewinień możemy umieścić w nawiasie.", zasady);
        format(zasady, sizeof(zasady), "%s\n \n5. Podawany powód musi być czytelny i jednoznaczny, powinien wyraźnie informować co gracz zrobił źle.", zasady);
        format(zasady, sizeof(zasady), "%s\n \n6. Jeżeli gracz złożył skargę na forum, stopień kary może zostać podniesiony o 1 poziom (lub wartość może być zwiększona x2 w przypadku kar specjalnych)", zasady);
        format(zasady, sizeof(zasady), "%s\n \n7. Jeżeli jakaś czynność jest wykonywana nagminnie, pomimo upomnień i kar, można nadać warna \n\t(nawet jeżeli stopień przewinienia tego nie przewiduje) lub zwiększyć wartość kary x2 lub stopień kary o 1.", zasady);
        format(zasady, sizeof(zasady), "%s\n \n8. Jeżeli czynność dotyczy wielu graczy, jest wykonywana na więcej niż 3 osobach, mamy prawo podnieść stopień kary o 1. \n\t(np. HK dla 10 = Masowe HK i kara stopnia 5)\n\tUwaga! Przy nadawaniu takiej kary administrator musi wpisać w powodzie słowo \"Masowe\".\n\tUwaga! Zasada działa jedynie dla następujących przewinień: DM(i jego odmiany), DB, HK, CK, DD/CZ", zasady);
	...
	{
		new zasady[2350];
		format(zasady, sizeof(zasady), "To odgrywanie prawdziwego życia w świecie wirtualnym. Role Play na naszym serwerze polega na kreowaniu swojej postaci i odgrywaniu prawdziwego życia w grze. \nOznacza to że możemy pracować (jako policjant, mechanik czy też płatny morderca), kupować domy, samochody oraz wiele innych rzeczy.");
        format(zasady, sizeof(zasady), "%s\nOdgrywamy także różne sytuacje, np. napady na bank, wyścigi uliczne, sprzedaż narkotyków. Nasze postacie zarabiają pieniądze oraz zdobywają umiejętności i znajomości.\n\tAby grać zgodnie z zasadami RP - po prostu wczuj się w postać tak, jakbyś to ty nią był.", zasady);
        format(zasady, sizeof(zasady), "%s\nJednak na naszym podejściu panuje także tak zwane \"Luźne RP\". Oznacza to, że na naszym serwerze odgrywanie prawdziwego życia\n\tprzystosowuje się do warunków jakie daje GTA SA i to, że nie musimy szanować życia swojej postaci jak własnego.", zasady);
        format(zasady, sizeof(zasady), "%s\nW ten sposób aby odegrywać prawidłowo nie pytamy się siebie w myśli - \"co bym zrobił gdybym był \n\tw prawdziwym życiu\" - tylko - \"co bym zrobił w prawdziwym życiu gdyby prawdziwe życie wyglądało tak jak sytuacja ogólna w grze\".", zasady);
        format(zasady, sizeof(zasady), "%s\n \nAby wczuć się w Role Play panujący na serwerze, prezentujemy krótki rys fabularny serwera: \n ", zasady);           
        format(zasady, sizeof(zasady), "%s\nLos Santos jest miastem opanowanym przez przestępców, nawet cywile chodzą po mieście uzbrojeni. Każdy walczy o swoje miejsce siłą a tych,\n\t\tktórzy stoją na drodze do ich celu traktują serią z kałacha.", zasady);
        format(zasady, sizeof(zasady), "%s\nSantos to nie el dorado pokoju gdzie ludzie żyją w zgodzie z Policją i wszyscy poświęcają się radosnej i owocnej współpracy.\n\tTo miasto, w którym co drugi obywatel jest kryminalistą a na każdym zakręcie może czekać cię śmierć.", zasady);
        format(zasady, sizeof(zasady), "%s\nWięc nie czekaj, weź sprawy w swoje ręcę i dołącz do ludzi siejących zamęt w Los Santos lub grup, które mają na celu uspokojenie motłochu i sprawienie by sprawiedliwość zatriumfowała.\n\tBądź zostań (nie)uczciwym pracownikiem i stań się najbogatszym człowiekiem w LS.", zasady);
        format(zasady, sizeof(zasady), "%s\n \nJak życie twojej postaci potoczy się dalej, zależy tylko i wyłącznie od ciebie! Powodzenia!", zasady);
	...
	if( page == HELP_PROFITYFORO )
	{
        ...
        format(zasady, sizeof(zasady), "%s\n"#KARA_TEKST2"Branie udziału w życiu społeczności\t"#KARA_BANICJI"Nie\t"#KARA_NIEZNACZNA"Tak", zasady);
        format(zasady, sizeof(zasady), "%s\n"#KARA_TEKST2"Dołączanie do frakcji\t"#KARA_SREDNIA"Zazwyczaj nie\t"#KARA_NIEZNACZNA"Tak", zasady);
        format(zasady, sizeof(zasady), "%s\n"#KARA_TEKST2"Dostęp do poradników\t"#KARA_BANICJI"Nie\t"#KARA_NIEZNACZNA"Tak", zasady);
        format(zasady, sizeof(zasady), "%s\n"#KARA_TEKST2"Dostęp do informacji o aktualizacjach\t"#KARA_BANICJI"Nie\t"#KARA_NIEZNACZNA"Tak", zasady);
        format(zasady, sizeof(zasady), "%s\n \n \n"#KARA_TEKST2"Nie czekaj! Zarejestruj się na {FFFFFF}M-RP", zasady);
...
hq_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == 1596)
	{
		...
		if(IsPlayerInGroup(playerid, 1) || PlayerInfo[playerid][pLider] == 1)
		{
	        ...
	        format(string, sizeof(string), ""#HQ_COLOR_TEKST2"Informacja o zgłoszeniu");
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_PLACEHOLDER"====================================================", string);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Treść: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_tresc]);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Godzina: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_kiedy]);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Zgłosił: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_nadal]);
	        ...
	        switch(Zgloszenie[dg_data][zgloszenie_status]) {
	            case 0: statusTxt = "Nie podjęto jeszcze decyzji";
	            case 1: statusTxt = "Akceptowane";
	            case 2: statusTxt = "Odrzucone";
	            case 3: statusTxt = "Fałszywe";
	        ...
	        if(Zgloszenie[dg_data][zgloszenie_status] == 1) {
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Przyjął: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
	            if(!strcmp(Zgloszenie[dg_data][zgloszenie_przyjal], GetNick(playerid))) {
	                ...
	                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_ODRZUCONE"Oznacz jako: FAŁSZYWE", string);    
	                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_ANULOWANE"Oznacz jako: ANULOWANE", string);
	            }
	        } else if(Zgloszenie[dg_data][zgloszenie_status] == 0) {
	            format(string, sizeof(string), "%s\n ", string);
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_AKCEPTOWANE"Akceptuj zgłoszenie", string);
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_ODRZUCONE"Odrzuć zgłoszenie", string);    
	        } else if(Zgloszenie[dg_data][zgloszenie_status] == 2) {
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Odrzucił: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
	        } else {
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Wydał decyzję: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
			}

			SetPVarInt(playerid, "zarzadzajZgl", dg_data);

			ShowPlayerDialogEx(playerid, 1597, DIALOG_STYLE_LIST, "Szczegóły Zgloszenia", string, "Wybierz", "Wróć");
		}
		if(IsPlayerInGroup(playerid, 3) || PlayerInfo[playerid][pLider] == 3)
		{
	        ...
	        format(string, sizeof(string), ""#HQ_COLOR_TEKST2"Informacja o zgłoszeniu");
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_PLACEHOLDER"====================================================", string);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Treść: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_tresc]);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Godzina: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_kiedy]);
	        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Zgłosił: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_nadal]);
	        ...
	        if(ZgloszenieSasp[dg_data][zgloszenie_status] == 1) {
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Przyjął: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
	            ...
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Odrzucił: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
	        } else {
	            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Wydał decyzję: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
			}
	...
	if(dialogid == 1597)
	{
		...
		if(IsPlayerInGroup(playerid, 1) || PlayerInfo[playerid][pLider] == 1)
		{
	        ...
	        if(listitem == 9)
	        {
	            ...
	            if(zglstatus == 0)
	            {
	                if(!strcmp(Zgloszenie[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zgłoszeń nie można akceptować!");
	            ...
	            new zglstatus = Zgloszenie[szczegol][zgloszenie_status];
	            if(zglstatus == 0) {
	                if(!strcmp(Zgloszenie[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zgłoszeń nie można odrzucać!");
	                strmid(Zgloszenie[szczegol][zgloszenie_przyjal], GetNick(playerid), 0, MAX_PLAYER_NAME, MAX_PLAYER_NAME);
    	...
    	if(IsPlayerInGroup(playerid, 3) || PlayerInfo[playerid][pLider] == 3)
		{
	        ...
	        if(listitem == 9)
	        {
	            ...
	            if(zglstatus == 0)
	            {
	                if(!strcmp(ZgloszenieSasp[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zgłoszeń nie można akceptować!");
	            ...
	            new zglstatus = ZgloszenieSasp[szczegol][zgloszenie_status];
	            if(zglstatus == 0) {
	                if(!strcmp(ZgloszenieSasp[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zgłoszeń nie można odrzucać!");
	                strmid(ZgloszenieSasp[szczegol][zgloszenie_przyjal], GetNick(playerid), 0, MAX_PLAYER_NAME, MAX_PLAYER_NAME);
...
ShowHeadquarters(playerid, page=HQ_MAIN)
{
	if(page == HQ_MAIN)
	{
        ...
        format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_TEKST"Ostatnie zgłoszenia", string);

  		ShowPlayerDialogEx(playerid, 1595, DIALOG_STYLE_LIST, "HeadQuarters/MDC", string, "Ok", "Wyjdź");
	}
	if(page == HQ_WL)
	{
		...
		if(IsPlayerInGroup(playerid, 1) || PlayerInfo[playerid][pLider] == 1)
		{
			for(new i = 0, j=OSTATNIE_ZGLOSZENIA; i<j; i++) {
		        if(Zgloszenie[i][zgloszenie_status] == 0) {
			        ...
			        format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    »» "#HQ_COLOR_FALSZYWE"[Fałszywe] %s\n", string, Zgloszenie[i][zgloszenie_tresc]);
        	...
        	ShowPlayerDialogEx(playerid, 1596, DIALOG_STYLE_LIST, ("Ostatnie zgłoszenia"), string, "Ok", "Wstecz");
		}
		if(IsPlayerInGroup(playerid, 3) || PlayerInfo[playerid][pLider] == 3)
		{
