        ...
        else if(PlayerHeist[playerid][p_RobTime] == 1)
        {
            switch(PlayerHeist[playerid][p_State])
            {
                case STATE_OPENING:
                {
                    ...
                    PlayerHeist[playerid][p_RobTime] = 30;
                    PlayerHeist[playerid][p_State] = STATE_OPENING2;
                    PlayerTextDrawSetString(playerid, RobberyText[playerid], "~b~~h~Sejf~n~~n~~w~Otwieranie drzwi sejfu...~n~Do wykonania ~r~00:30");
                    RemovePlayerAttachedObject(playerid, 4);
                    ApplyAnimation(playerid, "ROB_BANK", "CAT_Safe_Open", 4.0, 0, 0, 0, 0, 0, 1);
                    PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                    new radioMsg[256];
                    format(radioMsg, sizeof radioMsg, "W pomieszczeniu %s zostają wyważane drzwi sejfu. Udaj się tam jak najszybciej!!!", Heist[heist][h_Name]);
                ...
                case STATE_ROBBING:
                {
                    ...
                    new radioMsg[256];
                    format(radioMsg, sizeof radioMsg, "W pomieszczeniu %s sejf zostal okradziony!!!", Heist[heist][h_Name]);
                    SendRadioMessage(1, COLOR_YELLOW2, radioMsg);
                    SendRadioMessage(2, COLOR_YELLOW2, radioMsg);
                    SendRadioMessage(1, COLOR_YELLOW2, radioMsg);
                    SendRadioMessage(2, COLOR_YELLOW2, radioMsg);
