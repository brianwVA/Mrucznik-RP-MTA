...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case D_LISTANAPADOW:
        {
            ...
            if(!Heist[id][h_UID]) return sendTipMessage(playerid, "Wystąpił nieoczekiwany błąd.");
        ...
        case D_NAPAD_C_CONFIRM:
        {
            ...
            if(heist == -1) return sendTipMessage(playerid, "Wystąpił nieoczekiwany błąd przy tworzeniu napadu.");
            else _MruGracz(playerid, sprintf(" Pomyślnie stworzono napad o UID: %d", Heist[heist][h_UID]));
        }
    }
    return 1;
}
