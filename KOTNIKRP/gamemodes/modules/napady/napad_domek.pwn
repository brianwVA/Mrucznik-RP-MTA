...
#define SAFEROBTIME 90
#define DOORKICKTIME 35
#define DOORCLOSETIME 450
#define MINMONEY 1600
#define MAXMONEY 4500
...
new Float:lastRadioRobSafe = 0.0;
...
stock OnGameModeNapadDomek(){
	...
	RTH[textdooorss] = CreateDynamic3DTextLabel("{4169E1}Dom z sejfami\n{FFFFFF}Wpisz {4169E1}/wyłam {FFFFFF}aby wyłamać drzwi",COLOR_LIGHTGREENNAPAD,2403.6008,-1714.6058,14.2733,9);
	RTH[textsafee1] = CreateDynamic3DTextLabel("{4169E1}Sejf\n{FFFFFF}Wpisz {4169E1}/wlamanie {FFFFFF}aby włamać się do sejfu",COLOR_LIGHTGREENNAPAD,2402.9250,-1701.0734,14.2110,3);
	RTH[textsafee2] = CreateDynamic3DTextLabel("{4169E1}Sejf\n{FFFFFF}Wpisz {4169E1}/wlamanie {FFFFFF}aby włamać się do sejfu",COLOR_LIGHTGREENNAPAD,2398.5591,-1700.7568,14.2110,3);
...
YCMD:wlamanie(playerid, params[]){
	if(IsPlayerInRangeOfPoint(playerid, 1.0, 2402.9250,-1701.0734,14.2110) || IsPlayerInRangeOfPoint(playerid, 1.0, 2398.5591,-1700.7568,14.2110)){
		...
		if(GetPlayerVirtualWorld(playerid) != 0) return SendClientMessage(playerid, -1, "Dom można okradać tylko na vw 0!");
		if(!RTH[openeedd] || RTH[TimerKickdooorss] != 0){
			SendClientMessage(playerid, -1, "Drzwi są zamknięte");
		}
		else{
			if(IsPlayerInRangeOfPoint(playerid, 1.0, 2402.9250,-1701.0734,14.2110)){
				if(RTH[TimerRobbing1] != 0 || RTH[safeopeneedd1]){
					SendClientMessage(playerid, -1, "Nie możesz już okraść tego sejfu.");
				}
				else if((RTH[TimeToClose] - 10) < SAFEROBTIME){
					SendClientMessage(playerid, -1, "Uciekaj stąd, nie zdążyłbyś");
				}
				else{
					...
					if (gettime() - lastRadioRobSafe >= 10.0)
					{
						if (random(100) < 50)
						{
							SendRadioMessage(1, COLOR_LIGHTRED, "HQ: Trwa rabunek na dom w dzielnicy Ganton!!!", 0);
							SendRadioMessage(2, COLOR_LIGHTRED, "HQ: Trwa rabunek na dom w dzielnicy Ganton!!!", 0);
							lastRadioRobSafe = gettime();
						}
					}
...
YCMD:wylam(playerid, params[]){
	if(IsPlayerInRangeOfPoint(playerid, 3.0, 2402.765625, -1713.649658, 13.2043))
	{
		...
		if(GetPlayerVirtualWorld(playerid) != 0) return SendClientMessage(playerid, -1, "Dom można okradać tylko na vw 0!");
		if(RTH[openeedd])
			return SendClientMessage(playerid, -1, "Nie możesz otworzyć drzwi, są otwarte bądź ktoś je otwiera");
		if(RTH[WaitTimer] != 0)
			return SendClientMessage(playerid, -1, "Nie możesz teraz użyć tej komendy");
...
public RobSafe(playerid, id){
	...
	if(id == 1){
		...
		if(RTH[TimeToRob1] <= 0){
			...
			format(string, sizeof string, "Zdobyłeś %d$, a teraz się sprężaj z ucieczką, do zamknięcia drzwi pozostało %dsek.", ile, RTH[TimeToClose]);
...
public Kickdooorss(playerid){
	...
	if(RTH[TimeToKick] <= 0){
		...
		format(string, sizeof string, "{4169E1}Dom z sejfami\n{FF0000}\n{FFFFFF}Do zamknięcia się drzwi pozostało: {FFFF00}%d sekund", RTH[TimeToClose]);
...
public Closedooorss(){
	...
	if(RTH[TimeToClose] <= 0){
		...
		UpdateDynamic3DTextLabelText(RTH[textsafee1], COLOR_LIGHTGREENNAPAD, "{1FA7FF}Sejf\nWpisz {FFFFFF}/wlamanie {1FA7FF}aby włamać się do sejfu");
		UpdateDynamic3DTextLabelText(RTH[textsafee2], COLOR_LIGHTGREENNAPAD, "{1FA7FF}Sejf\nWpisz {FFFFFF}/wlamanie {1FA7FF}aby włamać się do sejfu");
		new string[128];
		format(string, sizeof string, "{1FA7FF}Dom z sejfami\n{FF0000}\n{FFFFFF}Możliwość otwarcia drzwi zostanie przywrócona za {FFFF00}%d sekund", RTH[WaitTime]);
		UpdateDynamic3DTextLabelText(RTH[textdooorss], COLOR_LIGHTGREENNAPAD, string);
	}
	else{
		new string[128];
