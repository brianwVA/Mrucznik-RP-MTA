...
command_setvregistration_Impl(playerid, registerText[32])
{
    if(PlayerInfo[playerid][pAdmin] >= 4000 || IsAScripter(playerid))
    {
        if(IsPlayerInAnyVehicle(playerid))
        {
            ...
            format(string, sizeof(string), "Administrator %s ustawił tablicę dla pojazdu %d - nowy text [%s]", GetNickEx(playerid), vehID, registerText);
            SendMessageToAdminEx(string, COLOR_P@, 2);
            CarData[VehicleUID[vehID][vUID]][c_Rejestracja] = registerText; 
            sendTipMessage(playerid, "Teraz należy zrespawnować pojazd!"); 
        }
        else
        {
            sendErrorMessage(playerid, "Musisz być w pojeździe!");
        }
    }
    return 1;
}
