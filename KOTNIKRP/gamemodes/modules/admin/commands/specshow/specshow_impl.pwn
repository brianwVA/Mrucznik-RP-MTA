    ...
    if(PlayerInfo[playerid][pAdmin] >= 2000 || IsAScripter(playerid))
    {
        ...
        if(playerSeeSpec[playerid] == INVALID_SPECTATE_ID)
        {
            if(valueSpec == -1)
            {
                ...
                sendTipMessageEx(playerid, COLOR_RED, "Włączyłeś podgląd użycia komendy /spectate na wszystkich administratorów"); 
                return 1;
            }
            if(IsPlayerConnected(valueSpec))
            {
                ...
                if(playerSeeSpec[playerid] == INVALID_SPECTATE_ID)
                {
                    format(string, sizeof(string), "Oznaczyłeś %s jako swój cel poglądu użyć komendy /spectate!", GetNick(valueSpec)); 
                    playerTargetSpec[valueSpec] = playerid; 
                    playerSeeSpec[playerid] = valueSpec;
                    sendTipMessageEx(playerid, COLOR_RED, string); 
                    sendTipMessage(playerid, "Każde użycie komendy przez tą osobę, zostanie Ci wyświetlone"); 
                }
                else
                {
                    sendErrorMessage(playerid, "Masz już włączony show-spectate! Wystąpił błąd."); 
                    return 1;
                }
            }
            else
        ...
        else
        {
            ...
            sendTipMessage(playerid, "Wyłączyłeś podgląd spectate"); 
        }
    }
    else 
    {
