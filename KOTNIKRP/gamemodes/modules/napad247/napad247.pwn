/* 
			...
			Dodać ograniczenie tworzenia napadów co 1 godzinę
...
stock LoadActorsToRob()
{
	...
	printf("Załadowano %d aktorów do obrabowania dla organizacji przestepczych!", rob);
}

forward SendCallToPolice(playerid);
public SendCallToPolice(playerid)
{ 
	GroupSendMessage(1, COLOR_RED, "ALARM: W sklepie 24/7 w okolicy urzędu miasta uruchomił się alarm!");
	format(C_STRING, sizeof(C_STRING), "[**] Monitoring CCTV: W sklepie 24/7 obok urzędu urchomił się alarm! [**]");
...
public CheckCopPlayers(playerid)
{
    ...
    } else sendErrorDialogMessage(playerid, "Nie ma funkcjonariuszy LSPD/FBI na służbie!");
}
...
public StartPlayerRob(playerid)
{
	...
	robInProgress[playerid] = 1;
 	...
 	sendTipDialogMessage(playerid, "Rozpocząłeś okradanie sprzedawcy sklepu\nsłużby zostały poinformowane o zaistniałej sytuacji!");
 	format(C_STRING, sizeof(C_STRING), "[# NAPAD #] Gracz %s [%d] zorganizował napad w sklepie 24/7", GetNick(playerid), playerid);
 	SendCommandLogMessage(C_STRING);
 	RobLog(C_STRING);

 	if(PlayerInfo[playerid][pMaska] == 1) {
 		sendTipMessage(playerid, "Jesteś zamaskowany, nie otrzymujesz WL za zorganizowanie napadu!");
 		return 1;
 	} 
 	else
 	{
...
{
	if(!IsAPrzestepca(playerid)) return sendErrorDialogMessage(playerid, "Nie jesteś z organizacji przestępczej!!");
	if(IsPlayerInRangeOfPoint(playerid, 2, NPCPosition[0][0], NPCPosition[0][1], NPCPosition[0][2]) || IsPlayerInRangeOfPoint(playerid, 2, NPCPosition[1][0], NPCPosition[1][1], NPCPosition[1][2]) || IsPlayerInRangeOfPoint(playerid, 2, NPCPosition[2][0], NPCPosition[2][1], NPCPosition[2][2])) {
		if(robSync >= 1) return sendErrorDialogMessage(playerid, "Ten sklep został niedawno obrabowany, spróbuj później");
		if(CheckMembersFromOrg(playerid) < MIN_ORG_ONLINE) return sendTipDialogMessage(playerid, "Nie ma w pobliżu Ciebie członków twojej organizacji (min. 3 osoby)"); 
		if(!GetPlayerWeapon(playerid)) return sendErrorDialogMessage(playerid, "Nie trzymasz żadnej broni w ręce!");
		CheckCopPlayers(playerid);
		return 1;
	} 
	else sendErrorDialogMessage(playerid, "Nie jesteś w pobliżu sprzedawcy!");
	return 1;
}
