...
stock PhonePanel(playerid, itemid, bool:action = false)
{
	if(action) 
	{
		ProxDetector(30.0, playerid, sprintf("* %s wyciąga telefon z kieszeni", GetNick(playerid)), COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_USECELLPHONE);
		SetPlayerAttachedObject(playerid, 4, -2002, 6, 0, 0.006, 0.015, -28.8, 0, 2.8);
	}
	new options[248];
	format(options, sizeof options, "Zadzwoń\nWyślij SMS\nPrzelew na telefon\n%s", (Item[itemid][i_Used]) ? ("Wyłącz") : ("Włącz"));
...
stock PhoneSms(playerid, to, number1, number2, text[])
{
	...
	if(PlayerInfo[playerid][pJailTime] > 0) return SendClientMessage(playerid, COLOR_GREY, "Nie możesz tego użyć będąc w więzieniu!");
	...
	if(number2 >= 100 && number2 <= 150)
	{
		...
		if(kaska[playerid] < smsCost) return sendErrorMessage(playerid, "Nie masz wystarczającej ilości środków!");
	...
	if(to == INVALID_PLAYER_ID)
		{PhoneError(playerid, false); return SendClientMessage(playerid, COLOR_GREY, "Nie udało się wysłać wiadomości - gracz o takim numerze jest offline.");}
	...
	SavePlayerSentMessage(playerid, sprintf("%s wysłał SMS do %s: %s", GetNick(playerid), GetNick(to), text), FROMME);
	SavePlayerSentMessage(to, sprintf("%s otrzymał SMS od %s: %s", GetNick(to), GetNick(playerid), text), TOME);
	...
	if(slotKontaktu >= 0)
		format(string, sizeof(string), "Wysłano SMS: %s, Odbiorca: %s (%d).", text, Kontakty[playerid][slotKontaktu][eNazwa], number2);
	else
		format(string, sizeof(string), "Wysłano SMS: %s, Odbiorca: %d.", text, number2);
...
{
	if(PlayerInfo[playerid][pLevel] < 3) return sendTipMessage(playerid, "Musisz mieć 3 level!");
	...
	if(PlayerInfo[id][pAccount]+money > MAX_MONEY_IN_BANK) return sendErrorMessage(playerid, "Gracz do którego próbowałeś przelać gotówkę - ma zbyt dużo pieniędzy na koncie."); 
	if(id == playerid) return sendErrorMessage(playerid, "Nie możesz wysłać przelewu samemu sobie.");

	//Czynności:
	SetPlayerBank(playerid, PlayerInfo[playerid][pAccount] - money);
	SetPlayerBank(id, PlayerInfo[id][pAccount] + money);

	//Komunikaty:
	new string[256];
	format(string, sizeof string, "Powiadomienie: Na twoje konto wpłyneło %d$ od numeru %d, Nadawca: BANK", money, GetPhoneNumber(playerid));
	SendClientMessage(id, COLOR_YELLOW, string);
	format(string, sizeof string, "Powiadomienie: Z twojego konta został wykonany przelew o wartości %d$ dla numeru %d, Nadawca: BANK", money, phone);
	SendClientMessage(playerid, COLOR_YELLOW, string);
	...
	{
		format(string, sizeof(string), "Gracz %s wysłał przelew do %s w wysokości %d$", GetNick(playerid), GetNick(id), money);
