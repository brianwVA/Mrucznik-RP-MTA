...
timer ListNearItems[3000](playerid)
{
    ...
    foreach(new i : Items)
	{
		...
		switch(Item[i][i_ItemType]) //opisy przedmiotów
		{
			case ITEM_TYPE_WEAPON:
			{
				...
				else if(Item[i][i_ValueSecret] == 0)
					format(oinfo, sizeof oinfo, " (nielegalne)");	
				else if(Item[i][i_ValueSecret] == 2)
					continue;		
			}
		}
	...
	if(strlen(string) < 13) return sendTipMessage(playerid, "Nie znaleziono żadnych przedmiotów w pobliżu.");
	ShowPlayerDialogEx(playerid, D_ITEM_PICKUP, DIALOG_STYLE_TABLIST_HEADERS, "Przedmioty w pobliżu", string, "Podnieś", "Zamknij");
    return 1;
}
