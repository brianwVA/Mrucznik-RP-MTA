...
//Opis:
/*
	Anty-cheat serwera + integracje z zewnętrznymi anty-cheatami.
*/
...
NexACDecodeCode(code)
{
	...
	switch(code)
	{
		...
		case 75:	format(code_decoded, sizeof(code_decoded), "KRP-AC: NoSpread (2)");
		case 76:	format(code_decoded, sizeof(code_decoded), "KRP-AC: TankMode");
...
PlayerCanSpawnWihoutTutorial(playerid)
{
	if(PlayerInfo[playerid][pLevel] == 1 && (PlayerInfo[playerid][pSex] == 0 || PlayerInfo[playerid][pOrigin] == 0 || PlayerInfo[playerid][pAge] == 0)) //anty sobeit spawn
	{
		...
		GetPlayerIp(playerid, ip, sizeof(ip));
		SendMessageToAdmin(
			sprintf("Anti-Cheat: %s [ID: %d] [IP: %s] najprawdopodobniej czituje. | Spawn pomijając tutorial", GetNickEx(playerid), playerid, ip), 
			0xFF00FFFF
		);
		sendErrorMessage(playerid, "Zespawnowałeś się bez wyboru płci/pochodzenia/wieku postaci! Zostałeś wyrzucony z serwera.");
		KickEx(playerid);
		return 0;
	}
	return 1;
...
public CheckCode2003(killerid, playerid)
{
	...
	{
    	MruDialog(killerid, "ACv2: Kod #2003", "Zostałeś wyrzucony za weapon hack.");
		format(string, sizeof string, "ACv2 [#2003]: %s został wyrzucony za weapon hack.", GetNickEx(killerid));
...
forward OznaczCziteraAC(playerid, reason[128]);
public OznaczCziteraAC(playerid, reason[128])
{
	printf("%s[%d] jest podejrzany o %s", GetNick(playerid), playerid, reason);
	SetPVarInt(playerid, "AC_oznaczony_ac", 1);
	if(strfind(reason, "CLEO", true) != -1) PlayerAC[playerid][acCleo]++;
	else if(strfind(reason, "S0beit", true) != -1) PlayerAC[playerid][acSobeit]++;
	else if(strfind(reason, "SampFuncs", true) != -1) PlayerAC[playerid][acSampFuncs]++;
	//else if(strfind(reason, "Vorbis", true) != -1) PlayerAC[playerid][acVorbis]++;
	else if(strfind(reason, "WH", true) != -1) PlayerAC[playerid][acWh]++;
	//else if(strfind(reason, "SilentPatch", true) != -1) PlayerAC[playerid][acSilentPatch]++;
	else if(strfind(reason, "Silent Aim", true) != -1) PlayerAC[playerid][acSilentAim]++;
	MarkPotentialCheater(playerid);
	return 1;
}

forward SendMessageFromNewAC(playerid);
public SendMessageFromNewAC(playerid)
{
	new cheats[256];
	new string[256];
	cheats[0] = '\0';
	if (PlayerAC[playerid][acCleo] > 0) 
    	format(cheats, sizeof cheats, "CLEO [%d/7] ", PlayerAC[playerid][acCleo]);
	if (PlayerAC[playerid][acSobeit] > 0) 
		format(cheats, sizeof cheats, "%sS0beit [%d/3] ", cheats, PlayerAC[playerid][acSobeit]);
	if (PlayerAC[playerid][acSampFuncs] > 0) 
		format(cheats, sizeof cheats, "%sSampFuncs [%d/2] ", cheats, PlayerAC[playerid][acSampFuncs]);
	if (PlayerAC[playerid][acVorbis] > 0) 
		format(cheats, sizeof cheats, "%sVorbisFile.dll ", cheats);
	if (PlayerAC[playerid][acWh] > 0) 
		format(cheats, sizeof cheats, "%sWH ", cheats);
	if (PlayerAC[playerid][acSilentPatch] > 0) 
		format(cheats, sizeof cheats, "%sSilentPatch ", cheats);
	if (PlayerAC[playerid][acSilentAim] > 0) 
		format(cheats, sizeof cheats, "%sSilentAim ", cheats);
	if(strlen(cheats) > 0)
	{
		format(string, sizeof(string), "%s[%d] jest podejrzany o %s", GetNick(playerid), playerid, cheats);
...
forward SetUsingOmp(playerid, set);
public SetUsingOmp(playerid, set)
{
	SetPVarInt(playerid, "IsUsingOmp", set);
}

stock IsUsingOmp(playerid)
{
	return GetPVarInt(playerid, "IsUsingOmp");
}
...
CreatePotentialCheatersTxd()
{
	...
	TextDrawColour(PotentialCheatersTitleTxd, -1);
	...
	for(new i; i<sizeof(PotentialCheatersTxd); i++)
	{
		...
		TextDrawColour(PotentialCheatersTxd[i], COLOR_RED);
	...
	forPlayerArray (top_cheaters => playerid) {
		if(PotentialCheaters[playerid] > 0)
		{
			...
			if(GetPVarInt(playerid, "AC_oznaczony")) {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_PANICRED);
			} else if(GetPVarInt(playerid, "AC_oznaczony_ac")) {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_GOLD);
			} else if(GetPVarInt(playerid, "AC_oznaczony_new")) {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_BLUE);
			} else if(PlayerInfo[playerid][pLevel] == 1) {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_NEWS);
			} else if(PotentialCheaters[playerid] >= 10) {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_RED);
			} else {
				TextDrawColour(PotentialCheatersTxd[i], COLOR_GREY);
			}
			TextDrawBoxColor(PotentialCheatersTxd[i], 50);
...
ACKickMessage(playerid, code)
{
	new ip[16];
	GetPlayerIp(playerid, ip, sizeof(ip));
	SendMessageToAdmin(sprintf("Anti-Cheat: %s [ID: %d] [IP: %s] dostał kicka. | %s [%d]", 
		GetNickEx(playerid), playerid, ip, NexACDecodeCode(code), code), 
		0x9ACD32AA);
	if(IsAProductionServer()) SendClientMessage(playerid, 0x9ACD32AA, sprintf("Anti-Cheat: Dostałeś kicka. | %s [%d]", NexACDecodeCode(code), code));
	if(IsAProductionServer()) SendClientMessage(playerid, 0x9ACD32AA, "Jeżeli uważasz, że antycheat zadziałał nieprawidłowo, zgłoś to administracji, podając kod z jakim otrzymałeś kicka.");
	Log(punishmentLog, WARNING, "%s dostał kicka od antycheata, powód: kod %d", GetPlayerLogName(playerid), code);
}
	...
	|| !Player_CanUseCar(playerid, vehicleid) && PlayerCuffed[playerid] < 1 && !IsAScripter(playerid))
	{
		...
		if(GetPVarInt(playerid, "AntyCheatOff") == 0)
		{
			if(!IsPlayerNPC(playerid))
			{
				...
				MruDialog(playerid, "ACv2: Kod #2001", "Dostałeś kicka za kierowanie samochodem bez wymaganych uprawnień");
				format(string, sizeof string, "ACv2 [#2001]: %s dostał kicka za jazdę bez uprawnień [Veh: %d]", GetNickEx(playerid), GetPlayerVehicleID(playerid));
				SendCommandLogMessage(string);
				Log(warningLog, WARNING, string);
				Log(punishmentLog, WARNING, string);
				KickEx(playerid);
