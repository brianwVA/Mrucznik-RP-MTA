...
ac2_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == DIALOG_AC_PANEL_CHANGE)
    {
        if(response)
        {
            ...
            Log(adminLog, WARNING, "Admin %s ustawił funkcję anty-cheata %s[%d] na %s", 
...
ac_ShowDialog(playerid, page = 1)
{
    ...
    ShowPlayerDialogEx(playerid, DIALOG_AC_PANEL, DIALOG_STYLE_LIST, sprintf("Panel Anty-Cheat'a (%d/%d)", page, pages), string, "Zmień", "Wyjdź");
    return 1;
}
