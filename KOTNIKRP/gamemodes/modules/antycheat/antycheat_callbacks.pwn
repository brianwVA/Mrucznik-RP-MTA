...
{
	#if defined MRP_MTA_RUNTIME
		// MTA posiada wlasny AC. Nex-AC zostaje tylko jako zgodnosc kompilacji PAWN.
		return 1;
	#else
	...
	if(type == 0) //Type of cheating (when 0 it returns the ID, when 1 - IP)
	{
		...
		{
			//kod wyłączony, jeśli wykryto (zapobiega dublowaniu komunikatów o wykryciu kodu nim gracz zostanie skickowany).
			return 1;
		}
		...
		if(code == 3 || code == 2) // Anty tp hack (in veh), często zgłaszany błąd w ac, po lagu kickuje, prawdopodobny fix
		{
		...
		{
			// wyłączony konkretny kod AC lub wszystkie
			return 1;
		}
...
AntiSpawn_Timer()
{
	...
	foreach(new playerid : Player)
	{
		if(gPlayerLogged[playerid] == 0)
		{
			...
			{
				Log(punishmentLog, WARNING, "%s dostał kicka od antycheata, powód: spawn będąc niezalogowanym");
				SendClientMessage(playerid, COLOR_PANICRED, "Zostałeś zkickowany za spawn jako niezalogowany");
				KickEx(playerid);
				continue;
			}
		}
...
hook OnPlayerGiveDamage(playerid, damageid, Float:amount, weaponid, bodypart)
{
	...
	// Oznaczanie graczy, którzy zadali obrażenia graczowi wedle raportowania oprawcy
