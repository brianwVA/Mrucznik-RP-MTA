...
{
	Create3DTextLabel("{adc7e7}Roznosiciel Gazet\nAby zebrać gazetę kliknij klawisz {ffffff}Y", 0xFFFFFFFF, 1870.5071,-1686.9442,13.5430, 40, 0, 0);
	return 1;
}
...
stock GazeciarzOnEnterCheckpoint(playerid)
{
	if(IsGazeciarz[playerid] == 1)
	{
		...
		{
			ShowPlayerDialogEx(playerid, 10101, DIALOG_STYLE_MSGBOX, "NIE BUGUJ!", "Nie buguj checkpointa, praca została zresetowana!", "OK", "");
			IsGazeciarz[playerid] = 0;
			GazeciarzAC[playerid] = 0;
			BlockDeska[playerid] = 0;
		}
...
public GazeciarzGazeta(playerid)
{
	if(IsPlayerInRangeOfPoint(playerid,1.5,1897.3784,-1645.8386,13.5468) || IsPlayerInRangeOfPoint(playerid,1.5,1907.5267,-1631.5620,13.5468) || IsPlayerInRangeOfPoint(playerid,1.5,1977.6581,-1651.5457,13.5463) || IsPlayerInRangeOfPoint(playerid,1.5,1971.7686,-1687.9669,13.5463) || IsPlayerInRangeOfPoint(playerid,1.5,1978.9882,-1695.9998,13.5549))
	{
		...
		new finalPrice = PRICE_GAZECIARZ;
		...
		new string[128];
		format(string, sizeof(string), "Dostarczyles gazete!~n~~g~$%d~n~~w~", finalPrice);
		TextDrawInfoOn(playerid, string, 3000);
		IsGazeciarz[playerid] = 0;
		BlockDeska[playerid] = 0;
...
CMD:dmevent(playerid, params[])
{
	if(PlayerOnedeData[playerid] == false) {
		if(Onede_Status == true) {
			...
			SendClientMessage(playerid, -1, "Arena DeathMatch jest wyłączona.");
	}
	else {
		...
		if(response) {
			switch(listitem) {
				case 0: {
					...
					SendClientMessageToAll(KOLOR_CZERWONY, "Arena DeathMatch została włączona aby dołączyć wpisz /dmevent");
					return 1;
				}
				...
				case 1: {
					...
					SendClientMessageToAll(KOLOR_CZERWONY, "Arena DeathMatch została wyłączona");
					return 1;
				}
			}
		}
