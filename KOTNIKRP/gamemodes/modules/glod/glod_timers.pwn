...
ptask CheckHunger[1000](playerid)
{
	if(gPlayerLogged[playerid])
	{
		...
		{
			_MruGracz(playerid, " Twoja postać zaczyna odczuwać głód, udaj się do restauracji aby coś zjeść!");
		...
		{
			_MruGracz(playerid, " Twoja postać zaczyna odczuwać pragnienie, udaj się do restauracji, aby kupić coś do picia!");
		...
		if(PlayerInfo[playerid][pThirst] >= 96.5 && GetPVarInt(playerid, "bw-cooldown") < gettime() && !CheckBW(playerid)) //bw
		{
			...
			ShowPlayerInfoDialog(playerid, "M-RP", "Twoja postać straciła przytomność z powodu odwodnienia.");
			NadajRanny(playerid, INJURY_TIME_EXCEPTION);
			ProxDetector(15.0, playerid, sprintf("* %s traci przytomność z powodu odwodnienia.", GetNick(playerid)), COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE, COLOR_PURPLE);
		}
