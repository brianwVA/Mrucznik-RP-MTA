...
kuracja_akceptuj(playerid)
{
    ...
    {
        sendErrorMessage(playerid, "Nikt nie oferował Ci kuracji.");
        return 1;
    }

    if(!IsAtHealingPlace(playerid))
    {
        sendErrorMessage(playerid, "Kurację można akceptować tylko w szpitalu / wnętrzu karetki.");
        return 1;
    }

    if(giveplayerid == INVALID_PLAYER_ID || !IsPlayerConnected(giveplayerid))
    {
        SendClientMessage(playerid, COLOR_GREY, "   Gracz, który oferował Ci kurację, wyszedł z gry.");
        return 1;
    }
    ...
    {
        sendErrorMessage(playerid, "Nie stać Cię na kurację!");
        return 1;
    }

    //body
    SendClientMessage(playerid, COLOR_LIGHTBLUE, sprintf("* Akceptowałeś kurację %s, koszt %d$.", DiseaseData[disease][Name], cost));
    SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, sprintf("* Gracz %s akceptował kurację kurację %s, koszt %d$.", GetNick(playerid), DiseaseData[disease][Name], cost));
    SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, "* Nie odchodź od gracza i wykorzystaj czas kuracji na wykonywanie akcji RP.");

    ZabierzKaseDone(playerid, cost);
    DajKaseDone(giveplayerid, commission);
    Sejf_Add(FRAC_ERS, cost-commission);
    Log(payLog, WARNING, "%s zapłacił %s $%d (prowizja $%d) za kurację choroby %s", 
        GetPlayerLogName(playerid), GetPlayerLogName(giveplayerid), cost, commission, DiseaseData[disease][Name]
    );

    ChatMe(giveplayerid, sprintf("podłącza %s do aparatury i rozpoczyna kurację.", GetNick(playerid)));
...
command_kuracja_Impl(playerid, giveplayerid, disease[], money)
{
    ...
    if(!IsAtHealingPlace(playerid))
    {
        sendErrorMessage(playerid, "Kurację można oferować tylko w szpitalu / wnętrzu karetki.");
        return 1;
    }
    ...
    if(money < 0) 
    {
        sendErrorMessage(playerid, "Prowizja musi być większa niż 0.");
        return 1;
    }

    if(IsPlayerTreated(giveplayerid)) 
    {
        sendErrorMessage(playerid, "Ten gracz ma już przeprowadzaną kurację.");
        return 1;
    }
    ...
    SendClientMessage(playerid, COLOR_LIGHTBLUE, sprintf("* Oferujesz %s %s kurację %s za $%d$ (prowizja %d$).", 
        GetNick(giveplayerid), 
        DiseaseData[diseaseID][CureTime] >= 60 ? sprintf("%d minutową", DiseaseData[diseaseID][CureTime]/60) : sprintf("%d sekundową", DiseaseData[diseaseID][CureTime]), 
        disease, cost, money
    ));
    SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, sprintf("* %s oferuje Ci kurację choroby %s.", GetNick(playerid), disease));
    SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, sprintf("* Kuracja potrwa %s i będzie kosztować %d$.", 
    ...
    SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, sprintf("* Szanse powodzenia kuracji to %d procent. Wpisz /akceptuj kuracja aby się zgodzić.", chance));
