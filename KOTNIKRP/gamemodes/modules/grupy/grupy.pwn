...
GetGroupBusinessPerm(groupid)
{
	if(!IsValidGroup(groupid)) return 0;
	if(GroupHavePerm(groupid, PERM_RESTAURANT)) return PERM_RESTAURANT;
	if(GroupHavePerm(groupid, PERM_CLUB)) return PERM_CLUB;
	if(GroupHavePerm(groupid, PERM_GUNDEALER)) return PERM_GUNDEALER;
	if(GroupHavePerm(groupid, PERM_LEGALGUNDEALER)) return PERM_LEGALGUNDEALER;
	return 0;
}
...
GroupSendAnnouncement(playerid, groupid, msg[])
{
	...
	new bizPerm = GetGroupBusinessPerm(groupid);
	if(bizPerm != 0 && BusinessTypeCooldown[bizPerm] >= 1)
	{
		format(string, sizeof(string), "Twoja grupa musi odczekać jeszcze %d minut przed wysłaniem ogłoszenia dla tego typu biznesu", BusinessTypeCooldown[bizPerm]);
	...
	if(strfind(msg, content, false) == -1)
	{
		...
		BusinessTypeCooldown[bizPerm] = 30;
		BusinessTypeKomunikatTime[bizPerm] = SetTimerEx("BusinessTypeKomunikatCzas", 60000, true, "i", bizPerm);
		return 1;
	}

	SendClientMessage(playerid, -1, " "); 
	sendTipMessageEx(playerid, COLOR_WHITE, "Wysłałeś ogłoszenie o tej samej treści w czasie mniejszym jak {AC3737}5 minut!");
	sendTipMessageEx(playerid, COLOR_WHITE, "{C0C0C0}Zostajesz ukarany karą Anty-Spam na {AC3737}60 minut");
	komunikatTime[playerid] = SetTimerEx("KomunikatCzas", 60000, true, "i", playerid);		
	PlayerInfo[playerid][pBlokadaPisaniaFrakcjaCzas] = 60;
	format(string, sizeof(string), "[ANTY_SPAM] %s otrzymał blokadę na 60 minut za wysłanie 2x tego samego komunikatu!", GetNickEx(playerid));
...
GroupShowInfo(playerid, groupid, admin = 0)
{
	...
	if(GroupIsVLeader(playerid, groupid) || admin || GroupHavePerm(groupid, PERM_RESTAURANT)) format(string, sizeof(string), "%s{FFFFFF}Kasa w sejfie: {BA782D}%d{FFFFFF}$\n", string, GroupInfo[groupid][g_Money]);
	...
	format(string, sizeof(string), "%s{FFFFFF}Główny lider: %s", string, nick);
	strcat(string, "\n{FFFFFF}Uprawnienia: ");
...
GroupShowLeaderPanel(playerid, group, type, bool:asadmin = false, page = 1)
{
	...
	switch(type)
	{
		...
		{
			new options[256];
			strcat(options, "»» Zarządzanie rangami\n");
			strcat(options, "»» Zarządzaj pojazdami\n");
			strcat(options, "»» Zarządzaj pracownikami\n");
			strcat(options, "»» Zarządzaj liderami\n");
			strcat(options, "»» Zmień kolor grupy\n");
			strcat(options, "»» Zmień nazwę grupy\n");
			strcat(options, "»» Zmień skrót\n");
			strcat(options, "»» Sprawdź uprawnienia grupy\n");
			if(GroupHavePerm(group, PERM_RESTAURANT))
				strcat(options, "»» Zarządzaj restauracją\n");
		...
		case 3: //Employees
		{
			...
			mysql_format(Database, query, sizeof(query), "SELECT COUNT(*) FROM `mru_konta` WHERE `Grupa1` = '%d' OR `Grupa2` = '%d' OR `Grupa3` = '%d'", GroupInfo[group][g_UID], GroupInfo[group][g_UID], GroupInfo[group][g_UID]);
			result = mysql_query(Database, query);
			new ilosc;
			cache_get_value_index_int(0, 0, ilosc);
			cache_delete(result);

			if(ilosc == 0)
				return sendErrorMessage(playerid, "Ta grupa nie ma żadnych pracowników.");
			...
			inline OnEmployeesLoaded()
			{
				for(new i = 0; i < cache_num_rows(); i++)
				{
					new uid, connected, nick[24], pgroup[3], pgrouprank[3], slot;
					//p<|>dds[24]dddddd
					cache_get_value_index_int(i, 0, uid), cache_get_value_index_int(i, 1, connected), cache_get_value_index(i, 2, nick);
					cache_get_value_index_int(i, 3, pgroup[0]), cache_get_value_index_int(i, 4, pgroup[1]), cache_get_value_index_int(i, 5, pgroup[2]);
					cache_get_value_index_int(i, 6, pgrouprank[0]), cache_get_value_index_int(i, 7, pgrouprank[1]), cache_get_value_index_int(i, 8, pgrouprank[2]);
					//if(uid == PlayerInfo[playerid][pUID]) continue;
					for(new j = 0; j < 3; j++)
					{
						if(pgroup[j] == GroupInfo[group][g_UID])
						{
							slot = j;
						}
					}
				...
				if( ilosc > (page*20) )
				{
					format(employees, sizeof(employees), "%s\n\n{888888}Następna strona >>>", employees);
					DynamicGui_AddRow(playerid, -3000);
				}
				SetPVarInt(playerid, "group-panel", group+1);
				DynamicGui_SetDialogValue(playerid, page);
				ShowPlayerDialogEx(playerid, D_GROUPLEADEREMPLOYEES, DIALOG_STYLE_TABLIST_HEADERS, header, employees, "Zarządzaj", "Zamknij");
			}
			mysql_format(Database, query, sizeof(query), "SELECT `UID`, `connected`, `Nick`, `Grupa1`, `Grupa2`, `Grupa3`, `Grupa1Rank`, `Grupa2Rank`, `Grupa3Rank` FROM `mru_konta` WHERE `Grupa1` = '%d' OR `Grupa2` = '%d' OR `Grupa3` = '%d' LIMIT %d,%d", GroupInfo[group][g_UID], GroupInfo[group][g_UID], GroupInfo[group][g_UID], offset, 20);
			MySQL_TQueryInline(Database, using inline OnEmployeesLoaded, query);
		}
		case 4: //Vehicle
		{
			new string[1024];
            ...
            ShowPlayerDialogEx(playerid, D_GROUPLEADERVEHICLE, DIALOG_STYLE_TABLIST_HEADERS, MruTitle("Zarządzanie pojazdami"), string, "Zarządzaj", "Cofnij");
		}
		case 5: //VLeader
		{
			...
			SetPVarInt(playerid, "group-panel", group+1);
			ShowPlayerDialogEx(playerid, D_GROUP_MANAGE_LEADERS, DIALOG_STYLE_LIST, sprintf("Zarządzanie liderami w %s", GroupInfo[group][g_ShortName]), string, "Zarządzaj", "Cofnij");
		}
	}
	return 1;
}
...
GroupAddVLeader(groupid, uid, playerid = -1, info = 1)
{
	...
	if(info && playerid != -1)
	{
		foreach(new i : Player)
		{
			...
			{
				va_SendClientMessage(i, COLOR_LIGHTBLUE, "(Zostałeś mianowany v-liderem w grupie %s przez %s)", GroupInfo[groupid][g_Name], GetNick(playerid));
				return 1;
			}
		}
	}
...
GroupRemovePlayer(playerid, groupid, kicked, kickedby = INVALID_PLAYER_ID)
{
	...
	if(GroupIsVLeader(playerid, groupid, 0)) //Usuń v-lidera
	{
		GroupRemoveVLeader(groupid, PlayerInfo[playerid][pUID]);
	}
	if(kicked == 1)
	{
		va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "(Zostałeś wyrzucony z grupy %s przez %s)", GroupInfo[groupid][g_Name], (IsPlayerConnected(kickedby)) ? (GetNick(kickedby)) : ("SYSTEM"));
		if(IsPlayerConnected(kickedby))
		{
			va_SendClientMessage(kickedby, COLOR_LIGHTBLUE, "(Wyrzuciłeś %s z grupy %s)", GetNick(playerid), GroupInfo[groupid][g_Name]);
		}
		Log(serverLog, WARNING, "%s został wyrzucony z grupy %s przez %s", GetPlayerLogName(playerid), GetGroupLogName(groupid), (IsPlayerConnected(kickedby)) ? (GetPlayerLogName(kickedby)) : ("SYSTEM"));
	}
	else
	{
		va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "Opuściłeś grupę %s.", GroupInfo[groupid][g_Name]);
		SendLeaderRadioMessage(groupid, COLOR_LIGHTGREEN, sprintf("> %s opuscił grupę %s", GetNickEx(playerid), GroupInfo[groupid][g_Name]));
		Log(serverLog, WARNING, "%s opuścił grupę %s", GetPlayerLogName(playerid), GetGroupLogName(groupid));
	}
	return 1;
}
...
GroupAddPlayer(playerid, groupid, rank = 0, invitedby = INVALID_PLAYER_ID)
{
	...
	{
		va_SendClientMessage(invitedby, COLOR_LIGHTBLUE, "(Przyjąłeś %s do swojej grupy %s, ranga: %s)", GetNick(playerid), GroupInfo[groupid][g_Name], GroupRanks[groupid][rank]);
        va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "(Lider %s przyjął Cię do grupy %s, ranga: %s)", GetNick(invitedby), GroupInfo[groupid][g_Name], GroupRanks[groupid][rank]);
		Log(serverLog, WARNING, "%s przyjął %s do grupy %s", GetPlayerLogName(invitedby), GetPlayerLogName(playerid), GetGroupLogName(groupid));
	}
	else
	{
		va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "(Zostałeś przyjęty do grupy %s, ranga: %s)", GroupInfo[groupid][g_Name], GroupRanks[groupid][rank]);
	}
	return 1;
}
...
GroupSendMessageOOC(playerid, group, params[], bool:fromchat = false)
{
	...
	{
		new allowedSlots = IsPlayerPremiumOld(playerid) ? MAX_PLAYER_GROUPS_PREMIUM : MAX_PLAYER_GROUPS;
		if(group < 0 || group > allowedSlots)
		{
			return sendTipMessage(playerid, "Użyj (/ro)oc[slot] [tekst]");
		}
		if(isnull(params))
		{
			new help[64];
			format(help, sizeof(help), "Użyj %s%d [tekst]", (fromchat) ? ("@") : ("/ro"), group);
			sendTipMessage(playerid, help);
			return 1;
		}
		if(gRO[playerid] == 1) return sendTipMessage(playerid, "Nie możesz pisać na (/ro), zablokowałeś ten czat (Użyj /togro)");
		...
		{
			sendTipMessage(playerid, "Pod tym slotem nie masz żadnej grupy!");
			return 1;
		}
	}
	return 1;
...
GroupSendMessageRadio(playerid, group, params[], bool:fromchat = false)
{
	if(IsPlayerConnected(playerid))
    {
		...
		if(group < 0 || group > allowedSlots)
		{
			return sendTipMessage(playerid, "Użyj (/r)adio[slot] [tekst]");
		}
		if(isnull(params))
		{
			new help[64];
			format(help, sizeof(help), "Użyj %s%d [tekst]", (fromchat) ? ("!") : ("/r"), group);
			sendTipMessage(playerid, help);
			return 1;
		}
		if(PlayerInfo[playerid][pMuted] == 1)
		{
			sendTipMessageEx(playerid, TEAM_CYAN_COLOR, "Nie możesz pisać ponieważ jesteś wyciszony");
			return 1;
		}
		if(GetPlayerAdminDutyStatus(playerid) == 1)
		{
			new help[128];
			format(help, sizeof(help), "Dobry admin nie powinien robić OOC w IC! Pisz poprzez /ro%d [treść]", group);
...
CreateGroupTextDraws(playerid)
{
	...
	PlayerTextDrawColour(playerid, SystemGrup[playerid], -1);
	...
	for(new i=0;i<allowedSlots;i++) {
		...
		PlayerTextDrawColour(playerid, NazwaGrupy[playerid][i], -1);
	...
	for(new i=0;i<allowedSlots;i++) {
		...
		PlayerTextDrawColour(playerid, DutyGrupy[playerid][i], -1);
	...
	for(new i=0;i<allowedSlots;i++) {
		...
		PlayerTextDrawColour(playerid, InfoGrupy[playerid][i], -1);
	...
	for(new i=0;i<allowedSlots;i++) {
		...
		PlayerTextDrawColour(playerid, PojazdyGrupy[playerid][i], -1);
	...
	for(new i=0;i<allowedSlots;i++) {
		...
		PlayerTextDrawColour(playerid, OnlineGrupy[playerid][i], -1);
...
DestroyGroupTextDraws(playerid)
{
	...
	new allowedSlots = IsPlayerPremiumOld(playerid) ? MAX_PLAYER_GROUPS_PREMIUM : MAX_PLAYER_GROUPS;
	for(new i=0;i<allowedSlots;i++) PlayerTextDrawDestroy(playerid, NazwaGrupy[playerid][i]);
	for(new i=0;i<allowedSlots;i++) PlayerTextDrawDestroy(playerid, DutyGrupy[playerid][i]);
	for(new i=0;i<allowedSlots;i++) PlayerTextDrawDestroy(playerid, InfoGrupy[playerid][i]);
	for(new i=0;i<allowedSlots;i++) PlayerTextDrawDestroy(playerid, PojazdyGrupy[playerid][i]);
	for(new i=0;i<allowedSlots;i++) PlayerTextDrawDestroy(playerid, OnlineGrupy[playerid][i]);
}
...
GroupCanPlayerJoin(playerid, groupid)
{
	for(new i = 0; i < MAX_PLAYER_GROUPS_PREMIUM; i++)
	{
		...
		if(pgrp > 0 && (GroupHavePerm(pgrp, PERM_POLICE) || GroupHavePerm(pgrp, PERM_MEDIC)))
		{
			if(GroupHavePerm(groupid, PERM_GANG) || GroupHavePerm(groupid, PERM_CRIMINAL) || GroupHavePerm(groupid, PERM_MAFIA))
			{
				sendErrorMessage(playerid, "Nie możesz dołączyć do grupy przestępczej będąc w grupie porzadkowej.");
				return 0;
			}
			break;
		}
	...
	{
		new pgrp = PlayerInfo[playerid][pGrupa][i];
		if(pgrp > 0 && (GroupHavePerm(pgrp, PERM_GANG) || GroupHavePerm(pgrp, PERM_CRIMINAL) || GroupHavePerm(pgrp, PERM_MAFIA)))
		{
			if(GroupHavePerm(groupid, PERM_GANG) || GroupHavePerm(groupid, PERM_CRIMINAL) || GroupHavePerm(groupid, PERM_MAFIA))
			{
				sendErrorMessage(playerid, "Nie możesz być w 2 grupach przestepczych.");
				return 0;
			}
			break;
		}
	...
	mysql_format(Database, query, sizeof(query), 
		...
		GroupInfo[groupid][g_UID], PlayerInfo[playerid][pUID]);
	new Cache:result = mysql_query(Database, query);
	new banned = 0;
	cache_get_value_index_int(0, 0, banned);
	cache_delete(result);
	
	if(banned > 0) return 0; //bana ma frajer!
...
stock Restaurant_CanBuyProduct(playerid, groupid, price)
{
	if(GroupIsLeader(playerid, groupid) || GroupIsVLeader(playerid, groupid)) return 1;
	if(PlayerInfo[playerid][pGroupMoneySpent] + price >= D_RESTAURANT_HOUR_LIMIT && (PlayerInfo[playerid][pGroupMoneySpentTime] == 0 || PlayerInfo[playerid][pGroupMoneySpentTime]+3600 > gettime()))
		return 0;
	return 1;
}
...
stock Admin_PermPanel(playerid, groupid, info = 0)
{
	...
	ShowPlayerDialogEx(playerid, D_GROUP_ADMIN_PERM, DIALOG_STYLE_LIST, MruTitle("Zmiana uprawnień grupy"), string, "Zmień", "Zapisz");
