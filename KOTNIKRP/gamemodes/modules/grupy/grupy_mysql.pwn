...
stock GroupsLoad()
{
	new lID, count, Cache:result;
	//Default fix
	mysql_query(Database, "UPDATE `mru_groups` SET `vLeader` = '0' WHERE `vLeader` = ''");
	mysql_query(Database, "UPDATE `mru_groups` SET `Ranks` = '-' WHERE `Ranks` = ''");
	mysql_query(Database, "UPDATE `mru_groups` SET `Skins` = '0' WHERE `Skins` = ''");
	mysql_query(Database, "UPDATE `mru_groups` SET `Flags` = '0' WHERE `Flags` = ''");
	//
    result = mysql_query(Database, "SELECT * FROM `mru_groups` WHERE 1");
    for(new i = 0; i < cache_num_rows(); i++)
    {
		new rangi[256], ranga[MAX_RANG][MAX_RANG_LEN], skiny[256], vliderzy[32], flagi[256];
		cache_get_value_index_int(i, 0, lID);
    	cache_get_value_index_int(i, 0,         GroupInfo[lID][g_UID]);
		cache_get_value_index(i, 1,             GroupInfo[lID][g_Name]);
		cache_get_value_index(i, 2,             GroupInfo[lID][g_ShortName]);
		cache_get_value_index_int(i, 3,                 GroupInfo[lID][g_Color]);
		cache_get_value_index_float(i, 4,       GroupInfo[lID][g_Spawn][0]);
		cache_get_value_index_float(i, 5,       GroupInfo[lID][g_Spawn][1]);
		cache_get_value_index_float(i, 6,       GroupInfo[lID][g_Spawn][2]);
		cache_get_value_index_float(i, 7,       GroupInfo[lID][g_Spawn][3]);
		cache_get_value_index_int(i, 8,         GroupInfo[lID][g_Int]);
		cache_get_value_index_int(i, 9,         GroupInfo[lID][g_VW]);
		cache_get_value_index(i, 10,            flagi);
		cache_get_value_index(i, 11,            rangi);
		cache_get_value_index_int(i, 12,                GroupInfo[lID][g_Leader]);
		cache_get_value_index(i, 13,            vliderzy);
		cache_get_value_index_int(i, 14,                GroupInfo[lID][g_Money]);
		cache_get_value_index_int(i, 15,                GroupInfo[lID][g_Mats]);
		cache_get_value_index(i, 16,            skiny);
		cache_get_value_index_int(i, 17,                GroupInfo[lID][g_MaxDuty]);
        ...
        {
			if(strlen(ranga[j]) > 1) format(GroupRanks[lID][j], 25, "%s", ranga[j]);
			else format(GroupRanks[lID][j], 25, "-");
		}
...
stock GroupSave(lID, bool:savehalf = false)
{
	...
	mysql_escape_string(GroupInfo[lID][g_Name], name_escaped);
	mysql_escape_string(GroupInfo[lID][g_ShortName], shortname_escaped);
	...
	if(lQuery[0]) mysql_query(Database, lQuery);
	if(savehalf == false)
	{
		//Rangi - Save
	...
	printf("[GRUPY]: Zapisano grupę UID %d [%s] ", GroupInfo[lID][g_UID], name_escaped);
	return 1;
}
...
stock GroupDelete(groupid)
{
	...
	foreach(new i : Player)
	{
		...
		if(IsPlayerInGroup(i, groupid))
		{
			...
			sendErrorMessage(i, "Zostałeś wyrzucony z grupy, ponieważ została ona usunięta!");
		}
	}
	for(new i = 0; i < 3; i++)
	{
		mysql_tquery_format("UPDATE `mru_konta` SET `Grupa%d` = '0', `Grupa%dRank` = '0', `Grupa%dSkin` = '0' WHERE `Grupa%d` = '%d'", i+1, i+1, i+1, i+1, groupid);
	}
	//
	mysql_tquery_format("DELETE FROM `mru_groups` WHERE `UID` = '%d'", groupid);
...
stock GroupCreate(name[], Float:x, Float:y, Float:z, Float:a, interior, VW, color = 1, shortname[] = "")
{
	...
	mysql_escape_string(name, name_escaped);
	mysql_escape_string(short, shortname_escaped);
	...
	if(mysql_query(Database, lQuery)) 
	{
		...
		printf("[GRUPY]: Stworzono grupę UID %d [%s] ", GroupInfo[id][g_UID], name_escaped);
		return id;
	}
	else return -2;
}
...
{
	if(!IsValidGroup(groupid)) return 0;
	new count = 0, Cache:result;
	result = mysql_query_format("SELECT `UID` FROM `mru_konta` WHERE `Grupa1` = '%d' OR `Grupa2` = '%d' OR `Grupa3` = '%d'", groupid, groupid, groupid);
