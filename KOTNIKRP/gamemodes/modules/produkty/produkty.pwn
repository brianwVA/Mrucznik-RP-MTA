...
stock product_Add(name[], org, price, value1, value2, itemtype, quant)
{
	...
	if(!mysql_query_format("INSERT INTO `mru_products` (`orgID`, `product_name`, `price`, `value1`, `value2`, `item_type`, `quant`) VALUES ('%d', '%s', '%d', '%d', '%d', '%d', '%d')", org, name, price, value1, value2, itemtype, quant))
		return -1;
	Product[id][p_UID] = cache_insert_id();
...
{
	if(frommysql) mysql_tquery_format("DELETE FROM `mru_products` WHERE `UID` = %d", Product[pID][p_UID]);
...
{
	mysql_tquery_format("UPDATE `mru_products` SET `orgID` = '%d', \
	`product_name` = '%s', \
	`price` = '%d', \
	`value1` = '%d', \
	`value2` = '%d', \
...
stock produkt_akceptuj(playerid)
{
	...
	if(!IsPlayerConnected(offer) || !Product[product][p_UID] || Product[product][p_Price] < 0) 
	{
		...
		return sendErrorMessage(playerid, "Gracz oferujący wyszedł z gry/produkt przestał istnieć. Oferta anulowana!");
	}
	new quant = GetPVarInt(offer, "selected-product-quant");
	if(kaska[playerid] < Product[product][p_Price]*quant)
	{
		va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Nie stać Cię na przedmiot %s x%d za cenę $%d", Product[product][p_ProductName], quant, Product[product][p_Price]*quant);
		return 1;
	}
	...
	va_SendClientMessage(playerid, COLOR_GREY, "* Kupiłeś produkt %s x%d od %s [%d] za %d$!", Product[product][p_ProductName], quant, GetNick(offer), offer, Product[product][p_Price]*quant);
	va_SendClientMessage(offer, COLOR_GREY, "* Gracz %s [%d] zaakceptował twoją oferte. Otrzymujesz %d$!", GetNick(playerid), playerid, (Product[product][p_Price]*quant)/2);
