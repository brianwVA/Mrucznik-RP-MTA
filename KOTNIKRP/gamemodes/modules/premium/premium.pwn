...
premium_ConvertToNewSystem(playerid)
{
	if(PlayerInfo[playerid][pDonateRank] != 0)
	{
		...
		_MruGracz(playerid, "Uwaga! Twoje premium zostało przeniesione na nowy system!");
		_MruGracz(playerid, "Otrzymałeś Konto Premium na 3 miesiące i dodatkowe 500 KC do wykorzystania.");

		Log(premiumLog, WARNING, "%s otrzymał konwersje KP %d na nowy system", GetPlayerLogName(playerid), PlayerInfo[playerid][pDonateRank]);
...
premium_loadForPlayer(playerid)
{
	...
	if(kpActive)
	{
		...
		{
			sendErrorMessage(playerid, "Twoje konto premium wygasło!");
			PremiumInfo[playerid][pKP] = 0;
			ZabierzKP(playerid);
		}
		else
		{
			...
			{
				_MruAdmin(playerid, "Jesteś posiadaczem konta premium! :)");
			}
...
{
	return _MruGracz(playerid, sprintf("Aktualnie na Twoim koncie znajduje się %d MrucznikCoins.", PremiumInfo[playerid][pMC]));
}
...
ZabierzKP(playerid)
{
	...
	{
		Log(premiumLog, WARNING, "%s zabrano KP", GetPlayerLogName(playerid));
		PremiumInfo[playerid][pKP] = 0;
...
DajKP(playerid, time, bool:msg=true)
{
	if(IsPlayerConnected(playerid))
    {
		Log(premiumLog, WARNING, "%s nadano KP na %d", GetPlayerLogName(playerid), time);
        ...
        if(expirationTime > 0 && time != 0)
        {
        	...
        	{
	            _MruAdmin(playerid, 
					sprintf("Otrzymałeś Konto Premium. Wygasa ono za %d dni i %d godzin.", 
						floatround(floatdiv(expirationTime, 86400), floatround_floor), 
						floatround(floatdiv(expirationTime, 3600), floatround_floor)%24
					)
				);
        ...
        {
            _MruAdmin(playerid, "Otrzymałeś konto Premium na czas nieokreślony.");
            PremiumInfo[playerid][pKP] = 1;
        }
    }
}
...
DajMC(playerid, mc)
{
	Log(premiumLog, WARNING, "%s nadano %dKC, poprzedni stan: %dKC", GetPlayerLogName(playerid), mc, PremiumInfo[playerid][pMC]);
	if(mc <= 0)
	{
	...
	new query[256];
	format(query, sizeof(query), "UPDATE `mybb_users` SET `samp_kc`=`samp_kc`+'%d' WHERE `uid` = '%d'", mc, PlayerInfo[playerid][pGID]);
...
{
	Log(premiumLog, WARNING, "%s zabrano %dKC, poprzedni stan: %dKC", GetPlayerLogName(playerid), mc, PremiumInfo[playerid][pMC]);
	if(mc <= 0)
	{
...
KupKP(playerid)
{
	...
	SendClientMessage(playerid, COLOR_LIGHTGREEN, "Gratulację! Zakupiłeś konto premium. Od teraz masz dostęp do możliwości premium. Dziękujemy za wspieranie serwera!"); 
}
...
PrzedluzKP(playerid)
{
	...
	SendClientMessage(playerid, COLOR_LIGHTGREEN, "Konto premium przedłużone :D!"); 
}
...
KupPojazdPremium(playerid, id)
{
	...
	{
		sendErrorMessage(playerid, "Nie stać Cię na ten pojazd");
...
KupPrzedmiotPremium(playerid, id)
{
	...
	{
		sendErrorMessage(playerid, "Nie stać Cię na ten przedmiot!");
		return DialogPrzedmioty(playerid);
	}

	ZabierzMC(playerid, PrzedmiotyPremium[id][Cena]);

	Log(premiumLog, WARNING, "%s kupił unikatowy przedmiot %d za %dKC",
	...
	_MruAdmin(playerid, sprintf("Gratulujemy dobrego wyboru. Kupiłeś przedmiot o ID %d za %d KC.", PrzedmiotyPremium[id][Model], PrzedmiotyPremium[id][Cena]));
	_MruAdmin(playerid, "Listę swoich przedmiotów premium znajdziesz pod komendą /dodatki");
...
KupSlotPojazdu(playerid)
{
	...
	{
		sendErrorMessage(playerid, "Nie stać Cię na zakup dodatkowego slotu.");
		return DialogSlotyPojazdu(playerid);
	}

	if(MRP_GetPlayerCarSlots(playerid) >= 10)
	{
		sendErrorMessage(playerid, "Masz już maksymalną ilość slotów.");
	...
	Log(premiumLog, WARNING, "%s kupił slot wozu za "#CAR_SLOT_CENA"KC",
		GetPlayerLogName(playerid));
	_MruAdmin(playerid, sprintf("Kupiłeś sobie slot na auto za %d KC. Masz teraz %d slotów.", CAR_SLOT_CENA, MRP_GetPlayerCarSlots(playerid)));
...
KupZmianeNicku(playerid)
{
	...
	{
		sendErrorMessage(playerid, "Nie stać Cię na zakup dodatkowej zmiany nicku");
	...
	Log(premiumLog, WARNING, "%s kupil zmiane nicku za "#ZMIANA_NICKU_CENA"KC",
		GetPlayerLogName(playerid));
	_MruAdmin(playerid, sprintf("Kupiłeś sobie zmianę nicku za %d KC. Masz teraz %d zmian nicku.", ZMIANA_NICKU_CENA, MRP_GetPlayerNickChanges(playerid)));
...
KupSkinPremium(playerid, id)
{
	...
	{
		sendErrorMessage(playerid, "Nie stać Cię na ten skin");
		return DialogSkiny(playerid);
	}

	MruMySQL_InsertSkin(playerid, id);

	Log(premiumLog, WARNING, "%s kupił unikatowy skin %d za %dKC",
		GetPlayerLogName(playerid), 
	...
	_MruAdmin(playerid, sprintf("Gratulujemy dobrego wyboru. Kupiłeś skin o ID %d za %d KC.", SkinyPremium[id][Model], SkinyPremium[id][Cena]));
	_MruAdmin(playerid, "Listę swoich skinów premium znajdziesz pod komendą /skiny");
...
KupNumerTelefonu(playerid, string:_numer[])
{
	...
	if(MruMySQL_IsPhoneNumberAvailable(numer))
	{
		...
		{
			sendErrorMessage(playerid, "Nie stać Cię na ten numer telefonu");
		...
		Log(premiumLog, WARNING, "%s kupil numer telefonu %d za %dKC.",
	...
	{
		sendErrorMessage(playerid, "Ten numer jest już zajęty!");
		return DialogTelefon(playerid);
	}
