...
MruMySQL_CookedMealsDialog(playerid)
{
	...
	MySQL_TQueryInline(Database, using inline OnMealsLoaded, sprintf("SELECT id, model, name, weight, type FROM mru_player_cooking WHERE owner='%d' ORDER BY id DESC LIMIT 50", 
...
{
	mysql_query(Database, sprintf("INSERT INTO mru_player_cooking (owner, model, name, weight, type) VALUES ('%d', '%d', '%s', '%d', '%d')",
...
MruMySQL_EatCookedMeal(playerid, id)
{
	...
	inline OnMealLoaded()
	{
		...
		mysql_tquery(Database, sprintf("DELETE FROM mru_player_cooking WHERE id='%d'", id));
	}

	MySQL_TQueryInline(Database, using inline OnMealLoaded, sprintf("SELECT name, weight, type FROM mru_player_cooking WHERE id='%d'", id));
	return;
}
