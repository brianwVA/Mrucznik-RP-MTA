...
EatCookedMeal(playerid, name[], weight, type)
{
	...
	{//Dolphin or Turtle
		...
		SetPlayerCriminal(playerid,INVALID_PLAYER_ID, "Spożywanie zagrożonych gatunków");
	}
	if(type < sizeof(FishNames))
	{
		IncreasePlayerImmunity(playerid, 1);
		SendClientMessage(playerid, COLOR_LIGHTBLUE, sprintf("* Zjadłeś: %s o wadze %dg i dostałeś +%dhp i 1pkt odporności.", name, weight, weight/10));
	}
	else
	{
		SendClientMessage(playerid, COLOR_LIGHTBLUE, sprintf("* Zjadłeś: %s o wadze %dg i dostałeś +%dhp.", name, weight, weight/10));
	}
	...
	{//1% szans na zatrucie
		SendClientMessage(playerid, COLOR_RED, "To co zjadłeś, chyba Ci zaszkodziło!");
	}
}
