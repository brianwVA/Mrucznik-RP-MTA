...
//Opis:
/*
	Moduł łączący gamemode z Mrucznikowym Discordem.
*/
...
// Stary samp-discord-connector nie jest bezpieczny w warstwie mta-amx (jego
// wątek kończy proces MTA po starcie gamemode). Integracja Discord jest
// wyłączona wyłącznie w buildzie MTA; komunikacja w grze nadal działa.
LoadDiscordChannels() { return 1; }
DiscordConnectInit() { return 1; }
...
YCMD:reloaddiscord(playerid, params[], help)
{
	#pragma unused params, help
	if(PlayerInfo[playerid][pAdmin] >= 1000 || IsAScripter(playerid))
		return sendTipMessage(playerid, "Integracja Discord jest wyłączona w wydaniu MTA.");
...
LoadDiscordChannels()
{
    new type, org_id, channel_id[64], Cache:result;
    result = mysql_query(Database, "SELECT `type`, `org_id`, `channel_id` FROM `mru_discord` ORDER BY `id` ASC");

    for(new i = 0; i < cache_num_rows(); i++)
    {
		//dds[64]
		cache_get_value_index_int(i, 0, type), cache_get_value_index_int(i, 1, org_id), cache_get_value_index(i, 2, channel_id);
...
SendDiscordMessage(channel, message[])
{
	if(g_OtherChannel[channel] == 0) return 1;
	new dest[512];
...
SendDiscordLogMessage(fractionid, message[])
{
	if(g_LogChannel[fractionid] == 0) return 1;
	new dest[512];
	utf8encode(dest, message);
...
SendDiscordGroupMessage(fractionid, message[])
{
	if(g_GroupChannel[fractionid] == 0) return 1;
	new dest[512];
	utf8encode(dest, message);
...
public DCC_OnMessageCreate(DCC_Message:message)
{
	...
	if(strcmp(content, "mrucznik", true) == 0)
	{
		new count = 0;
		...
		new str[128];
		if(count > 0)
			format(str, sizeof str, "Na mruczniku gra aktualnie %d %s!", count, (count > 1 ? "graczy" : "gracz"));
		else
			format(str, sizeof str, "Na mruczniku aktualnie nikt nie gra!");
