...
MRP_ShowAnim(playerid)
{
	...
	ShowPlayerDialogEx(playerid, D_ANIMLIST, 2, "{4876FF}M-RP » {FFFFFF}Animacje", dialog, "Wybierz", "Wyjdź");
	return 1;
}

MRP_DoAnimation(playerid, arg1[])
{
	if(MaPaczke[playerid] == 1 || PodnoszeniePaczki[playerid] == 1) return 0;
	new var0 = 0;
	...
	{
		//4.1, 0, 1, 1, 1, 1, 1
		ApplyAnimation(playerid, "BEACH", "ParkSit_W_loop", 4.1, 0, 1, 1, 1, 1, 1);
		SetPVarInt(playerid, "anim_do", 1);
		return 1;
	}
	else if(strcmp("@rolki", arg1, true) == 0)
	{
		SendClientMessage(playerid, -1, "Ta animacja została wyłączzona na czas tworzenia systemu przedmiotów!");
		return 1;
	}
