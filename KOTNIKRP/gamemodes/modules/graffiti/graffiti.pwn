...
/*
	...
	/graffiti stworz (na strefie przejętej/nieprzejętej - na SC & tylko dla gangów - na koniec do dodania) tworzy dialog
	...
	Następnie od razu te graffiti się usuwa i tworzy od nowa poleceniem graffiti_Load(id) aby zaaplikować "\n" w tekście.
	To samo z edycją graffiti, po edycji wszystko zapisujemy oraz usuwamy + tworzymy od nowa funkcją load_mysql.

	Admin ma dostęp do komendy(1@lvl>) /graffiti sprawdz - wysyła mu ID najblizszego graffiti (inRange 200-300)
	Pozniej ma dostep do /graffiti edytuj <ID_graffiti> w którym otwiera sie dialog taki sam jak u gracza
	czyli: mozliwosc zmiany tekstu, koloru, edycji położenia lub usunięcia. Admin moze edytowac kazdego gracza graffiti
	...
	Po stworzeniu graffiti, wysyła się ono na @czat oraz do logów<robota dla kogos innego, mrucznik pls>
	Edycja tekstu - logi
	Admin usunał graffiti - @czat

	Do dodania jest także rozpuszczalnik za X$ w 24/7, pozwoli to PD/mechanikom usuwać graffiti, mechanicy dostaną np. 3-4k$ za każde usunięcie,
	po usunięciu muszą przeczekać payday aby usunąć następne(żeby nie wyczyścili całego SC z graffiti na raz)
	
	Limit dla gracza to 3 graffiti, limit na całe SC - 75-100 graffiti
*/
...
graffiti_EditGraffiti(playerid, f)
{
	...
	Log(serverLog, WARNING, "Gracz %s zaaktualizował graffiti. Tekst:%s", GetPlayerLogName(playerid), GraffitiInfo[f][grafText]);
...
graffiti_CreateGraffiti(playerid)
{
	...
	if(f == INVALID_GRAFID) 
	{
		sendTipMessage(playerid, "Wykorzystano limit graffiti(100) na mapę.");
		return 1;
	}
	SetPVarInt(playerid, "GraffitiID", f);
	SetPVarInt(playerid, "CreatingGraff", 1);
	...
	graffiti_ReloadForPlayers(f);
	EditDynamicObject(playerid, GraffitiInfo[f][gID]);
	Log(serverLog, WARNING, "Gracz %s stworzył graffiti. Tekst: %s", GetPlayerLogName(playerid), GraffitiInfo[f][grafText]);
	return 1;
}
graffiti_ZerujListe(playerid)
{
