...
graffiti_ShowEditTextDialog(playerid, f)
{
	...
	ShowPlayerDialogEx(playerid, GRAFFITI_DIALOG_EDIT_TEXT, DIALOG_STYLE_INPUT, "Graffiti", "Wprowadź tekst(max 120 znaków)\n\nZachęcamy do zajrzenia w poradnik na forum dot. tworzenia graffiti!", "Dalej", "Anuluj");
}
graffiti_ShowEditColorDialog(playerid)
{
	ShowPlayerDialogEx(playerid, GRAFFITI_DIALOG_EDIT_COLOR, DIALOG_STYLE_LIST, "Wybierz podstawowy kolor", "{000000}Czarny\n{FFFFFF}Biały\n{F81414}Czerwony\n{00FF00}Zielony\n{00CED1}Niebieski\n{C3C3C3}Szary\n{E87E15}Pomarańczowy\n{FFE100}Żółty\n{8400FF}Fioletowy\n{EE00FF}Różowy", "Wybierz", "Anuluj");
}
graffiti_ShowCreationDialog(playerid)
{
	ShowPlayerDialogEx(playerid, GRAFFITI_DIALOG_ENTER_TEXT, DIALOG_STYLE_INPUT, "Graffiti", "Wprowadź tekst(max 120 znaków)\n\nZachęcamy do zajrzenia w poradnik na forum dot. tworzenia graffiti!", "Dalej", "Anuluj");
}
graffiti_ShowListDialog(playerid)
{
	...
	ShowPlayerDialogEx(playerid, GRAFFITI_DIALOG_LIST, DIALOG_STYLE_LIST, "Lista graffiti", dialogstring, "Znajdź", "X");
}
	...
	else if(dialogid == GRAFFITI_DIALOG_LIST)
	{
		if(response)
		{
			...
			if(graffiti_existing == 1) 
			{
				...
				sendTipMessage(playerid, "Stworzono znacznik gdzie to graffiti się znajduje.");
			}
		}
	}
	return 1;
