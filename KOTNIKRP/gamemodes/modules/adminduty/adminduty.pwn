...
//Opis:
/*
	Zawiera system służby administratora.
	W tym pliku dokładnie:
		> Główne CMD adminduty
...
CanUseAdminCommand(playerid)
{
	new valueAdminStatus = GetPVarInt(playerid, "dutyadmin");
	return (valueAdminStatus || PlayerInfo[playerid][pAdmin] >= 5000 || Uprawnienia(playerid, ACCESS_SKRYPTER)); 
}
AdminDutyPlayer(playerid, status)
{
	new string[128];	
	if(status == 1)//Jeżeli ma wejść
	{
		AdminDutyTimer[playerid] = SetTimerEx("AdminDutyCzas", 60000, true, "i", playerid);
		format(string, sizeof(string), "Administrator %s wszedł na służbę administratora!", GetNickEx(playerid));
	...
	else//Jeżeli ma zejść
	{	
		SetPlayerColor(playerid, TEAM_HIT_COLOR);
		//Komunikaty
		format(string, sizeof(string), "@DUTY: Wykonałeś ->  %d banów | %d warnów | %d kicków | %d innych akcji!", iloscBan[playerid],iloscWarn[playerid],iloscKick[playerid], (iloscInne[playerid]+iloscAJ[playerid])); 
		sendErrorMessage(playerid, string); 
		MSGBOX_Show(playerid, "Admin Duty ~r~OFF", MSGBOX_ICON_TYPE_OK);
		sendTipMessage(playerid, "Dziękujemy za sumienną służbę, administratorze!"); 	
		//Timer's kill
