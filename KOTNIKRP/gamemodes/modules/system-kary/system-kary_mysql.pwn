...
{
	new query[128],gid,group,Cache:result;
	...
	if(globalid)
	{
		...
		result = mysql_query(Database, query);
		cache_get_value_index_int(0, 0, group);
		cache_delete(result);
		if(group==5)
		{
			return true;
...
{	
	...
	mysql_escape_string(reason, validreason);
	...
	if(playerid == -1) {
		...
		result = mysql_query_format("SELECT UID, uid_forum FROM mru_konta WHERE Nick = '%s'", playername);

		if(cache_num_rows() > 0)
		{
			cache_get_value_index_int(0, 0, uid);
			cache_get_value_index_int(0, 1, gid);
		}
		cache_delete(result);
		ip_gracza = "nieznane";
	...
	if(type == PENALTY_WARN) {
		...
		if(CheckBlock(uid, BLOCK_CHAR) || CheckBlock(uid, BLOCK_CHAR_BAN) ) {
			...
			format(query, sizeof(query), 
				"UPDATE `mru_kary` SET `active` = 0 WHERE `player_uid` = '%d' AND `type` = 4 AND `active` = 1 ORDER BY `time` DESC LIMIT 1;", 
				uid
			);
			mysql_tquery(Database, query);
		} else {
			...
			if(CheckBlock(uid, BLOCK_BAN) || CheckBlock(uid, BLOCK_CHAR_BAN)) {
				...
				new query[256];
				format(query, sizeof(query), 
					"UPDATE `mru_kary` SET `active` = 0 WHERE `player_uid` = '%d' AND `type` = 3 AND `active` = 1 ORDER BY `time` DESC LIMIT 1;", 
					uid
				);
				mysql_tquery(Database, query);
			} else {
    ...
    format(str, sizeof(str), 
        "INSERT INTO `mru_kary` (`penalty_id`, `player_uid`, `player_gid`, `player_ip`, `admin_uid`, `admin_gid`, `time`, `type`, `value`, `reason`) VALUES (NULL, '%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%s');", 
        uid, gid, ip_gracza, validadmin_uid, validadmin_gid, time, type, value, validreason
    );
	} else {
		format(str, sizeof(str), 
			"INSERT INTO `mru_kary` (`penalty_id`, `player_uid`, `player_gid`, `player_ip`, `admin_uid`, `admin_gid`, `time`, `type`, `value`, `reason`) VALUES (NULL, '%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%s');", 
			PlayerInfo[playerid][pUID], PlayerInfo[playerid][pGID], ip_gracza, validadmin_uid, validadmin_gid, time, type, value, validreason
		);
	}
