...
//Opis:
/*
	Podstawowe chaty oraz narzędzia do ich pisania.
	Lista chatów:
		- [ic] /ad 
		- /live (/wywiad)
		- /news
...
sendFractionMessageToAll(playerid, groupid, text[])
{
	...
	format(sContent, sizeof(sContent), "Wiadomość dotarła do %d graczy", fractionMessageRange); 
...
sprawdzReklame(text[], playerid)
{
	...
	|| strfind(text, "fox", true)>=0)
	{
		...
		{
			format(string, sizeof(string), "AdmWarning: %s - Wykryto reklamę", text);
		}
...
sprawdzWulgaryzmy(text[], playerid)
{
	...
	if(CheckVulgarityString(text) != 0)
	{
		if(playerid != 666)
		{
			SendClientMessage(playerid, COLOR_GRAD2, "Zostałeś ukarany grzywną za wulgaryzmy! Kara: ("#PRICE_WULGARYZMY"$)");
			ZabierzKaseDone(playerid, PRICE_WULGARYZMY);
			Log(punishmentLog, WARNING, "Gracz %s został ukarany karą "#PRICE_WULGARYZMY"$ za przeklinanie.", GetPlayerLogName(playerid));
		}
...
PlayerTalkIC(playerid, text[], jakMowi[], Float:rangeTalk, bool:chatBooble=true)
{
	...
	if(strlen(jakMowi) <= 1)
	{
		sendErrorMessage(playerid, "Błąd! Zbyt krótka wartość 'jakMowi'"); 
		return 1;
	}
	...
	if(!IsPlayerInAnyVehicle(playerid) || IsABoat(idvehicle) || IsABike(idvehicle) || IsACarBezSzyb(idvehicle)){
		...
		if(IsVehicleWindowClosed(idvehicle, FL_DOOR) && IsVehicleWindowClosed(idvehicle, FR_DOOR) && IsVehicleWindowClosed(idvehicle, BL_DOOR) && IsVehicleWindowClosed(idvehicle, BR_DOOR)){
			chatBooble = false;
			if(strfind(newtext[ostatnialitera], ".", true, 0)  != -1 || strfind(newtext[ostatnialitera], "?", true, 0)  != -1 || strfind(newtext[ostatnialitera], "!", true, 0) != -1 || strfind(newtext[ostatnialitera], ":", true, 0)  != -1 || strfind(newtext[ostatnialitera], "*", true, 0) != -1 || strfind(newtext[ostatnialitera], " ", true, 0)  != -1) format(string, sizeof(string), "%s %s (w pojeździe): %s", GetNick(playerid), jakMowi, newtext);
			else format(string, sizeof(string), "%s %s (w pojeździe): %s.", GetNick(playerid), jakMowi, newtext);
			foreach(new i : Player){
				if(IsPlayerInVehicle(i, idvehicle) || (GetPlayerState(i) == PLAYER_STATE_SPECTATING && gPlayerLogged[i] == 1 && GetPlayerSpectateID(i) == idvehicle)){
					SendClientMessage(i, COLOR_FADE1, string);
					SetPlayerChatBubble(playerid, "**mówi coś w pojeździe**", KOLOR_JA, 20.0, 8000);
				}
			}
		}
		else{
...
PlayerTalkOOC(playerid, text[], Float:rangeTalk)
{
	...
	if(IsPlayerConnected(playerid))
	{
	    ...
	    if(strfind(text[ostatnialitera], ".", true, 0)  != -1 || strfind(text[ostatnialitera], "?", true, 0)  != -1 || strfind(text[ostatnialitera], "!", true, 0) != -1 || strfind(text[ostatnialitera], ":", true, 0)  != -1 || strfind(text[ostatnialitera], "*", true, 0) != -1 || strfind(text[ostatnialitera], " ", true, 0)  != -1) format(string, sizeof(string), "(( %s Napisał: %s ))", GetNick(playerid), text);
		else format(string, sizeof(string), "(( %s Napisał: %s. ))", GetNick(playerid), text);
...
stock ChatIC(playerid, text[])
{
	...
	{ //(w pojeździe)
		...
		format(string, sizeof(string), "%s Mówi(w pojeździe): %s", GetNick(playerid), text);
...
stock Krzyk(playerid, text[])
{
	...
	{ //(w pojeździe)
		...
		format(string, sizeof(string), "%s Krzyczy(w pojeździe): %s!!", GetNick(playerid), text);
...
stock Szept(playerid, text[])
{
	...
	{ //(w pojeździe)
		...
		format(string, sizeof(string), "%s Szepcze(w pojeździe): %s", GetNick(playerid), text);
...
{ //wiadomość wyświetlana w określonym zasięgu
	foreach(new i : Player)
	{
...
{ //wiadomość wyświetlana w określonym zasięgu kolorowana w zależności od odległości
	new Float: x, Float:y, Float:z;
	GetPlayerPos(playerid, x,y,z);
