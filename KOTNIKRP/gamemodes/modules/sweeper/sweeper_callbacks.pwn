...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(oldstate == PLAYER_STATE_DRIVER && SweeperJob[playerid])
	{
		...
		if(IsPlayerPremiumOld(playerid)){ money += floatround(money * 0.1); }
		new h, m, s;
		gettime(h, m, s);
		if(h >= 18) { money += floatround(money * 0.15); }
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(PRESSED(KEY_SUBMISSION) && GetVehicleModel(GetPlayerVehicleID(playerid)) == 574 && !SweeperJob[playerid])
	{
		...
		PlayerTextDrawColour(playerid, SweeperText[playerid], -1);
	    ...
	    SendClientMessage(playerid, -1, "Musisz jeździć po ulicach i je czyścić!");
	    SendClientMessage(playerid, -1, "Dostaniesz pieniądze, jeśli skończysz swoją prace.");
