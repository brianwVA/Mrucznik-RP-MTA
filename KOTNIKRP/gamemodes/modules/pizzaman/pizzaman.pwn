...
public PizzaboyTimer(playerid)
{
	...
	else if(Pizzaboy_State[playerid] == PIZZABOY_STATE_PACKING)
	{
		...
		} else
		{
			...
			if(Pizzaboy_Only == 1) {
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Zapakowałeś pizzę! Udaj się teraz do skutera pizzerii i zawieź pizzę klientom!");
			}
			else {
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Zapakowałeś pizzę! Udaj się teraz do pojazdu i zawieź pizzę klientom!");
			}
	...
	else if(Pizzaboy_State[playerid] == PIZZABOY_STATE_PICKUP)
	{
		...
		else
		{
			...
			if(Pizzaboy_Pizza_Quant[playerid] > 0)
			{
				...
				if(TimePassed <= 15)
				{
					//format(str, sizeof(str), "AdmWarning: %s [%d] dostarczył pizzę w mniej niż 15 sekund.", GetNick(playerid), playerid);
				...
				if(math <= 10)
				{
					...
					format(str, sizeof(str), "AdmWarning: %s [%d] najprawdopodobniej teleportuje się na Pizzaboyu.", GetNick(playerid), playerid);
					SendAdminMessage(COLOR_YELLOW, str);
					kick = 1;
					liczba_adminow = 0;
    				...
    				{
    					format(str, sizeof(str), "Zostałeś skickowany z podejrzeniem o TPHack");
				...
				SendClientMessage(playerid, COLOR_GREEN, "-------------- {FFFFFF}PIZZABOY {33AA33}--------------");
				new finalPrice = bonus + Pizzaboy_Earned[playerid];
				...
				format(str, sizeof(str), "Zarobiłeś $%d i bonusowe $%d. Łącznie dostajesz $%d!.",Pizzaboy_Earned[playerid], bonus, finalPrice);
				SendClientMessage(playerid, COLOR_LIGHTBLUE, str);
				format(str, sizeof(str), "Aby kontynuować jedź po kolejną pizzę. Lub wpisz{FFFFFF} /pizzaboy{00C2EC} by zakończyć pracę.");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, str);
...
Pizzaboy_CreateTextDraws(playerid)
{
	...
	PlayerTextDrawColour(playerid, pizza_td[playerid], -1);
...
{
	SendClientMessage(playerid, COLOR_GREEN, 	 "-------------- {FFFFFF}PIZZABOY {33AA33}--------------");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Witaj w pracy dostawcy pizzy! Idź do pizzerii i {FFFFFF}wez pizze {00C2EC}z kuchni.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Aby anulować wpisz jeszcze raz{FFFFFF} /pizzaboy");
...
Pizzaboy_End(playerid) // warto dodać do OnPlayerDisconnect, OnPlayerDeath itd. po sprawdzeniu if(Pizzaboy_Active[playerid])
{
...
Pizzaboy_Delete_Customer(playerid)
{
	if(Pizzaboy_Actor[playerid] != 0) // być może nie potrzebny if
	{
