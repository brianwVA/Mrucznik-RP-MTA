...
public Tattoo_Create(playerid, creatorid, id, index, bone, Float:fX, Float:fY, Float:fZ, Float:fRX, Float:fRY, Float:fRZ)
{
    ...
    new query[256];
    mysql_format(Database, query, sizeof(query), "INSERT INTO `mru_tattoo` (`ID`, `offsetX`, `offsetY`, `offsetZ`, `rX`, `rY`, `rZ`, `owner`, `bone`) VALUES ('%d', '%f', '%f', '%f', '%f', '%f', '%f', '%d', '%d')", id, fX, fY, fZ, fRX, fRY, fRZ, PlayerInfo[playerid][pUID], bone);
    mysql_query(Database, query);
    if(cache_insert_id() < 1) return sendErrorMessage(playerid, "Wystąpił błąd zapisu!");
    PlayerTattoo[playerid][slot][t_UID] = cache_insert_id();
    ...
    _MruGracz(playerid, "Tatuaż wykonany!");
    _MruGracz(creatorid, "Tatuaż wykonany!");
...
{
    mysql_tquery_format("DELETE FROM `mru_tattoo` WHERE `UID` = '%d'", PlayerTattoo[playerid][slot][t_UID]);
    ...
    sendErrorMessage(playerid, sprintf("Usunąłeś tatuaż o ID: %d", slot+1));
    return 1;
}
...
stock Tattoo_StartMaking(playerid, creatorid, id, index, bone, Float:fX, Float:fY, Float:fZ, Float:fRX, Float:fRY, Float:fRZ)
{
    ...
    if(!IsPlayerConnected(creatorid)) return sendErrorMessage(playerid, "Gracz, który składał Ci ofertę wyszedł z gry.");
    if(!IsPlayerNear(playerid, creatorid)) return sendErrorMessage(playerid, "Nie jesteś w pobliżu gracza, który składał Ci ofertę.");
    if(!GroupPlayerDutyPerm(creatorid, PERM_TATTOO)) return sendErrorMessage(playerid, "Gracz, który składał Ci ofertę zszedł ze służby!");
    if(kaska[playerid] < price) return sendErrorMessage(playerid, "Nie masz tylu pieniędzy!");

    new groupid = GetPlayerGroupUID(creatorid, OnDuty[creatorid]-1);
    if(!IsValidGroup(groupid)) return sendErrorMessage(playerid, "Gracz, który składał Ci ofertę zszedł ze służby!");
    DajKase(playerid, -price);
    DajKase(creatorid, price/2);
    Sejf_Add(groupid, price/2);

    va_SendClientMessage(creatorid, COLOR_LIGHTBLUE, "Otrzymujesz $%d za wykonanie tatuażu.", price/2);
    va_SendClientMessage(playerid, COLOR_LIGHTBLUE, "Tatuażysta %s zaczął wykonywać tatuaż, poczekaj 60 sekund.", GetNick(creatorid));
