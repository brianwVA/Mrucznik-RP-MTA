    ...
    if(GetPVarInt(playerid, "Tattoo-EditingObj") == 1)
    {
        if(response)
        {
            ...
            if(oldScale[0] != fScaleX || oldScale[1] != fScaleY || oldScale[2] != fScaleZ)
            {
                ...
                sendErrorMessage(playerid, "Nie możesz zmienić skali tatuażu!");
                return -2;
            }
            if(fOffsetX > 0.20 || fOffsetX < -0.50)
            {
                SetPlayerAttachedObject(playerid, index, modelid, boneid, oldoffset[0], oldoffset[1], oldoffset[2], oldRot[0], oldRot[1], oldRot[2], oldScale[0], oldScale[1], oldScale[2]);
                EditAttachedObject(playerid, index);
                sendErrorMessage(playerid, "Wartość X wykracza poza granicę obszaru!");
                return -2;
            }
            if(fOffsetY > 0.16 || fOffsetY < -0.16)
            {
                SetPlayerAttachedObject(playerid, index, modelid, boneid, oldoffset[0], oldoffset[1], oldoffset[2], oldRot[0], oldRot[1], oldRot[2], oldScale[0], oldScale[1], oldScale[2]);
                EditAttachedObject(playerid, index);
                sendErrorMessage(playerid, "Wartość Y wykracza poza granicę obszaru!");
                return -2;
            }
            if(fOffsetZ > 0.14 || fOffsetZ < -0.14)
            {
                SetPlayerAttachedObject(playerid, index, modelid, boneid, oldoffset[0], oldoffset[1], oldoffset[2], oldRot[0], oldRot[1], oldRot[2], oldScale[0], oldScale[1], oldScale[2]);
                EditAttachedObject(playerid, index);
                sendErrorMessage(playerid, "Wartość Z wykracza poza granicę obszaru!");
                return -2;
            }
        ...
        {
            RemovePlayerAttachedObject(playerid, index);
            sendTipMessage(playerid, "Przerwałeś tworzenie tatuażu!");
            if(IsPlayerConnected(GetPVarInt(playerid, "Tattoo-Offer"))) sendErrorMessage(GetPVarInt(playerid, "Tattoo-Offer"), "Gracz, któremu robiłeś tatuaż anulował ofertę.");
        ...
        case D_TATTOO:
        {
            ...
            if(!response)
            {
                ...
                if(IsPlayerConnected(offer))
                    sendErrorMessage(offer, "Gracz, któremu oferowałeś tatuaż anulował ofertę.");
                sendErrorMessage(playerid, "Anulowałeś ofertę tatuażu.");
                return 1;
            }
            ...
            if(!IsPlayerNear(playerid, offer)) return sendErrorMessage(playerid, "Nie znajdujesz się w pobliżu tego gracza!");

            SetPVarInt(playerid, "Tattoo-ModelIndex", listitem);
            ShowPlayerDialogEx(playerid, D_TATTOO_BONE, DIALOG_STYLE_LIST, "Tatuaż - Część ciała", "+ Lewa ręka\n+ Prawa ręka\n+ Lewa noga\n+ Prawa noga", "Dalej", "Zamknij");
            return 1;
        }
        case D_TATTOO_BONE:
        {
            ...
            {
                Tattoo_Failure(playerid);
                SendClientMessage(offer, COLOR_LIGHTBLUE, "* Gracz, któremu robiłeś tatuaż przekroczył limit obiektów przyczepialnych.");
                SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Przekroczyłeś limit obiektów przyczepialnych, tatuaż nie mógł zostać wykonany!");
                return 1;
            }
