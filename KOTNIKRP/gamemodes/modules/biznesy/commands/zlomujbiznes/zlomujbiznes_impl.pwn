...
command_zlomujbiznes_Impl(playerid)
{
    ...
    {
        sendErrorMessage(playerid, "Nie jesteś właścicielem tego biznesu"); 
        return 1;
    }
    if(GetPVarInt(playerid, "Wpisal_zlomujbiznes") == 0)
    {
        sendTipMessage(playerid, "Uwaga! Stracisz swój biznes, dostaniesz połowę jego ceny jako zwrot!"); 
        sendTipMessage(playerid, "Jeżeli akceptujesz powyższe warunki wpisz ponownie /zlomujbiznes"); 
        SetPVarInt(playerid, "Wpisal_zlomujbiznes", 1);
        return 1;
    }
    else if(GetPVarInt(playerid, "Wpisal_zlomujbiznes") == 1)
    {
        new stringName[MAX_PLAYER_NAME + 1]; 
        new string[128]; 
        Business[businessID][b_ownerUID] = 0;  
        format(stringName, sizeof(stringName), "Brak - na sprzedaż");
        Business[businessID][b_Name_Owner] = stringName; 
        PlayerInfo[playerid][pBusinessOwner] = INVALID_BIZ_ID; 
        format(string, sizeof(string), "%s zezłomował biznes %d za $%d", GetNick(playerid), businessID, valueMoneyB); 
        SendMessageToAdmin(string, COLOR_RED); 

        Log(payLog, WARNING, "%s zezłomował biznes %s za $%d",
        ...
        sendTipMessage(playerid, "Pomyślnie zezłomowałeś swój biznes!"); 
    }
    return 1;
}
