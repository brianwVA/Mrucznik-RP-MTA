...
biznesy_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
		...
		if(response)
		{
			...
			if(GetPVarInt(playerid, "bizWhatToDo") == 1)//Przyjmij pracownika
			{
				if(IsPlayerConnected(giveplayerid))
				{
					...
					{
						sendErrorMessage(playerid, "Ten gracz ma już biznes!"); 
						return 1; 
					}
					if(playerid == giveplayerid)
					{
						sendErrorMessage(playerid, "Nie możesz przyjąć samego siebie!"); 
						return 1;
					}
					PlayerInfo[giveplayerid ][pBusinessMember] = PlayerInfo[playerid][pBusinessOwner]; 
					format(string, sizeof(string), "Zostałeś przyjęty do %s przez %s", Business[PlayerInfo[playerid][pBusinessOwner]][b_Name], GetNick(playerid));
					sendTipMessageEx(giveplayerid , COLOR_BLUE, string); 
					format(string, sizeof(string), "Przyjąłeś %s do swojego biznesu!", GetNick(giveplayerid)); 
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 2)//Zwolnij pracownika
			{
				if(IsPlayerConnected(giveplayerid))
				{
					...
					{
						format(string, sizeof(string), "Gracz %s nie jest członkiem twojego biznesu!", GetNick(giveplayerid)); 
						sendErrorMessage(playerid, string); 
						return 1;
					}
					PlayerInfo[giveplayerid ][pBusinessMember] = INVALID_BIZ_ID; 
					format(string, sizeof(string), "Zostałeś zwolniony z %s przez %s", Business[PlayerInfo[playerid][pBusinessOwner]][b_Name], GetNick(playerid));
					sendTipMessageEx(giveplayerid , COLOR_BLUE, string); 
					format(string, sizeof(string), "Zwolniłeś %s ze swojego biznesu!", GetNick(giveplayerid )); 
					sendTipMessageEx(playerid, COLOR_BLUE, string); 
				}
			}
			else if(GetPVarInt(playerid, "bizWhatToDo") == 3)
			{
				...
				{
					sendErrorMessage(playerid, "Nie możesz wsadzić takiej gotówki - sejf jest przepełniony!"); 
					return 1;
				}
				if(valueMoney <= 0)
				{
					sendErrorMessage(playerid, "Wartość nie może być podana z ''-''"); 
					return 1;
				}
				format(string, sizeof(string), "Wpłaciłeś do sejfu $%d, jest w nim teraz $%d", 
					valueMoney, 
				...
				Log(payLog, WARNING, "%s wpłacił do biznesu %s kwotę %d$. Nowy stan: %d$",
			...
			else if(GetPVarInt(playerid, "bizWhatToDo") == 4)
			{
				...
				Log(payLog, WARNING, "%s wypłacił z biznesu %s kwotę %d$. Nowy stan: %d$",
					GetPlayerLogName(playerid),
					GetBusinessLogName(businessID), 
					valueMoney,
					Business[businessID][b_moneyPocket]
	...
	else if(dialogid == D_BIZ_WRITE)
	{
		if(!response)
		{
			sendErrorMessage(playerid, "Wyszedłeś z panelu tworzenia biznesu"); 
			return 1;
		}
		...
		if(response)
		{
			if(GetPVarInt(playerid, "MustBe") == 0)//Kwota kasy za jaka gracz ma kupic
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "M-RP", "Wprowadź poniżej maksymalny zysk\nktóry gracz będzie mógł otrzymać co godzine.", "Dalej", "Wyjdź"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 1)
			{
				...
				ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "M-RP", "Wpisz poniżej nazwę biznesu", "Dalej", "Wyjdź"); 
				return 1;
			}
			if(GetPVarInt(playerid, "MustBe") == 2)
			{
				if(strlen(inputtext) > 3 && strlen(inputtext) < 64)
				{
					...
					sendTipMessage(playerid, string); 
					sendTipMessage(playerid, "Teraz trzeba wybrać pozycję wejścia - stań w miejscu i wpisz /stworzbiznes"); 
					SetPVarInt(playerid, "ActionCreateBiz", 1); 
				}
				else
				{
					ShowPlayerDialogEx(playerid, D_BIZ_WRITE, DIALOG_STYLE_INPUT, "M-RP", "Wpisz poniżej nazwę biznesu\nMINIMALNIE 3 ZNAKI", "Dalej", "Wyjdź"); 
				}
				return 1;
			}
		}
