...
Car_Create(model, Float:x, Float:y, Float:z, Float:angle, color1, color2)
{
    ...
    if(mysql_query(Database, lStr))
    {
        lUID = cache_insert_id();
        ...
        CarData[idx][c_MaxHP] = 1000.0;
        CarData[idx][c_UpgradeWindows] = 0;
        CarData[idx][c_UpgradeWheels] = 0;
...
Car_Load()
{
    new lStr[512], lLoad=gCars, Cache:result;
    ...
    for(new i = 0; i < cache_num_rows(); i++)
    {
        ...
        cache_get_value_index_bool(i, 13,         CarData[gCars][c_bHydraulika]);
        ...
        cache_get_value_index(i, 24,             CarData[gCars][c_Rejestracja]);
        cache_get_value_index_int(i, 25,         CarData[gCars][c_Sideskirt]);
        cache_get_value_index_int(i, 26,         CarData[gCars][c_Hood]);
        cache_get_value_index_int(i, 27,         CarData[gCars][c_Exhaust]);
        cache_get_value_index_int(i, 28,         CarData[gCars][c_Vents]);
        cache_get_value_index_int(i, 29,         CarData[gCars][c_Lamps]);
        cache_get_value_index_int(i, 30,         CarData[gCars][c_Roof]);
        cache_get_value_index_int(i, 31,         CarData[gCars][c_Siren]);
        cache_get_value_index_float(i, 32,         CarData[gCars][c_MaxHP]);
        if(CarData[gCars][c_MaxHP] > 1500.0) CarData[gCars][c_MaxHP] = 1500.0;
        cache_get_value_index_int(i, 33,         CarData[gCars][c_UpgradeWindows]);
        //cache_get_value_index_int(i, 34,         CarData[gCars][c_UpgradeWheels]);
    ...
    for(new i=lLoad;i<gCars;i++)
    {
        ...
        if(CarData[i][c_OwnerType] == CAR_OWNER_FRACTION && CarData[i][c_Owner] == FRAC_GOV)
        {
            ...
            if(CarData[i][c_Model] == 496)
            {
                ...
                SetDynamicObjectMaterialText(elkaDMV, 0, "Urząd Miasta\nLos Santos", 90, "Ariel", 32, 1, -1, 0, 1);
	...
	for(new i=0;i<MAX_VEHICLES;i++)
	{
        ...
        if(CarData[VehicleUID[i][vUID]][c_UID] > 0)
		{
			...
			result = mysql_query(Database, qText);
			if(cache_num_rows())
			{
...
Car_LoadEx(lUID)
{
    ...
    {
        new i = 0;
        cache_get_value_index_int(i, 0,         CarData[lVehID][c_UID]);
        cache_get_value_index_int(i, 1,         CarData[lVehID][c_OwnerType]);
        cache_get_value_index_int(i, 2,         CarData[lVehID][c_Owner]);
        cache_get_value_index_int(i, 3,         CarData[lVehID][c_Model]);
        cache_get_value_index_float(i, 4,         CarData[lVehID][c_Pos][0]);
        cache_get_value_index_float(i, 5,         CarData[lVehID][c_Pos][1]);
        cache_get_value_index_float(i, 6,         CarData[lVehID][c_Pos][2]);
        cache_get_value_index_float(i, 7,         CarData[lVehID][c_Rot]);
        cache_get_value_index_float(i, 8,         CarData[lVehID][c_HP]);
        cache_get_value_index_int(i, 9,         CarData[lVehID][c_Tires]);
        cache_get_value_index_int(i, 10,         CarData[lVehID][c_Color][0]);
        cache_get_value_index_int(i, 11,         CarData[lVehID][c_Color][1]);
        cache_get_value_index_int(i, 12,         CarData[lVehID][c_Nitro]);
        cache_get_value_index_bool(i, 13,         CarData[lVehID][c_bHydraulika]);
        cache_get_value_index_int(i, 14,         CarData[lVehID][c_Felgi]);
        cache_get_value_index_int(i, 15,         CarData[lVehID][c_Malunek]);
        cache_get_value_index_int(i, 16,         CarData[lVehID][c_Spoiler]);
        cache_get_value_index_int(i, 17,         CarData[lVehID][c_Bumper][0]);
        cache_get_value_index_int(i, 18,         CarData[lVehID][c_Bumper][1]);
        cache_get_value_index_int(i, 19,         CarData[lVehID][c_Keys]);
        cache_get_value_index_int(i, 20,         CarData[lVehID][c_Neon]);
        cache_get_value_index_int(i, 21,         CarData[lVehID][c_Rang]);
        cache_get_value_index_int(i, 22,         CarData[lVehID][c_Int]);
        cache_get_value_index_int(i, 23,         CarData[lVehID][c_VW]);
        cache_get_value_index(i, 24,         CarData[lVehID][c_Rejestracja]);
        cache_get_value_index_int(i, 25,         CarData[lVehID][c_Sideskirt]);
        cache_get_value_index_int(i, 26,         CarData[lVehID][c_Hood]);
        cache_get_value_index_int(i, 27,         CarData[lVehID][c_Exhaust]);
        cache_get_value_index_int(i, 28,         CarData[lVehID][c_Vents]);
        cache_get_value_index_int(i, 29,         CarData[lVehID][c_Lamps]);
        cache_get_value_index_int(i, 30,         CarData[lVehID][c_Roof]);
        cache_get_value_index_int(i, 31,         CarData[lVehID][c_Siren]);
        cache_get_value_index_float(i, 32,         CarData[lVehID][c_MaxHP]);
        if(CarData[lVehID][c_MaxHP] > 1500.0) CarData[lVehID][c_MaxHP] = 1500.0;
        cache_get_value_index_int(i, 33,         CarData[lVehID][c_UpgradeWindows]);
        //cache_get_value_index_int(i, 34,         CarData[lVehID][c_UpgradeWheels]);
...
Car_LoadForPlayer(playerid)
{
    ...
    for(new i=0;i<MAX_VEHICLES;i++)
    {
        ...
        if(CarData[lcar][c_OwnerType] == CAR_OWNER_PLAYER && CarData[lcar][c_Owner] == lPUID)
        {
            ...
            strcat(lList, lStr);
            //Przypisanie auta, które już istenije w grze - brak potrzeby ponownego ładowania do systemu, tylko pobranie z bazy.
    ...
    MySQL_TQueryInline(Database, using inline OnCarsLoaded, lStr);
...
Car_AddTune(vehicleid)
{
    ...
    ChangeVehicleColours(vehicleid, CarData[uid][c_Color][0], CarData[uid][c_Color][1]);
}
...
Car_Spawn(lUID, bool:loaddesc=true)
{
    ...
    {
        if(VehicleUID[v][vUID] == lUID && GetVehicleModel(v) != 0)
        {
            CarData[lUID][c_ID] = v;
            return v;
        }
    ...
    {
        expire = 1200; //Respawn co 20 minut dla pojazdów należących do pracy dorywczej
    }
...
/*
    Czyszczenie pojazdu z pamięci - OverLoad Protect
    Podczas unspawnowania pojazdu, sprawdzane jest, czy na serwerze obecnie jest
    własciciel pojazdu. Jeżeli jest, czyszczenie zostaje pominięte.
    W przeciwnym wypadku, pojazd ulega usunięciu z pamięci, do kolejki zostaje
    dodany wolny index.
*/
...
Car_ClearMem(lVehID)
{
    ...
    //Uznajemy index za ponowny do użycia.
...
Car_Unspawn(v, bool:playercall=false)
{
    ...
    new lUID = VehicleUID[v][vUID];
    if(lUID == 0) return 0;

    GetVehicleHealth(v, CarData[lUID][c_HP]);
    new panels, doors, lights;
    GetVehicleDamageStatus(v, panels, doors, lights, CarData[lUID][c_Tires]);
    ...
    //OverLoad Protect:
    if(CarData[lUID][c_OwnerType] == CAR_OWNER_PLAYER && !playercall)
    {
        new lOwner = CarData[lUID][c_Owner], bool:lExist=false;
        new keys = CarData[lUID][c_Keys];
...
Car_Save(lUID, lType)
{
    ...
    switch(lType)
    {
        ...
        case CAR_SAVE_TUNE:
        {
            ...
            new message[128];
            format(message, sizeof(message), "%s", CarData[lUID][c_Rejestracja]);
            mysql_escape_string(message, message);
            format(lStr, sizeof(lStr), "UPDATE `mru_cars` SET `max_hp`='%f', `bulletproof`='%d', `Rejestracja` = '%s' WHERE `UID`='%d'", CarData[lUID][c_MaxHP],CarData[lUID][c_UpgradeWindows], message, CarData[lUID][c_UID]);
            //printf(lStr);
            mysql_tquery(Database, lStr);
        }
        default:
...
IsABoatModel(carid)
{
	if(carid == 472 || carid == 473 || carid == 493 || carid == 595 || carid == 484 || carid == 430 || carid == 453 || carid == 452 || carid == 446 || carid == 454)//łodzie
	{
		return 1;
	}
	return 0;
...
Car_PrintOwner(car)
{
    ...
    switch(type)
    {
        ...
        case CAR_OWNER_PLAYER:
        {
            ...
            if(cache_num_rows())
            {
                cache_get_value_index(0, 0, lStr);
                cache_delete(result);
            }
			else
			{
				format(lStr, sizeof(lStr), "Nieistniejący");
			}
        }
        case CAR_OWNER_JOB:
        {
...
ShowCarEditDialog(playerid)
{
    ...
    format(lStr, sizeof(lStr), "{FFFFFF}Model:\t\t{8FCB04}%d{FFFFFF}\t[ {8FCB04}%s {FFFFFF}]\nWłaściciel:\t{8FCB04}%s{FFFFFF} » {8FCB04}%s{FFFFFF} (UID: {8FCB04}%d{FFFFFF})\nRanga:\t\t{8FCB04}%d{FFFFFF}\nOpis Pojazdu\nStan:\t\t{8FCB04}%.1f{FFFFFF}\%\nZaparkuj tutaj\nUsuń kluczyki\n{%06x}Kolor I\n{%06x}Kolor II", CarData[car][c_Model], VehicleNames[CarData[car][c_Model]-400], (CarData[car][c_OwnerType] >= 6) ? ("ZBUGOWANY WŁAŚCICIEL") : (CarOwnerNames[CarData[car][c_OwnerType]]), Car_PrintOwner(car), CarData[car][c_Owner], CarData[car][c_Rang], lHP/10, RGBAtoRGB(color1), RGBAtoRGB(color2));
    ShowPlayerDialogEx(playerid, D_EDIT_CAR_MENU, DIALOG_STYLE_LIST, "{8FCB04}Edycja {FFFFFF}pojazdów", lStr, "Wybierz", "Wróć");
    return 1;
}
...
ShowPlayerCarDialog(playerid, type)
{
    ...
    for(new i = 0; i<sizeof(SalonAut); i++)
    {
        ...
        {
            strcat(string, sprintf("%d\t%s~n~~g~$%s\n", SalonAut[i][sModel], SalonAut[i][sModel] < 611 ? VehicleNames[SalonAut[i][sModel]-400] : "Inny", SalonAut[i][sCena]));
        }
    }
...
CreateSalonDialog(playerid, id)
{
    ...
    PlayerTextDrawColour(playerid, Salon_Background[playerid], -1);
    ...
    PlayerTextDrawColour(playerid, Salon_Name[playerid], -1);
    ...
    PlayerTextDrawColour(playerid, Salon_Button[playerid][0], -1);
    ...
    PlayerTextDrawColour(playerid, Salon_Button[playerid][1], -1);
    ...
    PlayerTextDrawColour(playerid, Salon_Model[playerid], -1);
    ...
    PlayerTextDrawColour(playerid, Salon_Desc[playerid], -1);
...
RefreshSalon()
{
	...
	printf("[Salon Aut] Odswiezono salon");
}
