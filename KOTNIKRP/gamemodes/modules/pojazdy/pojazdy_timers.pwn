...
//----[                                                                                                 ]----//
//----------------------------------------------------*------------------------------------------------------//
// Autor: Mrucznik & Sandał & Creative
...
public CruiseControl_Static_TurnOn(playerid, type)
{
    ...
    //pozniej trzeba dodać nowy typ w pojazdy_timers.pwn
    if(type == 0 && (IsPlayerInAnyVehicle(playerid) && GetPlayerVehicleSeat(playerid) == 0)) 
	{
		SendClientMessage(playerid, COLOR_YELLOW, "Została ci nałożona blokada prędkości do 50km/h, nie posiadasz prawa jazdy.");
