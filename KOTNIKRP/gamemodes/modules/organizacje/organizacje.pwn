...
organizacje_toggleSektaKey(playerid, giveplayerid)
{
    ...
    {
        format(var, sizeof(var), "Dałeś klucz do wrot %s", GetNick(giveplayerid));
        sendTipMessageEx(playerid, COLOR_LIGHTBLUE, var);
        sendTipMessageEx(giveplayerid, COLOR_LIGHTBLUE, "Otrzymałeś klucz do wrot.");
        SektaKey[giveplayerid] = 1;
    }
    else
    {
        format(var, sizeof(var), "Zabrałeś klucz do wrot %s", GetNick(giveplayerid));
        sendTipMessageEx(playerid, COLOR_LIGHTBLUE, var);
...
organizacje_wepchnijSekta(playerid, giveplayerid)
{
    ...
    {
        if(GetDistanceBetweenPlayers(playerid,giveplayerid) > 3) return sendErrorMessage(playerid, "Gracz nie jest w pobliżu.");
		if(SektaUwieziony[giveplayerid] == 1) return sendErrorMessage(playerid, "Gracz jest już uwięziony.");
        format(var, sizeof(var), "* %s ogłuszył(a) i wrzucił(a) do celi %s.", GetNick(playerid) ,GetNick(giveplayerid));
    ...
    {
        sendTipMessage(playerid, "Nie jesteś przy celi/wrotach");
    }
	return 1;
}
...
organizacje_wypuscSekta(playerid, giveplayerid)
{
    ...
    {
        if(SektaUwieziony[giveplayerid] == 0) return sendErrorMessage(playerid, "Gracz nie jest uwięziony.");
        SetPlayerPos(giveplayerid, 867.2818,-1107.7255,5.7544);
        format(var, sizeof(var), "* %s wypuścił z celi %s.", GetNick(playerid) ,GetNick(giveplayerid));
        ProxDetector(20.0, playerid, var, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
