...
command_koszyk_Impl(playerid)
{
	...
	if(GetPVarInt(mechanik, "Tune_active") == 1 && GetPVarInt(mechanik, "Tune_giveplayerid") == playerid)
	{
		...
		for(new i = 0; i<MAX_TUNE_PARTS; i++)
		{
			...
			{
				if(TuneRecipe[playerid][i][tuneR_Type] == TUNE_TYPE_BUMPER || TuneRecipe[playerid][i][tuneR_Type] == TUNE_TYPE_BUMPER_FRONT || TuneRecipe[playerid][i][tuneR_Type] == TUNE_TYPE_BUMPER_REAR || TuneRecipe[playerid][i][tuneR_Type] == TUNE_TYPE_SPOILER)
					format(custom, sizeof custom, "%s", TuneRecipe[playerid][i][tuneR_Custom] == 0 ? " (Fabryczny)" : " (Niestandardowy)");
				else
					format(custom, sizeof custom, "");
				format(string, sizeof(string), "%s\n%d\t%s%s\t%s\t%d$", string, i, TuneRecipe[playerid][i][tuneR_Name], custom, TuneTypeNames[TuneRecipe[playerid][i][tuneR_Type]], TuneRecipe[playerid][i][tuneR_Cost]);
		...
		return SendClientMessage(playerid, COLOR_GRAD2, "Nie jesteś podczas tuningu!");
}
