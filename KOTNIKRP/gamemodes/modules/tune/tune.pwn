...
CancelTune(playerid, bool:cancel = true)
{
	...
	if(cancel)
	{
		if(IsPlayerConnected(giveplayerid)) SendClientMessage(giveplayerid, COLOR_GRAD2, "Tuning został anulowany!");
		if(playerid != giveplayerid) SendClientMessage(playerid, COLOR_GRAD2, "Tuning został anulowany!");
...
FinishTune(playerid)
{
	...
	{
		SendClientMessage(giveplayerid, COLOR_GRAD2, sprintf("Nie stać Cię na ten tuning, kosztuje on %d$! Tuner może teraz usunąć część tuningu.", cost));
		SendClientMessage(playerid, COLOR_GRAD2, sprintf("Tego gracza nie stać na ten tuning! Oferta została anulowana i możesz teraz ponownie użyć /tuning %d by usunąć część tuningu.", giveplayerid));
		SetPVarInt(giveplayerid, "Tune_PendingInvite", 0);
		return 1;
	}
	...
	for(new i = 0; i<MAX_TUNE_PARTS; i++)
	{
		if(TuneRecipe[giveplayerid][i][tuneR_Custom] == 0)
		{
			switch(TuneRecipe[giveplayerid][i][tuneR_Type])
			{
				...
				case TUNE_TYPE_UPGRADE_WINDOWS:
					CarData[VehicleUID[vehicleid][vUID]][c_UpgradeWindows] = TuneRecipe[giveplayerid][i][tuneR_Val];
				case TUNE_TYPE_UPGRADE_ARMOR:
					CarData[VehicleUID[vehicleid][vUID]][c_MaxHP] = float(TuneRecipe[giveplayerid][i][tuneR_Val]);
				case TUNE_TYPE_UPGRADE_BPROOFWHEELS:
					CarData[VehicleUID[vehicleid][vUID]][c_UpgradeWheels] = TuneRecipe[giveplayerid][i][tuneR_Val];
			}
		}
		TuneRecipe[giveplayerid][i][tuneR_Active] = 0;
	}

	SendClientMessage(giveplayerid, COLOR_GRAD2, sprintf("Akceptujesz tuning od gracza %s [%d]. Zapłaciłeś %d$!", GetNick(playerid), playerid, cost));
	SendClientMessage(playerid, COLOR_GRAD2, sprintf("Gracz %s [%d] zaakceptował ofertę tuningu. Dostałeś %d$!", GetNick(giveplayerid), giveplayerid, floatround(float(cost)*0.3)));
...
ShowTunePanel(playerid)
{
	...
	if(!GetPVarInt(playerid, "Tune_check")) format(string, sizeof(string), "%s» Usuń tuning\n» Zakończ tuning", string);
...
Float:tuneRX = 0.0, Float:tuneRY = 0.0, Float:tuneRZ = 0.0)
{
	...
	sendErrorMessage(GetPVarInt(playerid, "Tune_mechanik"), "Ten gracz nie ma już miejsca na kolejny tuning!");
	return 1;
}
...
AcceptTuneOffer(playerid, giveplayerid)
{
	...
	SendClientMessage(giveplayerid, COLOR_GRAD2, sprintf("Akceptujesz ofertę tuningu gracza %s [%d]. Wpisz /koszyk by zobaczyć listę tuningową, lub wpisz /anuluj tuning, aby anulować!", GetNick(playerid), playerid));
	return SendClientMessage(playerid, COLOR_GRAD2, sprintf("Gracz %s [%d] zaakceptował ofertę tuningu. Wpisz {bfbfbf}/tuning %d{BFC0C2} aby rozpocząć!", GetNick(giveplayerid), giveplayerid, giveplayerid));
}
