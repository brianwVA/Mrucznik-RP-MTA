        ...
        switch (response)
        {
            case EDIT_RESPONSE_FINAL:
            {
                ...
                if(tuneid == -1) 
                {
                    ...
                    return sendErrorMessage(playerid, "Wystąpił błąd. (Brak identyfikatora tuningu)");
                }
...
Tune_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        ...
        {   
            ...
            switch(listitem)
            {
                case TUNE_TYPE_NITRO:
                {
                    ...
                    "Pojemność\tSkill\tCena\n\
                    » 2 użycia\t1\t{33AA33}$1000\n\
                    » 5 użyć\t2\t{33AA33}$2000\n\
                    » 10 użyć\t3\t{33AA33}$3000\n\
                ...
                case TUNE_TYPE_FINISH:
                {
                    ...
                    ShowPlayerDialogEx(playerid, D_TUNE_PANEL_FINISH_CONFIRM, DIALOG_STYLE_MSGBOX, "{99e805}Panel Tuningu {ffffff}» Zakończ", "Czy na pewno chcesz zakończyć tuning?\nSpowoduje to wysłanie oferty graczowi, a Ty nie będziesz mógł już nic zmienić.", "Tak", "Nie");
                }
                ...
                case TUNE_TYPE_NEON:
                {
                    ...
                    if(IsPlayerPremiumOld(giveplayerid))
                        ShowPlayerDialogEx(playerid, D_TUNE_PANEL_SELECT_NEON, DIALOG_STYLE_LIST, "{99e805}Panel Tuningu {ffffff}» Neony", "»{FFFFFF} Biały\n»{FF0000} Czerwony\n»{0000FF} Niebieski\n»{9ACD32} Zielony\n»{DAA520} Żółty\n»{C2A2DA} Różowy", "Wybierz", "Anuluj");
                    else
                        ShowPlayerDialogEx(playerid, D_TUNE_PANEL_SELECT_NEON, DIALOG_STYLE_LIST, "{99e805}Panel Tuningu {ffffff}» Neony", "»{FFFFFF} Biały", "Wybierz", "Anuluj");
                }
                case TUNE_TYPE_UPGRADE:
                {
                    ShowPlayerDialogEx(playerid, D_TUNE_PANEL_UPGRADE, DIALOG_STYLE_TABLIST_HEADERS, sprintf("{99e805}Panel Tuningu {ffffff}» %s", TuneTypeNames[listitem]), 
                    "Ulepszenie\tSkill\tCena\n\
                    » Pancerz karoserii\t4\t{33AA33}$5000\n\
                    » Kuloodporne szyby\t4\t{33AA33}$5000",
                    "Zamontuj", "Anuluj");
                }
                ...
                case TUNE_TYPE_PAINTJOB:
                {
                    ...
                    sendTipMessage(playerid, "Ten pojazd nie może zostać pomalowany.");
                ...
                default:
                {
                    ...
                    {
                        ShowTunePanel(playerid);
                        return sendErrorMessage(playerid, "Musisz mieć 4 skill mechanika!");
                    }
    ...
    else if(dialogid == D_TUNE_PANEL_FINISH_CONFIRM)
    {
        if(response)
        {
            ...
            format(string, sizeof(string),  "%s zakończył tuning. Wpisz {bfbfbf}/akceptuj tuning{BFC0C2} by zaakceptować lub {bfbfbf}/anuluj tuning{BFC0C2} by anulować.", GetNick(playerid));
            SendClientMessage(giveplayerid, COLOR_GRAD2, string);
            format(string, sizeof(string),  "Zakończyłeś tuning i oferujesz %s jego akceptację. Poczekaj, aż zaakceptuje Twoją ofertę!", GetNick(giveplayerid));
    ...
    else if(dialogid == D_TUNE_PANEL_SELECT)
    {
        ...
        switch(listitem)
        {
            ...
            case 1: // CUSTOM
            {   
                sendErrorMessage(playerid, "Tymczasowo wyłączone!");
                return ShowTunePanel(playerid);
                //new type = GetPVarInt(playerid, "Tune_type");
        ...
        if(response)
        {
            ...
            switch(listitem)
            {
                ...
                default:
                {
                    ...
                    {
                        sendErrorMessage(playerid, sprintf("Musisz mieć %d skill mechanika!", listitem+1));
                    ...
                    if(listitem == 0) 
                    {
                        model = 1009; // 2x
                        cost = 1000;
                        format(tuneName, sizeof tuneName, "Nitro 2x");
                    }
                    else if(listitem == 1) 
                    {
                        model = 1008; // 5x
                        cost = 2000;
                        format(tuneName, sizeof tuneName, "Nitro 5x");
                    }
                    else if(listitem == 2)
                    {
                        model = 1010; // 10x
                        cost = 3000;
                        format(tuneName, sizeof tuneName, "Nitro 10x");
                    }
                    else return ShowTunePanel(playerid);
                    AddVehicleComponent(GetPVarInt(playerid, "Tune_vehicleid"), model);
                    AddTuneToRecipe(GetPVarInt(playerid, "Tune_giveplayerid"), tuneName, TUNE_TYPE_NITRO, cost, model, listitem, 0);
    ...
    else if (dialogid == D_TUNE_PANEL_UPGRADE)
    {
        if(!response) return ShowTunePanel(playerid);
        if(GetPVarInt(playerid, "Tune_check")) return ShowTunePanel(playerid);
        switch(listitem)
        {
            case 0: // Pancerz
            {
                ...
                {
                    sendErrorMessage(playerid, "Musisz mieć 4 skill mechanika!");
                    return ShowTunePanel(playerid);
                }
                AddTuneToRecipe(GetPVarInt(playerid, "Tune_giveplayerid"), "Pancerz karoserii", TUNE_TYPE_UPGRADE_ARMOR, 5000, -1001, 1500, 0);
            }
            case 1: // Szyby
            {
                if(PlayerInfo[playerid][pMechSkill] < 200)
                ...
                AddTuneToRecipe(GetPVarInt(playerid, "Tune_giveplayerid"), "Kuloodporne szyby", TUNE_TYPE_UPGRADE_WINDOWS, 5000, -1001, 1, 0);
            }
            //case 2: // Opony kuloodporne
            //{
            //    if(PlayerInfo[playerid][pMechSkill] < 200)
            //    {
            //        sendErrorMessage(playerid, "Musisz mieć 4 skill mechanika!");
            //        return ShowTunePanel(playerid);
            //    }
            //    AddTuneToRecipe(GetPVarInt(playerid, "Tune_giveplayerid"), "Kuloodporne opony", TUNE_TYPE_UPGRADE_BPROOFWHEELS, 5000, -1001, 1, 0);
            //}
        }
        return 1;
    }   
