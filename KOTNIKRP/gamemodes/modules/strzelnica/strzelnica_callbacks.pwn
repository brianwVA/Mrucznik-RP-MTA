...
hook OnGameModeInit()
{ 
    SetTimer("st_initPlaces", 100, false);
    return 1;
}
...
hook OnPlayerLeaveDynArea(playerid, area)
{
    if(IsValidDynamicArea(area))
    {
        ...
        if(extraid >= ST_CHECKSUM) //found
        {
            ...
            ShowPlayerDialogEx(playerid, D_STLEAVE, DIALOG_STYLE_MSGBOX, "Strzelnica", "Naciśnij 'OK' aby opuścić strzelnice i wrócić do poprzedniej pozycji\n{FF0000}UWAGA! TWÓJ SKILL NIE WZROŚNIE JEŻELI WYJDZIESZ", "OK", "-");
        }
    }
    return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case D_STLEAVE:
        {
            ...
            st_stop(playerid, "");
            sendErrorMessage(playerid, "Pamiętaj, że gdy wymuszasz wyjście ze strzelnicy twój skill nie wzrośnie.");
        }
        case D_STWEAPON:
        {
            if(!response)
            ...
            if(kaska[playerid] < price)
            {
                ...
                {
                    sendErrorMessage(seller, "Tego gracza nie stać na to!");
                }
            ...
            ShowPlayerDialogEx(playerid, D_STPAYMENT, DIALOG_STYLE_MSGBOX, "Strzelnica - Zakup wstępu", sprintf("{FFFFFF}Czy na pewno chcesz kupić wstęp na strzelnice za ${00FF00}%d{FFFFFF}?\nTwoja wybrana broń: {FF0000}%s", price, gunname), "Tak", "Nie");
        }
        case D_STPAYMENT:
        {
            if(!response)
            ...
            if(seller != INVALID_PLAYER_ID && IsPlayerConnected(seller))
            {
                ...
                va_SendClientMessage(seller, COLOR_BLUE, "SPRZEDAŻ: %s kupił od Ciebie wstęp na strzelnice. Otrzymujesz %d$", GetNick(playerid), money);
                Log(payLog, WARNING, "%s kupił od %s wstęp na strzelnice za %d$ (prowizja %d$)", GetPlayerLogName(playerid), GetPlayerLogName(seller), price, money);
