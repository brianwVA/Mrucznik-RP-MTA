...
command_strzelnica_Impl(playerid)
{
    ...
    {
        return sendErrorMessage(playerid, "Nie znajdujesz się na strzelnicy!");
    }
    foreach(new i : Player)
    {
        ...
        return va_SendClientMessage(playerid, COLOR_RED, "Nie możesz użyć tej komendy, ponieważ ktoś z biznesu strzelnicy jest na służbie! ID tej osoby: %s (%d)", GetNick(i), i);
    }
    if(st_getFreePlace() == -1)
        return sendErrorMessage(playerid, "Na strzelnicy aktualnie brak wolnych miejsc, wróć później jak się zwolni.");
    ...
    //Cena 2 razy wyższa gdy wstęp jest zakupowany poprzez komende
    ShowPlayerDialogEx(playerid, D_STWEAPON, DIALOG_STYLE_TABLIST_HEADERS, "Strzelnica - Wybór broni", weapons, "Dalej", "Wyjdź");
    return 1;
}
