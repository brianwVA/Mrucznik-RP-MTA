...
forward st_initPlaces();
public st_initPlaces()
{
    for(new i = 0; i < sizeof(st_spawn); i++)
    {
        stData[i][st_areaid] = CreateDynamicSphere(st_spawn[i][0], st_spawn[i][1], st_spawn[i][2], 0.8);
        new string[32];
        format(string, sizeof(string), "Stanowisko %d", i+1);
        CreateDynamic3DTextLabel(string, COLOR_WHITE, st_spawn[i][0], st_spawn[i][1], st_spawn[i][2], 1.8);
...
stock st_addSkillToWeapon(playerid, weaponid)
{
    ...
    switch(PlayerInfo[playerid][pWeaponSkill][skillid])
    {
        case 10, 20, 30, 40:
        {
            ...
            SendClientMessage(playerid, COLOR_BLUE, "** Pamiętaj, że możesz sprawdzić swoje umiejętności za pomocą komendy /wskill");
            return va_SendClientMessage(playerid, COLOR_YELLOW, "Twoje umiejętności używania broni %s wzrosły na %d level. Twój DMG z tej broni wzrósł do %.0f!", gunname, st_getPlayerWeaponSkill(playerid, weaponid), GetWeaponDamage(weaponid)*st_getWeaponMultiplier(playerid, weaponid));
        }
    }
    return 1;
}
...
stock st_stop(playerid, reason[] = "Koniec czasu.")
{
    ...
    format(string, sizeof(string), "{FFFFFF}Broń: {BA7229}%s\n{FFFFFF}Celne strzały: {BA7229}%d\n{FFFFFF}Niecelne strzały: {BA7229}%d\n{FFFFFF}Punkty: {BA7229}%d", gunname, accurate, misses, points);
...
stock st_start(playerid, weapon)
{
    ...
    if(id == -1 || id > MAX_ST)
    {
        return _MruGracz(playerid, "Brak wolnego miejsca na strzelnicy, wróć ponownie później!");
    }
...
stock st_createplayerTextDraw(playerid)
{
    ...
    PlayerTextDrawColour(playerid, strzelnica_textdraw[playerid], -1);
