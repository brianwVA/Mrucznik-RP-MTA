...
ptask stTimer[1000](playerid)
{
    ...
    if(stPlayer[playerid][stp_lefttime] <= 0)
    {
        ...
        {
            sendErrorMessage(playerid, "Niestety twoje umiejętności nie wzrosły. Masz zbyt dużo niecelnych strzałów/za mało punktów, popracuj nad tym!");
        }
        st_stop(playerid);
        return 1;
    }
    ...
    {
        ShowPlayerInfoDialog(playerid, "Strzelnica - Nieaktywność", "Za chwilę zostaniesz wyrzucony ze strzelnicy, jeżeli nie wykonasz żadnej czynności.");
        SetPVarInt(playerid, "stafkinfo", 1);
    }
    else if(lastshot >= 50 && stPlayer[playerid][stp_lastshot] > 0)
    {
        return st_stop(playerid, "Brak jakiejkolwiek aktywności.");
    }
