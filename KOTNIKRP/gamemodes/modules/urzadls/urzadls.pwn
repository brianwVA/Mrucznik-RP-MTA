...
//Opis:
/*
	Automatyzacja Urzędu Miasta w Los Santos - boty, skrypt botów, prawa jazdy [..] .
*/
...
CreateActorsInDMV(playerid)
{
	...
	{
		format(string, sizeof(string), "|____________Urząd Miasta otwarty_____________|");
		SendClientMessageToAll(COLOR_LIGHTGREEN, string);	
		return 1;
	}
	format(string, sizeof(string), "|____________Urząd Miasta otwarty przez %s_____________|", GetNick(playerid));
	SendClientMessageToAll(COLOR_LIGHTGREEN, string);
	return 1;
}
DestroyActorsInDMV()
...
{
	DmvLicenseCost[0] = 20;
	DmvLicenseCost[1] = 100;
	DmvLicenseCost[2] = 1000;
	DmvLicenseCost[3] = 120;
	DmvLicenseCost[4] = 130;
	DmvLicenseCost[5] = 140;
	DmvLicenseCost[6] = 120;
	DmvLicenseCost[7] = 7000;
