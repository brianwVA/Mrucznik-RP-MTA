...
new Float:Drwal_Lokacje[][3] =
{
	...
	{2351.5801, -653.5382, 128.0547}  // środek tartaku - donoszenie drzew
};
...
public Drwal_Timer_Actions(playerid, etap, time) // główny timer wszystkich czynności.
{
	...
	if(etap == 1)
	{
		...
		else
		{
			...
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Gratulacje! Udało Ci się ściąć drzewo");
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Teraz musisz je porąbać na mniejsze kawałki, wpisz {FFFFFF}/porab");
	...
	else if(etap == 2)
	{
		...
		else
		{
			...
			SendClientMessage(playerid, COLOR_GREEN, 	 "-------------- {FFFFFF}DRWAL {33AA33}--------------");
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Dobrze! Teraz musisz zanieść wszystkie kawałki do tartaku!");
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Aby to zrobić wpisz {FFFFFF}/podnies");
	...
	else if(etap == 4)
	{
		...
		else
		{
			...
			new string[128];
			SendClientMessage(playerid, COLOR_GREEN, 	 "-------------- {FFFFFF}DRWAL {33AA33}--------------");
			format(string, sizeof(string), "Zebrałeś już {FFFFFF}%d/3 {00C2EC}drzew!", DrzewoStage[playerid]);
			...
			if(DrzewoStage[playerid] >= 3)
			{
				...
				format(string, sizeof(string), "Udało Ci się wykonać zadanie! W nagrodę otrzymujesz {FFFFFF}$%d{00C2EC}!", finalPrice);
				SendClientMessage(playerid, COLOR_LIGHTBLUE, string);
...
Drwal_Start(playerid) // zaczyna działanie systemu po zaakceptowaniu dialogu.
{
	SendClientMessage(playerid, COLOR_GREEN, 	 "-------------- {FFFFFF}DRWAL {33AA33}--------------");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Na twoim radarze pojawił się znacznik w którym znajduje się drzewo do ścięcia.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Udaj się tam i wykonaj swoje zadanie! Aby ściąć drzewo wpisz {FFFFFF}/zetnij {00C2EC}przy drzewie.");
...
Drwal_End(playerid) // konczy dzialanie systemu i przywraca domyślne wartości. Warto dodać do OnPlayerDisconnect.
{
	DestroyDynamicMapIcon(DrzewoIcon[playerid]);
	DestroyDynamicObject(DrzewoObject[playerid][0]);
...
hook OnGameModeInit() // dodać do OnGameModeInit
{
	Create3DTextLabel("Praca Drwala\nWpisz /drwal aby pracować", COLOR_LIGHTBLUE, Drwal_Lokacje[0][0], Drwal_Lokacje[0][1], Drwal_Lokacje[0][2]+0.5000, 20, 0, 0);
}
	...
	if(IsPlayerConnected(playerid))
	{
		if(DrzewoStage2[playerid] == 1)
		{
			if(DrzewoWykonuje[playerid] == 0)
			{
				...
				} else return sendErrorMessage(playerid, "Jesteś za daleko!");
			} else return sendErrorMessage(playerid, "Już wykonujesz tą czynność!");
		}
	}
	return 1;
}
...
YCMD:drwal(playerid, params[])
{
	if(IsPlayerConnected(playerid))
	{
		...
		{	
			if(DrzewoStage2[playerid] == 0)
			{
				ShowPlayerDialogEx(playerid, D_DRWAL, DIALOG_STYLE_MSGBOX, "Praca drwala", "Twoim zadaniem jest ścięcie drzewa, a następnie przeniesienie go do tartaku", "Akceptuj", "Anuluj");
			} else return sendErrorMessage(playerid, "Wykonujesz już pracę drwala!");
		}
	}
	return 1;
}
...
YCMD:gotodrwal(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] > 0)
	{
		...
		return 1; 
	} else return sendErrorMessage(playerid, "Brak uprwanień!");
}