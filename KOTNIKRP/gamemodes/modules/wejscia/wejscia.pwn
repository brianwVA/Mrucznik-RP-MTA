...
DodajWejscie(Float:fx1, Float:fy1, Float:fz1, Float:fx2, Float:fy2, Float:fz2, vw1=0, int1=0, vw2=0, int2=0, nazwain[]="", nazwaout[]="", wejdzUID=0, playerLocal=255, bool:specialCome=false)
{
	...
	if(isnull(nazwaout)) 
	{
		CreateDynamicPickup(1239, 2, fx2, fy2, fz2, vw2, int2);
	}
	else 
	{
...
Sprawdz_UID_Wchodzenie(playerid, Check_ID)
{
	...
	{
		//tutaj ewentualne link do muzyki - odkomentować niżej
		//new muzik[128];
	...
	else if(Check_ID == 7)//Wejscie do VINYL
	{
		if(vinylStatus == 0)
		{
			...
			{
				sendErrorMessage(playerid, "Klub jest teraz zamknięty!"); 
				return 1;
			}
		}
		else
	...
	else if(Check_ID == 14)
	{
		...
		sendTipMessageEx(playerid, COLOR_P@, "Włodarze:");
	...
	else if(Check_ID == 15)
	{
		...
		sendTipMessage(playerid, "Zapraszamy na rzeź!");

	}
	else if(Check_ID == 16)//Więzienie stanowe - wejście i wyjście
	{
		if(!IsAPolicja(playerid, 0) && !IsABOR(playerid, 0) && PlayerInfo[playerid][pJob] != 2 && !CheckPlayerPerm(playerid, PERM_LAWYER))
		{
			SendClientMessage(playerid, COLOR_WHITE, "Simon_Mrucznikov mówi: Zaraz zaraz kolego! A ty gdzie? Nie możesz tu wejść!"); 
			noAccessCome[playerid] = 1;
			return 1;
		}
	...
	else if(Check_ID == 20)
	{
		if(GunshopLSLock == 1)
		{
			sendTipMessage(playerid, "Drzwi są zamknięte.");
			noAccessCome[playerid] = 1;
			return 1;
		}
	}
	...
	else if(Check_ID == 45) //AS Pizzeria
	{
		...
		sendTipMessage(playerid, "Jeżeli w restauracji nikogo nie ma, wpisz /kupjedzenie przy ladzie.");
	}
	return 0; 
}
Sprawdz_UID_Wychodzenie(playerid, Check_ID)
{
	...
	else if(Check_ID == 16)//Więzienie stanowe - główne wejście
	{
		if(PlayerInfo[playerid][pJailed] > 0)
		{
			SendClientMessage(playerid, COLOR_WHITE, "Pavlo_Rudovy mówi: Koleszko? Nie pomyliło Ci się coś? Wracaj do celi!!"); 
			noAccessCome[playerid] =1;
			return 1;
		}
		GameTextForPlayer(playerid, "~w~by~n~Simeone & Rozalka", 5000, 1);
			...
			if(IsPlayerInAnyVehicle(playerid))
			{
				...
				foreach(new i2 : Player)
				{
					if(GetPlayerVehicleID(i2) == pVehAcID && GetPlayerVehicleSeat(i2) != 0)
					{
						...
						sendTipMessage(playerid, "Zmienianie pogody - pomyślnie wykonano!");
					}
				}
			}
			else
		...
		for(new i; i<valueWjedz; i++)
		{
			if(IsPlayerInRangeOfPoint(playerid, wjazdy[i][RangeofPoint], wjazdy[i][wj_X], wjazdy[i][wj_Y], wjazdy[i][wj_Z]))//Wejście
			{
				if(GetPVarInt(playerid, "JestPodczasWjezdzaniaPasazer") == 1)
				{
					...
					sendTipMessage(playerid, "Wykonano wjedź dla pasażera"); 
			...
			else if(IsPlayerInRangeOfPoint(playerid, wjazdy[i][RangeofPoint], wjazdy[i][wy_X], wjazdy[i][wy_Y], wjazdy[i][wy_Z]))//Wejście
			{
				if(GetPVarInt(playerid, "JestPodczasWjezdzaniaPasazer") == 1)
				{
					...
					SetPVarInt(playerid, "CodeACDisable", 0);
					SetAntyCheatForPlayer(playerid, 0);
					sendTipMessage(playerid, "Wykonano wyjedź dla pasażera!"); 
					timeSecWjedz[playerid] = 0; 
					return 1;
				}
