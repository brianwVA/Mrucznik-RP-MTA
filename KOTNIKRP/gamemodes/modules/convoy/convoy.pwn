...
StartConvoy(playerid, vehicleid)
{
	...
	foreach(new i : Player)
	{
		...
		{
			SendClientMessage(i, COLOR_ADD, sprintf("%s rozpoczął konwój. Chroń dostawę, aby dowieść cały ładunek!", GetNick(playerid)));
		}
		else
		{
			SendClientMessage(i, COLOR_ADD, sprintf("%s rozpoczął przewóz dyń z narkotykami!", GetNick(playerid)));
			SendClientMessage(i, COLOR_ADD, "Jedź do czerwonego markera i ostrzelaj konwój aby zdobyć łup!");
		}
	}
}
...
StopConvoy(CONVOY_STOP_REASONS:reason)
{
	...
	switch(reason)
	{
		...
		{
			SendClientMessageToAll(COLOR_ADD, "Konwój z dyniami został rozbity! Los Santos znowu jest bezpieczne.");
		}
		case CONVOY_STOP_ADMIN:
		{
			SendClientMessageToAll(COLOR_ADD, "Konwój z dyniami został przerwany przez admina.");
		}
	}
	return 1;
}
...
timer AfterDropBox[500](playerid, boxid, Float:x, Float:y, Float:z, Float:angle)
{
	...
	{
		ChatMe(playerid, "wrzuca dynię spowrotem do konwojowego furgonu.");
	...
	if(actorid != -1 && !IsInAConvoyTeam(playerid))
	{
		...
		SendClientMessage(playerid, COLOR_LIGHTBLUE, sprintf("Gratulacje, dostarczyłeś dynię z konwoju. Otrzymujesz %d$ i %dMC.", money, mc));
		DajKase(playerid, money);
		Log(payLog, WARNING, "Gracz %s otrzymał %d$ za dynie", 
			GetPlayerLogName(playerid), money
		);
		DajMC(playerid, mc);
		Log(premiumLog, WARNING, "Gracz %s otrzymał %dMC za dynie", 
			GetPlayerLogName(playerid), mc
		);

		DestroyBox(boxid);
		ActorChat(actorid, CONVOY_ACTOR_NAME, "dzięki, oto twoja działka.");
