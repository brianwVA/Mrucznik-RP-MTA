...
stock IbizaPanelCheck(){
	...
	if(ibizaStrobes == false){
		format(txt_ibiza_strobes, 128, "{FFFFFF}Stroboskopy: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_strobes, 128, "{FFFFFF}Stroboskopy: \t{00FF00}Włączone\n");
	}
	if(ibizaNeons == false){
		format(txt_ibiza_neons, 128, "{FFFFFF}Neony: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_neons, 128, "{FFFFFF}Neony: \t{00FF00}Włączone\n");
	}
	if(ibizaLights == false){
		format(txt_ibiza_lights, 128, "{FFFFFF}Oświetlenie: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_lights, 128, "{FFFFFF}Oświetlenie: \t{00FF00}Włączone\n");
	}
	if(ibizaSmokes == false){
		format(txt_ibiza_smokes, 128, "{FFFFFF}Dym: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_smokes, 128, "{FFFFFF}Dym: \t{00FF00}Włączone\n");
	}
	if(ibizaTextOne == false){
		format(txt_ibiza_txtOne, 128, "{FFFFFF}Tekst #1: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_txtOne, 128, "{FFFFFF}Tekst #1: \t{00FF00}Włączone\n");
	}
	if(ibizaTextTwo == false){
		format(txt_ibiza_txtTwo, 128, "{FFFFFF}Tekst #2: \t{FF0000}Wyłączone\n");
	}else{
		format(txt_ibiza_txtTwo, 128, "{FFFFFF}Tekst #2: \t{00FF00}Włączone\n");
	}
...
stock IbizaNadajBilet(playerid, giveplayerid, bilet)
{
	...
	{
		sendTipMessage(playerid, "Jesteś za daleko.");
		return 1;
	}
	if(bilet == 1) nazwa_biletu = "NORMAL";
	if(bilet == 2) nazwa_biletu = "VIP";
	if(bilet == 3) nazwa_biletu = "SUPERVIP";
	format(var, sizeof(var), "Dałeś bilet %s dla %s", nazwa_biletu, GetNick(giveplayerid));
    sendTipMessageEx(playerid, COLOR_LIGHTBLUE, var);
    format(var, sizeof(var), "Otrzymałeś bilet %s od %s", nazwa_biletu, GetNick(playerid));
