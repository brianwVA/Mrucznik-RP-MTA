			...
			switch(listitem){
				case 0:{
					if(kaska[playerid] >= ibiza_priceNormal && ibiza_priceNormal != 0){
						...
						SendClientMessage(playerid, -1, "Kasjerka mówi: Dziękujemy za zakup biletu!");
						Sejf_Add(FAMILY_IBIZA, ibiza_priceNormal);
						Log(payLog, WARNING, "%s kupił bilet normalny (Ibiza) za $ %d", GetPlayerLogName(playerid), ibiza_priceNormal);
					}else{
						if(ibiza_priceNormal == 0) SendClientMessage(playerid, -1, "Klub jest zamknięty.");
						else SendClientMessage(playerid, -1, "Nie masz wystarczającej ilości pieniędzy!");
					}
				}
				case 1:{
					if(kaska[playerid] >= ibiza_priceVIP && ibiza_priceVIP != 0){
						...
						SetPlayerWeather(playerid, 27);
						SendClientMessage(playerid, -1, "Kasjerka mówi: Dziękujemy za zakup biletu VIP!");
						Sejf_Add(FAMILY_IBIZA, ibiza_priceVIP);
						Log(payLog, WARNING, "%s kupił bilet VIP (Ibiza) za $ %d", GetPlayerLogName(playerid), ibiza_priceNormal);
					}else{
						if(ibiza_priceVIP == 0) SendClientMessage(playerid, -1, "Klub jest zamknięty.");
						else SendClientMessage(playerid, -1, "Nie masz wystarczającej ilości pieniędzy!");
					}
				}
			...
			switch(listitem){
				...
				case 4:{
					if(ibizaTextOne == false){
						ibizaTextOne = true;
						ShowPlayerDialogEx(playerid, DIALOG_IBIZA_TEXT_ONE, DIALOG_STYLE_INPUT, "IbizaClub - Ustaw tekst #1", "Wpisz tekst", "Zatwierdź", "Anuluj");
					}else{
						ibizaTextOne = false;
						ibizaTextOneOff();
					}
				...
				case 5:{
					if(ibizaTextTwo == false){
						ibizaTextTwo = true;
						ShowPlayerDialogEx(playerid, DIALOG_IBIZA_TEXT_TWO, DIALOG_STYLE_INPUT, "IbizaClub - Ustaw tekst #2", "Wpisz tekst", "Zatwierdź", "Anuluj");
					}else{
						ibizaTextTwo = false;
						ibizaTextTwoOff();
					}
				...
				case 6:{
					ShowPlayerDialogEx(playerid, DIALOG_IBIZA_NORMAL_TICKET, DIALOG_STYLE_INPUT, "IbizaClub - Normalny bilet", "Wpisz cenę biletu normalnego\nMax 80 $", "Zatwierdź", "Anuluj");
				}
				case 7:{
					ShowPlayerDialogEx(playerid, DIALOG_IBIZA_VIP_TICKET, DIALOG_STYLE_INPUT, "IbizaClub - Normalny bilet", "Wpisz cenę biletu VIP\nMax 200 $", "Zatwierdź", "Anuluj");
				}
			}
		}
	}
