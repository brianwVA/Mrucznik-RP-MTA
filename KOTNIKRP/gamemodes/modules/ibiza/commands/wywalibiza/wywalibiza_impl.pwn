...
{
    if(IsPlayerInGroup(playerid, 18, 1) && GroupPlayerDutyRank(playerid) >= 2) 
    {
        new Float:x,
            Float:y,
            Float:z;
        ...
        if(IsPlayerInRangeOfPoint(playerid, 3.0, x, y, z))
        {
            if(GetPlayerVirtualWorld(giveplayerid) == 21 || GetPlayerVirtualWorld(giveplayerid) == 22 || GetPlayerVirtualWorld(giveplayerid) == 23 || GetPlayerVirtualWorld(giveplayerid) == 24 || GetPlayerVirtualWorld(giveplayerid) == 26 || GetPlayerVirtualWorld(giveplayerid) == 27)
            {
                ...
                format(var, sizeof(var), "[IBIZA_INFO]%s wyrzucił %s z ibizy!", GetNick(playerid), GetNick(giveplayerid));
                GroupSendMessage(18, COLOR_P@, var);
        ...
        {
            sendTipMessage(playerid, "Nie znajdujesz się obok niego.");
        }
    }
    else
    {
