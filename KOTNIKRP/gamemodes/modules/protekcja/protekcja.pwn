...
public DisableProtection(playerid)
{
    ...
    SendClientMessage(playerid, COLOR_RED, "Ochrona przed obrażeniami zostala wylaczona.");
    return 1;
}
...
hook OnPlayerDamage(&playerid, &issuerid)
{
    ...
    {
        SendClientMessage(playerid, COLOR_YELLOW, "Nie możesz umrzeć podczas 20-sekundowej ochrony!");
        SendClientMessage(issuerid, COLOR_YELLOW, "Twój cel jest chroniony i nie możesz go zranić!");
        return 0;
    }
    return 1;
}
...
YCMD:usunalldym(playerid, params[])
{
    ...
    for(new smokeid = 0; smokeid < sizeof(PlantedSmokes); smokeid++)
    {
        ...
        va_SendClientMessage(playerid, -1, "Usunąłeś zadymiarkę ID: %d", smokeid+1);
    }
    return 1;
}
...
YCMD:usunallflara(playerid, params[])
{
    ...
    for(new flareid = 0; flareid < sizeof(PlantedFlares); flareid++)
    {
        ...
        va_SendClientMessage(playerid, -1, "Usunąłeś flarę ID: %d", flareid+1);
    }
    return 1;
}
