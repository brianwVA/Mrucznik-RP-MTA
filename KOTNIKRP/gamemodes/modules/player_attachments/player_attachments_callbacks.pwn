	...
	if(AttachedObjects[playerid][index][ao_active] == true)
	{
		if(response)
		{
			if(!CheckEditionBoundaries(playerid, fOffsetX, fOffsetY, fOffsetZ, fScaleX, fScaleY, fScaleZ))
			{
				...
				//TODO: jeżeli gracz kliknie wyjdz 2 razy w krótkim czasie, nie właczy mu się edycja
				return -2;
			}

			SendClientMessage(playerid, COLOR_GREEN, "Edytowałeś pozycję swojego przedmiotu.");
		...
		{
			SendClientMessage(playerid, COLOR_RED, "Anulowałeś edycję pozycję swojego przedmiotu.");
...
attachemnts_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_PRZEDMIOTYGRACZA)
	{
		if(response)
		{
			...
			if(HasPlayerActiveAttachedObject(playerid, item))
			{
				...
				sendErrorMessage(playerid, "Masz już przyczepiony ten obiekt.");
				return 1;
			}

			if(GetFreeAttachedObjectSlot(playerid) == INVALID_ATTACHED_OBJECT_INDEX)
			{
				sendErrorMessage(playerid, "Masz zbyt dużo założonych przedmiotów. Użyj komendy /zdejmij aby zdjąć jakiś przedmiot.");
				return 1;
			}
			...
			{
				sendErrorMessage(playerid, "Błąd systemu doczepianych obiektów! Zgłoś okoliczności na forum");
				return 1;
			}

			ShowPlayerDialogEx(playerid, DIALOG_PRZEDMIOTYGRACZA_EDYCJA, DIALOG_STYLE_MSGBOX, "Przedmioty - edycja", "Czy chcesz edytować pozycję przedmiotu?", "Tak", "Nie");
	...
	if(dialogid == DIALOG_PRZEDMIOTYGRACZA_KOSC)
	{
		if(response)
		{
			...
			EditAttachedObject(playerid, index);
			sendTipMessage(playerid, "Wybierz pozycję obiektu.");
		}
		return 1;
	}
	...
	if(dialogid == DIALOG_PRZEDMIOTYGRACZA_ZDEJMIJ_ADMIN)
	{
		if(response)
		{
			...
			SendClientMessage(giveplayerid, COLOR_RED, sprintf("Administrator %s zdjął twój obiekt, popraw jego ułożenie, inaczej możesz otrzymać karę.", GetNickEx(playerid)));
			sendTipMessage(playerid, sprintf("Zdjąłeś obiekt graczowi %s", GetNick(giveplayerid)));
		}
		return 1;
	}
	return 0;
