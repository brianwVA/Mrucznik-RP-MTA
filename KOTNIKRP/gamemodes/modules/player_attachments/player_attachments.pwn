...
CheckEditionBoundaries(playerid, Float:x, Float:y, Float:z, Float:sx, Float:sy, Float:sz)
{
	...
	if(0.5 > sx || sx > 1.5) 
	{
		sendErrorMessage(playerid, "Ten obiekt jest za duży w skali X!");
		return 0;
	} 
	if(0.5 > sy || sy > 1.5) 
	{
		sendErrorMessage(playerid, "Ten obiekt jest za duży w skali Y!");
		return 0;
	} 
	if(0.5 > sz || sz > 1.5) 
	{
		sendErrorMessage(playerid, "Ten obiekt jest za duży w skali Z!");
		return 0;
	} 
	return 1;
}
...
DialogAdminRemoveAttachedItems(playerid, giveplayerid)
{
	...
	if(count==0) return sendErrorMessage(playerid, "Ten gracz nie ma założonych przedmiotów.");
...
DialogBoneSelect(playerid, bone=0)
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_PRZEDMIOTYGRACZA_KOSC, DIALOG_STYLE_LIST, "Wybierz gdzie przyczepić przedmiot.", 
		string, 
		"Przyczep",
		"Wyjdź"
	);
