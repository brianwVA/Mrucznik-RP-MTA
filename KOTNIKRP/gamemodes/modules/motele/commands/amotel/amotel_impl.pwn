...
command_amotel_Impl(playerid, action[64], value[256])
{
    ...
    if(motelID != -1)
    {
        ...
        format(string2, sizeof(string2), "» Nazwa (%s)\n» Cena ({11d625}$%d)\n» Ilość pokoi (%d/%d)\n» Zmień interior\n» Usuń", Motels[motelID][motName], Motels[motelID][motPrice], Motels[motelID][motOccupied], Motels[motelID][motRooms]);
        ...
        if(strfind(action, "crea", true) != -1 || strfind(action, "stw", true) != -1) {
            ...
            {
                sendTipMessage(playerid, "Użyj /amotel stworz [InX] [InY] [InZ] [InVW] [InInterior] [Cena] [Pokoje] [Nazwa]");
