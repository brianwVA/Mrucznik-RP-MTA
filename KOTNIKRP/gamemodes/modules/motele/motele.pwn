...
// UPDATE `mru_konta` SET `Money`=`Money`-200 WHERE `UID` in (1, 3) // przykładowe niwelowanie hajsu graczy offline o 8:00

// VW pokoi: mtrUID + 13000

//-----------------<[ Funkcje: ]>-------------------

/* Funkcje zarządzania, wchodzenia gracza */
...
{
	new motelID = Motel_GetCurrent(playerid, false, false); // Sprawdza czy gracz jest w motelu przy drzwiach wyjściowych
	if(motelID != -1)
	{
...
Motel_Room_Wejdz(playerid, roomID)
{
	...
	if(motelID != -1)
	{
		...
		// Gdy gracz nie ma dostępu, drzwi są zamknięte lub nie jest właścicielem wyświetla się komunikat o zamkniętych drzwiach
		if(!access && !MotelRooms[roomID][mtrDoors] && MotelRooms[roomID][mtrOwnerUID] != PlayerInfo[playerid][pUID]) return Motel_ShowError(playerid, motelID, "Drzwi do tego pokoju są zamknięte, a Ty nie masz dostępu.");
...
Motel_Room_CheckWyjdz(playerid)
{
	...
	if(roomUID != -1 && roomID != -1) // Znaleziono UID pokoju i ID w tablicy, można przejść dalej
	{
...
Motel_Rent(playerid, UID, roomNum, roomInterior)
{
	...
	if(motelID != -1)
	{
		...
		if(MotelRooms[roomID][mtrUID] == 0) // Pokój jeszcze nie jest stworzony w bazie, więc system go tworzy
		{
			MotelRooms[roomID][mtrUID] = MruMySQL_CreateMotelRoom(roomID);
		}
		else // Pokój już istnieje w bazie i ma swoje UID, więc tylko zapisuje
		{
		...
		SendClientMessage(playerid, -1, "Właśnie wynająłeś pokój w motelu. Opłata będzie pobierana raz dziennie o godzinie 8:00.");
		SendClientMessage(playerid, -1, "Pamiętaj by mieć środki na swoim koncie bankowym, inaczej zostaniesz wyeksmitowany!");
		Log(moteleLog, WARNING, "%s wynajął pokój %d w motelu %d", GetPlayerLogName(playerid), MotelRooms[roomID][mtrUID], Motels[motelID][motUID]);
		...
		// Zabiera kaske za 'wzięcie klucza'
		ZabierzKaseDone(playerid, Motels[motelID][motPrice]);

		GameTextForPlayer(playerid, "~w~Witamy w twoim nowym pokoju", 5000, 1);
	} else return SendClientMessage(playerid, COLOR_RED, "WYSTĄPIŁ BŁĄD Z WYNAJMEM POKOJU (NIEPRAWIDŁOWE ID MOTELU)"); // Prawdopodobnie nie występujący błąd
	return 1;
}

Motel_RoomDestroy(roomID)
{
	Log(moteleLog, WARNING, "Pokój %d został zniszczony", MotelRooms[roomID][mtrUID]);
...
Motel_Unrent(playerid, reason = 0, offline = 0)
{
	...
	Log(moteleLog, WARNING, "%s zrezygnował z wynajmu pokoju %d z powodu %d (offline: %d)", GetPlayerLogName(playerid), PlayerInfo[playerid][pMotRoom], reason, offline);
	if(!offline)
	{
		// Ustawianie danych dot. motelu w tablicy gracza
	...
	if(reason == 0)
	{
		...
		SendClientMessage(playerid, -1, string);
		SendClientMessage(playerid, -1, "Właśnie zrezygnowałeś z wynajmu pokoju w tym motelu. Do zobaczenia następnym razem!");
	}
	else if(reason == 1)
	{
		if(motelID != -1 || motelID == 0) format(string, sizeof(string), "{e8c205}| -------------- [ {FFFFFF}%s{e8c205} ] -------------- |", Motels[motelID][motName]);
		...
		SendClientMessage(playerid, -1, string);
		SendClientMessage(playerid, -1, "Zostałeś eksmitowany, ponieważ motel w którym miałeś pokój został usunięty.");
	}
	else if(reason >= 2)
	{
		if(motelID != -1 || motelID == 0) format(string, sizeof(string), "{e8c205}| -------------- [ {FFFFFF}%s{e8c205} ] -------------- |", Motels[motelID][motName]);
		...
		SendClientMessage(playerid, -1, string);
		SendClientMessage(playerid, -1, "Zostałeś eksmitowany, ponieważ nie masz pieniędzy aby zapłacić za pokój.");
		if(reason > 2) // W reason zapisywana jest też wartość mtrPayOffline gdy pokój zostaje zniszczony.
		{
			format(string, sizeof(string), "Dodatkowo z twojego konta zostało pobrane łącznie {11d625}$%d{FFFFFF} gdy byłeś offline.", reason-2);
			SendClientMessage(playerid, -1, string);
		}
	}
	else if(reason == -1)
	{
		...
		SendClientMessage(playerid, -1, string);
		SendClientMessage(playerid, -1, "Zostałeś eksmitowany, ponieważ twoje konto było zablokowane.");
	}
	else if(reason == -2)
	{
		if(motelID != -1 || motelID == 0) format(string, sizeof(string), "{e8c205}| -------------- [ {FFFFFF}%s{e8c205} ] -------------- |", Motels[motelID][motName]);
		...
		SendClientMessage(playerid, -1, string);
		SendClientMessage(playerid, -1, "Zostałeś eksmitowany, ponieważ twoje konto nie było zalogowane od 7 dni.");
	}

	// Wyrzuca wszystkich z pokoju jeśli są w nim
	if(!offline)
	{
...
{
	if(motelID != -2) return ShowPlayerInfoDialog(playerid, sprintf("{e8c205} %s {ff0000}» Błąd", Motels[motelID][motName]), string); // Wyświetla błąd
	else return ShowPlayerInfoDialog(playerid, "{e8c205} Kreator moteli {ff0000}» Błąd", string); // Wyświetla błąd
}

Motel_GetCurrent(playerid, outside, both = false, inside_everywhere = false, inside_room = false) // Sprawdza przy/w jakim motelu gracz się znajduje
{
	for(new i = 0; i < MAX_MOTELS; i++)
    {
        if(Motels[i][motUID] != 0)
		{
			if ((!outside || both) && inside_everywhere) // Działa wszędzie we wnętrzu motelu
			{
			...
			if(outside || both) // Działa przy wejściu do motelu ( na zewnątrz )
			{
			...
			else if (!outside && !both) // Działa przy wyjściu do motelu ( w środku )
			{
...
Motel_Create(playerid, Float:InX, Float:InY, Float:InZ, InVW, InInterior, Price, Rooms, MotelName[64])
{
	...
	if(motelID != -1) return sendErrorMessage(playerid, "W tym miejscu nie można stworzyć motelu!");
	...
	if(Motels[motelID][motUID] != 0)
	{
		...
		Motels[motelID][motInText] = CreateDynamic3DTextLabel("Wyjście", 0x008080FF, Motels[motelID][motInX], Motels[motelID][motInY], Motels[motelID][motInZ]+0.3, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, Motels[motelID][motInVW], Motels[motelID][motInInt]);
		sendTipMessage(playerid, "Pomyślnie stworzono motel!");
		Log(adminLog, WARNING, "%s stworzył nowy motel %s [%d]", GetPlayerLogName(playerid), Motels[motelID][motName], Motels[motelID][motUID]);
	} else sendErrorMessage(playerid, "Nie udało się stworzyć motelu... (Błąd MySQL)");
	return 1;
}
...
Motel_Destroy(playerid, motelID)
{
	...
	Log(adminLog, WARNING, "%s usunął motel %s [%d] wraz ze wszystkimi pokojami", GetPlayerLogName(playerid), tempMotelName, tempMotelUID);
	return 1;
}
	...
	for(new i = 1; i < MAX_MOTEL_ROOMS; i++)
	{
		if(MotelRooms[i][mtrUID] != 0 && MotelRooms[i][mtrMotUID] != 0)
		{
			if(MotelRooms[i][mtrOwnerUID] != 0)
			{
				...
				if(motelID == -1)
				{
					Log(moteleLog, WARNING, "Motel %d nie istnieje, UID %d został eksmitowany!", MotelRooms[i][mtrOwnerUID]);
					MruMySQL_MotelSetEvictState(MotelRooms[i][mtrOwnerUID], 1);
				}
				else
				{
					...
					for(new j = 0; j < MAX_PLAYERS; j++)
					{
						if(IsPlayerConnected(j) && gPlayerLogged[j])
						{
							if(PlayerInfo[j][pUID] == MotelRooms[i][mtrOwnerUID])
							{
								...
								{
									SetPlayerBank(j, PlayerInfo[j][pAccount]-Motels[motelID][motPrice]);
									format(string, sizeof(string), "{e8c205}| -------------- [ {FFFFFF}%s{e8c205} ] -------------- |", Motels[motelID][motName]);
									SendClientMessage(j, -1, string);
									format(string, sizeof(string), "Z twojego konta zostało pobrane {11d625}$%d{FFFFFF}.", Motels[motelID][motPrice]);
									SendClientMessage(j, -1, string);
									Log(payLog, WARNING, "%s zapłacił $%d za pokój %d w motelu %d", GetPlayerLogName(j), Motels[motelID][motPrice], MotelRooms[PlayerInfo[j][pMotRoom]][mtrUID], Motels[motelID][motUID]);
									break;
								}
								else
								{
									Log(moteleLog, WARNING, "UID %d nie miał pieniędzy by zapłacić za pokój %d w motelu %d", MotelRooms[i][mtrOwnerUID], MotelRooms[i][mtrUID], Motels[motelID][motUID]);
									Motel_Unrent(j, 2);
									break;
								}
							}
					...
					if(!found)
					{
						...
						if(!evicted)
						{
							...
							{
								Log(moteleLog, WARNING, "UID %d jest zbanowany i został eksmitowany z pokoju %d w motelu %d", MotelRooms[i][mtrOwnerUID], MotelRooms[i][mtrUID], Motels[motelID][motUID]);
								MruMySQL_MotelSetEvictState(MotelRooms[i][mtrOwnerUID], -1);
								Motel_RoomDestroy(i);
							}
							else
							{
								if(MruMySQL_GetAccInt("Bank", MruMySQL_GetNameFromUID(MotelRooms[i][mtrOwnerUID])) < Motels[motelID][motPrice]) {
									Log(moteleLog, WARNING, "UID %d jest offline i nie ma pieniędzy by zapłaić, został eksmitowany z pokoju %d w motelu %d", MotelRooms[i][mtrOwnerUID], MotelRooms[i][mtrUID], Motels[motelID][motUID]);
									MruMySQL_MotelSetEvictState(MotelRooms[i][mtrOwnerUID], MotelRooms[i][mtrPayOffline]+2);
									Motel_RoomDestroy(i);
								} else {
									Log(moteleLog, WARNING, "UID %d jest offline, zapłacił $%d za pokój %d w motelu %d", MotelRooms[i][mtrOwnerUID], Motels[motelID][motPrice], MotelRooms[i][mtrUID], Motels[motelID][motUID]);
			...
			{
				Log(moteleLog, WARNING, "Motel %d nie ma właściciela, usuwam",  MotelRooms[i][mtrUID]);
				Motel_RoomDestroy(i);
			}
		}
	}
