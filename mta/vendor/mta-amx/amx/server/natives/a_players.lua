...
function SetPlayerSkin(amx, player, skin)
	local model, customSkin = mrpResolveSkin(g_SkinReplace[skin] or skin)
	local skinset = setElementModel(player, model)
	if skinset then
		mrpApplyCustomSkin(player, customSkin)
...
function GetPlayerSkin(amx, player)
	local customSkin = getElementData(player, 'mrp:customSkin')
	if customSkin then return customSkin end
...
function PlayCrimeReportForPlayer(amx, player, suspectid, crimeid)
	...
	clientCall(player, 'PlayCrimeReportForPlayer', crimeid)
	return true
end
...
function SetPlayerAttachedObject(amx, player, index, modelid, bone, fOffsetX, fOffsetY, fOffsetZ, fRotX, fRotY, fRotZ, fScaleX, fScaleY, fScaleZ, materialcolor1, materialcolor2)
	...
	local runtimeModel, customModel = mrpResolveObjectModel(modelid)
	...
	if obj then
		...
		local attachment = {
			index = index, model = modelid, bone = bone,
			x = fOffsetX, y = fOffsetY, z = fOffsetZ,
			rx = fRotX, ry = fRotY, rz = fRotZ,
			sx = fScaleX, sy = fScaleY, sz = fScaleZ,
			materialcolor1 = materialcolor1, materialcolor2 = materialcolor2
		}
		setElementData(obj, 'amx:attachedObject', attachment, false)
		if customModel then
			setElementData(obj, 'mrp:customObjectModel', customModel)
		end
...
function EditAttachedObject(amx, player, index)
	if not player or index < 0 or index > 9 then return false end
	local playerID = getElemID(player)
	local obj = g_Players[playerID] and g_Players[playerID].attachedObjects[index]
	if not isElement(obj) then return false end
	local attachment = getElementData(obj, 'amx:attachedObject')
	if type(attachment) ~= 'table' then return false end
	g_AttachmentEditors = g_AttachmentEditors or {}
	g_AttachmentEditors[player] = table.deepcopy(attachment)
	clientCall(player, 'EditAttachedObject', attachment)
	return true
end

local function applyAttachmentEdit(player, index, values)
	local playerID = getElemID(player)
	local obj = playerID and g_Players[playerID] and g_Players[playerID].attachedObjects[index]
	if not isElement(obj) or type(values) ~= 'table' then return false end
	local attachment = getElementData(obj, 'amx:attachedObject')
	if type(attachment) ~= 'table' then return false end
	local names = { 'x', 'y', 'z', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz' }
	for _, name in ipairs(names) do
		local value = tonumber(values[name])
		if not value or value ~= value or math.abs(value) > 10000 then return false end
		attachment[name] = value
	end
	if attachment.sx <= 0 or attachment.sy <= 0 or attachment.sz <= 0 then return false end
	detachElementFromBone(obj)
	setObjectScale(obj, attachment.sx, attachment.sy, attachment.sz)
	attachElementToBone(obj, player, g_BoneMapping[attachment.bone], attachment.x, attachment.y, attachment.z,
		attachment.rx, attachment.ry, attachment.rz)
	setElementData(obj, 'amx:attachedObject', attachment, false)
	return attachment
end

addEvent('amx:onAttachmentEditPreview', true)
addEventHandler('amx:onAttachmentEditPreview', resourceRoot, function(index, values)
	if not client or source ~= resourceRoot then return end
	applyAttachmentEdit(client, tonumber(index), values)
end)

addEvent('amx:onAttachmentEditFinish', true)
addEventHandler('amx:onAttachmentEditFinish', resourceRoot, function(index, response, values)
	if not client or source ~= resourceRoot then return end
	index = tonumber(index)
	local original = g_AttachmentEditors and g_AttachmentEditors[client]
	if not original or original.index ~= index then return end
	local attachment
	if response then
		attachment = applyAttachmentEdit(client, index, values)
	else
		attachment = applyAttachmentEdit(client, index, original)
	end
	if not attachment then return end
	g_AttachmentEditors[client] = nil
	procCallOnAll('OnPlayerEditAttachedObject', getElemID(client), response and 1 or 0, index,
		attachment.model, attachment.bone,
		float2cell(attachment.x), float2cell(attachment.y), float2cell(attachment.z),
		float2cell(attachment.rx), float2cell(attachment.ry), float2cell(attachment.rz),
		float2cell(attachment.sx), float2cell(attachment.sy), float2cell(attachment.sz))
end)

addEventHandler('onPlayerQuit', root, function()
	if g_AttachmentEditors then g_AttachmentEditors[source] = nil end
end)
...
function PlayerTextDrawSetProportional(amx, player, textdrawID, proportional)
	...
	g_PlayerTextDraws[player][textdrawID].proportional = proportional
	return true
end
...
function PlayerTextDrawSetPreviewModel(amx, player, textdrawID, model)
	...
	g_PlayerTextDraws[player][textdrawID].previewmodel = model
	return true
end
...
function PlayerTextDrawSetPreviewRot(amx, player, textdrawID, rX, rY, rZ, zoom)
	...
	g_PlayerTextDraws[player][textdrawID].previewrot = { rX, rY, rZ, zoom }
	return true
end
...
function PlayerTextDrawSetPreviewVehCol(amx, player, textdrawID, color1, color2)
	...
	g_PlayerTextDraws[player][textdrawID].previewcolors = { color1, color2 }
	return true
end
...
function EnableStuntBonusForAll(amx, enable)
	g_StuntBonusEnabled = enable and true or false
	clientCall(root, 'setStuntBonusEnabled', g_StuntBonusEnabled)
	return true
end

function EnableStuntBonusForPlayer(amx, player, enable)
	if not player then return false end
	setElementData(player, 'amx:stuntBonusEnabled', enable and true or false, false)
	clientCall(player, 'setStuntBonusEnabled', enable and true or false)
	return true
end
...
function StartRecordingPlayerData(amx, player, type, name)
	if not player or (type ~= 1 and type ~= 2) then return false end
	g_PlayerRecordings = g_PlayerRecordings or {}
	if g_PlayerRecordings[player] then StopRecordingPlayerData(amx, player) end
	name = tostring(name or ''):gsub('[^%w_%-]', '_'):sub(1, 48)
	if name == '' then return false end
	local path = 'recordings/' .. name .. '.rec'
	if fileExists(path) then fileDelete(path) end
	local file = fileCreate(path)
	if not file then return false end
	fileWrite(file, '# MRP-MTA player recording v1\n')
	fileWrite(file, 'time,type,x,y,z,rx,ry,rz,vx,vy,vz,vehicle\n')
	local recording = { file = file, started = getTickCount(), type = type }
	recording.timer = setTimer(function()
		if not isElement(player) or not g_PlayerRecordings[player] then return end
		local x, y, z = getElementPosition(player)
		local rx, ry, rz = getElementRotation(player)
		local vx, vy, vz = getElementVelocity(player)
		local vehicle = getPedOccupiedVehicle(player)
		fileWrite(file, string.format('%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d\n',
			getTickCount() - recording.started, type, x, y, z, rx, ry, rz, vx, vy, vz,
			vehicle and (getElemID(vehicle) or -1) or -1))
		fileFlush(file)
	end, 100, 0)
	g_PlayerRecordings[player] = recording
	return true
end

function StopRecordingPlayerData(amx, player)
	local recording = g_PlayerRecordings and g_PlayerRecordings[player]
	if not recording then return false end
	if isTimer(recording.timer) then killTimer(recording.timer) end
	if recording.file then fileClose(recording.file) end
	g_PlayerRecordings[player] = nil
	return true
end

addEventHandler('onPlayerQuit', root, function()
	if g_PlayerRecordings and g_PlayerRecordings[source] then
		StopRecordingPlayerData(nil, source)
	end
end)

addEventHandler('onResourceStop', resourceRoot, function()
	if not g_PlayerRecordings then return end
	for player in pairs(g_PlayerRecordings) do
		StopRecordingPlayerData(nil, player)
	end
end)
