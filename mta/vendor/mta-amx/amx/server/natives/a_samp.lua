...
function IsPlayerNPC(amx, player)
	return player and getElementData(player, 'amx:npc') == true or false
end
...
function TextDrawSetPreviewRot(amx, textdraw, rX, rY, rZ, zoom)
	textdraw.previewrot = { rX, rY, rZ, zoom }
	return true
end

function TextDrawSetPreviewVehCol(amx, textdraw, color1, color2)
	textdraw.previewcolors = { color1, color2 }
	return true
end
...
function NetStats_MessagesRecvPerSecond(amx, player)
	local networkStat = getNetworkStats(player)
	return math.floor(networkStat.messagesInResendBuffer or networkStat.messagesInSendBuffer or 0)
end
