...
{
	new title_str[128];
	format(title_str, 128, "M-RP ª {00b33c}%s", text);
	return title_str;
}
...
timer StartLogin[100](playerid, sessionID)
{
	...
	if(result == -1 || result == 1) //logowanie
	{
        //Logowanie
		new string[256];
		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze M-RP! Zaloguj siÍ aby rozpoczπÊ grÍ.");
	...
	else //rejestracja
	{
        ...
        {
        SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze M-RP! Zarejestruj swoje konto aby rozpoczπÊ grÍ.");
...
UnFrakcja(playerid, para1, bool:respawn = true)
{
	...
	if(PlayerInfo[para1][pLider] > 0 && PlayerInfo[para1][pLiderValue] == 1)
	{
		...
		ShowPlayerDialogEx(playerid, DIALOG_UNFRAKCJA, DIALOG_STYLE_MSGBOX, "M-RP", sprintf("%s jest g≥Ûwnym liderem organizacji.\nCzy chcesz zwolniÊ WSZYSTKICH liderÛw z organizacji?\n(Zabierze VLD)", GetNick(para1)), "Tak", "Nie");
		return 1;
	}
...
ShowPersonalization(playerid, value)
{
	...
	if(value == 2)
	{
		...
		ShowPlayerDialogEx(playerid, D_PERS_CHAT, DIALOG_STYLE_TABLIST, "M-RP", string, "Akceptuj", "OdrzuÊ");
