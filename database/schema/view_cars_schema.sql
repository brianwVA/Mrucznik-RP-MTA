...
/*!50001 VIEW `view_cars` AS select `mru_cars`.`UID` AS `UID`,`mru_cars`.`ownertype` AS `ownertype`,`mru_cars`.`owner` AS `owner`,`mru_cars`.`model` AS `model`,`CAR_NAME`(`mru_cars`.`model`) AS `Nazwa` from `mru_cars` order by `mru_cars`.`UID` */;
